...
CREATE TABLE IF NOT EXISTS sessions (
    ...
    title TEXT,
    runtime_activation_snapshot TEXT,
...
class SessionDB:
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 7:
                try:
                    cursor.execute(
                        "ALTER TABLE sessions ADD COLUMN runtime_activation_snapshot TEXT"
                    )
                except sqlite3.OperationalError:
                    pass  # Column already exists
    ...
    def create_session(
        ...
        runtime_activation_snapshot: Dict[str, Any] = None,
    ) -> str:
        """Create a new session record. Returns the session_id."""
        def _do(conn):
            conn.execute(
                """INSERT OR IGNORE INTO sessions (id, source, user_id, model, model_config,
                   system_prompt, parent_session_id, started_at, runtime_activation_snapshot)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (
                    session_id,
                    source,
                    ...
                    parent_session_id,
                    time.time(),
                    json.dumps(runtime_activation_snapshot) if runtime_activation_snapshot is not None else None,
                ),
            )
        self._execute_write(_do)
        return session_id

    def update_runtime_activation_snapshot(
        self,
        session_id: str,
        snapshot: Dict[str, Any] | None,
    ) -> None:
        """Persist the runtime activation snapshot for a session."""

        def _do(conn):
            conn.execute(
                "UPDATE sessions SET runtime_activation_snapshot = ? WHERE id = ?",
                (
    ...
    def get_session(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        if not row:
            return None
        session = dict(row)
        snapshot_raw = session.get("runtime_activation_snapshot")
        if isinstance(snapshot_raw, str) and snapshot_raw:
            try:
                session["runtime_activation_snapshot"] = json.loads(snapshot_raw)
            except Exception:
                pass
        return session
        ...
        def _do(conn):
            cursor = conn.execute(
                """INSERT INTO messages (session_id, role, content, tool_call_id,
                   ...
                   reasoning, reasoning_details, codex_reasoning_items, metadata)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (
                    session_id,
                    role,
