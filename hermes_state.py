...
class SessionDB:
    ...
    def __init__(
        ...
        self.database_config = database_config or (
            StateDatabaseConfig(backend="sqlite", db_path=Path(db_path or DEFAULT_DB_PATH))
        ...
        try:
            if read_only:
                if self.backend != "sqlite":
                    raise ValueError("read_only SessionDB is only supported for SQLite state databases")
