...
- Timed record wrapper with optional workspace priming (macOS): `starter-kits/agent-launch-closeout-kit/scripts/demo-capture-timed-record-wrapper.sh`
...
- Recorded asset:
  - Recording path: starter-kits/agent-launch-closeout-kit/demo-artifacts/raw-demo-capture-2026-04-22T07-13-02-0500.mov
  - Duration: 00:01:19
  - Edited asset path: starter-kits/agent-launch-closeout-kit/demo-artifacts/edited-demo-capture-2026-04-22T07-13-02-0500.mp4
  - Posted URL (if published): 
  - Notes: timed screen recording wrapper; raw capture copied to edited asset path for immediate attachment readiness
## Cross-note closeout
...
- [x] Weekly factory note updated with launch execution result
- [x] Pipeline note updated with launch execution result
- [x] CEO note updated with launch execution result
- [x] Ship checklist updated with launch execution result
...
5. Use the recorded walkthrough asset as the primary attachment unless publish timing or format constraints force a fallback to the proof still.
6. On macOS, prefer `bash starter-kits/agent-launch-closeout-kit/scripts/demo-capture-timed-record-wrapper.sh --prime-workspace` when one-command workspace priming + timed capture is the fastest path; otherwise capture manually and then run the headless finalize helper.
7. Run `bash starter-kits/agent-launch-closeout-kit/scripts/demo-capture-post-finalize-verify.sh`, then log the verified asset path here.
