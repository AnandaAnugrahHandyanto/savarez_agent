...
For the lowest-friction macOS capture path when you want the walkthrough workspace primed, recorded, finalized, and verified in one block, use:

```bash
bash starter-kits/agent-launch-closeout-kit/scripts/demo-capture-timed-record-wrapper.sh --prime-workspace
```

That wrapper reruns `--prepare` by default, can call `demo-capture-launcher.sh --no-prepare` to open the exact walkthrough surfaces and activate QuickTime first, records the main display with native `screencapture`, copies the raw recording into the edited-asset slot when needed, finalizes `launch-execution-log.md`, and runs the post-finalize verifier automatically.
