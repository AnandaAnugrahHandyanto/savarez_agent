...
# Unset HERMES_* behavioral vars too. Kanban worker context pins live
# board/task/run ids and must never leak into test runs; tests use fixtures
# or explicit env setup when they need Kanban context.
unset HERMES_YOLO_MODE HERMES_INTERACTIVE HERMES_QUIET HERMES_TOOL_PROGRESS \
      ...
      HERMES_HOME_MODE HERMES_KANBAN_HOME HERMES_KANBAN_DB \
      HERMES_KANBAN_BOARD HERMES_KANBAN_WORKSPACES_ROOT HERMES_KANBAN_TASK \
      HERMES_KANBAN_RUN_ID HERMES_KANBAN_CLAIM_LOCK HERMES_KANBAN_WORKSPACE \
      HERMES_KANBAN_TENANT HERMES_KANBAN_ASSIGNEE 2>/dev/null || true
