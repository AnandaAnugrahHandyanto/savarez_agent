#!/usr/bin/env python3
"""Live test harness for Savarez AI Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any savarez imports.
ORIGINAL_HOME = os.environ.get("SAVAREZ_HOME")
ORIGINAL_AUTH = Path.home() / ".savarez" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.savarez/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.savarez/.env`` so
    ...
    real_env_file = Path.home() / ".savarez" / ".env"
    if real_env_file.exists():
        ...
        load_hermes_dotenv(hermes_home=str(Path.home() / ".savarez"))
...
def reset_module_state():
    """Drop cached modules so the new SAVAREZ_HOME takes effect."""
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["SAVAREZ_HOME"] = str(home)
...
def main():
    ...
    # Restore original SAVAREZ_HOME
    if ORIGINAL_HOME is not None:
        os.environ["SAVAREZ_HOME"] = ORIGINAL_HOME
    else:
