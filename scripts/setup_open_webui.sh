...
# Bootstrap Open WebUI against Savarez AI Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.savarez/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-savarez.sh
...
#   OPEN_WEBUI_NAME='Johnny Savarez'
...
#   HERMES_API_MODEL_NAME='Savarez AI Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Savarez AI Agent WebUI}"
...
HERMES_ENV_FILE="${HERMES_ENV_FILE:-$HOME/.savarez/.env}"
...
HERMES_API_MODEL_NAME="${HERMES_API_MODEL_NAME:-Savarez AI Agent}"
HERMES_API_BASE_URL="http://${HERMES_API_CONNECT_HOST}:${HERMES_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-savarez.sh"
LOG_DIR="$HOME/.savarez/logs"
...
install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.savarez.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.savarez</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.savarez"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.savarez"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-savarez.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Savarez AI Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-savarez.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.savarez/logs/openwebui.log
StandardError=append:%h/.savarez/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-savarez.service
}
...
main() {
  ...
  log 'Ensuring Savarez API server is configured...'
  ...
  log 'Restarting Savarez gateway so API server settings take effect...'
  savarez gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${HERMES_API_CONNECT_HOST}:${HERMES_API_PORT}/health" >/dev/null; then
    log 'Savarez API server did not answer on the first check. Trying to start gateway in the background...'
    nohup savarez gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  ...
  log "Savarez API endpoint: ${HERMES_API_BASE_URL}"
