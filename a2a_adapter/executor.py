...
from a2a.types import InvalidParamsError, Part, TaskState, TextPart
...
# Default ceiling on turns running at once. Each turn is a full, blocking agent
# loop; without a bound, many peers would saturate asyncio's shared default
# thread pool and stall the whole process. Overridable via env for operators who
# size their deployment differently.
DEFAULT_MAX_CONCURRENCY = 16


def _default_max_concurrency() -> int:
    raw = os.environ.get("HERMES_A2A_MAX_CONCURRENCY", "").strip()
...
class HermesAgentExecutor(AgentExecutor):
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        session = self._store.get_or_create(task.context_id)
        ...
        callbacks = {
            ...
            "tool_progress_callback": make_tool_progress_cb(updater, loop),
            "step_callback": make_step_cb(updater, loop),
        ...
        try:
            result = await loop.run_in_executor(
                self._pool(),
                functools.partial(
                    session.run_turn, user_text, task.id, callbacks=callbacks
                ),
            )
