...
def _tool_io_mode() -> str:
    """How much tool I/O to expose to the remote peer in status metadata.

    ``preview`` (default) bounds args/results to a size limit; ``none`` sends
    tool names only (no args or results — for deployments handling sensitive
    data); ``full`` sends them unbounded. Read per-call so it can be changed
    without restarting and overridden in tests.
    """
    mode = os.environ.get("HERMES_A2A_TOOL_IO", "preview").strip().lower()
    return mode if mode in {"preview", "none", "full"} else "preview"


def _tool_call_metadata(name: str | None, args: Any) -> dict[str, Any]:
    ...
    if mode == "none":
        return metadata
    metadata["hermes/args"] = _json_safe(args) if mode == "full" else _bounded(args)
    return metadata


def _tool_result_metadata(name: str | None, result: Any) -> dict[str, Any]:
    ...
    metadata["hermes/result"] = (
        _json_safe(result) if mode == "full" else _bounded(result)
    )
    return metadata
    ...
    def _cb(
        ...
        **_kwargs: Any,
        ...
        if event_type != "tool.started":
            return
        _emit_working(
            updater, loop, f"⚙ {name}", metadata=_tool_call_metadata(name, args)
        )
