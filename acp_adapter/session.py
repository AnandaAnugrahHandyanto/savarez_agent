"""ACP session manager — maps ACP sessions to Savarez AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.savarez/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Savarez itself is running in WSL.

    Windows ACP clients can launch ``savarez acp`` inside WSL while still sending
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Savarez from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["savarez-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Savarez AI agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Savarez AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Savarez runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.savarez/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``SAVAREZ_HOME`` dynamically rather than relying on
