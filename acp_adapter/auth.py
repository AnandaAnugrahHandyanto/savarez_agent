"""ACP auth helpers — detect and advertise Savarez authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "savarez-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Savarez runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Savarez can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Savarez.
    ...
    not have Savarez provider credentials configured yet, so Savarez always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Savarez using the currently configured "
