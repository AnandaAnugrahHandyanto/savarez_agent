"""CLI entry point for the savarez-agent ACP adapter.

Loads environment variables from ``~/.savarez/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from SAVAREZ_HOME (default ``~/.savarez``)."""
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="savarez-acp",
        description="Run Savarez AI Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Savarez version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _run_check() -> None:
    ...
    from acp_adapter.server import SavarezACPAgent  # noqa: F401

    print("Savarez ACP check OK")
...
def _run_setup() -> None:
    ...
    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "savarez", "model"]
        hermes_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``savarez postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting savarez-agent ACP adapter")
    ...
    import acp
    from .server import SavarezACPAgent
