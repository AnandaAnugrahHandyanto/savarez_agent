...
小夜内置了云崽风格的帮助菜单系统，用户发送 `帮助` 指令即可获得分类帮助图片。
...
| 分类名 | 参数 | 内容 | 权限 |
|--------|------|------|------|
| 人设互动 | `帮助 人设` | 自我介绍、问候、情感互动 | 所有用户 |
| 会话管理 | `帮助 会话` | 新建/恢复/分支会话 | 所有用户 |
| 系统信息 | `帮助 系统` | 状态、版本、用量 | 所有用户 |
| 审计系统 | `帮助 审计` | 审计开关、日志、备份 | 管理员 |
| 系统更新 | `帮助 更新` | 更新、回滚、镜像 | 管理员 |
| 管理员专区 | `管理帮助` | 网关管理、调试工具 | 管理员 |

### 权限分级

| 等级 | 身份 | 可见内容 |
|------|------|---------|
| `user` | 普通用户 | 人设互动、会话管理、系统信息 |
| `admin` | 管理员 | 以上 + 审计系统、系统更新 |
| `dev` | 开发者 | 所有指令 + 危险操作 |
...
在 `config.yaml` 中添加管理员：

```yaml
xiaoye:
  owner_openid: "你的QQ OpenID"
  admin_users:
    - "管理员1的OpenID"
    - "管理员2的OpenID"
```
...
Hermes 原生 `/help` 命令已通过网关钩子拦截，发送 `/help` 现在会显示自定义帮助菜单而非原生命令列表。
...
**配置文件位置：** `~/.hermes/skills/xiaoye-help/help-config.yaml`
