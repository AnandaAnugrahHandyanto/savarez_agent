"""Centralized logging setup for Savarez AI Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.savarez/logs/`` (profile-aware via ``get_hermes_home()``).
...
# Used by _ComponentFilter and exposed for ``savarez logs --component``.
COMPONENT_PREFIXES = {
...
def setup_logging(
    *,
    ...
    """Configure the Savarez logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    hermes_home
        Override for the Savarez home directory.  Falls back to
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the savarez group. However, both ``_open()`` (initial creation)
