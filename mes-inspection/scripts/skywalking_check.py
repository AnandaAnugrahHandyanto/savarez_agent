...
class SkyWalkingChecker(BaseChecker):
    ...
    def _check_response_time(self, target: Dict[str, Any] = None) -> CheckResult:
        """检查服务 P95 响应时间。"""
        target = target or {}
        service = target.get("service_name", self.config.get("service_name", "mes-service"))
        query = RESP_TIME_QUERY.format(service=service)
        try:
            data = self._sw_graphql(query, target)
    ...
    def _check_sla(self, target: Dict[str, Any] = None) -> CheckResult:
        """检查服务 SLA。"""
        target = target or {}
        service = target.get("service_name", self.config.get("service_name", "mes-service"))
    ...
    def _check_slow_endpoints(self, target: Dict[str, Any] = None) -> CheckResult:
        """检查 Top N 慢接口。"""
        target = target or {}
        service = target.get("service_name", self.config.get("service_name", "mes-service"))
        now = datetime.utcnow()
    ...
    def _check_alarms(self, target: Dict[str, Any] = None) -> CheckResult:
        """检查活跃告警。"""
        try:
    ...
    def check_node(self, target: Dict[str, Any]) -> InspectionReport:
        """对单个 SkyWalking 节点执行巡检。"""
        checks: List[CheckResult] = []
        metadata: Dict[str, Any] = {}

        checks.append(self._check_oap_health(target))
        checks.append(self._check_response_time(target))
        checks.append(self._check_sla(target))
        checks.append(self._check_slow_endpoints(target))
        checks.append(self._check_alarms(target))
