...
class AIAgent:
    ...
    def run_conversation(
        ...
        try:
            ...
            for r in _pre_results if isinstance(_pre_results, (list, tuple)) else []:
                if isinstance(r, dict):
                    # Context injection
                    if r.get("context"):
                        _ctx_parts.append(str(r["context"]))
                    # Runtime override
                    if r.get("runtime_override"):
                        _runtime_override = r["runtime_override"]
                elif isinstance(r, str) and r.strip():
                    _ctx_parts.append(r)
            if _ctx_parts:
                _plugin_user_context = "\n\n".join(_ctx_parts)

            # Apply runtime override from plugin
            if isinstance(_runtime_override, dict):
                _new_model = _runtime_override.get("model") or self.model
                _new_provider = _runtime_override.get("provider") or self.provider
                _new_base = _runtime_override.get("base_url") or self.base_url
                _new_key = _runtime_override.get("api_key") or self.api_key

                logger.info(
                    "Applying runtime override: provider=%s model=%s",
                    _new_provider,
                    _new_model,
                )
                logger.info(
                    "Current runtime before switch: provider=%s model=%s",
                    self.provider,
                    self.model,
                )
                ...
                try:
                    ...
                    logger.info(
                        "Runtime after switch: provider=%s model=%s",
                        self.provider,
                        self.model,
                    )

                except Exception as _override_exc:
                    logger.warning(
                        "Failed applying runtime override: %s",
                        _override_exc,
                    )

                # Apply system_prompt override from plugin
                _plugin_system_prompt = _runtime_override.get("system_prompt")
                if _plugin_system_prompt and isinstance(_plugin_system_prompt, str):
                    _plugin_system_prompt = _plugin_system_prompt.strip()
                    if _plugin_system_prompt:
                        logger.info(
                            "Applying system_prompt override from plugin (%d chars)",
                            len(_plugin_system_prompt),
                        )
                        if self.ephemeral_system_prompt:
                            ...
                            self.ephemeral_system_prompt = _plugin_system_prompt

                # Capture response_suffix from plugin for post-processing
                _plugin_response_suffix = _runtime_override.get("response_suffix")
                if _plugin_response_suffix and isinstance(_plugin_response_suffix, str):
                    _plugin_stripped = _plugin_response_suffix.strip()
                    if _plugin_stripped:
                        logger.info(
                            "Captured response_suffix from plugin (%d chars)",
                            len(_plugin_stripped),
                        )
                        self._pending_response_suffix = _plugin_stripped
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            try:
                ...
                if assistant_message.tool_calls:
                    ...
                    # Append response_suffix from plugin (e.g. signature tag)
                    _resp_suffix = getattr(self, "_pending_response_suffix", None)
                    if _resp_suffix and isinstance(_resp_suffix, str):
                        _resp_suffix = _resp_suffix.strip()
                        if _resp_suffix:
                            logger.info(
                                "Appending response_suffix to final_response (%d chars)",
                                len(_resp_suffix),
                            )
                            final_response = (final_response + _resp_suffix).strip()
                    # Clear after use so it doesn't persist across turns
                    self._pending_response_suffix = None
