...
class AIAgent:
    ...
    _SKILL_REVIEW_PROMPT = (
        "Review the conversation above and decide whether the skill "
        "library needs an update.\n\n"
        "**Default stance: do nothing.** If no concrete signal listed "
        "below fired in the actual conversation, say 'Nothing to save.' "
        "and stop. Inaction is the correct outcome of a smooth session. "
        "Be ACTIVE *when a real signal fired* — but do not invent lessons "
        "to look productive. An opportunity is missed only when a genuine "
        "lesson existed and you ignored it, not when no lesson existed at "
        "all.\n\n"
        "Signals that warrant action (must be observed in the actual "
        "conversation, not derived from common sense or speculation):\n"
        "  • User corrected your style, tone, format, legibility, or "
        "verbosity. Frustration signals like 'stop doing X', 'this is too "
        "verbose', 'don't format like this', 'why are you explaining', "
        "'just give me the answer', 'you always do Y and I hate it', or "
        "an explicit 'remember this' are FIRST-CLASS skill signals, not "
        "just memory signals. Embed the preference into the skill that "
        "governs that task so the next session starts already knowing.\n"
        "  • User corrected your workflow, approach, or sequence of "
        "steps. Encode the correction as a pitfall or explicit step in "
        "the skill that governs that class of task.\n"
        "  • A non-trivial technique, fix, workaround, debugging path, "
        "or tool-usage pattern was *observed working* this session. The "
        "lesson must come from something that actually happened, not a "
        "hypothesis about what might happen.\n"
        "  • A skill that got loaded or consulted this session turned "
        "out to be concretely wrong, missing a step, or outdated *as "
        "evidenced by an observed failure in this session*. Patch it.\n\n"
        "If none of the above fired, stop. Do not reach further to "
        "manufacture a justification. 'Be active' applies WITHIN the act "
        "of updating, not to the decision to update.\n\n"
        "Target shape of the library: CLASS-LEVEL skills, each with a "
        "rich SKILL.md and a `references/` directory for session-"
        "specific detail. Not a long flat list of narrow one-session-"
        "one-skill entries.\n\n"
        "Preference order when you do act — earliest action that fits:\n"
        ...
        "use the right directory per kind:\n"
        "     • `references/<topic>.md` — session-specific detail (error "
        "transcripts, reproduction recipes, provider quirks) AND "
        "condensed knowledge banks: quoted research, API docs, external "
        "authoritative excerpts, or domain notes. Concise and "
        "task-focused, not a full mirror of upstream docs.\n"
        "     • `templates/<name>.<ext>` — starter files meant to be "
        "copied and modified. **The file MUST be syntactically valid for "
        "its extension** — a `.js` template is JavaScript, a `.py` "
        "template is Python, a `.md` template is markdown. Do not "
        "package markdown text under a `.js` extension or vice versa.\n"
        "     • `scripts/<name>.<ext>` — statically re-runnable actions "
        "(verification scripts, fixture generators, deterministic "
        "probes). Same syntax-validity rule.\n"
        "     Add via skill_manage action=write_file with file_path "
        "starting 'references/', 'templates/', or 'scripts/'. The "
        "umbrella's SKILL.md should gain a one-line pointer.\n"
        "  4. CREATE A NEW CLASS-LEVEL UMBRELLA SKILL when no existing "
        "skill covers the class. The name MUST be at the class level. "
        "The name MUST NOT be a specific PR number, error string, "
        "feature codename, library-alone name, or 'fix-X / debug-Y / "
        "audit-Z-today' session artifact. If the proposed name only "
        "makes sense for today's task, fall back to (1), (2), or (3).\n\n"
        "User-preference embedding (important): when the user expressed "
        "a style/format/workflow preference, the update belongs in the "
        "SKILL.md body, not just in memory. Memory captures 'who the "
        "user is and what the current situation and state of your "
        "operations are'; skills capture 'how to do this class of task "
        "for this user'. When they complain about how you handled a "
        "task, the skill that governs that task needs to carry the "
        "lesson.\n\n"
        "If you notice two existing skills that overlap, note it in your "
        "reply — the background curator handles consolidation at scale.\n\n"
        "**Hard rules for the patch itself (real failure modes observed "
        "2026-05-12):**\n"
        "  1. NEVER write placeholder strings into the file. Strings "
        "like `... (existing content) ...`, `... (previous content) "
        "...`, `... (rest of the skill)`, `(omitted for brevity)`, "
        "`(rest unchanged)` are not valid replacements — they become "
        "literal text and corrupt the file. Your `new_string` must be "
        "the FULL, FINAL text the file should contain at that location.\n"
        "  2. NEVER use skill_manage(patch) to 're-emit' an existing "
        "section just to add something nearby. Patch only the smallest "
        "span you actually changed. If you want to add a pitfall row, "
        "patch the table; don't reproduce the whole section header.\n"
        "  3. Before patching, call skill_view first and copy "
        "`old_string` EXACTLY from the current file (no abbreviation, "
        "no rewriting, no summarization). If the patch fails 'old_string "
        "not found', re-read the file and fix the selector — do not "
        "retry with another guess.\n"
        "  4. NEVER invent a diagnosis. If the conversation's diagnostic "
        "output contradicts your hypothesis (e.g. `current_value` shows "
        "the field was filled but you want to write 'field-not-filled "
        "bug'), the lesson is elsewhere — re-read before patching.\n\n"
        ...
        "'Nothing to save.' is the correct response when no signal fired. "
        "Use it without apology — a clean session needs no patch."
    )

    _COMBINED_REVIEW_PROMPT = (
        "Review the conversation above and decide whether memory or the "
        "skill library needs an update. **Default stance on both: do "
        "nothing unless a concrete signal fired in this conversation.**\n\n"
        "**Memory**: who the user is. Did the user reveal persona, "
        "desires, preferences, personal details, or expectations about "
        "how you should behave? Save facts about the user and durable "
        "preferences with the memory tool. If nothing was revealed, "
        "skip.\n\n"
        "**Skills**: how to do this class of task. Be ACTIVE *when a "
        "real signal fired* below — but do not invent lessons to look "
        "productive. An opportunity is missed only when a genuine lesson "
        "existed and you ignored it, not when no lesson existed at all.\n\n"
        "Target shape of the skill library: CLASS-LEVEL skills with a rich "
        "SKILL.md and a `references/` directory for session-specific detail. "
        "Not a long flat list of narrow one-session-one-skill entries.\n\n"
        "Signals that warrant a skill update (must be observed in this "
        "conversation, not speculated):\n"
        ...
        "was *observed working* this session.\n"
        "  • A skill that was loaded or consulted turned out concretely "
        "wrong, missing, or outdated *as evidenced by an observed "
        "failure* — patch it now.\n\n"
        "If none of the above fired, skip the skill update.\n\n"
        ...
        "for starter files meant to be copied and modified (**file MUST "
        "be syntactically valid for its extension** — `.js` is "
        "JavaScript, `.md` is markdown, no mixing); `scripts/<name>.<ext>` "
        "for statically re-runnable actions (verification, fixture "
        "generators, probes). Add a one-line pointer in SKILL.md so "
        "future agents find them.\n"
        "  4. CREATE A NEW CLASS-LEVEL UMBRELLA when nothing exists. "
        "Name at the class level — NOT a PR number, error string, "
        "codename, library-alone name, or 'fix-X / debug-Y' session "
        "artifact. The name MUST NOT be a session-specific artifact. If "
        "the name only fits today's task, fall back to (1), (2), or (3).\n\n"
        ...
        "**Hard rules for the patch itself (real failure modes observed "
        "2026-05-12):**\n"
        "  1. NEVER write placeholder strings into the file. "
        "`... (existing content) ...`, `... (previous content) ...`, "
        "`... (rest of the skill)`, `(omitted for brevity)`, "
        "`(rest unchanged)` are NOT valid replacements — they become "
        "literal text and corrupt the file. Your `new_string` must be "
        "the FULL, FINAL text the file should contain at that location.\n"
        "  2. NEVER re-emit a whole section just to add a nearby line. "
        "Patch the smallest span you actually changed.\n"
        "  3. Before patching, call skill_view first and copy "
        "`old_string` EXACTLY from the current file. If the patch fails "
        "'old_string not found', re-read and fix the selector — do not "
        "retry with another guess.\n"
        "  4. NEVER invent a diagnosis. If diagnostic output in the "
        "conversation contradicts your hypothesis (e.g. `current_value` "
        "shows the field was filled), the lesson is elsewhere — "
        ...
        "'Nothing to save.' and stop. That is the correct response for a "
        "clean session, not a failure mode."
    )
