...
class AIAgent:
    ...
    _SKILL_REVIEW_PROMPT = (
        "Review the conversation above and update the skill library ONLY "
        "when there is a clear, concrete signal that genuinely new knowledge "
        "exists. Quality over quantity — a session with no signal should "
        "produce no update.\n\n"
        ...
        "WRITE HYGIENE — follow these rules on every update to prevent "
        "SKILL.md files from accumulating duplicate content over time:\n"
        "  • READ BEFORE WRITE: always call skill_view to read the current "
        "SKILL.md before patching or editing it. Never write from memory.\n"
        "  • NO DUPLICATES: if the concept you want to add already exists "
        "under any wording, modify the existing entry — do NOT add a new "
        "one alongside it. Semantic duplicates are still duplicates.\n"
        "  • PREFER patch OVER edit: use action='patch' (find-and-replace) "
        "for targeted changes. Only use action='edit' (full rewrite) when "
        "restructuring an entire section — full rewrites risk silently "
        "re-introducing or multiplying content.\n"
        "  • NO DATE STAMPS: do not write 'Updated YYYY-MM-DD', "
        "'NEW — Session YYYY-MM-DD', '<!-- added ... -->', or any "
        "temporal markers into SKILL.md content. Skills are timeless "
        "guidelines, not changelogs.\n"
        "  • DELETE DEPRECATED content: if removing an outdated section, "
        "actually delete it. Never leave it with a DEPRECATED label — "
        "that label accumulates as dead weight.\n\n"
        ...
        "'Nothing to save.' is a valid and correct outcome for sessions "
        "that ran smoothly with no corrections and no new technique. "
        "Use it freely when the signal list above was not triggered."
    )
    ...
    _COMBINED_REVIEW_PROMPT = (
        ...
        "**Skills**: how to do this class of task. Update skills ONLY "
        "when there is a clear, concrete signal that genuinely new "
        "knowledge exists. Quality over quantity — a session with no "
        "signal should produce no skill update.\n\n"
        ...
        "WRITE HYGIENE — follow these rules on every skill update to "
        "prevent SKILL.md files from accumulating duplicate content:\n"
        "  • READ BEFORE WRITE: always call skill_view to read the current "
        "SKILL.md before patching or editing it. Never write from memory.\n"
        "  • NO DUPLICATES: if the concept you want to add already exists "
        "under any wording, modify the existing entry — do NOT add a new "
        ...
        "  • NO DATE STAMPS: do not write 'Updated YYYY-MM-DD', "
        "'NEW — Session YYYY-MM-DD', '<!-- added ... -->', or any "
        "temporal markers into SKILL.md content.\n"
        "  • DELETE DEPRECATED content: actually remove it rather than "
        "leaving a DEPRECATED label.\n\n"
        ...
        "'Nothing to save.' is a valid and correct outcome when the "
        "signal list above was not triggered."
    )
