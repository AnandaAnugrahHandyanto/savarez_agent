...
class AIAgent:
    ...
    def __init__(
        ...
        if self.api_mode == "anthropic_messages":
            ...
            if _is_bedrock_anthropic:
                ...
                # Seed the bearer token env var so _resolve_model_id can geo-prefix
                # the model ID on the very first call (before any pool rotation).
                if api_key and api_key != "aws-sdk":
                    import os as _os
                    _os.environ["AWS_BEARER_TOKEN_BEDROCK"] = api_key
                    _os.environ.setdefault("AWS_REGION", _br_region)
                self._anthropic_client = build_anthropic_bedrock_client(_br_region)
                self._anthropic_api_key = api_key or "aws-sdk"
                self._anthropic_base_url = base_url
                self._is_anthropic_oauth = False
                self.api_key = api_key or "aws-sdk"
                self.client = None
                self._client_kwargs = {}
            ...
            # Seed bearer token so _resolve_model_id works on the first call.
            if api_key and api_key != "aws-sdk":
                import os as _os
                _os.environ["AWS_BEARER_TOKEN_BEDROCK"] = api_key
                _os.environ.setdefault("AWS_REGION", self._bedrock_region)
    ...
    def _build_system_prompt_parts(self, system_message: str = None) -> Dict[str, str]:
        ...
        # what it can route subagents to.
    ...
    def _build_keepalive_http_client(base_url: str = "") -> Any:
        try:
            ...
            return _httpx.Client(
                ...
                limits=_httpx.Limits(max_retries=0),
                proxy=_proxy,
            )
        except Exception:
            return None
    ...
    def _create_openai_client(self, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
        ...
        # Disable HTTP client retries to let credential pool rotation handle 429s.
    ...
    def _swap_credential(self, entry) -> None:
        ...
        if self.api_mode == "anthropic_messages":
            if self.provider == "bedrock":
                # Bedrock pool rotation: update bearer env var, region, and rebuild
                # the AnthropicBedrock client bound to the new regional endpoint.
                if runtime_key and runtime_key != "aws-sdk":
                    import os as _os
                    _os.environ["AWS_BEARER_TOKEN_BEDROCK"] = runtime_key
                _new_base = runtime_base.rstrip("/") if isinstance(runtime_base, str) else ""
                _region_m = re.search(r"bedrock-runtime\.([a-z0-9-]+)\.", _new_base)
                if _region_m:
                    _new_region = _region_m.group(1)
                    import os as _os
                    _os.environ["AWS_REGION"] = _new_region
                    self._bedrock_region = _new_region
                _new_region = getattr(self, "_bedrock_region", None) or "us-east-1"
                try:
                    self._anthropic_client.close()
                except Exception:
                    pass
                from agent.anthropic_adapter import build_anthropic_bedrock_client
                self._anthropic_client = build_anthropic_bedrock_client(_new_region)
                self._anthropic_api_key = runtime_key
                self._anthropic_base_url = _new_base
                self.api_key = runtime_key
                self.base_url = _new_base
                try:
                    from agent.auxiliary_client import _evict_cached_clients
                    _evict_cached_clients("bedrock")
                except Exception:
                    pass
                return
        ...
        if self.api_mode == "bedrock_converse":
            # Sync os.environ so boto3 and _resolve_model_id use the new key.
            # Also update _bedrock_region from the new entry's base_url and evict
            # the stale boto3 client so the next call connects via the right endpoint.
            if runtime_key and runtime_key != "aws-sdk":
                import os as _os
                _os.environ["AWS_BEARER_TOKEN_BEDROCK"] = runtime_key
            _new_base = runtime_base.rstrip("/") if isinstance(runtime_base, str) else ""
            ...
            if _region_m:
                ...
                import os as _os
                _os.environ["AWS_REGION"] = _new_region
                _old_region = getattr(self, "_bedrock_region", None)
                if _old_region and _old_region != _new_region:
                    try:
                        from agent.bedrock_adapter import invalidate_runtime_client
                        invalidate_runtime_client(_old_region)
                    except Exception:
                        pass
                self._bedrock_region = _new_region
            self.api_key = runtime_key
    ...
    def _rotate_credential_before_call(self) -> None:
        """Per-call credential rotation — fires pool.select() before every LLM API call.

        Only applies to providers with a credential pool that uses strategies
        benefiting from pre-call rotation (ollama-cloud round-robin balanced across
        every turn). Skips rotation for anthropic/bedrock/codex modes where the
        reactive error-recovery path is the right mechanism.
        """
        pool = getattr(self, "_credential_pool", None)
        if pool is None:
            return
        provider = getattr(self, "provider", None) or ""
        # Only rotate before each call for specific pool-backed providers.
        # ollama-cloud round-robin needs per-call rotation to distribute load.
        if provider not in {"ollama-cloud"}:
            return
        entry = pool.select()
        if entry is not None:
            _label = getattr(entry, "label", None) or entry.id[:8]
            self._swap_credential(entry)
            logger.info(
                "Per-call credential rotation: provider=%s entry=%s",
                provider, _label,
            )
    ...
    def _interruptible_api_call(self, api_kwargs: dict):
        ...
        provider fallback.
        """
        self._rotate_credential_before_call()
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        if self.api_mode == "anthropic_messages":
            ...
            _model = self.model
            if self.provider == "bedrock":
                from agent.bedrock_adapter import resolve_model_id_for_region
                # Use self._bedrock_region directly instead of reading from env var
                # AWS_BEARER_TOKEN_BEDROCK is a shared env var that auxiliary clients
                # (compression, title-gen) overwrite with their own pool entry, causing
                # a region mismatch: main session on ap-southeast-1 would resolve model
                # using us-west-2 prefix → "us.anthropic.claude-sonnet-4-6" sent to
                # ap-southeast-1 endpoint → HTTP 400 "model identifier is invalid".
                _br_region = getattr(self, "_bedrock_region", None) or "us-east-1"
                _model = resolve_model_id_for_region(_model, _br_region)
        ...
        try:
            ...
            if self.api_mode == "codex_responses":
                ...
                if self.api_mode == "anthropic_messages":
                    ...
                    _sum_model = self.model
                    if self.provider == "bedrock":
                        # Resolve geo-prefixed model ID for the active Bedrock region.
                        # self.model holds the bare config value; build_kwargs passes it
                        # directly to the AnthropicBedrock client which rejects bare IDs.
                        try:
                            from agent.bedrock_adapter import resolve_model_id_for_region
                            _sum_model = resolve_model_id_for_region(
                                _sum_model,
                                getattr(self, "_bedrock_region", None) or "us-east-1",
                            )
                        except Exception:
                            pass
                    _ant_kw = _tsum.build_kwargs(model=_sum_model, messages=api_messages, tools=None,
            ...
            if final_response:
                ...
                if self.api_mode == "codex_responses":
                    ...
                    _retry_model = self.model
                    if self.provider == "bedrock":
                        try:
                            from agent.bedrock_adapter import resolve_model_id_for_region
                            _retry_model = resolve_model_id_for_region(
                                _retry_model,
                                getattr(self, "_bedrock_region", None) or "us-east-1",
                            )
                        except Exception:
                            pass
                    _ant_kw2 = _tretry.build_kwargs(model=_retry_model, messages=api_messages, tools=None,
                                    is_oauth=self._is_anthropic_oauth,
                                    max_tokens=self.max_tokens, reasoning_config=self.reasoning_config,
                                    preserve_dots=self._anthropic_preserve_dots())
