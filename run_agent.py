...
class AIAgent:
    ...
    def __init__(
        ...
        if api_mode in {
            ...
            provider_name is None and self._base_url_hostname == "api.anthropic.com"
        ):
        ...
        if (
            self.provider == "openrouter" or self._is_openrouter_url()
        ...
        if self.api_mode == "anthropic_messages":
            ...
            if _is_bedrock_anthropic:
                ...
                if not self.quiet_mode:
                    print(
                        f"🤖 AI Agent initialized with model: {self.model} (AWS Bedrock + AnthropicBedrock SDK, {_br_region})"
                    )
            else:
                ...
                self._is_anthropic_oauth = (
                    _is_oat(effective_key) if _is_native_anthropic else False
                )
                ...
                if not self.quiet_mode:
                    ...
                    if effective_key and len(effective_key) > 12:
                        print(
                            f"🔑 Using token: {effective_key[:8]}...{effective_key[-4:]}"
                        )
            ...
            try:
                ...
                if _gr.get("guardrail_identifier") and _gr.get("guardrail_version"):
                    ...
                    if _gr.get("stream_processing_mode"):
                        self._bedrock_guardrail_config["streamProcessingMode"] = _gr[
            ...
            if not self.quiet_mode:
                ...
                print(
                    f"🤖 AI Agent initialized with model: {self.model} (AWS Bedrock, {self._bedrock_region}{_gr_label})"
                )
        else:
            ...
            # Merge per-provider custom HTTP headers (e.g. CF-Access-Client-Id/Secret
            # for Cloudflare Tunnel-protected endpoints) into default_headers.
            if self.provider_headers:
        ...
        # Stable routing key for per-agent CodeAct dispatcher registration.
        # Keep this separate from self.session_id because the latter can rotate
        # during compaction/session rollover while the agent instance persists.
        self._codeact_dispatch_session_id = uuid.uuid4().hex

        # Resolve CodeAct profile for the active provider/model capabilities.
        self._codeact_profile = self._resolve_codeact_profile()
        self._codeact_kernel = None  # initialised below when mode is active

        # Thinking-mode toggle for buun-server (and any backend that accepts
        # chat_template_kwargs in extra_body).  None = use server default.
        self._chat_template_kwargs: dict | None = None

        # Show tool configuration and store valid tool names for validation.
        self._refresh_tool_configuration_for_codeact()
        if self.tools:
            tool_names = sorted(self.valid_tool_names)
            if not self.quiet_mode:
                if self._codeact_profile["codeact_mode"]:
                    print(f"🐍 CodeAct mode active — {len(self.tools)} tool (run_code)")
                else:
        ...
        if not skip_memory:
            try:
                _mem_provider_name = (
                    mem_config.get("provider", "") if mem_config else ""
                )
                ...
                if _mem_provider_name:
                    ...
                    if self._memory_manager.providers:
                        ...
                        if self._session_db:
                            try:
                                _st = self._session_db.get_session_title(
                                    self.session_id
                                )
                                if _st:
                        ...
                        if self._gateway_session_key:
                            _init_kwargs["gateway_session_key"] = (
                                self._gateway_session_key
                            )
        ...
        qwen_aware_anchor_first_assistant = bool(
            _qa_cfg.get("anchor_first_assistant", False)
        )
        qwen_aware_threshold_absolute_max = _qa_cfg.get(
            "threshold_absolute_max"
        )  # None or int
        ...
        try:
            _ctx_cfg = (
                _agent_cfg.get("context", {}) if isinstance(_agent_cfg, dict) else {}
            )
            ...
            try:
                self.context_compressor.on_session_start(
                    ...
                    context_length=getattr(
                        self.context_compressor, "context_length", 0
                    ),
                )
        ...
        if (
            ...
            try:
                _detected = query_ollama_num_ctx(
                    self.model, self.base_url, api_key=self.api_key or ""
                )
        ...
        if (
            ...
            and _ollama_num_ctx_override
            is None  # don't override explicit ollama_num_ctx
    ...
    def _resolve_codeact_profile(self) -> dict:
        """Resolve CodeAct behavior from explicit provider/model capabilities."""
        ...
        codeact_cfg = cfg.get("codeact", {}) if isinstance(cfg, dict) else {}
        if not isinstance(codeact_cfg, dict):
            codeact_cfg = {}

        override = codeact_cfg.get("enabled", "auto")
        if override is True:
            profile["codeact_mode"] = True
            if not profile.get("structured_envelope"):
                profile["structured_envelope"] = True
            if profile.get("envelope_enforcement") in (None, "", "none"):
                profile["envelope_enforcement"] = "prompt"
        elif override is False:
            profile["codeact_mode"] = False

        envelope_cfg = codeact_cfg.get("envelope", {})
        if isinstance(envelope_cfg, dict):
            envelope_enabled = envelope_cfg.get("enabled", "auto")
            if envelope_enabled is True:
                profile["structured_envelope"] = True
            elif envelope_enabled is False:
                profile["structured_envelope"] = False

            enforcement = envelope_cfg.get("enforcement", "auto")
            if isinstance(enforcement, str):
                enforcement_norm = enforcement.strip().lower()
                if enforcement_norm in {"grammar", "prompt", "none"}:
                    profile["envelope_enforcement"] = enforcement_norm

        if (
            profile.get("codeact_mode")
            and profile.get("structured_envelope")
            and profile.get("envelope_enforcement") in (None, "", "none")
        ):
            profile["envelope_enforcement"] = "prompt"

        return profile

    def _resolve_codeact_enabled_tool_names(self) -> set | None:
        """Return the tool-name filter used inside the CodeAct namespace."""
        ...
        for toolset_name in self.enabled_toolsets:
            if validate_toolset(toolset_name):
                enabled_names.update(resolve_toolset(toolset_name))
        return enabled_names

    def _refresh_tool_configuration_for_codeact(self) -> None:
        """Refresh visible tools after resolving the current CodeAct profile."""
        if self._codeact_profile.get("codeact_mode"):
            from model_tools import build_codeact_tool_schema

            enabled_names = self._resolve_codeact_enabled_tool_names()
            envelope = bool(self._codeact_profile.get("structured_envelope", False))
            self.tools = [
                build_codeact_tool_schema(
                    enabled_tool_names=enabled_names,
                    envelope_mode=envelope,
                )
            ]
            if self._codeact_kernel is None:
                import agent.codeact_tool  # noqa: F401 — side-effect: registers run_code

                self._codeact_kernel = self._init_codeact_kernel(enabled_names)
        else:
            if self._codeact_kernel is not None:
                self._shutdown_codeact_kernel()
    ...
    def switch_model(
        ...
        if api_mode == "anthropic_messages":
            ...
            effective_key = (
                (api_key or self.api_key or resolve_anthropic_token() or "")
                if _is_native_anthropic
                else (api_key or self.api_key or "")
            )
            self.api_key = effective_key
            self._anthropic_api_key = effective_key
            ...
            self._is_anthropic_oauth = (
                _is_oauth_token(effective_key) if _is_native_anthropic else False
            )
            self.client = None
            self._client_kwargs = {}
        else:
    ...
    def _check_compression_model_feasibility(self) -> None:
        ...
        try:
            ...
            if aux_context < threshold:
                ...
                if main_ctx:
                    self.context_compressor.threshold_percent = new_threshold / main_ctx
        ...
        for msg in review_messages or []:
            ...
            if not tcid:
                ...
                if (
                    ...
                    and content_str in existing_tool_contents
                ):
                    continue
            try:
    ...
    def _build_memory_write_metadata(
        ...
        *,
        ...
        metadata: Dict[str, Any] = {
            ...
            or getattr(self, "_memory_write_origin", "assistant_tool"),
            "execution_context": (
                execution_context
    ...
    def _apply_pending_steer_to_tool_results(
        ...
        target_idx = None
        for j in range(
            len(messages) - 1, max(len(messages) - num_tool_msgs - 1, -1), -1
        ):
            msg = messages[j]
    ...
    def close(self) -> None:
        ...
        # 6. Shut down the CodeAct kernel (persistent Python subprocess)
        self._shutdown_codeact_kernel()
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        # CodeAct behavioral guidance: overrides generic tool-use enforcement
        # when run_code is the model's only tool.  Without this, models get
        # stuck in loops calling run_code with print() to "present" answers.
        if "run_code" in self.valid_tool_names:
            prompt_parts.append(CODEACT_BEHAVIORAL_GUIDANCE)
    ...
    def _create_openai_client(
        ...
        if self.provider == "google-gemini-cli" or str(
            ...
            safe_kwargs = {
                ...
                if k
                in {"api_key", "base_url", "default_headers", "project_id", "timeout"}
            }
    ...
    def _run_codex_create_stream_fallback(self, api_kwargs: dict, client: Any = None):
        ...
        fallback_kwargs = self._get_transport().preflight_kwargs(
            ...
            def _bedrock_call():
                try:
                    ...
                    result["response"] = stream_converse_with_callbacks(
                        ...
                        if self.reasoning_callback or self.stream_delta_callback
                        else None,
        ...
        def _call_chat_completions():
            ...
            if _provider_timeout_cfg is not None:
                ...
                _stream_read_timeout = float(
                    os.getenv("HERMES_STREAM_READ_TIMEOUT", 120.0)
                )
    ...
    def _try_activate_fallback(self, reason: "FailoverReason | None" = None) -> bool:
        ...
        if reason in (FailoverReason.rate_limit, FailoverReason.billing):
            ...
            primary_provider = (
                ((self._primary_runtime or {}).get("provider") or "").strip().lower()
            )
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        if self.api_mode == "codex_responses":
            ...
            is_github_responses = base_url_host_matches(
                self.base_url, "models.github.ai"
            ...
            is_xai_responses = (
                self.provider == "xai" or self._base_url_hostname == "api.x.ai"
            )
            ...
            return _ct.build_kwargs(
                ...
                github_reasoning_extra=self._github_models_reasoning_extra_body()
        ...
        _is_tokenhub = base_url_host_matches(
            self._base_url_lower, "tokenhub.tencentmaas.com"
        )
        ...
        # CodeAct grammar enforcement: require a tool call only until the model
        # has completed the first real action. After at least one tool result is
        # present in the conversation, relax back to ``auto`` so the model can
        # finish with a plain assistant answer instead of getting trapped in a
        # "present the answer via another run_code call" loop.
        _codeact_tool_choice = None
        if (
            getattr(self, "_codeact_kernel", None) is not None
            and isinstance(self.tools, list)
            and len(self.tools) == 1
        ):
            _has_prior_tool_results = any(
                isinstance(msg, dict) and msg.get("role") == "tool"
                for msg in _msgs_for_chat
            )
            _codeact_tool_choice = "auto" if _has_prior_tool_results else "required"

        _ctk = getattr(self, "_chat_template_kwargs", None)
        _extra_body_additions = {"chat_template_kwargs": _ctk} if _ctk else None

        return _ct.build_kwargs(
            model=self.model,
            messages=_msgs_for_chat,
            tools=self.tools,
            tool_choice=_codeact_tool_choice,
            base_url=self.base_url,
            timeout=self._resolved_api_call_timeout(),
            max_tokens=self.max_tokens,
            ephemeral_max_output_tokens=_ephemeral_out,
            ...
            qwen_prepare_inplace_fn=self._qwen_prepare_chat_messages_inplace
            if _is_qwen
            else None,
            qwen_session_metadata=_qwen_meta,
            ...
            lmstudio_reasoning_options=self._lmstudio_reasoning_options_cached()
            if _is_lmstudio
            else None,
            anthropic_max_output=_ant_max,
    ...
    def _compress_context(
        ...
        *,
        ...
        if summary_error:
            if (
                getattr(self, "_last_compression_summary_warning", None)
                != summary_error
            ):
            ...
            _aux_fail_model = getattr(
                self.context_compressor, "_last_aux_model_failure_model", None
            )
            _aux_fail_err = getattr(
                self.context_compressor, "_last_aux_model_failure_error", None
            )
            if _aux_fail_model:
        ...
        # Soft-reset the CodeAct kernel so user variables from compacted
        # turns don't outlive the context the model can see.  The model
        # can't reference them in the compressed conversation, so the
        # kernel shouldn't have them either.
        if self._codeact_kernel is not None:
            try:
                self._codeact_kernel.soft_reset()
            except Exception:
                pass
    ...
    def _invoke_tool(
        ...
        elif function_name == "run_code" and self._codeact_kernel is not None:
            return self._invoke_run_code(function_args)
        else:
    ...
    def _invoke_run_code(self, function_args: Any) -> str:
        """Execute a CodeAct ``run_code`` tool call without re-parsing.

        When Hermes already parsed ``{"thoughts": ..., "code": ...}`` from the
        tool-call arguments, send the ``code`` field straight to the persistent
        kernel. Falling back through the generic dispatcher reintroduces the
        envelope parser and can bounce valid bare Python like ``print(...)`` or
        ``pass`` back to the model as a fake parse error.
        """
        if self._codeact_kernel is None:
            return json.dumps(
                {
                    "error": (
                        "CodeAct kernel not initialised. "
                        "This tool requires codeact_mode to be enabled."
                    )
                }
            )
        ...
        if isinstance(function_args, dict):
            ...
            from agent.codeact_dispatcher import dispatch as _codeact_dispatch

            return _codeact_dispatch(
                str(function_args),
                self._codeact_kernel,
                envelope_mode=False,
            )

        if thoughts:
            logger.debug("CodeAct thoughts: %.120s", thoughts)
        if not code:
    ...
    def _init_codeact_kernel(self, enabled_tool_names: set | None):
        ...
        Builds the namespace source from the registry, creates a tool
        dispatcher that routes kernel tool-call requests back to
        handle_function_call, and starts the subprocess.
        """
        ...
        from agent.codeact_tool import set_kernel_dispatcher

        _registry = __import__("tools.registry", fromlist=["registry"]).registry

        # Build a SkillNamespaceInjector with config-driven limits.
        _max_skills = 20
        _recent_count = 5
        try:
            from hermes_cli.config import load_config as _load_cfg

            _skills_cfg = (_load_cfg().get("codeact") or {}).get("skills") or {}
            _max_skills = int(_skills_cfg.get("max_injected", 20))
            _recent_count = int(_skills_cfg.get("recently_used_count", 5))
        except Exception:
            pass

        _skill_injector = SkillNamespaceInjector(
            registry=_registry,
            max_skills=_max_skills,
            recently_used_count=_recent_count,
        )

        namespace_source = build_tool_namespace_source(
            registry=_registry,
            enabled_tool_names=enabled_tool_names,
            skill_injector=_skill_injector,
        )

        # Read promotion config once.
        _auto_promote = False
        try:
            _promo_cfg = (_load_cfg().get("codeact") or {}).get("promotion") or {}
            _auto_promote = bool(_promo_cfg.get("auto_promote", False))
        except Exception:
            pass
        _session_id_for_promo = getattr(self, "session_id", None)

        def _tool_dispatcher(tool_name: str, args: dict) -> str:
            """Route a kernel tool call back through handle_function_call."""
            # __promote_skill__ — Phase 5 skill promotion pipeline.
            if tool_name == "__promote_skill__":
                ...
                try:
                    ...
                    candidate = PromotionCandidate(
                        ...
                        session_id=_session_id_for_promo,
                        occurrence_count=1,
                    )
                    if _auto_promote:
                        path = write_promoted_skill(candidate)
                        return _json.dumps(
                            {
                                "status": "promoted",
                        ...
                        return _json.dumps(
                            {
                                "status": "flagged",
                                "message": (
                                    f"Skill '{candidate.fn_name}' flagged for review. "
                                    "Run `hermes curator promote` to approve."
                                ),
                            }
                        )
                except Exception as exc:
                    return _json.dumps({"error": f"Promotion failed: {exc}"})
            return handle_function_call(
                tool_name,
                args,
                self.task_id or "",
                session_id=self.session_id or "",
                enabled_tools=list(enabled_tool_names) if enabled_tool_names else None,
            )
        ...
        # Wire dispatcher so the registered run_code handler can also reach the kernel.
        set_kernel_dispatcher(
            self._codeact_dispatch_session_id,
            lambda args: __import__(
                "agent.codeact_dispatcher", fromlist=["dispatch"]
            ).dispatch(
                args.get("code", ""),
                kernel,
                envelope_mode=False,  # handler receives pre-parsed args dict
            )
        )

        return kernel

    def _shutdown_codeact_kernel(self) -> None:
        """Tear down the CodeAct kernel if active."""
        if self._codeact_kernel is not None:
            try:
                self._codeact_kernel.shutdown()
            except Exception:
                pass
            self._codeact_kernel = None
        from agent.codeact_tool import set_kernel_dispatcher

        set_kernel_dispatcher(self._codeact_dispatch_session_id, None)
    ...
    def _execute_tool_calls_concurrent(
        ...
        for tool_call in tool_calls:
            ...
            if block_message is not None:
                ...
                guardrail_decision = self._tool_guardrails.before_call(
    ...
    def _execute_tool_calls_sequential(
        ...
        for i, tool_call in enumerate(assistant_message.tool_calls, 1):
            ...
            if _block_msg is not None:
                ...
                function_result = self._guardrail_block_result(
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        try:
            ...
            _summary_temperature = (
                None if _omit_summary_temperature else _raw_summary_temp
            )
            _is_nous = "nousresearch" in self._base_url_lower
            ...
            _is_lmstudio_summary = (
                self.provider or ""
            ).strip().lower() == "lmstudio" and self._supports_reasoning_extra_body()
            ...
            if self.api_mode == "codex_responses":
                ...
                if self.api_mode == "anthropic_messages":
                    ...
                    _summary_result = self._get_transport().normalize_response(
                        summary_response
                    )
                    final_response = (_summary_result.content or "").strip()
            ...
            if final_response:
                ...
                if self.api_mode == "codex_responses":
                    ...
                    _retry_result = self._get_transport().normalize_response(
                        summary_response
                    )
                    final_response = (_retry_result.content or "").strip()
    ...
    def run_conversation(
        ...
        if not self.quiet_mode:
            ...
            self._safe_print(
                f"💬 Starting conversation: '{_print_preview[:60]}{'...' if len(_print_preview) > 60 else ''}'"
            )
        ...
        while (
            ...
            request_logger = getattr(self, "logger", None) or logging.getLogger(
                __name__
            )
            ...
            if not self.quiet_mode:
                ...
                if self.thinking_callback:
                    ...
                    not self._has_stream_consumers()
                    and self._should_start_quiet_spinner()
                ):
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    if response_invalid:
                        ...
                        self._vprint(
                            f"{self.log_prefix}   ⏱️  {_failure_hint}", force=True
                        )
                        ...
                        self._vprint(
                            f"{self.log_prefix}⏳ Retrying in {wait_time:.1f}s ({_failure_hint})...",
                            force=True,
                        )
                        logging.warning(
                        ...
                        _thinking_exhausted = (
                            ...
                            and (
                                (
                                    ...
                                    and not self._has_content_after_think_block(
                                        _trunc_content
                                    )
                                )
                    ...
                    if (
                        ...
                        and getattr(self, "_unicode_sanitization_passes", 0) < 2
                    ):
                        ...
                        if _is_ascii_codec:
                            ...
                            if isinstance(
                                getattr(self, "prefill_messages", None), list
                            ):
                            ...
                            if isinstance(active_system_prompt, str):
                                _sanitized_system = _strip_non_ascii(
                            ...
                            if isinstance(
                                getattr(self, "ephemeral_system_prompt", None), str
                            ):
                            ...
                            _default_headers = (
                                ...
                                if isinstance(
                                    getattr(self, "_client_kwargs", None), dict
                                )
                                else None
                            )
                            ...
                            if _raw_key:
                                ...
                                if _clean_key != _raw_key:
                                    ...
                                    if getattr(
                                        self, "client", None
                                    ) is not None and hasattr(self.client, "api_key"):
                    ...
                    if (
                        ...
                        try:
                            ...
                            pass
                        print(
                            f"{self.log_prefix}🔐 Nous 401 — Portal authentication failed."
                        )
                        if _body_text:
                            print(f"{self.log_prefix}   Response: {_body_text}")
                        print(
                            f"{self.log_prefix}   Most likely: Portal OAuth expired, account out of credits, or agent key revoked."
                        )
                        print(f"{self.log_prefix}   Troubleshooting:")
                        print(
                            f"{self.log_prefix}     • Re-authenticate: hermes login --provider nous"
                        )
                        print(
                            f"{self.log_prefix}     • Check credits / billing: https://portal.nousresearch.com"
                        )
                        print(
                            f"{self.log_prefix}     • Verify stored credentials: {_dhh}/auth.json"
                        )
                        print(
                            f"{self.log_prefix}     • Switch providers temporarily: /model <model> --provider openrouter"
                        )
                    if (
                        ...
                        if self._try_refresh_copilot_client_credentials():
                            self._vprint(
                                f"{self.log_prefix}🔐 Copilot credentials refreshed after 401. Retrying request..."
                            )
                            continue
                    if (
                        self.api_mode == "anthropic_messages"
                        ...
                        print(
                            f"{self.log_prefix}     • For API keys: verify at https://platform.claude.com/settings/keys"
                        )
                        print(
                            f"{self.log_prefix}     • For Claude Code: run 'claude /login' to refresh, then retry"
                        )
                        print(
                            f'{self.log_prefix}     • Legacy cleanup: hermes config set ANTHROPIC_TOKEN ""'
                        )
                        print(
                            f'{self.log_prefix}     • Clear stale keys: hermes config set ANTHROPIC_API_KEY ""'
                        )
                    ...
                    if self._is_openrouter_url() and "support tool use" in error_msg:
                        self._vprint(
                    ...
                    if is_payload_too_large:
                        ...
                        if compression_attempts > max_compression_attempts:
                            ...
                            self._vprint(
                                f"{self.log_prefix}   💡 Try /new to start a fresh conversation, or /compress to retry compression.",
                                force=True,
                            )
                            logging.error(
                    ...
                    if is_context_length_error:
                        ...
                        available_out = parse_available_output_tokens_from_error(
                            error_msg
                        )
                        ...
                        _base_lower = (
                            (getattr(self, "base_url", "") or "").rstrip("/").lower()
                        )
                        ...
                        if parsed_limit and parsed_limit < old_ctx:
                            ...
                            self._vprint(
                                f"{self.log_prefix}Context limit detected from API: {new_ctx:,} tokens (was {old_ctx:,})",
                                force=True,
                            )
                        ...
                        if (
                            ...
                            self._vprint(
                                f"{self.log_prefix}   💡 The conversation has accumulated too much content. Try /new to start fresh, or /compress to manually trigger compression.",
                                force=True,
                            )
                            logging.error(
                    ...
                    if is_client_error:
                        ...
                        if (
                            ...
                            if _provider == "openai-codex" and status_code == 401:
                                self._vprint(
                                    f"{self.log_prefix}   💡 Codex OAuth token was rejected (HTTP 401). Your token may have been",
                                    force=True,
                                )
                                self._vprint(
                                    f"{self.log_prefix}      refreshed by another client (Codex CLI, VS Code). To fix:",
                                    force=True,
                                )
                                self._vprint(
                                    f"{self.log_prefix}      1. Run `codex` in your terminal to generate fresh tokens.",
                                    force=True,
                                )
                                self._vprint(
                                    f"{self.log_prefix}      2. Then run `hermes auth` to re-authenticate.",
                                    force=True,
                                )
                            else:
                                self._vprint(
                    ...
                    if retry_count >= max_retries:
                        ...
                        # per API call block.
                        if (
                            not primary_recovery_attempted
            ...
            try:
                ...
                normalized = _transport.normalize_response(
                    response, **_normalize_kwargs
                )
                ...
                if self.api_mode == "codex_responses" and finish_reason == "incomplete":
                    ...
                    if (
                        ...
                        interim_codex_message_items = interim_msg.get(
                            "codex_message_items"
                        )
                        duplicate_interim = (
                ...
                if assistant_message.tool_calls:
                    ...
                    if self._tool_guardrail_halt_decision is not None:
                        ...
                        final_response = self._toolguard_controlled_halt_response(
                            decision
                        )
                        self._emit_status(
                    ...
                    if not self._has_content_after_think_block(final_response):
                        ...
                        if self._has_content_after_think_block(_partial_streamed):
                            ...
                            _recovered = self._strip_think_blocks(
                                _partial_streamed
                            ).strip()
                            logger.info(
                        ...
                        _prefill_exhausted = (
                            _has_structured and self._thinking_prefill_retries >= 2
                        )
                        if (
                            _truly_empty
                ...
                logger.debug(
                    "Outer loop error in API call #%d", api_call_count, exc_info=True
                )
        ...
        if final_response is None and (
            ...
            _turn_exit_reason = (
                f"max_iterations_reached({api_call_count}/{self.max_iterations})"
            )
            self._emit_status(
