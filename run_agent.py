...
class AIAgent:
    ...
    def __init__(
        ...
        self._tool_registry_injection = _agent_section.get(
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        # Tool registry injection: controls how available tools are surfaced to the
        # model in the system prompt.  Controlled by config.yaml
        # agent.tool_registry_injection:
        #   "disabled" (default) — no injection
        #   "nudge"     — soft recommendation to check tool registry when relevant
        #   "full"      — always surface all available tools
        if self.valid_tool_names and self._tool_registry_injection != "disabled":
            _all_tool_names = ", ".join(sorted(self.valid_tool_names))
            if self._tool_registry_injection == "full":
                prompt_parts.append(
                    TOOL_REGISTRY_FULL_GUIDANCE.format(tool_names=_all_tool_names)
                )
            elif self._tool_registry_injection == "nudge":
                prompt_parts.append(
                    TOOL_REGISTRY_NUDGE_GUIDANCE.format(tool_names=_all_tool_names)
                )
                ...
                try:
                    ...
                    if not is_context_length_error and not status_code:
                        ...
                        if _is_server_disconnect:
                            ...
                            _is_large = (
                                approx_tokens > ctx_len * 0.6 or len(api_messages) > 200
                            )
                            if _is_large:
                                is_context_length_error = True
                                self._vprint(
                ...
                if assistant_message.tool_calls:
                    ...
                    if not self._has_content_after_think_block(final_response):
                        ...
                        if reasoning_text:
                            ...
                            self._vprint(
                                f"{self.log_prefix}ℹ️  Empty response (no content or reasoning)."
                            )
