...
from agent.codex_responses_adapter import _deterministic_call_id as _codex_deterministic_call_id
from agent.codex_responses_adapter import _split_responses_tool_id as _codex_split_responses_tool_id
...
from agent.tool_result_classification import FILE_MUTATING_TOOL_NAMES as _FILE_MUTATING_TOOLS
...
class AIAgent:
    ...
    def __init__(
        ...
        extended_thinking_budget_ratio: float = 0.2,
    ):
        """
        ...
        Args:
            ...
            load_soul_identity (bool): If True, still use ~/.hermes/SOUL.md as the primary
                ...
                remain skipped.
            enable_extended_thinking (bool): If True, enable extended thinking for tasks that
                involve reasoning (keywords: "review", "debug", "refactor", "design"). This
                allows the model to allocate a portion of its token budget to internal reasoning
                before generating the response, improving accuracy on complex tasks. (default: False)
            extended_thinking_budget_ratio (float): Ratio of token budget to allocate for thinking
                (0.0-1.0). Only used when enable_extended_thinking=True. For example, 0.2 means
                20% of available output tokens are reserved for thinking. (default: 0.2)
        """
        _install_safe_stdio()
        ...
        self.extended_thinking_budget_ratio = extended_thinking_budget_ratio
                        ...
                        for _fb in _fb_entries:
                            ...
                            if _fb_client is not None:
                                ...
                                if _provider_timeout is not None:
                                    client_kwargs["timeout"] = _provider_timeout
                                if (
                                    hasattr(_fb_client, "_default_headers")
                                    and _fb_client._default_headers
                                ):
                                    client_kwargs["default_headers"] = dict(
                                        _fb_client._default_headers
                                    )
                                _fb_resolved = True
                                break
                        if not _fb_resolved:
    ...
    def _build_system_prompt_parts(self, system_message: str = None) -> Dict[str, str]:
        ...
        return {
            ...
            "volatile": "\n\n".join(
                p.strip() for p in volatile_parts if p and p.strip()
            ),
        }
                ...
                for event in stream:
                    ...
                    try:
                        ...
                        if _diag.get("first_chunk_at") is None:
                            _diag["first_chunk_at"] = last_chunk_time["t"]
                        try:
                            _diag["bytes"] = int(_diag.get("bytes", 0)) + len(
                                repr(event)
                            )
                        except Exception:
                            pass
        ...
        for i, (tc, name, args, block_result, blocked_by_guardrail) in enumerate(
            ...
            if r is None:
                ...
                if is_error:
                    ...
                    result_preview = (
                        _err_text[:200] if len(_err_text) > 200 else _err_text
                    )
                    logger.warning(
            ...
            if self._should_emit_quiet_tool_messages():
                ...
                if self.verbose_logging:
                    ...
                    response_preview = (
                        ...
                        if len(_preview_str) > self.log_prefix_chars
                        else _preview_str
                    )
                    print(
        ...
        for i, tool_call in enumerate(assistant_message.tool_calls, 1):
            ...
            if not self.quiet_mode:
                if self.verbose_logging:
                    ...
                    response_preview = (
                        _fr_str[: self.log_prefix_chars] + "..."
                        if len(_fr_str) > self.log_prefix_chars
                        else _fr_str
                    )
                    print(
        ...
        while (
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    # If extended thinking is enabled AND the task involves reasoning
                    # (detected by keywords), add thinking budget to the API request.
                    if self.enable_extended_thinking:
                        _task_text = (user_message or "").lower()
                        _reasoning_keywords = {
                            "review",
                            "debug",
                            "refactor",
                        ...
                        _has_reasoning_task = any(
                            kw in _task_text for kw in _reasoning_keywords
                        )
                        if _has_reasoning_task:
                            # Calculate thinking budget based on max_tokens and ratio
                            _max_output_tokens = (
                                api_kwargs.get("max_tokens") or self.max_tokens or 4096
                            )
                            _thinking_budget = max(
                                int(
                                    _max_output_tokens
                                    * self.extended_thinking_budget_ratio
                                ),
                                1000,
                            )

                            # Add thinking configuration to api_kwargs
                            # This is a feature flag approach — only add if the field doesn't exist
                            ...
                            logger.debug(
                                "Extended thinking enabled for reasoning task: budget_tokens=%d (ratio=%f)",
                                _thinking_budget,
                                self.extended_thinking_budget_ratio,
                            )
