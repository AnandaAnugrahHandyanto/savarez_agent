        ...
        if not output:
            ...
            if fallback_text:
                ...
                output = [SimpleNamespace(
                    ...
                    content=[SimpleNamespace(type="output_text", text=fallback_text)],
                )]
                response.output = output
            elif normalized_status in {"queued", "in_progress", "incomplete", "completed"}:
                ...
                try:
                    ...
                    if self.api_mode == "codex_responses":
                        ...
                        if response is None:
                            ...
                            _out_text_stripped = output_text.strip() if isinstance(output_text, str) else ""
                            if _out_text_stripped:
                                logger.debug(
                                    "Codex response.output is empty but output_text is present (%d chars); deferring to normalization.",
                                    len(_out_text_stripped),
                                )
                            elif isinstance(response_status, str) and response_status.strip().lower() in {"queued", "in_progress", "incomplete", "completed"}:
                                logger.debug(
                                    "Codex response.output is empty with continuable status=%s; deferring to normalization.",
                                    response_status,
                                )
                            else:
                                _resp_incomplete = getattr(response, "incomplete_details", None)
                                logger.warning(
                                    "Codex response.output is empty after stream backfill (status=%s, incomplete_details=%s, model=%s). %s",
                                    response_status, _resp_incomplete, getattr(response, "model", None),
