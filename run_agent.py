...
class AIAgent:
    ...
    def run_conversation(
        ...
        # Plugin hook: pre_context_compress
        # Fired before context compression. Allows alternative context assembly.
        assembled_messages = None
        try:
            from hermes_cli.plugins import invoke_hook as _invoke_hook

            _assemble_results = _invoke_hook(
                "pre_context_compress",
                session_id=self.session_id,
                messages=list(messages),
                token_budget=self.context_compressor.threshold_tokens
                if self.compression_enabled
                else None,
                model=self.model,
                platform=getattr(self, "platform", None) or "",
            )
            for result in _assemble_results:
                if isinstance(result, list) and len(result) > 0:
                    assembled_messages = result
                    logger.info(
                        "Using plugin context assembly: %d messages", len(result)
                    )
                    break
        except Exception as exc:
            logger.debug("pre_context_compress hook failed: %s", exc)
