...
class AIAgent:
    _CONTEXT_PRESSURE_COOLDOWN = 15 * 60
    ...
    def _context_pressure_tier(self, progress: float) -> float:
        if progress >= 0.95:
            return 0.95
        if progress >= 0.85:
            return 0.85
    ...
    def _emit_context_pressure(self, progress: float, compressor: Any) -> None:
        """Emit user-facing context-pressure warnings for CLI/gateway consumers."""
        try:
            threshold_tokens = int(getattr(compressor, 'threshold_tokens', 0) or 0)
            context_length = int(getattr(compressor, 'context_length', 0) or 0)
            threshold_ratio = (threshold_tokens / context_length) if threshold_tokens and context_length else 0.0
            is_cli = (self.platform or 'cli') == 'cli'
            if is_cli:
                message = format_context_pressure(
                    progress,
                    threshold_tokens,
                    threshold_ratio,
                ...
                message = format_context_pressure_gateway(
                    progress,
                    threshold_ratio,
                    compression_enabled=self.compression_enabled,
                )
            if self.status_callback:
                try:
                    self.status_callback('context_pressure', message)
                except Exception:
                    logger.debug('status_callback error in _emit_context_pressure', exc_info=True)
        except Exception:
            logger.debug('Failed to emit context pressure warning', exc_info=True)
    ...
    def _compress_context(self, messages: list, system_message: str, *, approx_tokens: int = None, task_id: str = "default", focus_topic: str = None) -> tuple:
        ...
        _post_ratio = (_compressed_est / self.context_compressor.threshold_tokens) if getattr(self.context_compressor, 'threshold_tokens', 0) else 0.0
        if _post_ratio < 0.85:
            self._context_pressure_warned_at = 0.0
            self.__class__._context_pressure_last_warned.pop(self.session_id or 'default', None)
