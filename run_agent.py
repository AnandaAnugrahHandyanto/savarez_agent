...
class AIAgent:
    ...
    _MEMORY_REVIEW_PROMPT = (
        "Review the conversation above for durable memory updates. "
        "Classify the destination first; usefulness alone is not enough to write "
        "to built-in memory.\n\n"
        "Routing rules:\n"
        "  • USER.md: only stable user identity, durable preferences, communication "
        "style, standing expectations, or recurring personal context that should "
        "shape future sessions.\n"
        "  • MEMORY.md: stable assistant/environment/tool/project facts, local "
        "conventions, or lessons about how this deployment works.\n"
        "  • session history: task progress, one-off outcomes, temporary TODO state, "
        "meeting/event minutiae, and facts useful only for recalling this session.\n"
        "  • skills: reusable procedures, workflow corrections, debugging paths, "
        "tool quirks, or any 'how to do this next time' learning.\n\n"
        "Skip memory writes for useful facts that belong in session history, skills, "
        "wiki/reference stores, or the current answer only. When memory is warranted, "
        "prefer replace or consolidation over add if the store is already crowded or "
        "an existing entry covers the same concept. If nothing belongs in built-in "
        "memory, just say 'Nothing to save.' and stop."
    )
    ...
    _COMBINED_REVIEW_PROMPT = (
        "Review the conversation above for two independent self-improvement "
        "dimensions. Act only where there is a real signal.\n\n"
        "**Memory**: Classify the memory destination first; usefulness alone is "
        "not enough to write to built-in memory. USER.md is only for stable "
        "user identity, durable preferences, communication style, standing "
        "expectations, or recurring personal context. MEMORY.md is only for "
        "stable assistant/environment/tool/project facts, local conventions, "
        "or lessons about how this deployment works. Task progress, one-off "
        "outcomes, and temporary state belong in session history; reusable "
        "procedures belong in skills; detailed reference material belongs in "
        "the appropriate reference/wiki store rather than built-in memory. "
        "Skip memory writes for useful-but-wrong-store facts. When memory is "
        "warranted, prefer replace or consolidation over add if an existing "
        "entry covers the same concept.\n\n"
        ...
        "you handled a task, update the relevant skill(s) that govern that "
        "task — memory alone isn't enough. Memory only captures the narrow "
        "durable facts above; skills say 'how to do this class of task for "
        "this user'.\n\n"
        ...
        "genuinely nothing belongs in memory or skills, say 'Nothing to save.' "
        "and stop."
    )
