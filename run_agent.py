...
class AIAgent:
    ...
    _SKILL_REVIEW_PROMPT = (
        ...
        "skill covers the class. Before calling skill_manage action=create, "
        "call skill_candidates with a concise summary of the proposed skill "
        "or task. Treat its output as a read-only candidate check across "
        "installed skills and hub metadata. If an installed candidate fits, "
        "patch or extend it instead. If a hub candidate is clearly better, "
        "mention that it should be installed rather than creating a local "
        "duplicate. Create only when no candidate covers the class. The "
    ...
    _COMBINED_REVIEW_PROMPT = (
        ...
        "Before creating, call skill_candidates with a concise summary of "
        "the proposed skill or task. If an installed candidate fits, patch "
        "or extend it. If a hub candidate is clearly better, mention that "
        "it should be installed instead of creating a local duplicate. "
        "Create only when no candidate covers the class. Name at the class "
        "level, NOT a PR number, error string, "
