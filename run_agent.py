...
class AIAgent:
    ...
    def _emit_stream_drop(
        ...
        *,
        ...
        try:
            self._emit_status(
                t("run_agent.stream_reconnect",
                  provider=provider, kind=kind,
                  error_type=type(error).__name__, suffix=_suffix,
    ...
    # Map auxiliary task names to i18n keys for user-visible translation.
    _AUX_TASK_I18N: dict[str, str] = {
        "background review": "run_agent.aux_task_background_review",
        "title generation": "run_agent.aux_task_title_generation",
    }
    ...
    def _emit_auxiliary_failure(self, task: str, exc: BaseException) -> None:
        ...
        task_key = self._AUX_TASK_I18N.get(task)
        task_display = t(task_key) if task_key else task
        self._emit_warning(t("run_agent.auxiliary_failed", task=task_display, detail=detail))
        ...
        def _run_review():
            ...
            try:
                ...
                if actions:
                    ...
                    self._safe_print(f"  {t('run_agent.self_improvement_review', summary=summary)}")
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    if response_invalid:
                        ...
                        if self._fallback_index < len(self._fallback_chain):
                            self._emit_status(t("run_agent.empty_malformed_response"))
                        if self._try_activate_fallback():
                            retry_count = 0
                            compression_attempts = 0
                            primary_recovery_attempted = False
                        ...
                        if retry_count >= max_retries:
                            ...
                            self._emit_status(t("run_agent.max_retries_invalid_try_fallback", max_retries=max_retries))
                            if self._try_activate_fallback():
                                retry_count = 0
                                compression_attempts = 0
                                primary_recovery_attempted = False
                                continue
                            self._emit_status(t("run_agent.max_retries_invalid_giving_up", max_retries=max_retries))
                        ...
                        self._vprint(f"{self.log_prefix}{t('run_agent.retrying_in', wait_time=f'{wait_time:.1f}', hint=_failure_hint)}", force=True)
                    ...
                    if finish_reason == "length":
                        self._emit_status(t("run_agent.response_truncated"))
                    ...
                    if is_rate_limited and self._fallback_index < len(self._fallback_chain):
                        ...
                        if not pool_may_recover:
                            self._emit_status(t("run_agent.rate_limited_switch_fallback"))
                    ...
                    if is_payload_too_large:
                        ...
                        self._emit_status(t("run_agent.payload_too_large", attempt=compression_attempts, max_attempts=max_compression_attempts))
                        ...
                        if len(messages) < original_len:
                            self._emit_status(t("run_agent.compressed_retrying", original=original_len, compressed=len(messages)))
                    ...
                    if is_context_length_error:
                        ...
                        self._emit_status(t("run_agent.context_too_large", tokens=f"{approx_tokens:,}", attempt=compression_attempts, max_attempts=max_compression_attempts))
                    ...
                    if is_client_error:
                        ...
                        self._emit_status(t("run_agent.non_retryable_error_try_fallback", status_code=status_code))
                        if self._try_activate_fallback():
                            retry_count = 0
                            compression_attempts = 0
                            primary_recovery_attempted = False
                        ...
                        self._emit_status(
                            t("run_agent.non_retryable_error", status_code=status_code,
                    ...
                    if retry_count >= max_retries:
                        ...
                        self._emit_status(t("run_agent.max_retries_exhausted_try_fallback", max_retries=max_retries))
                        if self._try_activate_fallback():
                            retry_count = 0
                            compression_attempts = 0
                            primary_recovery_attempted = False
                        ...
                        if is_rate_limited:
                            self._emit_status(t("run_agent.rate_limited_after_retries", max_retries=max_retries, summary=_final_summary))
                        else:
                            self._emit_status(t("run_agent.api_failed_after_retries", max_retries=max_retries, summary=_final_summary))
                    ...
                    if is_rate_limited:
                        self._emit_status(t("run_agent.rate_limited_waiting", wait_time=f"{wait_time:.1f}", attempt=retry_count + 1, max_retries=max_retries))
                    else:
                        self._emit_status(t("run_agent.retrying_in_attempt", wait_time=f"{wait_time:.1f}", attempt=retry_count, max_retries=max_retries))
                    logger.warning(
                ...
                if assistant_message.tool_calls:
                    ...
                    if not self._has_content_after_think_block(final_response):
                        ...
                        if fallback and getattr(self, '_last_content_tools_all_housekeeping', False):
                            ...
                            self._emit_status(t("run_agent.empty_response_after_tool_calls"))
                            self._last_content_with_tools = None
                            self._last_content_tools_all_housekeeping = False
                            self._empty_content_retries = 0
                        ...
                        if (
                            ...
                            self._emit_status(t("run_agent.empty_after_tool_calls_nudge"))
                        ...
                        if _has_structured and self._thinking_prefill_retries < 2:
                            ...
                            self._emit_status(
                                t("run_agent.thinking_only_prefilling", attempt=self._thinking_prefill_retries)
                            )
                        ...
                        if _truly_empty and (not _has_structured or _prefill_exhausted) and self._empty_content_retries < 3:
                            ...
                            self._emit_status(
                                t("run_agent.empty_response_retrying", attempt=self._empty_content_retries)
                            )
                            continue
                        ...
                        if _truly_empty and self._fallback_chain:
                            ...
                            self._emit_status(
                                t("run_agent.empty_response_switch_fallback")
                            )
                            if self._try_activate_fallback():
                                self._empty_content_retries = 0
                                self._emit_status(
                                    t("run_agent.switched_to_fallback", model=self.model, provider=self.provider)
                                )
                                logger.info(
                        ...
                        if reasoning_text:
                            ...
                            self._emit_status(t("run_agent.reasoning_no_visible"))
                        else:
                            logger.warning(
                                "Empty response (no content or reasoning) "
                            ...
                            self._emit_status(
                                t("run_agent.no_content_after_retries",
                                   suffix=(" and fallback attempts." if self._fallback_chain else
