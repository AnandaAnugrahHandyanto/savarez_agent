...
//! SAVAREZ_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for SAVAREZ_HOME. Tests use this; production
...
/// Spawn the locally-built Savarez desktop binary, then close the installer
...
pub async fn launch_hermes_desktop(
    ...
    let exe_path = resolve_hermes_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built Savarez desktop at {}. The desktop build step \
    ...
    tracing::info!(?exe_path, "launching Savarez desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/Savarez
...
pub(crate) fn resolve_hermes_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    ...
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "Savarez.exe"),
            ("win-arm64-unpacked", "Savarez.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/Savarez.app/Contents/MacOS", "Savarez"),
            ("mac-arm64/Savarez.app/Contents/MacOS", "Savarez"),
        ]
    } else {
...
pub(crate) fn resolve_hermes_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    ...
    #[cfg(target_os = "macos")]
    {
        // .../Savarez.app/Contents/MacOS/Savarez -> .../Savarez.app
...
/// path so a bare re-open just opens Savarez instead of re-running setup.
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_hermes_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built Savarez desktop app")
    })?;
...
async fn run_bootstrap(
    ...
    // explicitly; we infer it from $SavarezHome which Stage-Repository clones
    // the repo INTO at $SavarezHome\savarez-agent. Mirrors hermes_constants.
    let hermes_home = args
        .hermes_home
        .clone()
        .unwrap_or_else(|| crate::paths::hermes_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&hermes_home).join("savarez-agent");

    // Copy ourselves to SAVAREZ_HOME/hermes-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_hermes_home() {
        tracing::warn!(?err, "failed to copy installer into SAVAREZ_HOME (non-fatal)");
        emit_log(&format!(
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("Savarez"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../Savarez.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("Savarez.exe");
    ...
    // what the updater ditto's over /Applications/Savarez.app). A regression in
