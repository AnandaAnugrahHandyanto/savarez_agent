...
//!
//! Driven when the installer is launched as `Savarez-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Savarez desktop process to fully exit (so the venv
...
///.savarez_cli/main.py (sys.exit(2)). We surface a targeted message for this.
...
pub async fn start_update(app: AppHandle) -> Result<(), String> {
        ...
        .is_err()
    {
        ...
        let mut stages = vec![
            stage_info("update", "Updating Savarez"),
...
async fn run_update(app: AppHandle) -> Result<()> {
    ...
    let install_root = hermes_home.join("savarez-agent");
    ...
    let hermes = resolve_hermes(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the hermes CLI under {}. Is Savarez installed? \
    ...
    // and the guard would only produce a false "Savarez is still running" stop.
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Savarez" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Savarez is still running. Close all Savarez windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Savarez manually."),
            );
        }
    } else if let Err(err) =
...
async fn wait_for_venv_free(install_root: &Path, app: &AppHandle) {
    ...
    emit_log(app, Some("update"), LogStream::Stdout, "[update] waiting for Savarez to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            ...
            emit_log(
                ...
                Some("update"),
                LogStream::Stdout,
                "[update] Savarez still holding the venv shim; force-killing stragglers…",
            );
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_hermes_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Savarez.app was found under {}",
...
mod tests {
    ...
    fn venv_hermes_is_under_install_root() {
        let root = Path::new("/x/savarez-agent");
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Savarez.app"]),
            Some(PathBuf::from("/Applications/Savarez.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Savarez.app");
        let tmp = base.join("Savarez.app.hermes-update-new");
        let old = base.join("Savarez.app.hermes-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Savarez.app");
        let tmp = base.join("Savarez.app.hermes-update-new"); // intentionally absent
        let old = base.join("Savarez.app.hermes-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Savarez.app");
        let tmp = base.join("Savarez.app.hermes-update-new"); // absent
        let old = base.join("Savarez.app.hermes-update-old");
        write_marker(&target, "OLD");
