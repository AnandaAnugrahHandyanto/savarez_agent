//! Savarez Setup — Tauri entrypoint.
//!
...
/// desktop app handing off via `Savarez-Setup.exe --update`).
///
...
pub fn run() {
    ...
    tracing::info!(?mode, force_setup, "Savarez installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // "Savarez" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "Savarez" shortcuts that install.ps1 creates, and a
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::hermes_home().join("savarez-agent");
        ...
        .expect("error while running Savarez Setup");
}
