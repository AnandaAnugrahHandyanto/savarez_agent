...
type SavarezRefType = (typeof HERMES_REF_TYPES)[number]
...
const ICON_PATHS: Record<SavarezRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as SavarezRefType] ?? ICON_FALLBACK
  ...
  const matches = [
    ...
    ...Array.from(text.matchAll(HERMES_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as SavarezRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: SavarezRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing Savarez directives (`@file:...`, `@image:...`) as
