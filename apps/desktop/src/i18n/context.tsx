...
import { getSavarezConfigRecord, type SavarezConfigRecord, saveSavarezConfig } from '@/hermes'
...
export interface I18nConfigClient {
  getConfig: () => Promise<SavarezConfigRecord>
  saveConfig: (config: SavarezConfigRecord) => Promise<{ ok: boolean }>
}
...
const defaultConfigClient: I18nConfigClient = {
  ...
  saveConfig: config => {
    ...
    return saveSavarezConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: SavarezConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: SavarezConfigRecord, locale: Locale): SavarezConfigRecord {
