...
const KEY_TABS = [
  { icon: Wrench, id: 'tool', labelKey: 'settings.keys.categories.tools' },
  { icon: Settings2, id: 'setting', labelKey: 'settings.keys.categories.settings' }
] as const
...
export function KeysSettings() {
  ...
  const groups = useMemo(() => {
    ...
    return KEY_TABS.map(t => t.id).flatMap(tab => {
      ...
      return entries.length === 0 ? [] : [{ category: tab, entries }]
    })
  }, [vars])
