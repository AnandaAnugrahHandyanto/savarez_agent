...
import { getSavarezConfigRecord, type SavarezGateway, saveSavarezConfig } from '@/hermes'
import { useI18n } from '@/i18n'
import { Wrench } from '@/lib/icons'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import { $activeSessionId } from '@/store/session'
import type { SavarezConfigRecord } from '@/types/hermes'

import { EmptyState, LoadingState, Pill, SettingsContent } from './primitives'
import { useDeepLinkHighlight } from './use-deep-link-highlight'

interface McpSettingsProps {
  gateway?: SavarezGateway | null
  onConfigSaved?: () => void
}
...
function getServers(config: SavarezConfigRecord | null): McpServers {
...
export function McpSettings({ gateway, onConfigSaved }: McpSettingsProps) {
  ...
  const [config, setConfig] = useState<SavarezConfigRecord | null>(null)
  ...
  const saveServer = async () => {
    ...
    try {
      ...
      await saveSavarezConfig(nextConfig)
