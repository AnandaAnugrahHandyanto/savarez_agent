...
// bug that drives the REAL useGatewayBoot hook + REAL SavarezGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshSavarezConfig: async () => undefined,
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForSavarez) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startSavarez()'s remote branch awaits waitForSavarez() for 45s before it
    ...
    // After ~45s waitForSavarez gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Savarez backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
