...
import { SavarezGateway } from '@/hermes'
...
interface GatewayBootOptions {
  ...
  onGatewayReady: (gateway: SavarezGateway | null) => void
  refreshSavarezConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: SavarezConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Savarez...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Savarez…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Savarez gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshSavarezConfig().catch(() => undefined)
    ...
    const gateway = new SavarezGateway()
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshSavarezConfig()
