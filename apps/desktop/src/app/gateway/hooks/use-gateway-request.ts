...
export function useGatewayRequest() {
  ...
  const gatewayRef = useRef<SavarezGateway | null>(null)
  ...
  const reconnectingRef = useRef<Promise<SavarezGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as SavarezGateway | null
      }),
    []
  )
  ...
  const requestGateway = useCallback(
    async <T>(method: string, params: Record<string, unknown> = {}) => {
      ...
      if (!gateway) {
        throw new Error('Savarez gateway unavailable')
      }
