...
import type { SavarezReadDirEntry, SavarezReadDirResult } from '@/global'

export type ProjectTreeEntry = SavarezReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: SavarezReadDirEntry) {
...
async function filterIgnored(entries: SavarezReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<SavarezReadDirResult> {
