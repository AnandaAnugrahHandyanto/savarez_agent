...
import type { SavarezReadDirResult } from '@/global'

import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<SavarezReadDirResult>>()
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): SavarezReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: SavarezReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<SavarezReadDirResult>(resolve => {
