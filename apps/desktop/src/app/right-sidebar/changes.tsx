...
export function ChangesPanel({ profile, sessionId }: ChangesPanelProps) {
  ...
  const canOpenFullFile = connection?.mode === 'local' && (!profile || profile === 'default')
  ...
  const openFullFileTab = useCallback(
    async (turn: SessionTurnChange, file: SessionTurnChangeFile) => {
      ...
      try {
        if (!canOpenFullFile) {
          throw new Error('Full-file preview is only available for local sessions.')
        }
    ...
    [canOpenFullFile, currentCwd, sessionId]
  )
                      ...
                      {turn.files.map(file => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <Tip label={file.status === 'deleted' ? 'File was deleted' : canOpenFullFile ? 'Open full file' : 'Full file is local-only for now'}>
                              <Button
                                ...
                                disabled={file.status === 'deleted' || !canOpenFullFile}
