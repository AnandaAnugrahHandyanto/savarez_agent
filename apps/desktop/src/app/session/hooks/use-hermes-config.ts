...
import { getSavarezConfig, getSavarezConfigDefaults } from '@/hermes'
...
interface SavarezConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useSavarezConfig({ activeSessionIdRef, refreshProjectBranch }: SavarezConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshSavarezConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getSavarezConfig(), getSavarezConfigDefaults().catch(() => ({}))])
  ...
  return { refreshSavarezConfig, sttEnabled, voiceMaxRecordingSeconds }
}
