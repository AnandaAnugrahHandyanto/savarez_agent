...
const DESKTOP_COMMAND_META = [
  ...
  ['/profile', 'Switch the active Savarez profile'],
  ...
  ['/version', 'Show Savarez Agent version'],
...
export function isDesktopSlashCommand(command: string): boolean {
  ...
  return DESKTOP_COMMANDS.has(canonical) || !isKnownSavarezSlashCommand(normalized)
}

/**
 * An "extension" command is anything the backend surfaces that is NOT one of
 * Savarez' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownSavarezSlashCommand(normalized)
}
...
function isKnownSavarezSlashCommand(command: string): boolean {
