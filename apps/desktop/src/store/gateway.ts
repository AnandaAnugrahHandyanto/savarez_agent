...
const isOpen = (gateway: SavarezGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<SavarezGateway | null>(null)
...
let primaryGateway: SavarezGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: SavarezGateway | null, profile = 'default'): void {
...
export function activeGateway(): SavarezGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<SavarezGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
