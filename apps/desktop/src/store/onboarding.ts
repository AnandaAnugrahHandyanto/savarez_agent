...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Savarez is ready', message: `${provider} connected.` })
}
...
async function fetchProviderDefaultModel(
  ...
  // `savarez model` does (for Nous it honors the user's free/paid tier, so a
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Savarez still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Savarez still cannot resolve a usable provider.'
}
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Savarez still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Savarez still cannot reach ${url}.` }
    }
