...
// no longer bundles the Savarez AI Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Savarez.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Savarez'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Savarez.exe'),
...
// Default SAVAREZ_HOME for non-sandboxed runs -- matches main.cjs's
// resolveSavarezHome(). On Windows it's %LOCALAPPDATA%\hermes; elsewhere
// it's ~/.savarez. The fresh-install sandbox launchFresh() sets its own
// SAVAREZ_HOME and never touches this.
const DEFAULT_SAVAREZ_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'hermes')
  }
  return path.join(os.homedir(), '.hermes')
})()
const VENV_ROOT = path.join(DEFAULT_SAVAREZ_HOME, 'savarez-agent', 'venv')
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Savarez-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Savarez-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Savarez-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Savarez-${version}-${os}-${arch}.${ext}'
...
function launchFresh() {
  ...
  console.log(`  SAVAREZ_HOME: ${hermesHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(hermesHome, 'savarez-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Savarez AI Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  const staleFactoryMarker = path.join(APP.resourcesPath, 'savarez-agent', 'hermes_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Savarez
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + SAVAREZ_HOME
