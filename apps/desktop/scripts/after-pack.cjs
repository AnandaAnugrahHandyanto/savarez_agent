 ...
 *
 * Stamps the Savarez icon + identity onto the packed Windows Savarez.exe via
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Savarez')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Savarez'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Savarez.exe keeps the stock Electron icon`)
  }
}
