#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Savarez icon + version metadata onto the
// built Savarez.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Savarez.exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.cjs <path-to-Savarez.exe>
//
...
// Stamp the Savarez icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Savarez',
      FileDescription: 'Savarez',
      CompanyName: 'Nous Research',
      LegalCopyright: 'Copyright (c) 2026 Nous Research'
    }
  })

  console.log('[set-exe-identity] done — Savarez icon + identity stamped')
}
