...
function normalizeRequestedProfileKey(profile) {
  const requested = profile && String(profile).trim() ? String(profile).trim() : primaryProfileKey()
  const primary = primaryProfileKey()

  // When Desktop is pinned to a non-default primary profile, stale renderer state
  // may still ask Electron for profile "default". Treat that as the primary
  // backend instead of spawning a second dashboard that can collide on ports.
  if (requested === 'default' && primary !== 'default') {
    return primary
  }
...
async function ensureBackend(profile) {
  const key = normalizeRequestedProfileKey(profile)
