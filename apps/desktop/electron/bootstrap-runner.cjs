 ...
 *
 * Drives apps/desktop's first-launch install of Savarez AI Agent by spawning
...
// checkout under ~/.hermes/savarez-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(hermesHome) {
  if (!hermesHome) return null
  const candidate = path.join(hermesHome, 'savarez-agent', 'scripts', installScriptName())
  try {
    fs.accessSync(candidate, fs.constants.R_OK)
    return candidate
  } catch {
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/AnandaAnugrahHandyanto/savarez_agent/${commit}/scripts/${scriptName}`
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, hermesHome } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(ps, fullArgs, hiddenWindowsChildOptions({
      ...
      env: {
        ...process.env,
        // Pass SAVAREZ_HOME through so install.ps1 respects the caller's
        // choice rather than re-computing the default.
        SAVAREZ_HOME: hermesHome || process.env.SAVAREZ_HOME || ''
      }
    }))
