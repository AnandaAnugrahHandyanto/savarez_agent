 ...
 *
 *   macOS:   …/Savarez.app/Contents/MacOS/Savarez  → …/Savarez.app
 *   Windows: …\Savarez\Savarez.exe                 → …\Savarez  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env = {}) {
  ...
  if (platform === 'darwin') {
    // …/Savarez.app/Contents/MacOS/Savarez → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Savarez.app
    if (appBundle.endsWith('.app')) return appBundle
    return null
  }

  if (platform === 'win32') {
    // NSIS per-user installs Savarez.exe directly in the install dir.
    const dir = p.dirname(exe)
    if (/[\\/]Savarez$/i.test(dir) || /[\\/]hermes-desktop$/i.test(dir)) return dir
    return null
  }
...
function buildWindowsCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, hermesHome }) {
  ...
  // a problem, but Savarez install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
  const lines = [
    '@echo off',
