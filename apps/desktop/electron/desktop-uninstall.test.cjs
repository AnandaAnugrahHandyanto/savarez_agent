...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Savarez.app/Contents/MacOS/Savarez', 'darwin'),
    '/Applications/Savarez.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Savarez.app/Contents/MacOS/Savarez', 'darwin'),
    '/Users/x/Applications/Savarez.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Savarez\\Savarez.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Savarez'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\hermes-desktop\\Savarez.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\hermes-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Savarez.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_SavarezXXXX/hermes', 'linux', { APPIMAGE: '/home/x/Apps/Savarez.AppImage' }),
    '/home/x/Apps/Savarez.AppImage'
  )
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Savarez.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Savarez.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.hermes/savarez-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.hermes/savarez-agent',
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.hermes/savarez-agent',
    agentRoot: '/home/x/.hermes/savarez-agent',
  ...
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.hermes\/savarez-agent'/)
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\hermes',
    uninstallArgs: ['-m', 'hermes_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Savarez',
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Savarez" >nul 2>&1/)
