...
def _resolve_cron_progress_config(job: dict, cfg: Any) -> dict:
    ...
    ``enabled: true`` activates progress for every cron run, including
    no-agent script jobs. ``enabled: auto`` uses a conservative classifier so
    normal reports and watchdogs stay quiet by default. Per-job ``progress``
    dicts may override these fields. Environment overrides are available for
    operators debugging a live gateway: ``HERMES_CRON_PROGRESS_ENABLED``,
    ``HERMES_CRON_PROGRESS_INITIAL_DELAY`` and ``HERMES_CRON_PROGRESS_INTERVAL``.
    """
...
def _deliver_cron_progress_update(
    ...
    *,
    ...
    This intentionally does *not* use the agent's ``messaging`` toolset. Cron
    agents run with interactive messaging disabled, so progress belongs in the
    scheduler/runtime wrapper. Failures are logged and returned but never raise
    into the cron run.
    """
    targets = _resolve_delivery_targets(job)
    if not targets:
