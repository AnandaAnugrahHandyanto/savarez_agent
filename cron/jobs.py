...
_SUPPORTED_WORKFLOW_BINDING_TYPES = frozenset({
...
def _normalize_job_record(job: Dict[str, Any]) -> Dict[str, Any]:
    ...
    workflow_binding_type = _coerce_job_text(
        normalized.get("workflow_binding_type")
    ).strip()
    workflow_binding_id = _coerce_job_text(
        normalized.get("workflow_binding_id")
    ).strip()
    normalized["workflow_binding_type"] = workflow_binding_type or None
    normalized["workflow_binding_id"] = workflow_binding_id or None

    return normalized


def _normalize_workflow_binding_fields(
    workflow_binding_type: Optional[Any], workflow_binding_id: Optional[Any]
) -> tuple[Optional[str], Optional[str]]:
    """Normalize optional cron workflow-binding fields.

    Workflow bindings are opt-in and must always travel as a pair. This slice
    only supports bound WhatsApp outreach plans.
    """

    normalized_type = _coerce_job_text(workflow_binding_type).strip() or None
    normalized_id = _coerce_job_text(workflow_binding_id).strip() or None

    if bool(normalized_type) != bool(normalized_id):
        raise ValueError(
            "workflow_binding_type and workflow_binding_id must be provided together"
        )
    if normalized_type and normalized_type not in _SUPPORTED_WORKFLOW_BINDING_TYPES:
        raise ValueError(f"unsupported workflow_binding_type: {normalized_type}")
...
def create_job(
    ...
    Args:
        ...
        workflow_binding_type: Optional canonical workflow binding type.
                This slice supports ``whatsapp_outreach_plan``.
        workflow_binding_id: Optional canonical workflow binding identifier.
                Must be provided together with ``workflow_binding_type``.
    ...
    normalized_workflow_binding_type, normalized_workflow_binding_id = (
        _normalize_workflow_binding_fields(
    ...
    job = {
        ...
        "workflow_binding_type": normalized_workflow_binding_type,
        "workflow_binding_id": normalized_workflow_binding_id,
    }
...
def update_job(job_id: str, updates: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    for i, job in enumerate(jobs):
        ...
        if "workflow_binding_type" in updates or "workflow_binding_id" in updates:
            binding_type, binding_id = _normalize_workflow_binding_fields(
                updates.get("workflow_binding_type", job.get("workflow_binding_type")),
                updates.get("workflow_binding_id", job.get("workflow_binding_id")),
            )
            updates["workflow_binding_type"] = binding_type
            updates["workflow_binding_id"] = binding_id
