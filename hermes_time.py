"""
Timezone-aware clock for Savarez.
...
Resolution order:
  ...
  2. ``timezone`` key in ``~/.savarez/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — Savarez never
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — Savarez requires 3.9+)
