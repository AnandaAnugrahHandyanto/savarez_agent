...
def create_test_dataset(num_prompts: int = 20) -> Path:
    ...
    with open(dataset_file, "w", encoding="utf-8") as f:
        for i in range(num_prompts):
            entry = {"prompt": f"Test prompt {i}: What is 2+2? Just answer briefly.", "test_id": i}
...
def test_current_implementation():
    ...
    if len(snapshots) == 0:
        ...
        for i, snapshot in enumerate(snapshots, 1):
            print(f"   {i}. [{snapshot['elapsed_seconds']:6.2f}s] {snapshot['completed_count']} prompts completed")
