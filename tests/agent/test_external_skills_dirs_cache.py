...
def hermes_home_with_config(tmp_path, monkeypatch):
    """Isolated ``~/.savarez/`` with a config.yaml referencing one external dir."""
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    """Two different SAVAREZ_HOMEs keep separate cache entries."""
    home_a = tmp_path / "home_a" / ".savarez"
    ...
    home_b = tmp_path / "home_b" / ".savarez"
    ...
    monkeypatch.setenv("SAVAREZ_HOME", str(home_a))
    assert get_external_skills_dirs() == [ext_a.resolve()]

    monkeypatch.setenv("SAVAREZ_HOME", str(home_b))
