...
def test_resolve_api_key_provider_skips_unconfigured_copilot_with_generic_github_token(
    ...
    def fail_if_called(provider_id):
        ...
        raise AssertionError(
            "unconfigured Copilot must not resolve generic GitHub tokens"
        )

    monkeypatch.setenv("GITHUB_TOKEN", "ghp_classic_pat_for_git_only")
    monkeypatch.setattr(
        "agent.auxiliary_client._select_pool_entry", lambda _pid: (False, None)
    )
    monkeypatch.setattr("hermes_cli.auth.PROVIDER_REGISTRY", fake_registry)
    monkeypatch.setattr(
        "hermes_cli.auth.resolve_api_key_provider_credentials", fail_if_called
    )
...
def test_resolve_api_key_provider_allows_explicit_copilot_token_env(monkeypatch):
    ...
    monkeypatch.setattr("hermes_cli.auth.PROVIDER_REGISTRY", fake_registry)
    monkeypatch.setattr(
        "hermes_cli.auth.resolve_api_key_provider_credentials", empty_creds
    )
