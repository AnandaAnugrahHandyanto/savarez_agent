...
class TestReadCodexAccessToken:
    ...
    def test_pool_without_selected_entry_falls_back_to_auth_store(self, tmp_path, monkeypatch):
        ...
        with patch("hermes_agent.agent.auxiliary_client._select_pool_entry", return_value=(True, None)), \
             patch("hermes_agent.cli.auth._read_codex_tokens", return_value={
...
class TestTryCodex:
    def test_pool_without_selected_entry_falls_back_to_auth_store(self):
        with (
            ...
            from hermes_agent.agent.auxiliary_client import _try_codex
...
class TestExpiredCodexFallback:
    ...
    def test_expired_codex_falls_through_to_next(self, tmp_path, monkeypatch):
        ...
        with patch("hermes_agent.agent.anthropic_adapter.build_anthropic_client") as mock_build:
            mock_build.return_value = MagicMock()
            from hermes_agent.agent.auxiliary_client import _resolve_auto, AnthropicAuxiliaryClient
...
class TestGetTextAuxiliaryClient:
    ...
    def test_codex_pool_entry_takes_priority_over_auth_store(self):
        ...
        with (
            ...
            patch("hermes_agent.cli.auth._read_codex_tokens", side_effect=AssertionError("legacy codex store should not run")),
        ):
            from hermes_agent.agent.auxiliary_client import _try_codex
...
class TestNousAuxiliaryRefresh:
    def test_try_nous_prefers_runtime_credentials(self):
        ...
        with (
            patch("hermes_agent.agent.auxiliary_client._read_nous_auth", return_value={"access_token": "stale-token"}),
            patch("hermes_agent.agent.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("hermes_agent.cli.models.get_nous_recommended_aux_model", return_value=None),
            patch("hermes_agent.agent.auxiliary_client.OpenAI") as mock_openai,
        ):
    ...
    def test_try_nous_uses_portal_recommendation_for_text(self):
        ...
        with (
            ...
            patch("hermes_agent.agent.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("hermes_agent.cli.models.get_nous_recommended_aux_model", return_value="minimax/minimax-m2.7") as mock_rec,
            patch("hermes_agent.agent.auxiliary_client.OpenAI") as mock_openai,
        ):
            from hermes_agent.agent.auxiliary_client import _try_nous
    ...
    def test_try_nous_uses_portal_recommendation_for_vision(self):
        ...
        with (
            patch("hermes_agent.agent.auxiliary_client._read_nous_auth", return_value={"access_token": "***"}),
            patch("hermes_agent.agent.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("hermes_agent.cli.models.get_nous_recommended_aux_model", return_value="google/gemini-3-flash-preview") as mock_rec,
            patch("hermes_agent.agent.auxiliary_client.OpenAI"),
        ):
            from hermes_agent.agent.auxiliary_client import _try_nous
    ...
    def test_try_nous_falls_back_when_recommendation_lookup_raises(self):
        ...
        with (
            patch("hermes_agent.agent.auxiliary_client._read_nous_auth", return_value={"access_token": "***"}),
            patch("hermes_agent.agent.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", fresh_base)),
            patch("hermes_agent.cli.models.get_nous_recommended_aux_model", side_effect=RuntimeError("portal down")),
            patch("hermes_agent.agent.auxiliary_client.OpenAI"),
        ):
            from hermes_agent.agent.auxiliary_client import _try_nous
            client, model = _try_nous()
...
class TestTryPaymentFallback:
    ...
    def test_returns_none_when_no_fallback(self):
        with patch("hermes_agent.agent.auxiliary_client._try_openrouter", return_value=(None, None)), \
             ...
             patch("hermes_agent.agent.auxiliary_client._try_codex", return_value=(None, None)), \
    ...
    def test_skips_to_codex_when_or_and_nous_fail(self):
        ...
        with patch("hermes_agent.agent.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("hermes_agent.agent.auxiliary_client._try_nous", return_value=(None, None)), \
             patch("hermes_agent.agent.auxiliary_client._try_custom_endpoint", return_value=(None, None)), \
             patch("hermes_agent.agent.auxiliary_client._try_codex", return_value=(mock_codex, "gpt-5.2-codex")), \
             patch("hermes_agent.agent.auxiliary_client._read_main_provider", return_value="openrouter"):
            client, model, label = _try_payment_fallback("openrouter")
...
def test_resolve_api_key_provider_skips_unconfigured_anthropic(monkeypatch):
    ...
    from hermes_agent.cli.auth import ProviderConfig
...
class TestAuxiliaryTaskExtraBody:
    def test_sync_call_merges_task_extra_body_from_config(self):
        ...
        with patch("hermes_agent.cli.config.load_config", return_value=config), patch(
            "agent.auxiliary_client._get_cached_client",
