...
class TestBuildSkillsSystemPrompt:
    ...
    def test_builds_category_only_index_with_skills(self, monkeypatch, tmp_path):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    ...
    def test_deduplicates_skills(self, monkeypatch, tmp_path):
        ...
        result = build_skills_system_prompt()
        # Skill names are intentionally omitted from the category-only index;
        # duplicate skills should still collapse into a single category count.
        assert "  tools: (1 skill)" in result
        assert "- search" not in result
        assert "Search stuff" not in result
    ...
    def test_excludes_incompatible_platform_skills(self, monkeypatch, tmp_path):
        ...
        assert "  apple: (1 skill)" in result
    ...
    def test_rebuilds_prompt_when_disabled_skills_change(self, monkeypatch, tmp_path):
        ...
        assert "  tools: (1 skill)" in first
        assert "cached-skill" not in first
    ...
    def test_includes_setup_needed_skills(self, monkeypatch, tmp_path):
        ...
        result = build_skills_system_prompt()
        assert "  media: (2 skills)" in result
    ...
    def test_includes_skills_with_met_prerequisites(self, monkeypatch, tmp_path):
        ...
        result = build_skills_system_prompt()
        assert "  media: (1 skill)" in result
...
class TestBuildSkillsSystemPromptConditional:
    ...
    def test_fallback_skill_shown_when_primary_unavailable(self, monkeypatch, tmp_path):
        ...
        assert "  search: (1 skill)" in result
        assert "duckduckgo" not in result
