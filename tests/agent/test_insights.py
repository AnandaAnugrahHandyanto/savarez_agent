...
def populated_db(db):
    ...
    db.append_message(
        "s1",
        role="assistant",
        content="Let me load the E2E skill.",
        tool_calls=[{"function": {"name": "skill_view", "arguments": '{"name":"playwright-authenticated-local-e2e"}'}}],
    )
    ...
    db.append_message(
        "s4",
        role="assistant",
        content="Load and update skill metadata.",
        tool_calls=[
            {"function": {"name": "skill_view", "arguments": '{"name":"playwright-authenticated-local-e2e"}'}},
            {"function": {"name": "skill_manage", "arguments": '{"name":"repo-docs-local-notes-split"}'}},
        ],
    )
...
class TestInsightsPopulated:
    ...
    def test_skill_breakdown(self, populated_db):
        ...
        assert top_skill["skill"] == "playwright-authenticated-local-e2e"
