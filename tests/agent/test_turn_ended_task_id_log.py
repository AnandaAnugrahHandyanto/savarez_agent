...
``effective_task_id`` in the conversation loop. This test guards that the Turn-ended
diagnostic log line carries that id as ``task=<id>`` and that the format string and
argument tuple stay in lockstep (a mismatch would raise at logging time).

This is a source-contract test: the emit block builds a local ``_diag_msg`` /
``_diag_args`` pair and passes it to both logger.info and logger.warning. We assert:
...
CONV_LOOP = Path(__file__).resolve().parents[2] / "agent" / "conversation_loop.py"
