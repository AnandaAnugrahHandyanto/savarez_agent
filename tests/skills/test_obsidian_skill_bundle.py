...
def test_skills_catalog_lists_additive_obsidian_skills():
    ...
    required_rows = {
        ...
        "obsidian-markdown": "note-taking/obsidian-markdown",
        "obsidian-cli": "note-taking/obsidian-cli",
        "obsidian-bases": "note-taking/obsidian-bases",
        "json-canvas": "note-taking/json-canvas",
    }

    missing = []
    for name, path in required_rows.items():
        row_pattern = (
            rf"\| (?:\[`{re.escape(name)}`\]\([^)]+\)|`{re.escape(name)}`) "
            rf"\| .* \| `{re.escape(path)}` \|"
        )
        if not re.search(row_pattern, catalog):
            missing.append(name)
