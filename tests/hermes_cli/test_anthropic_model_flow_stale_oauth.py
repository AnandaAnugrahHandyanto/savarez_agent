"""Tests for Bug #12905 fix — stale OAuth token detection in savarez model flow.

Bug 3: `savarez model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.savarez/.env but no valid
