...
def test_setup_cli_parser_accepts_voice_section():
    ...
    assert "hermes setup model|tts|terminal|gateway|tools|voice|agent" in main_source
    assert '"voice"' in main_source
...
def test_full_setup_runs_voice_dispatch_section(tmp_path, monkeypatch):
    ...
    assert calls == ["model", "terminal", "agent", "gateway", "tools", "voice"]
