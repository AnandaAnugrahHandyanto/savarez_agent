...
def _isolated_hermes_home(tmp_path):
    """Point SAVAREZ_HOME at a temp dir so tests never touch real config."""
    env_file = tmp_path / ".env"
    env_file.touch()
    with patch.dict(os.environ, {"SAVAREZ_HOME": str(tmp_path)}):
        yield tmp_path
...
class TestListNavigation:
    """savarez config set must preserve YAML list fields when using numeric
