...
subparser, placing the flag BEFORE the subcommand (e.g. 'savarez --yolo chat')
...
def _build_parser():
    """Build the savarez argument parser from the real code.
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `savarez gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `savarez <argv>` must exit 0 (help) rather than
