...
When ``SAVAREZ_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.savarez/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.savarez            (has its own auth.json fixture)
    * Profile     -> tmp_path/.savarez/profiles/coder   (active, SAVAREZ_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Savarez is not
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (SAVAREZ_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.savarez as the "real home" — which is NOT equal
    # to the SAVAREZ_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    hermes_home = tmp_path / "classic"
    ...
    # Classic mode: SAVAREZ_HOME is set to a custom path that is NOT under
    # ~/.savarez/profiles/ — get_default_hermes_root() returns SAVAREZ_HOME
