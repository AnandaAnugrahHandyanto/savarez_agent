...
Before the fix, ``savarez whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``savarez gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    savarez = home / ".savarez"
    savarez.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("SAVAREZ_HOME", str(savarez))
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``savarez whatsapp`` with an existing paired session and
