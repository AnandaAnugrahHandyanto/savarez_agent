"""Tests for savarez backup and import commands."""
...
def _make_hermes_tree(root: Path) -> None:
    """Create a realistic ~/.savarez directory structure for testing."""
    ...
    # savarez-agent repo (should be EXCLUDED)
    (root / "savarez-agent").mkdir(exist_ok=True)
    (root / "savarez-agent" / "run_agent.py").write_text("# big file\n")
    (root / "savarez-agent" / ".git").mkdir()
    (root / "savarez-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_hermes_agent(self):
        ...
        assert _should_exclude(Path("savarez-agent/run_agent.py"))
        assert _should_exclude(Path("savarez-agent/.git/HEAD"))
...
class TestBackup:
    ...
    def test_excludes_hermes_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include savarez-agent/ directory."""
        hermes_home = tmp_path / ".savarez"
        hermes_home.mkdir()
        _make_hermes_tree(hermes_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "savarez-agent" in n]
            assert agent_files == [], f"savarez-agent files leaked into backup: {agent_files}"
    ...
    def test_includes_nested_hermes_agent_in_skills(self, tmp_path, monkeypatch):
        ...
        (nested / "SKILL.md").write_text("# Savarez Agent Skill\n")
        (nested / "sub").mkdir()
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/savarez-backup-*.zip."""
        hermes_home = tmp_path / ".savarez"
        hermes_home.mkdir()
        ...
        zips = list(tmp_path.glob("savarez-backup-*.zip"))
        assert len(zips) == 1

    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside SAVAREZ_HOME."""
        hermes_home = tmp_path / ".savarez"
        hermes_home.mkdir()
        _make_hermes_tree(hermes_home)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Savarez backup."""
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into savarez home."""
        hermes_home = tmp_path / ".savarez"
        hermes_home.mkdir()
        monkeypatch.setenv("SAVAREZ_HOME", str(hermes_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_strips_hermes_prefix(self, tmp_path, monkeypatch):
        """Import strips .savarez/ prefix if all entries share it."""
        hermes_home = tmp_path / ".savarez"
        hermes_home.mkdir()
        monkeypatch.setenv("SAVAREZ_HOME", str(hermes_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".savarez/config.yaml": "model: test\n",
            ".savarez/skills/a/SKILL.md": "# A\n",
        })
    ...
    def test_rejects_non_hermes_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a savarez backup."""
        hermes_home = tmp_path / ".savarez"
        hermes_home.mkdir()
        monkeypatch.setenv("SAVAREZ_HOME", str(hermes_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        # traversal file should NOT exist outside savarez home
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".savarez"
        src_home.mkdir(parents=True)
        _make_hermes_tree(src_home)

        monkeypatch.setenv("SAVAREZ_HOME", str(src_home))
        ...
        dst_home = tmp_path / "dest" / ".savarez"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("SAVAREZ_HOME", str(dst_home))
        ...
        # savarez-agent should NOT be present
        assert not (dst_home / "savarez-agent").exists()
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without savarez markers fails validation."""
        import io
        from hermes_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_hermes(self):
        """Detects .savarez/ prefix wrapping all entries."""
        import io
        from hermes_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".savarez/config.yaml", "test")
            zf.writestr(".savarez/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".savarez/"
...
class TestBackupEdgeCases:
    def test_nonexistent_hermes_home(self, tmp_path, monkeypatch):
        """Backup exits when savarez home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".savarez"
        monkeypatch.setenv("SAVAREZ_HOME", str(fake_home))
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("savarez-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_hermes_home(self, tmp_path, monkeypatch):
        """Backup handles empty savarez home (no files to back up)."""
        hermes_home = tmp_path / ".savarez"
        hermes_home.mkdir()
    ...
    def test_skips_output_zip_inside_hermes(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside savarez root."""
        hermes_home = tmp_path / ".savarez"
        hermes_home.mkdir()
        (hermes_home / "config.yaml").write_text("model: test\n")
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert "savarez -p coder" in coder_wrapper
...
class TestQuickSnapshot:
    ...
    def hermes_home(self, tmp_path):
        """Create a fake SAVAREZ_HOME with critical state files."""
...
# ---------------------------------------------------------------------------
# Pre-update backup (savarez update safety net)
# ---------------------------------------------------------------------------

class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``savarez update``
    runs before touching anything."""

    @pytest.fixture
    def hermes_home(self, tmp_path):
        root = tmp_path / ".savarez"
        root.mkdir()
    ...
    def test_backup_contents_match_full_backup(self, hermes_home):
        ...
        ``savarez backup`` would, and should exclude the same directories."""
        from hermes_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(hermes_home=hermes_home)
        assert out is not None
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # savarez-agent repo excluded
        assert not any(n.startswith("savarez-agent/") for n in names)
    ...
    def test_skips_symlinked_files(self, hermes_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside SAVAREZ_HOME."""
        from hermes_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    def hermes_home(self, tmp_path, monkeypatch):
        ...
        root.mkdir()
        _make_hermes_tree(root)
        # Point SAVAREZ_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("SAVAREZ_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for hermes_cli.config + hermes_constants so they pick up SAVAREZ_HOME
...
# ---------------------------------------------------------------------------
# Pre-migration backup (savarez claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``savarez claw migrate`` runs before mutating ~/.savarez/."""
    ...
    def test_creates_backup_under_backups_dir(self, hermes_home):
        ...
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `savarez
    ...
    def test_backup_uses_shared_exclusion_rules(self, hermes_home):
        ...
        ``savarez backup`` / ``create_pre_update_backup`` — no drift."""
        from hermes_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(hermes_home=hermes_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_restorable_with_hermes_import(self, hermes_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Savarez
        backup — `savarez import` should accept it."""
    ...
    def test_missing_hermes_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.savarez yet — nothing to back up."""
        from hermes_cli.backup import create_pre_migration_backup
...
class TestRestoreCronJobsIfEmptied:
    """`savarez update` config migration can leave cron/jobs.json valid-but-empty,
