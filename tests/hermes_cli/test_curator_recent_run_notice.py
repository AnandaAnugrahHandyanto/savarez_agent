...
The notice prints the most recent curator run summary on `savarez update`,
...
session start) so users normally never see the rename map. `savarez update`
...
def test_prints_multiline_summary_with_rename_map(curator_env):
    ...
    summary = (
        ...
        "full report: savarez curator status"
    )
    _set_state(
        curator_env["curator"],
        last_run_at=now,
