"""Tests for --ignore-user-config and --ignore-rules flags on `savarez chat`.
...
files. In Savarez the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.savarez/config.yaml`` in ``load_cli_config()``
...
argparse → cmd_chat → cli.main() → SavarezCLI → AIAgent call chain.
"""
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``SavarezCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting HERMES_IGNORE_RULES=1 flips SavarezCLI.ignore_rules True."""
        monkeypatch.setenv("HERMES_IGNORE_RULES", "1")

        # Import SavarezCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of SavarezCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.SavarezCLI)
        # Replicate the exact logic from cli.py SavarezCLI.__init__:
...
class TestArgparseFlagsRegistered:
    ...
    def test_flags_present_in_chat_parser(self):
        ...
        import argparse
        parser = argparse.ArgumentParser(prog="savarez")
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real savarez parser accepts both flags."""
