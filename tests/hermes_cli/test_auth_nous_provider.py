...
class TestResolveVerifyFallback:
    ...
    def test_missing_hermes_ca_bundle_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.setenv("HERMES_CA_BUNDLE", "/nonexistent/savarez-ca.pem")
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `savarez model` → Nous Portal → Skip (keep current) after
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `savarez auth add nous`
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "Savarez is not logged into Nous Portal").
    ...
    # doesn't raise "Savarez is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `savarez auth add nous --type oauth --label <name>` silently
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", Savarez must surface an
    ...
    re-authenticate" string led users to report this as a Savarez persistence
    ...
    assert "savarez auth add nous" in message.lower()
...
def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    ...
    writing to the user's real ``~/.savarez/shared/`` across CI runs.
    """
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `savarez auth add nous
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    assert result["client_id"] == "savarez-cli"
...
def test_shared_store_survives_across_profile_switch(
    ...
    (different SAVAREZ_HOME) sees the same shared state and can rehydrate
    ...
    monkeypatch.setenv("SAVAREZ_HOME", str(profile_a))
    ...
    # Profile B: fresh SAVAREZ_HOME, no auth yet, but the shared store
    ...
    monkeypatch.setenv("SAVAREZ_HOME", str(profile_b))
