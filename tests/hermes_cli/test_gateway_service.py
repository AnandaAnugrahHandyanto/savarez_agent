...
class TestLaunchdPlistSemanticComparison:
    def test_launchd_plist_is_current_accepts_equivalent_path_variants(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.hermes.gateway.plist"
        monkeypatch.setattr(gateway_cli, "get_launchd_plist_path", lambda: plist_path)

        plist = plistlib.loads(gateway_cli.generate_launchd_plist().encode("utf-8"))
        env = plist["EnvironmentVariables"]
        env["PATH"] = ":".join(f"{part.rstrip('/')}/" for part in env["PATH"].split(":"))
