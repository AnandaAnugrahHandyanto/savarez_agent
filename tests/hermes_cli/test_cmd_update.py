...
class TestCmdUpdatePip:
    ...
    def test_update_pip_exports_virtualenv_from_sys_prefix(
        ...
        monkeypatch.setattr(hm.sys, "prefix", "/tmp/savarez-launcher-venv")
        monkeypatch.setattr(hm.sys, "base_prefix", "/usr")

        hm._cmd_update_pip(mock_args)

        assert mock_run.call_count == 1
        assert mock_run.call_args.args[0] == ["/usr/bin/uv", "pip", "install", "--upgrade", "savarez-agent"]
        assert mock_run.call_args.kwargs["env"]["VIRTUAL_ENV"] == "/tmp/savarez-launcher-venv"
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        origin but behind AnandaAnugrahHandyanto/savarez-agent silently misses updates.
        """
        from hermes_cli import main as hm
        ...
        with patch.object(
            hm,
            "_get_origin_url",
            return_value="https://github.com/example/savarez-agent.git",
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.savarez"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.savarez/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.savarez/profiles/work"))
...
class TestCmdUpdateBranchFlag:
    """``savarez update --branch <name>`` targets the requested branch.
...
class TestCmdUpdateCheckBranchFlag:
    """``savarez update --check --branch <name>`` honors the branch override.
...
class TestCmdUpdateZipBranchRefusal:
    """``savarez update --branch=<non-main>`` must refuse on the ZIP fallback path.
