...
`savarez gateway start/stop/restart` invocation should be routed to
...
def test_dispatch_renders_gateway_not_registered_friendly(
    ...
    """`savarez -p typo gateway start` should print a clear message and
    ...
    assert "savarez profile create typo" in out
...
def test_redirect_short_circuits_supervised_child(
    ...
    running execs `savarez gateway run --replace`, the
