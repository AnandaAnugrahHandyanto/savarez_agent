...
def _make_agent(hermes_home: Path) -> Path:
    ...
    agent_root = hermes_home / "savarez-agent"
...
def _make_gui_build(hermes_home: Path) -> None:
    ...
    desktop = hermes_home / "savarez-agent" / "apps" / "desktop"
    ...
    (hermes_home / "savarez-agent" / "node_modules").mkdir(parents=True)
...
def test_agent_is_installed_venv_only(tmp_path):
    ...
    hermes_home = tmp_path / ".hermes"
    (hermes_home / "savarez-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(hermes_home) is True
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    hermes_home = tmp_path / ".hermes"
    _make_agent(hermes_home)
    userdata = tmp_path / "Savarez-userdata"
    userdata.mkdir()
...
def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "Savarez"."""
    home = Path("/home/tester")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))

    monkeypatch.setattr(gu.sys, "platform", "darwin")
    assert gu.desktop_userdata_dir() == home / "Library" / "Application Support" / "Savarez"

    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    assert gu.desktop_userdata_dir() == home / ".config" / "Savarez"
...
def test_userdata_dir_windows(monkeypatch):
    ...
    assert gu.desktop_userdata_dir() == Path(r"C:\Users\tester\AppData\Roaming") / "Savarez"
...
def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    ...
    hermes_home = tmp_path / ".hermes"
    (hermes_home / "savarez-agent" / "hermes_cli").mkdir(parents=True)
    (hermes_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
