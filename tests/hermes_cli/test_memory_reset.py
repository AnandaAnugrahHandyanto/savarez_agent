"""Tests for the `savarez memory reset` CLI command.
...
- Profile-scoped reset (uses SAVAREZ_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake SAVAREZ_HOME with memory files."""
    ...
    (memories / "MEMORY.md").write_text(
        "§\nSavarez repo is at ~/.savarez/savarez-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `savarez memory reset` is run.
    """
...
class TestMemoryReset:
    """Tests for `savarez memory reset` subcommand."""
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's SAVAREZ_HOME."""
