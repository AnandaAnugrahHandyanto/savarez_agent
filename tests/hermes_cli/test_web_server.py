...
class TestPluginAPIAuth:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hermes_home):
        ...
        # Test middleware handles 404 vs 401 properly, so registering anything
        # underneath /api/plugins/ tests the auth middleware.
        # We must insert dummy routes directly BEFORE the SPA catchall route,
        # otherwise our test endpoints will just hit the SPA catchall (which returns 404
        # if the web dist isn't built yet, bypassing our auth checks).
        from fastapi import APIRouter

        test_router = APIRouter()

        @test_router.get("/board")
        def board_get():
            return {"status": "ok"}

        @test_router.post("/tasks")
        def tasks_post(data: dict):
            return {"status": "created"}

        @test_router.patch("/tasks/{task_id}")
        def tasks_patch(task_id: str, data: dict):
            return {"status": "updated"}

        @test_router.delete("/tasks/{task_id}")
        def tasks_delete(task_id: str):
            return {"status": "deleted"}

        @test_router.get("/other")
        def test_other():
            return {"status": "ok"}

        @test_router.websocket("/ws")
        async def test_ws(websocket):
            await websocket.accept()
            await websocket.close()

        app.include_router(test_router, prefix="/api/plugins/kanban")

        test_router2 = APIRouter()

        @test_router2.get("/scan-status")
        def scan_status():
            return {"status": "ok"}

        app.include_router(test_router2, prefix="/api/plugins/hermes-achievements")

        # Ensure catch-all route is at the very end
        catch_all_idx = None
        ...
        if catch_all_idx is not None:
            catch_all = app.router.routes.pop(catch_all_idx)
