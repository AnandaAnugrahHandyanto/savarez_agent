...
class TestLaunchdPlistPath:
    ...
    def test_plist_includes_configured_codex_app_server_sandbox_mode(self, monkeypatch):
        monkeypatch.setattr(
            gateway_cli,
            "read_raw_config",
            lambda: {"model": {"codex_app_server_sandbox_mode": "danger-full-access"}},
        )

        plist = gateway_cli.generate_launchd_plist()

        assert "<key>HERMES_CODEX_APP_SERVER_SANDBOX_MODE</key>" in plist
        assert "<string>danger-full-access</string>" in plist
    ...
    def test_plist_path_includes_ason_bin_when_present(self, tmp_path, monkeypatch):
        home = tmp_path / "home"
        ason_bin = home / ".ason" / "bin"
        ason_bin.mkdir(parents=True)
        monkeypatch.setattr(gateway_cli.Path, "home", lambda: home)

        plist = gateway_cli.generate_launchd_plist()

        assert str(ason_bin) in plist
