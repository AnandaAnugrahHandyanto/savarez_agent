"""Tests for IRC gateway configuration via `savarez setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Savarez install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Savarez install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#savarez")
            monkeypatch.setenv("IRC_NICKNAME", "savarez-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `savarez setup gateway` experience with IRC present."""
