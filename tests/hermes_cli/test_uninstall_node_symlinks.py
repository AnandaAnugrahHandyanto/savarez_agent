...
~/.local/bin pointing into $SAVAREZ_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Savarez-managed node dir.
"""
...
def _make_hermes_node(hermes_home: Path) -> Path:
    """Create a fake $SAVAREZ_HOME/node/bin/{node,npm,npx} tree."""
...
def test_removes_dangling_symlink_into_hermes_node(fake_home):
    """A link into the Savarez node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the Savarez links that exist; ignores the ones that don't."""
    hermes_home = fake_home / ".savarez"
    node_bin = _make_hermes_node(hermes_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are Savarez-managed; node is a real user binary.
