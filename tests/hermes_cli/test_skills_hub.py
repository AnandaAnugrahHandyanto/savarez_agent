...
def test_do_list_platform_env_is_ignored(three_source_env, monkeypatch):
    """`savarez skills list` reads the active profile's config via
    SAVAREZ_HOME (swapped by -p), so it must NOT pass a platform arg to
...
# ---------------------------------------------------------------------------
# Regression: full identifier must be recoverable from `savarez skills search`
