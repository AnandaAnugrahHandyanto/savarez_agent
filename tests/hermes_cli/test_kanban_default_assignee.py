...
def isolated_kanban_home(monkeypatch):
    """Spin up a fresh SAVAREZ_HOME with a clean kanban DB."""
    test_home = tempfile.mkdtemp(prefix="kanban_default_assignee_test_")
    monkeypatch.setenv("SAVAREZ_HOME", test_home)
    # Force-reimport so the fresh SAVAREZ_HOME is picked up.
    ...
    # gives each test its own monkeypatched SAVAREZ_HOME so no cross-test
    # contamination.
...
def test_dry_run_with_default_assignee_reports_without_mutating(isolated_kanban_home):
    ...
    `savarez kanban dispatch --dry-run` see the routing decision before
    committing."""
    kb, _home = isolated_kanban_home
    with kb.connect_closing() as conn:
        kb.create_board(slug="default", name="Test")
