"""Tests for `savarez portal` dispatch.

`savarez portal` (no subcommand) is the human-readable alias for the Nous Portal
one-shot onboarding (`savarez auth add nous --type oauth` / `savarez setup
--portal`). The prior status default moved to `savarez portal info`, with
...
def test_bare_portal_and_login_run_one_shot(monkeypatch, sub):
    """`savarez portal`, `savarez portal login` -> one-shot onboarding."""
...
def test_info_and_status_alias_run_status(monkeypatch, sub):
    """`savarez portal info` and the `status` back-compat alias -> status."""
    calls = {"login": 0, "status": 0}
...
def test_one_shot_delegates_to_model_flow_nous(monkeypatch):
    """`savarez portal` must run the quick-setup Nous flow (login + MODEL PICK +
    ...
    assert calls["model_flow"] == 1, (
        "`savarez portal` must route through _model_flow_nous so the model "
