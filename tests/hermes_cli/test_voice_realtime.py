...
def test_voice_realtime_defaults_are_safe():
    ...
    assert realtime["turn_detection"]["type"] == "server_vad"
    assert realtime["dispatch"]["max_active_delegates"] == 1
    assert realtime["dispatch"]["default_toolsets"] == []
...
class TestVoiceRealtimeEndpoints:
    ...
    def test_client_secret_mints_ephemeral_token_without_leaking_api_key(self, monkeypatch):
        ...
        data = resp.json()
        assert data == {
            "client_secret": {"value": "ephemeral-secret", "expires_at": 1893456000},
