"""Tests for the post-pull syntax guard in ``savarez update``.
...
becomes unbootable — every ``savarez`` invocation imports those files at
...
markers in ``hermes_cli/config.py``; users who ran ``savarez update`` in
the 7-minute window before #28458 landed could not run any ``savarez``
command afterward.
"""
...
# release; if a future ``savarez update`` would brick users, this test fails
# in CI first.
# ---------------------------------------------------------------------------
...
def test_production_tree_passes_syntax_guard():
    ...
    assert ok is True, (
        ...
        f"savarez update would brick users. Error: {error}"
    )
