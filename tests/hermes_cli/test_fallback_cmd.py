"""Tests for `savarez fallback` — chain reading, add/remove/clear, legacy migration."""
...
# ---------------------------------------------------------------------------
# Shared fixture — isolate SAVAREZ_HOME so save_config writes to tmp_path
# ---------------------------------------------------------------------------
...
def _write_config(home: Path, data: dict) -> None:
    config_path = home / ".savarez" / "config.yaml"
...
class TestReadChain:
    ...
    def test_reads_new_list_format(self):
        ...
        cfg = {
            "fallback_providers": [
                ...
                {"provider": "nous", "model": "Savarez-4-Llama-3.1-405B"},
            ]
        }
    ...
    def test_merges_new_and_legacy_formats(self):
        ...
        cfg = {
            ...
            "fallback_model": {"provider": "nous", "model": "Savarez-4"},
        }
        assert _read_chain(cfg) == [
            {"provider": "openrouter", "model": "anthropic/claude-sonnet-4.6"},
            {"provider": "nous", "model": "Savarez-4"},
        ]
...
class TestListCommand:
    def test_list_empty(self, isolated_home, capsys):
        ...
        assert "savarez fallback add" in out
...
class TestRemoveCommand:
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        # Picker returns index 1 (the middle entry, "nous / Savarez-4")
...
class TestArgparseWiring:
    """Verify `savarez fallback` is wired into main.py's argparse tree.
