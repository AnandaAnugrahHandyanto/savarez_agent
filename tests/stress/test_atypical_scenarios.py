...
normal tests assume away:
  ...
  - Filesystem: SAVAREZ_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own SAVAREZ_HOME, collect failures.
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "SAVAREZ_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
...
def _(home, kb):
    ...
    try:
        ...
        try:
            ...
            home_abs = str(Path(os.environ["SAVAREZ_HOME"]).resolve())
...
def _(home, kb):
    """SAVAREZ_HOME at a path with spaces — should work but catches
    ...
    weird = tempfile.mkdtemp(prefix="savarez with spaces ")
    os.environ["SAVAREZ_HOME"] = weird
    ...
    try:
        ...
        print(f"  SAVAREZ_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
...
def _(home, kb):
    """SAVAREZ_HOME with non-ASCII chars."""
    ...
    try:
        ...
        print(f"  SAVAREZ_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)
...
def _(home, kb):
    """SAVAREZ_HOME is a symlink to the real dir. _INITIALIZED_PATHS
    ...
    try:
        ...
        conn2.close()
        print("  symlinks to same SAVAREZ_HOME share DB correctly")
    finally:
