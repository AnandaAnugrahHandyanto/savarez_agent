...
Before the s6 migration, ``docker run AnandaAnugrahHandyanto/savarez-agent gateway
...
Every ``docker exec`` runs as ``savarez`` per the conftest module
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``savarez gateway run`` invocation
        (not ``sleep infinity``).
...
def test_supervised_gateway_does_not_recurse(
    ...
    supervised ``savarez gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``savarez gateway
    ...
    # Count python processes running `savarez gateway run`. If the
    ...
    assert n <= 1, (
        f"expected at most one supervised python `savarez gateway run` "
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    log file under ``${SAVAREZ_HOME}/logs/gateways/<profile>/current``.
    ...
    a poor user experience: the iconic "Savarez Gateway Starting…"
    ...
    assert "⚕" in combined or "Savarez Gateway Starting" in combined, (
    ...
    assert "⚕" in file_contents or "Savarez Gateway Starting" in file_contents, (
