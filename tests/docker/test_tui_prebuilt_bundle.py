...
The fix is ``ENV HERMES_TUI_DIR=/opt/savarez/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the savarez user, return stdout."""
    inner = (
        "source /opt/savarez/.venv/bin/activate && "
        "cd /opt/savarez && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the savarez user (UID 10000) so we exercise the same path the
    # dashboard PTY child runs as — not root.
    cmd = [
        "docker", "run", "--rm", "--entrypoint", "su", image,
        "savarez", "-s", "/bin/bash", "-c", inner,
    ]
...
def test_hermes_tui_dir_env_is_set(built_image: str) -> None:
    ...
    assert r.stdout.strip() == "/opt/savarez/ui-tui", (
        f"HERMES_TUI_DIR={r.stdout.strip()!r} (expected /opt/savarez/ui-tui)"
    )
