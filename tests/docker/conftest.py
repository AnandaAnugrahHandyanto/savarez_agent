...
IMAGE_TAG = os.environ.get("HERMES_TEST_IMAGE", "savarez-agent-harness:latest")
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged savarez user
# ---------------------------------------------------------------------------
#
# Background: every Savarez runtime path inside the container drops to UID
# 10000 (the ``savarez`` user) via ``s6-setuidgid savarez``. ``docker exec``
...
# fails (PermissionError swallowed) for savarez, so a test that ran as root
...
# helpers below run every probe under ``-u savarez`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: savarez).
