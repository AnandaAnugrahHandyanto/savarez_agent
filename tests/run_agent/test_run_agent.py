...
class TestRunConversation:
    ...
    def test_failed_patch_gets_auto_remediation_before_footer(self, agent):
        self._setup_agent(agent)
        ...
        tc = _mock_tool_call(name="patch", arguments=patch_args, call_id="c_patch")
        resp1 = _mock_response(content="", finish_reason="tool_calls", tool_calls=[tc])
        # This would previously be delivered immediately with only the
        # finalizer footer appended. The verifier should instead inject one
        # remediation turn before allowing any user-facing final response.
        resp2 = _mock_response(content="All requested edits are done.", finish_reason="stop")
        resp3 = _mock_response(content="I could not repair the edit after re-checking.", finish_reason="stop")
        ...
        with (
            ...
            patch.object(agent, "_file_mutation_verifier_enabled", return_value=True),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
            result = agent.run_conversation("patch the file")

        assert mock_handle_function_call.call_count == 1
        assert agent.client.chat.completions.create.call_count == 3
        assert "I could not repair the edit" in result["final_response"]
        assert "File-mutation verifier" in result["final_response"]
        assert "All requested edits are done." not in result["final_response"]
        synthetic_prompts = [
            m["content"]
            for m in result["messages"]
            if m.get("role") == "user" and m.get("_file_mutation_remediation_synthetic")
        ]
        assert len(synthetic_prompts) == 1
        assert "follow-up required before final answer" in synthetic_prompts[0]
        assert "/tmp/a.md" in synthetic_prompts[0]

    def test_successful_remediation_suppresses_file_mutation_footer(self, agent):
        self._setup_agent(agent)
        agent.tools = _make_tool_defs("patch")
        agent.valid_tool_names = {"patch"}
        ...
        fixed_args = json.dumps({
            "mode": "replace",
            "path": "/tmp/a.md",
            "old_string": "real",
            "new_string": "fixed",
        })
        tc_failed = _mock_tool_call(name="patch", arguments=failed_args, call_id="c_fail")
        tc_fixed = _mock_tool_call(name="patch", arguments=fixed_args, call_id="c_fix")
        resp1 = _mock_response(content="", finish_reason="tool_calls", tool_calls=[tc_failed])
        resp2 = _mock_response(content="Done.", finish_reason="stop")
        resp3 = _mock_response(content="", finish_reason="tool_calls", tool_calls=[tc_fixed])
        resp4 = _mock_response(content="Actually fixed and verified.", finish_reason="stop")
        ...
        assert result["final_response"] == "Actually fixed and verified."
