...
class TestRunConversation:
    ...
    def test_kanban_block_called_on_iteration_exhaustion(self, agent, monkeypatch):
        ...
        The exhaustion path appends a recovery handoff comment and routes
        through ``kanban_db._record_task_failure(outcome="timed_out")`` so
        ``consecutive_failures`` increments and the dispatcher's
        ``failure_limit`` breaker eventually trips.
        """
        self._setup_agent(agent)
        ...
        mock_handle_function_call = MagicMock(return_value="ok")

        with (
            patch("run_agent.handle_function_call", mock_handle_function_call),
        ...
        kanban_comment_calls = [
            c for c in mock_handle_function_call.call_args_list
            if c.args and c.args[0] == "kanban_comment"
        ]
        assert len(kanban_comment_calls) == 1, (
            "iteration exhaustion must append a durable kanban_comment "
            "so retry workers do not lose the handoff"
        )
        comment_args = kanban_comment_calls[0].args[1]
        ...
        payload = call.kwargs.get("event_payload_extra")
        assert isinstance(payload, dict)
        assert payload["budget_used"] == 2
        assert payload["budget_max"] == 2
        assert payload["recovery_summary"] == "Could not finish — budget exhausted."
