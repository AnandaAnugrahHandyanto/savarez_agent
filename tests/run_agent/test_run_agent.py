...
class TestRunConversation:
    ...
    def test_context_error_reduces_max_tokens_when_available_out_is_sufficient(
        ...
        """When available output >= 512, retry uses a reduced max_tokens cap instead of compressing."""
        ...
        def _fake_api_call(api_kwargs):
            captured_kwargs.append(dict(api_kwargs))
            result = responses.pop(0)
            if isinstance(result, Exception):
                raise result
            return result
        ...
        mock_parse.return_value = 1000  # 1000 >= 512 → ephemeral path, not compression
    ...
    def test_context_error_compresses_context_when_available_out_is_too_small(
        ...
        """When available output < 512, context compression is triggered instead of reducing max_tokens."""
        from agent.error_classifier import ClassifiedError, FailoverReason
        ...
        mock_parse.return_value = 300  # 300 < 512 → compression, not ephemeral path
