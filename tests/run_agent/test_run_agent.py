...
class TestRunConversation:
    ...
    def test_truncated_tool_call_second_retry_injects_concise_hint(self, agent):
        ...
        final_resp = _mock_response(content="Done!", finish_reason="stop")
        responses = iter([
            truncated_resp, truncated_resp, good_resp, final_resp,
        ])
        captured_messages = []
        ...
        assert "split it into smaller tool calls" in third_call_messages[-1]["content"]

    def test_truncated_tool_call_gives_up_after_concise_retry(self, agent):
        """If the concise retry also truncates, the agent refuses the partial tool."""
        self._setup_agent(agent)
        agent.valid_tool_names.add("write_file")
        bad_tc = _mock_tool_call(
            name="write_file",
