...
def _isolate(tmp_path, monkeypatch):
    """Isolate SAVAREZ_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal SavarezCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = SavarezCLI.__new__(SavarezCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["savarez-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Savarez' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Savarez' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Savarez' minimum context length."""
