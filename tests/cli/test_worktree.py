...
def _setup_worktree(repo_root):
    ...
    wt_name = f"savarez-{short_id}"
    branch_name = f"savarez/{wt_name}"
...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("savarez/savarez-")
...
class TestStaleWorktreePruning:
    ...
    def test_prunes_old_clean_worktree(self, git_repo):
        ...
        for entry in worktrees_dir.iterdir():
            if not entry.is_dir() or not entry.name.startswith("savarez-"):
                continue
            try:
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned savarez/* and pr-* branches."""

    def test_prunes_orphaned_hermes_branch(self, git_repo):
        """savarez/savarez-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "savarez/savarez-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "savarez/savarez-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "savarez/savarez-deadbeef" in result.stdout
        ...
        orphaned = [
            ...
            and (b.startswith("savarez/savarez-") or b.startswith("pr-"))
        ]
        assert "savarez/savarez-deadbeef" in orphaned
        ...
        assert "savarez/savarez-deadbeef" not in result.stdout
