...
minimal ``SavarezCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def hermes_home(tmp_path, monkeypatch):
    """Isolated SAVAREZ_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".savarez"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("SAVAREZ_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves SAVAREZ_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal SavarezCLI stub with an active goal wired in."""
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, hermes_home):
        ...
        src = inspect.getsource(SavarezCLI.chat)
