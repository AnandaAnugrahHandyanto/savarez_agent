...
class TestChromeDebugLaunch:
    def test_windows_launch_uses_browser_found_on_path(self):
        ...
        with patch("hermes_agent.cli.shutil.which", side_effect=lambda name: r"C:\Chrome\chrome.exe" if name == "chrome.exe" else None), \
             patch("hermes_agent.cli.os.path.isfile", side_effect=lambda path: path == r"C:\Chrome\chrome.exe"), \
    ...
    def test_windows_launch_falls_back_to_common_install_dirs(self, monkeypatch):
        ...
        with patch("hermes_agent.cli.shutil.which", return_value=None), \
             patch("hermes_agent.cli.os.path.isfile", side_effect=lambda path: path == installed), \
             patch("subprocess.Popen", side_effect=fake_popen):
