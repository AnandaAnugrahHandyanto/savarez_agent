...
def test_secret_capture_callback_can_be_completed_from_cli_state_machine():
    ...
    with patch("hermes_agent.cli.callbacks.save_env_value_secure") as save_secret:
...
def test_secret_capture_uses_getpass_without_tui():
    ...
    with patch("hermes_agent.cli.callbacks.getpass.getpass", return_value="secret-value"), patch(
...
def test_secret_capture_timeout_clears_hidden_input_buffer():
    ...
    with patch("hermes_agent.cli.callbacks.queue.Queue.get", side_effect=queue.Empty), patch(
