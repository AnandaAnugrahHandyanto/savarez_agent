...
Covers four shapes from Phase 4 of ``.savarez/plans/2026-05-21-dashboard-oauth-auth.md``:
    ...
    def test_rejects_malformed_client_id(self):
        with pytest.raises(ValueError, match="agent:"):
            nous_plugin.NousDashboardAuthProvider(
                client_id="savarez-dashboard", portal_url="https://x"
            )
...
class TestPluginRegister:
    ...
    def test_skips_when_client_id_malformed(self, monkeypatch):
        monkeypatch.setenv("HERMES_DASHBOARD_OAUTH_CLIENT_ID", "savarez-dashboard")
        ...
        assert "savarez-dashboard" in nous_plugin.LAST_SKIP_REASON
...
class TestStartLogin:
    ...
    def test_returns_login_start(self, provider):
        result = provider.start_login(
            redirect_uri="https:/.savarez.fly.dev/auth/callback"
        )
    ...
    def test_authorize_url_has_required_params(self, provider):
        ...
        assert params["redirect_uri"] == "https:/.savarez.fly.dev/auth/callback"
    ...
    def test_allows_http_with_arbitrary_host(self, provider):
        ...
        provider.start_login(redirect_uri="http:/.savarez.fly.dev/auth/callback")
