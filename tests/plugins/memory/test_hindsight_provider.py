...
class TestSyncTurn:
    ...
    def test_sync_turn_with_tool_calls(self, provider):
        """When retain_tool_calls=True and turn_messages provided, includes tools."""
        turn_msgs = [
        ...
        provider.sync_turn("what's the weather?", "It's 72°F and sunny in SF.", turn_messages=turn_msgs)
        if provider._sync_thread:
            provider._sync_thread.join(timeout=5.0)
        turns = json.loads(provider._client.aretain_batch.call_args.kwargs["items"][0]["content"])
        messages = turns[0]  # first (only) turn
        roles = [e["role"] for e in messages]
        assert "tool_call" in roles
        assert "tool_result" in roles
        assert any(e.get("name") == "get_weather" for e in messages)
        assert all("timestamp" in e for e in messages)

    def test_sync_turn_without_tool_calls(self, provider_with_config):
        """When retain_tool_calls=False, falls back to simple user/assistant."""
        p = provider_with_config(retain_tool_calls=False)
        turn_msgs = [
        ...
        p.sync_turn("hello", "hi there", turn_messages=turn_msgs)
        if p._sync_thread:
            p._sync_thread.join(timeout=5.0)
        turns = json.loads(p._client.aretain_batch.call_args.kwargs["items"][0]["content"])
        messages = turns[0]
        assert len(messages) == 2
        assert messages[0]["role"] == "user"
        assert messages[1]["role"] == "assistant"
...
class TestConfigSchema:
    def test_schema_has_all_new_fields(self, provider):
        ...
        expected_keys = {
            ...
            "retain_every_n_turns", "retain_tool_calls", "retain_async",
            "retain_context",
