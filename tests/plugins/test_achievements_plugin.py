"""Tests for the bundled savarez-achievements dashboard plugin.
...
``plugins/savarez-achievements/tests/`` covering the achievement engine
internals (tier math, secret-state handling, catalog invariants). These
tests live at the savarez-agent level and focus on the integration
...
def plugin_api(tmp_path, monkeypatch):
    """Load plugin_api with isolated ~/.savarez so state/snapshot files don't collide.
