...
def test_board_query_param_default_overrides_current_board_pointer(client):
    ...
    through to whichever board ``savarez kanban boards switch`` last pinned.
    """
...
def test_config_reads_dashboard_kanban_section(tmp_path, monkeypatch, client):
    home = Path(os.environ["SAVAREZ_HOME"])
...
def test_create_task_includes_warning_when_no_dispatcher(client, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: (False, "No gateway is running — start `savarez gateway start`."),
    )
    r = client.post(
        "/api/plugins/kanban/tasks",
...
def test_dashboard_bulk_actions_include_reclaim_first():
    ...
    assert "savarez-kanban-bulk-reclaim-first" in dist
    assert '"→ todo"' in dist
...
def test_dashboard_failed_card_highlight_class_exists():
    ...
    assert "savarez-kanban-card--failed" in js
    assert "savarez-kanban-card--failed" in css
