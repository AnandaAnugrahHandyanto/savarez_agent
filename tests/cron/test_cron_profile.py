...
SAVAREZ_HOME scoping, and tick() serialization for profile jobs.
"""
...
def isolated_cron_profile_home(tmp_path, monkeypatch):
    """Create an isolated Savarez root with a named profile and temp cron store."""
    root = tmp_path / "savarez-root"
    profile_home = root / "profiles" / "support"
    profile_home.mkdir(parents=True)
    (root / "cron").mkdir(parents=True)

    monkeypatch.setenv("SAVAREZ_HOME", str(root))
...
class TestCronjobToolProfile:
    ...
    def test_schema_advertises_profile(self):
        ...
        assert "savarez profile" in desc_lower
...
class TestRunJobProfileContext:
    ...
    def _install_agent_stubs(monkeypatch, observed: dict):
        ...
        class FakeAgent:
            def __init__(self, **kwargs):
                ...
                observed["env_home_during_init"] = os.environ.get("SAVAREZ_HOME")
            ...
            def run_conversation(self, *_a, **_kw):
                ...
                observed["env_home_during_run"] = os.environ.get("SAVAREZ_HOME")
    ...
    def test_run_job_sets_and_restores_profile_home(
        ...
        assert os.environ["SAVAREZ_HOME"] == str(root)
    ...
    def test_no_agent_profile_uses_profile_scripts_dir_and_restores_env(
        ...
        (scripts_dir / "print_home.py").write_text(
            "import os\nprint(os.environ.get('SAVAREZ_HOME', ''))\n",
            encoding="utf-8",
        )
        monkeypatch.setattr(sched, "_hermes_home", None)
