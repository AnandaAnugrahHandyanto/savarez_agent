...
class TestToolsetConsistency:
    ...
    def test_hermes_platforms_share_core_tools(self):
        """All savarez-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on savarez-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["savarez-cli", "savarez-telegram", "savarez-discord", "savarez-whatsapp", "savarez-slack", "savarez-signal", "savarez-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_hermes_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("savarez-whatsapp")

    def test_hermes_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("savarez-api-server")
