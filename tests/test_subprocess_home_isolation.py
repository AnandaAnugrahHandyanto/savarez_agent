...
class TestGetSubprocessHome:
    ...
    def test_explicit_subprocess_home_overrides_isolation_opt_out(self, tmp_path, monkeypatch):
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        (hermes_home / "home").mkdir()
        explicit_home = tmp_path / "forced-home"
        monkeypatch.setenv("HERMES_HOME", str(hermes_home))
        monkeypatch.setenv("HERMES_SUBPROCESS_HOME", str(explicit_home))
        monkeypatch.setenv("HERMES_PROFILE_HOME_ISOLATION", "0")
