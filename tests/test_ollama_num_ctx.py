...
from hermes_agent.agent.model_metadata import query_ollama_num_ctx
...
class TestQueryOllamaNumCtx:
    ...
    def test_returns_context_from_model_info(self):
        ...
        with patch("hermes_agent.agent.model_metadata.detect_local_server_type", return_value="ollama"):
    ...
    def test_returns_none_for_non_ollama_server(self):
        ...
        with patch("hermes_agent.agent.model_metadata.detect_local_server_type", return_value="lm-studio"):
            result = query_ollama_num_ctx("model", "http://localhost:1234")
        assert result is None

    def test_returns_none_on_connection_error(self):
        """Should return None if the server is unreachable."""
        with patch("hermes_agent.agent.model_metadata.detect_local_server_type", side_effect=Exception("timeout")):
