"""Tests for acp_adapter.server — SavarezACPAgent ACP server."""
...
from acp_adapter.server import SavarezACPAgent, HERMES_VERSION
...
def agent(mock_manager):
    """SavarezACPAgent backed by a mock session manager."""
    return SavarezACPAgent(session_manager=mock_manager)
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "savarez-agent"
        ...
        assert payloads == [
            {
                "args": ["--setup"],
                "description": (
                    "Open Savarez' interactive model/provider setup in a terminal. "
                    "Use this when Savarez has not been configured on this machine yet."
                ),
                "id": TERMINAL_SETUP_AUTH_METHOD_ID,
                "name": "Configure Savarez provider",
                "type": "terminal",
            }
        ]
...
class TestSessionOps:
    ...
    async def test_new_session_returns_model_state(self):
        ...
        acp_agent = SavarezACPAgent(session_manager=manager)
    ...
    async def test_load_session_replays_persisted_history_to_client(self, agent):
        ...
        state.history = [
            ...
            {"role": "assistant", "content": "SavarezACPAgent._ADVERTISED_COMMANDS controls them."},
            {
                "role": "assistant",
                "content": "",
                "tool_calls": [
        ...
        assert replay_calls[1].kwargs["update"].content.text.startswith("SavarezACPAgent")
...
class TestRegisterSessionMcpServers:
    ...
    async def test_registers_stdio_servers(self, agent, mock_manager):
        ...
        state.agent.enabled_toolsets = ["savarez-acp"]
    ...
    async def test_refreshes_agent_tool_surface(self, agent, mock_manager):
        ...
        mock_defs.assert_called_once_with(
            enabled_toolsets=["savarez-acp", "mcp-srv"],
            disabled_toolsets=None,
            quiet_mode=True,
        )
        assert state.agent.enabled_toolsets == ["savarez-acp", "mcp-srv"]
