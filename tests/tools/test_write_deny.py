...
class TestWriteDenyExactPaths:
    ...
    def test_hermes_env(self):
        # ``.env`` under the active SAVAREZ_HOME (profile-aware, not just
        # ``~/.savarez``) must be write-denied. The hermetic test conftest
        # points SAVAREZ_HOME at a tempdir — resolve via get_hermes_home()
    ...
    def test_hermes_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.savarez/.env`` (whose credentials are inherited by every profile)
        ...
        monkeypatch.setenv("SAVAREZ_HOME", str(profile_home))

        # Sanity check: SAVAREZ_HOME does point to the profile dir, not the root.
...
class TestWriteAllowed:
    ...
    def test_hermes_config_not_env(self):
        path = os.path.join(str(Path.home()), ".savarez", "config.yaml")
