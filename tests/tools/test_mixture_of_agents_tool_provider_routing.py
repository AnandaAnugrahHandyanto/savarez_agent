...
def test_anthropic_adapter_does_not_clobber_thinking_temperature(monkeypatch):
    ...
    def fake_normalize(_response):
        return SimpleNamespace(role="assistant", content=""), "stop"

    monkeypatch.setattr("agent.anthropic_adapter.build_anthropic_kwargs", fake_build_kwargs)
    monkeypatch.setattr("agent.anthropic_adapter.normalize_anthropic_response", fake_normalize)
