...
def test_format_suppresses_reconciled_dynamic_workflow_async_event(monkeypatch):
        ...
        dwt.handle_dynamic_workflow(
            {
                ...
                "objective": "Suppress reconciled completion",
                "dispatch_ready": True,
                "nodes": [{"node_id": "worker", "goal": "Do work"}],
            },
            parent_agent=object(),
        )
        dwt.handle_dynamic_workflow({"action": "status", "workflow_id": "wf_reconcile"})
...
def test_format_keeps_unreconciled_dynamic_workflow_async_event():
    from tools import dynamic_workflow_tool as dwt
    ...
    try:
        ...
        assert result is not None
        assert "ASYNC DELEGATION COMPLETE" in result
        assert "Worker finished cleanly." in result
    finally:
        dwt._reset_for_tests()
