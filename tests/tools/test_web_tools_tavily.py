...
class TestWebCrawlTavily:
    ...
    def test_crawl_dispatches_to_tavily(self):
        ...
        with patch("hermes_agent.tools.web_tools._get_backend", return_value="tavily"), \
             patch.dict(os.environ, {"TAVILY_API_KEY": "tvly-test"}), \
             patch("hermes_agent.tools.web_tools.httpx.post", return_value=mock_response), \
             patch("hermes_agent.tools.web_tools.check_website_access", return_value=None), \
             patch("hermes_agent.tools.web_tools.is_safe_url", return_value=True), \
             patch("hermes_agent.tools.interrupt.is_interrupted", return_value=False):
            from hermes_agent.tools.web_tools import web_crawl_tool
            result = json.loads(asyncio.get_event_loop().run_until_complete(
    ...
    def test_crawl_sends_instructions(self):
        ...
        with patch("hermes_agent.tools.web_tools._get_backend", return_value="tavily"), \
             patch.dict(os.environ, {"TAVILY_API_KEY": "tvly-test"}), \
             patch("hermes_agent.tools.web_tools.httpx.post", return_value=mock_response) as mock_post, \
             patch("hermes_agent.tools.web_tools.check_website_access", return_value=None), \
             patch("hermes_agent.tools.web_tools.is_safe_url", return_value=True), \
             patch("hermes_agent.tools.interrupt.is_interrupted", return_value=False):
            from hermes_agent.tools.web_tools import web_crawl_tool
