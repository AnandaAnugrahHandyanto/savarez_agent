...
bind-mounted $SAVAREZ_HOME may contain host-owned files Hermes does not manage,
...
def _toplevel_chown_loop(text: str) -> str:
    """Extract the `for f in … chown hermes:hermes "$SAVAREZ_HOME/$f" … done`
    ...
    block = m.group(1)
    assert 'chown hermes:hermes "$SAVAREZ_HOME/$f"' in block, (
