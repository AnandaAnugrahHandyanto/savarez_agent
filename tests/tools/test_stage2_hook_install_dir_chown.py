...
files under the savarez user's home directory ($SAVAREZ_HOME == /opt/data).
Runtime-writable trees under ``/opt/savarez`` must be explicitly chowned to the
new UID before services drop privileges. ``/opt/savarez/gateway`` is one such
