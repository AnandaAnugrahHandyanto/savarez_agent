...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_hermes_bin(self):
        """After _INSTALL_FAILED, manual install in SAVAREZ_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
...
class TestHermesHomeIsolation:
    def test_hermes_bin_dir_respects_hermes_home(self):
        ...
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"SAVAREZ_HOME": tmpdir}):
    ...
    def test_conftest_isolation_prevents_real_home_writes(self):
        ...
        hermes_home = os.getenv("SAVAREZ_HOME")
        assert hermes_home is not None, "SAVAREZ_HOME should be set by conftest"
        assert "hermes_test" in hermes_home, "Should point to test temp dir"

    def test_get_hermes_home_fallback(self):
        """Without SAVAREZ_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_hermes_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove SAVAREZ_HOME entirely. With HOME also absent, expanduser
