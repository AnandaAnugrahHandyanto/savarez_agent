...
``~/.savarez/.env`` entries. These tests confirm each STT provider now
...
class TestProviderSelectionGate:
    ...
    ``os.environ`` a user with keys in ``~/.savarez/.env`` would be told
...
class TestEndToEndRegressionGuard:
    ...
    ``~/.savarez/.env`` carrying the key while ``os.environ`` does not.
