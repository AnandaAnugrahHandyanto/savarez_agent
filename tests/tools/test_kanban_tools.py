...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    """Normal `savarez chat` sessions (no HERMES_KANBAN_TASK) must have
    ...
    schema = registry.get_definitions(set(resolve_toolset("savarez-cli")), quiet=True)
...
def worker_env(monkeypatch, tmp_path):
    """Simulate being a worker: SAVAREZ_HOME isolated, HERMES_KANBAN_TASK set
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    under an authoritative-looking name like ``savarez-system`` and
    ...
    from tools import kanban_tools as kt
    out = kt._handle_comment({
        "task_id": worker_env, "body": "hi", "author": "savarez-system",
    })
    ...
    try:
        ...
        # caller-supplied "savarez-system" override.
        assert comments[0].author == "test-worker"
    finally:
        conn.close()
...
# restarting Savarez. These tests pin that ``board=<slug>`` argument
...
def multi_board_env(monkeypatch, tmp_path):
    """Isolated Savarez home with two distinct kanban boards seeded.
