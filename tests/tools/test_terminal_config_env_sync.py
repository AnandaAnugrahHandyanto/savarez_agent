...
dicts) keeps the test independent of the user's ~/.savarez/config.yaml and
...
def _save_config_env_sync_keys() -> set[str]:
    """terminal config keys bridged by ``savarez config set foo bar``."""
...
def test_save_config_set_supports_critical_bridged_keys():
    ...
    assert not missing, (
        f"`savarez config set terminal.X` doesn't sync these load-bearing "
...
def test_docker_persist_across_processes_is_bridged_everywhere():
    ...
    container on Savarez exit or just stops it (keeping it for the next
...
def test_docker_orphan_reaper_is_bridged_everywhere():
    ...
    ``terminal.docker_orphan_reaper`` controls whether Savarez sweeps stale
