...
class TestDispatchDelegateTask(unittest.TestCase):
    ...
    def test_acp_args_forwarded(self, mock_creds, mock_cfg):
        """Both acp_command and acp_args reach _build_child_agent for
        ACP-capable tools (e.g. 'copilot').  acp_command='claude' now
        routes to the Claude CLI print-mode handoff and bypasses
        _build_child_agent entirely — see TestClaudeCliHandoff — so this
        forwarding test uses 'copilot' to exercise the ACP subprocess path.
        """
        mock_creds.return_value = {
