...
binary download are exercised manually by `savarez secrets bitwarden
...
def hermes_home(tmp_path, monkeypatch):
    """Point Savarez at an isolated home directory."""
    home = tmp_path / ".savarez"
    home.mkdir()
    monkeypatch.setenv("SAVAREZ_HOME", str(home))
    # Some modules cache get_hermes_home; clear if needed.
    import hermes_constants
    if hasattr(hermes_constants, "_SAVAREZ_HOME_CACHE"):
        hermes_constants._SAVAREZ_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
