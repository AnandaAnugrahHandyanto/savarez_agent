...
class TestIsForumParent:
    ...
    def test_forum_channel_class_instance(self, monkeypatch):
        adapter = DiscordAdapter(PlatformConfig(enabled=True, token="***"))
        fake_forum_cls = type("ForumChannel", (), {})
        monkeypatch.setattr(_discord_mod, "ForumChannel", fake_forum_cls, raising=False)
        ch = fake_forum_cls()
...
async def test_send_to_forum_creates_thread_post():
    ...
    forum_channel = _fake_forum_channel()
