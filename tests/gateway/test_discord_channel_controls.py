...
def test_config_bridges_thread_free_response_channels(monkeypatch, tmp_path):
    """gateway/config.py bridges discord.thread_free_response_channels to env var and adapter extra."""
    import yaml
    config_file = tmp_path / "config.yaml"
    config_file.write_text(yaml.dump({
        "discord": {
    ...
    assert config.platforms[Platform.DISCORD].extra["thread_free_response_channels"] == ["444", "555"]
