...
class TestExtractMedia:
    ...
    def test_windows_backslash_path(self):
        ...
        assert len(media) == 1
        assert media[0][0] == "C:\\Users\\test\\file.png"
    ...
    def test_windows_forward_slash_path(self):
        ...
        assert len(media) == 1
        assert media[0][0] == "C:/Users/test/file.png"

    def test_windows_path_with_spaces(self):
        """Windows path containing spaces."""
        media, cleaned = BasePlatformAdapter.extract_media(
            "MEDIA:C:\\Users\\test\\my file.png"
        )
        assert len(media) == 1
        assert media[0][0] == "C:\\Users\\test\\my file.png"

    def test_windows_path_with_optional_whitespace(self):
        """Windows path with optional whitespace after MEDIA:"""
        media, cleaned = BasePlatformAdapter.extract_media(
            "MEDIA: C:\\Users\\test\\file.png"
        )
        assert len(media) == 1
    ...
    def test_unix_absolute_path_still_works(self):
        """Unix absolute path — must not regress."""
        media, cleaned = BasePlatformAdapter.extract_media(
            "MEDIA:/tmp/file.png"
        )
        assert len(media) == 1
        assert media[0][0] == "/tmp/file.png"

    def test_unix_tilde_path(self):
        """Unix tilde path -- must not regress. On Windows expanduser
        resolves to the user's home dir, so check suffix."""
        media, cleaned = BasePlatformAdapter.extract_media(
            "MEDIA:~/file.png"
        )
        assert len(media) == 1
    ...
    def test_windows_path_in_mixed_text(self):
        """Windows MEDIA tag embedded in surrounding text."""
        media, cleaned = BasePlatformAdapter.extract_media(
            "Here is your file MEDIA:D:\\data\\image.png\nEnjoy!"
        )
        assert len(media) == 1
        assert media[0][0] == "D:\\data\\image.png"
        assert "Here is your file" in cleaned
        assert "Enjoy!" in cleaned

    def test_non_media_text_not_matched(self):
        """Regular text without MEDIA tag should not match."""
        media, cleaned = BasePlatformAdapter.extract_media("Just some text C:\\path not relevant")
        assert media == []
        assert cleaned == "Just some text C:\\path not relevant"
