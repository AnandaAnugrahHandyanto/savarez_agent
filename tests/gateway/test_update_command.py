...
class TestHandleUpdateCommand:
    ...
    async def test_managed_install_returns_package_manager_guidance(self, monkeypatch):
        ...
        assert "brew upgrade savarez-agent" in result
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        with patch("gateway.run._hermes_home", tmp_path):
            ...
            # project_root will be the real savarez-agent dir (which HAS .git).
    ...
    async def test_no_hermes_binary(self, tmp_path):
        """Returns error when savarez is not on PATH and hermes_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "savarez update" in result

    @pytest.mark.asyncio
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m hermes_cli.main when savarez not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "Starting Savarez update" in result
    ...
    async def test_resolve_hermes_bin_prefers_which(self, tmp_path):
        ...
        with patch("shutil.which", return_value="/custom/path/savarez"):
            result = _resolve_hermes_bin()

        assert result == ["/custom/path/savarez"]
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        with patch("gateway.run._hermes_home", hermes_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", side_effect=lambda x: "/usr/bin/savarez" if x == "savarez" else "/usr/bin/setsid"), \
...
class TestUpdateCommandPlatformGate:
    ...
    async def test_allows_plugin_platform_via_registry_fallback(self, monkeypatch):
        ...
        # Later steps may legitimately return success ("Starting Savarez
