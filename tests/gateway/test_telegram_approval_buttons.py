...
class TestTelegramExecApproval:
    ...
    async def test_send_update_prompt_escapes_dynamic_prompt(self):
        ...
        assert result.success is True
        assert sent["parse_mode"] in ("MarkdownV2", "MARKDOWN_V2") or "MARKDOWN_V2" in repr(sent["parse_mode"])
...
class TestTelegramApprovalCallback:
    ...
    async def test_approval_callback_escapes_dynamic_user_name(self):
        ...
        assert edit_kwargs["parse_mode"] in ("MarkdownV2", "MARKDOWN_V2") or "MARKDOWN_V2" in repr(edit_kwargs["parse_mode"])
