...
# The synadia_ai SDK mock lives in a sibling module so it travels with the
# NATS test files into the §4 Stage-4 reference-clone copy step (which only
# pulls files matching tests/gateway/test_nats_*.py + tests/gateway/_nats_sdk_mock.py
# and does NOT touch conftest.py).
from tests.gateway._nats_sdk_mock import _ensure_synadia_agents_mock
