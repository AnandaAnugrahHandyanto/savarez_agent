...
async def test_native_reply_transport_requires_adapter_capability_and_reply_context():
    ...
    consumer = GatewayStreamConsumer(
        adapter,
        "chat_123",
        StreamConsumerConfig(edit_interval=0.01, buffer_threshold=1, cursor="", transport="native"),
...
@pytest.mark.asyncio
async def test_native_reply_transport_respects_explicit_edit_transport():
    adapter = _WeComNativeAdapter()
    consumer = GatewayStreamConsumer(
        adapter,
        "chat_123",
        StreamConsumerConfig(edit_interval=0.01, buffer_threshold=1, cursor="", transport="edit"),
        metadata=_reply_metadata(),
    )
...
async def test_native_final_success_sends_single_finish_frame_when_done_arrives_with_first_delta():
    ...
    consumer = GatewayStreamConsumer(
        adapter,
        "chat_123",
        StreamConsumerConfig(edit_interval=0.01, buffer_threshold=100, cursor="", transport="native"),
        metadata=_reply_metadata(),
    )
...
async def test_native_streaming_tool_boundary_reuses_same_stream_key():
    ...
    consumer = GatewayStreamConsumer(
        adapter,
        "chat_123",
        StreamConsumerConfig(edit_interval=0.01, buffer_threshold=5, cursor="", transport="native"),
        metadata=_reply_metadata(),
    )
