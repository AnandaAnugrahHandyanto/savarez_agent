"""Gateway-flow tests for NDR DM handling."""
...
def _make_source() -> SessionSource:
    return SessionSource(
        ...
        user_id="peer-pubkey",
        chat_id="chat-ndr-123",
        user_name="peer-pubkey",
        chat_type="dm",
    )
...
def _make_runner(session_entry: SessionEntry):
    ...
    runner.config = GatewayConfig(
        platforms={
            Platform.NDR: PlatformConfig(
                enabled=True,
                extra={"bin": "/usr/local/bin/ndr"},
            )
        }
    )
    adapter = MagicMock()
...
@pytest.mark.asyncio
async def test_handle_message_persists_timeout_reply_for_ndr_dm(monkeypatch, tmp_path):
    ...
    result = await runner._handle_message(_make_event("Hi Hermes over NDR"))
    ...
    assert persisted_entries[1]["content"] == "Hi Hermes over NDR"
