...
class TestTelegramSendClarify:
    ...
    @pytest.mark.asyncio
    async def test_rigor_choices_render_single_row_without_other(self, monkeypatch):
        adapter = _make_adapter()
        mock_msg = MagicMock()
        ...
        result = await adapter.send_clarify(
            chat_id="12345",
            question="Research rigor for this one?",
            choices=["Brief", "Standard", "Deep"],
            clarify_id="cid-rigor",
            session_key="sk-rigor",
        )
