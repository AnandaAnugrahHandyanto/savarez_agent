...
class TestWeixinEventLoopHandling:
    def test_api_post_recovers_from_different_loop_error(self):
        import aiohttp
        from unittest.mock import AsyncMock, MagicMock, patch

        from gateway.platforms.weixin import _api_post

        fake_response = MagicMock()
        fake_response.ok = True
        fake_response.text = AsyncMock(return_value='{"ret": 0}')

        original_session = MagicMock()
        post_cm = AsyncMock()
        post_cm.__aenter__ = AsyncMock(
            side_effect=RuntimeError("Task attached to a different loop")
        )
        post_cm.__aexit__ = AsyncMock(return_value=False)
        original_session.post = MagicMock(return_value=post_cm)

        with patch("gateway.platforms.weixin.aiohttp.ClientSession") as SessionMock:
            new_session = MagicMock()
            new_post_cm = AsyncMock()
            new_post_cm.__aenter__ = AsyncMock(return_value=fake_response)
            new_post_cm.__aexit__ = AsyncMock(return_value=False)
            new_session.post = MagicMock(return_value=new_post_cm)
            SessionMock.return_value.__aenter__ = AsyncMock(return_value=new_session)
            SessionMock.return_value.__aexit__ = AsyncMock(return_value=False)
    ...
    def test_api_get_recovers_from_different_loop_error(self):
        import aiohttp
        from unittest.mock import AsyncMock, MagicMock, patch

        from gateway.platforms.weixin import _api_get
        ...
        original_session.get = MagicMock(return_value=get_cm)

        with patch("gateway.platforms.weixin.aiohttp.ClientSession") as SessionMock:
            new_session = MagicMock()
            new_get_cm = AsyncMock()
            new_get_cm.__aenter__ = AsyncMock(return_value=fake_response)
            new_get_cm.__aexit__ = AsyncMock(return_value=False)
            new_session.get = MagicMock(return_value=new_get_cm)
            SessionMock.return_value.__aenter__ = AsyncMock(return_value=new_session)
            SessionMock.return_value.__aexit__ = AsyncMock(return_value=False)
