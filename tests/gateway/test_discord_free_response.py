...
@pytest.mark.asyncio
async def test_discord_thread_free_channel_bypasses_mention_and_auto_threads(adapter, monkeypatch):
    """Threaded free-response channels should accept top-level messages and isolate them in threads."""
    monkeypatch.setenv("DISCORD_REQUIRE_MENTION", "true")
    monkeypatch.delenv("DISCORD_FREE_RESPONSE_CHANNELS", raising=False)
    monkeypatch.setenv("DISCORD_THREAD_FREE_RESPONSE_CHANNELS", "789")
    monkeypatch.delenv("DISCORD_AUTO_THREAD", raising=False)  # default true

    fake_thread = FakeThread(channel_id=999, name="threaded-free-response")
    adapter._auto_create_thread = AsyncMock(return_value=fake_thread)

    message = make_message(
        channel=FakeTextChannel(channel_id=789),
        content="ambient ops request",
    )
    ...
    adapter.handle_message.assert_awaited_once()
    event = adapter.handle_message.await_args.args[0]
    assert event.text == "ambient ops request"
    assert event.source.chat_type == "thread"
    assert event.source.thread_id == "999"
...
@pytest.mark.asyncio
async def test_discord_thread_free_channel_can_come_from_config_extra(adapter, monkeypatch):
    monkeypatch.delenv("DISCORD_REQUIRE_MENTION", raising=False)
    monkeypatch.delenv("DISCORD_FREE_RESPONSE_CHANNELS", raising=False)
    monkeypatch.delenv("DISCORD_THREAD_FREE_RESPONSE_CHANNELS", raising=False)
    monkeypatch.delenv("DISCORD_AUTO_THREAD", raising=False)
    adapter.config.extra["thread_free_response_channels"] = ["789", "999"]

    fake_thread = FakeThread(channel_id=1001, name="config-threaded-free-response")
    adapter._auto_create_thread = AsyncMock(return_value=fake_thread)

    message = make_message(
        channel=FakeTextChannel(channel_id=789),
        content="allowed from config and threaded",
    )
...
@pytest.mark.asyncio
async def test_discord_no_thread_channel_overrides_thread_free_response(adapter, monkeypatch):
    """Explicit no-thread channels should still win over threaded free-response."""
    monkeypatch.setenv("DISCORD_REQUIRE_MENTION", "true")
    monkeypatch.delenv("DISCORD_FREE_RESPONSE_CHANNELS", raising=False)
    monkeypatch.setenv("DISCORD_THREAD_FREE_RESPONSE_CHANNELS", "789")
