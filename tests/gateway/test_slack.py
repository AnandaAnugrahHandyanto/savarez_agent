...
class TestReactions:
    ...
    @pytest.mark.asyncio
    async def test_plankton_reaction_routes_reacted_message(self, adapter):
        """A :plankton: reaction should route the reacted-to message."""
        adapter._app.client.conversations_history = AsyncMock(return_value={
            "messages": [{
                "type": "message",
                "user": "U_ORIGINAL",
                "text": "Please train on this",
        ...
        assert msg_event.source.chat_id == "C_DEV"
        assert msg_event.source.thread_id == "1710000000.000100"
        assert "Please train on this" in msg_event.text

    @pytest.mark.asyncio
    async def test_reaction_target_channel_routes_response_top_level(self, adapter):
        """Configured reaction target channel should get a top-level response."""
        ...
        adapter._app.client.chat_getPermalink = AsyncMock(return_value={
            "ok": True,
            "permalink": "https://fleet.slack.com/archives/C_DEV/p1710000000000100",
        })
        adapter._app.client.conversations_replies = AsyncMock(return_value={"messages": []})
        adapter._app.client.users_info = AsyncMock(return_value={
            "user": {"profile": {"display_name": "Kevin"}}
    ...
    async def test_reaction_target_with_thread_routes_to_target_thread(self, adapter):
        """Configured C:thread reaction target should preserve that target thread."""
        adapter.config.extra["trigger_reaction_target"] = "C_TRAIN:1719999999.000001"
        adapter._app.client.conversations_history = AsyncMock(return_value={
            "messages": [{
                "type": "message",
                "user": "U_ORIGINAL",
        ...
        await adapter._handle_slack_reaction_added({
            ...
            "event_ts": "1710000001.000202",
            "team": "T_TEAM",
        })

        msg_event = adapter.handle_message.await_args.args[0]
        assert msg_event.source.chat_id == "C_TRAIN"
        assert msg_event.source.thread_id == "1719999999.000001"
        assert msg_event.reply_to_message_id == "1719999999.000001"
