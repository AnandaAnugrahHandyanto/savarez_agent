"""Regression tests for install.sh Node.js detection and PATH hygiene.

The desktop/bootstrap installer can run from a GUI process whose PATH has not
loaded the user's shell-managed Node (nvm, fnm, Volta, mise, asdf, etc.). It
must try the user shell PATH before installing a Hermes-managed Node, and a
managed Node install must not shadow those managers by linking node/npm/npx into
~/.local/bin.
"""
