...
function MdInline({ t, text }: { t: Theme; text: string }) {
  ...
  for (const m of text.matchAll(INLINE_RE)) {
    ...
    if (m[2] && m[3]) {
      ...
      parts.push(
        <Text backgroundColor={t.color.panelBg} color={t.color.chipAccentText} key={parts.length}>
          {' '}
          {m[8]}
          {' '}
        </Text>
  ...
  const nodes = useMemo(() => {
    ...
    while (i < lines.length) {
      ...
      if (fence) {
        ...
        const title = isDiff ? 'diff' : lang || 'code'

        nodes.push(
          <Box flexDirection="column" key={key}>
            <BlockShell t={t} title={title}>
              {block.map((l, j) => {
      ...
      if (/^\s*(?:>\s*)+/.test(line)) {
        ...
        nodes.push(
          <Box flexDirection="column" key={key}>
            <BlockShell t={t} title="quote">
              {quoteLines.map((ql, qi) => (
                <Text color={t.color.panelMuted} key={qi}>
