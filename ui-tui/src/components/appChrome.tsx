...
export function StatusRule({
  ...
  const leftWidth = Math.max(12, cols - cwdLabel.length - 8)
    ...
    >
      <Box flexShrink={1} width={leftWidth}>
        <Text color={t.color.statusFg} wrap="truncate-end">
          <Text backgroundColor={busy ? t.color.chipAccentBg : t.color.chipBg} color={busy ? t.color.chipAccentText : statusColor}>
            {' '}
            {status}
            {' '}
          </Text>
          <Text color={t.color.dim}>{busy ? ' ' : ''}</Text>
          {busy ? <FaceTicker color={t.color.amber} /> : null}
          <Text color={t.color.panelMuted}> · {model}</Text>
          {ctxLabel ? <Text color={t.color.panelMuted}> · {ctxLabel}</Text> : null}
          {bar ? (
            <Text color={t.color.panelMuted}>
              {' · '}
          ...
          {voiceLabel ? <Text color={t.color.panelMuted}> · {voiceLabel}</Text> : null}
          {bgCount > 0 ? <Text color={t.color.panelMuted}> · {bgCount} bg</Text> : null}
        </Text>
      </Box>

      <Text color={t.color.panelMuted}>  </Text>
      <Text color={t.color.label}>{cwdLabel}</Text>
    </Box>
  )
}

export function FloatBox({ children, color, t }: { children: ReactNode; color: string; t: Theme }) {
  return (
    <Box
