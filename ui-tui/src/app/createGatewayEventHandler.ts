    ...
    switch (ev.type) {
      ...
      case 'message.complete': {
        ...
        if (ev.payload?.usage || 'account_limits' in (ev.payload ?? {})) {
          const hasAccountLimits = 'account_limits' in (ev.payload ?? {})

          patchUiState(state => ({
            ...state,
            info: state.info && hasAccountLimits ? { ...state.info, account_limits: ev.payload?.account_limits ?? null } : state.info,
