...
export const coreCommands: SlashCommand[] = [
  ...
  {
    aliases: ['sb'],
    help: 'status bar position (on|off|top|bottom|collapsed)',
    name: 'statusbar',
    run: (arg, ctx) => {
      const mode = arg.trim().toLowerCase()
      ...
      const next: null | StatusBarMode =
        !mode || mode === 'toggle'
          ...
          : mode === 'on' || mode === 'top'
            ? 'top'
            : mode === 'off' || mode === 'bottom' || mode === 'collapsed'
              ? mode
              : null

      if (!next) {
        return ctx.transcript.sys('usage: /statusbar [on|off|top|bottom|collapsed|toggle]')
      }
