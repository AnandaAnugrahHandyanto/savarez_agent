...
React + Ink terminal UI for Savarez. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
- Input history is stored in `~/.savarez/.savarez_history` or under `SAVAREZ_HOME`.
...
```text
ui-tui/
  packages/savarez-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      savarez-ink.d.ts    type declarations for @savarez/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent SavarezCLI subprocess for slash commands
```
