...
```bash
# Prefer the installed Hermes venv; fall back to .venv for upstream-style checkouts.
source venv/bin/activate   # or: source .venv/bin/activate
```

`scripts/run_tests.sh` probes `venv` first, then `$HOME/.hermes/hermes-agent/venv`,
then `.venv` (for upstream-style checkouts).
