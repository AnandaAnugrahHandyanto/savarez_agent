            ...
            if tool_uses:
                ...
                for tu in tool_uses:
                    tc_list.append({
                        ...
                        "function": {
                            "name": tu.name,
                            "arguments": json.dumps(tu.input, ensure_ascii=True) if isinstance(tu.input, dict) else str(tu.input),
                        },
                    })
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            if structured is not None:
                ...
                return json.dumps({"result": structured}, ensure_ascii=True)
            return json.dumps({"result": text_result}, ensure_ascii=True)
...
def _make_list_resources_handler(server_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            return json.dumps({"resources": resources}, ensure_ascii=True)
...
def _make_read_resource_handler(server_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            return json.dumps({"result": "\n".join(parts) if parts else ""}, ensure_ascii=True)
...
def _make_list_prompts_handler(server_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            return json.dumps({"prompts": prompts}, ensure_ascii=True)
