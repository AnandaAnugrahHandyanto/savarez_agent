...
transport, discovers their tools, and registers them into the savarez-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.savarez/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.savarez/logs/mcp-stderr.log), tagged with the
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            hermes_home = os.path.expanduser(
                os.getenv(
                    "SAVAREZ_HOME", os.path.join(os.path.expanduser("~"), ".savarez")
                )
            )
            candidates = [
                ...
                # image (which the Savarez Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Savarez MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Savarez gateways know how to render.
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Savarez is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `savarez mcp login {server_name}` (or delete the tokens "
            f"file under ~/.savarez/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Savarez config file.
    ...
    ``os.environ`` (which includes ``~/.savarez/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Savarez' MEDIA: tag convention and out to messaging
            ...
            # Savarez' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Savarez's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Savarez registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``savarez tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Savarez registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple savarez
