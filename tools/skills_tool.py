...
SKILL_VIEW_SCHEMA = {
    "name": "skill_view",
    "description": "Skills allow for loading information about specific tasks and workflows, as well as scripts and templates. Load a skill's full content or access its linked files (references, templates, scripts). The first relevant skill load is mandatory. First call returns SKILL.md content plus a 'linked_files' dict showing available references/templates/scripts. To access those, call again with file_path parameter. skill-repeat-guard-v0: prompt guidance only, not runtime caching or enforcement. Avoid unnecessary repeat calls for the exact same skill/file in the same active visible context, but protective replay is correct when compression/resume/context loss may have removed prior content, a different file_path/reference is needed, the skill changed or may have changed, the user asked for refresh, the task entered a new phase/hard gate, or authority/safety/live-state ambiguity requires current source instructions.",
    "parameters": {
        "type": "object",
        "properties": {
            "name": {
