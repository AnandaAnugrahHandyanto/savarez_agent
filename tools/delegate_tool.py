...
def delegate_task(
    ...
    if n_tasks == 1:
        ...
        with ThreadPoolExecutor(max_workers=max_children) as executor:
            ...
            # Claude-CLI routed tasks are absent from `children`; the
            # fabricated-entry branches below treat missing indices as
            # role=None, which is correct (no child means no role).
            _child_by_index = dict(children)
