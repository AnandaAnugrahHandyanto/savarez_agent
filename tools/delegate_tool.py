...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        "agent (kimi, claude, codex, openclaw, hermes, deepseek), always use agent_id, never role."
    )
...
def _build_role_param_description() -> str:
    ...
    return (
        "Role of the child agent. ONLY use when the user does NOT specify "
        "a named agent. 'leaf' (default) = focused worker, cannot "
        ...
        "kimi/claude/codex/openclaw/hermes/deepseek, use agent_id instead; "
        "do NOT use role."
    )
