...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        "EXECUTION MODES:\n"
        "- Synchronous delegation (default): single-task and batch calls block "
        "the parent turn until every child finishes or is interrupted. If the "
        "parent is interrupted (user sends a new message, /stop, /new), active "
        "synchronous children are cancelled with status='interrupted'.\n"
        "- Asynchronous/background delegation: pass background=True with a "
        "single goal to return immediately with a delegation_id. The subagent "
        "continues independently and its final result is re-injected into the "
        "conversation when it finishes. Use this for long-running reasoning "
        "work such as research, builds, investigations, or review that can "
        "complete after the current turn.\n\n"
        "WHEN NOT TO USE (use these instead):\n"
        "- Mechanical multi-step work with no reasoning needed -> use execute_code\n"
        "- Single tool call -> just call the tool directly\n"
        "- Tasks needing user interaction -> subagents cannot use clarify\n"
        "- Long-running shell commands that do not need an LLM -> use "
        "terminal(background=True, notify_on_complete=True)\n"
        "- Durable scheduled/recurring work that must survive restarts or run "
        "on a schedule -> use cronjob (action='create')\n\n"
        "IMPORTANT:\n"
