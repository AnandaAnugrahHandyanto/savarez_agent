...
def gateway_pending_approval_counts() -> dict[str, int]:
    ...
    with _lock:
        return {session_key: len(entries) for session_key, entries in _gateway_queues.items() if entries}


def _write_gateway_approval_pulse() -> None:
    """Best-effort pulse update when approval queues change."""
    try:
        from gateway.live_pulse import write_live_pulse
        write_live_pulse()
    except Exception:
        logger.debug("Failed to write Agent Floor approval pulse", exc_info=True)
    ...
    def _drop_entry() -> None:
        ...
        with _lock:
            ...
            if entry in queue:
                ...
                changed = True
            if not queue:
                changed = changed or session_key in _gateway_queues
                _gateway_queues.pop(session_key, None)
        if changed:
            _write_gateway_approval_pulse()
