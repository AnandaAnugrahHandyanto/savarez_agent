...
DANGEROUS_PATTERNS = [
    ...
    (r'\b(bash|sh|zsh|ksh)\s+-[^\s]*i\b.*(?:/dev/(tcp|udp)/|>&|0>&1)', "interactive reverse shell"),
    (r'/dev/(tcp|udp)/[^\s;|&]+/\d+', "reverse shell via /dev/tcp or /dev/udp"),
    (r'\b(nc|ncat)\b[^\n]*(\s-e\s+|\s--exec\s+)(/bin/)?(sh|bash)\b', "netcat reverse shell"),
    (r'\bmkfifo\b[^\n]*(nc|ncat)\b|\b(nc|ncat)\b[^\n]*\bmkfifo\b', "mkfifo netcat reverse shell"),
    (r'\bsocat\b[^\n]*\bexec:', "socat reverse shell"),
    (r'\b(cat|less|more|head|tail|grep|rg|sed|awk)\b[^\n]*(~/\.hermes/(\.env|auth\.json)|(?:\$home|\$\{home\})/\.hermes/(\.env|auth\.json)|~/\.ssh/|(?:\$home|\$\{home\})/\.ssh/|~/\.(netrc|pgpass|npmrc|pypirc))', "read sensitive credential file"),
