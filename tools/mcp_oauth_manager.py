...
# ---------------------------------------------------------------------------
# SavarezMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------
...
def _make_hermes_provider_class() -> Optional[type]:
    ...
    class SavarezMCPOAuthProvider(OAuthClientProvider):
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`SavarezTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, SavarezTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import SavarezTokenStorage
                        if isinstance(storage, SavarezTokenStorage):
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import SavarezTokenStorage
            if not isinstance(storage, SavarezTokenStorage):
                return
    ...
    return SavarezMCPOAuthProvider
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`SavarezMCPOAuthProvider` directly using the helpers
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``savarez mcp remove <name>`` and (indirectly) by
        ``savarez mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
