...
MEMORY_SCHEMA = {
    ...
    "description": (
        ...
        "remove (delete -- old_text identifies it), patch (update existing -- pattern is a "
        "regex that locates and rewrites the matched span; tolerant of whitespace/wording "
        "drift, so prefer it over replace when the exact old_text is uncertain).\n\n"
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "content": {
                "type": "string",
                "description": "The entry content. Required for 'add', 'replace', and 'patch' (the replacement text)."
            },
            "old_text": {
                "type": "string",
