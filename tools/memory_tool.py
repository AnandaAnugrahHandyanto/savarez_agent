...
MEMORY_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "old_text": {
                "type": "string",
                "description": "REQUIRED for 'replace' and 'remove' actions. Short unique substring identifying the entry to update or delete. Must be non-empty and match exactly one existing entry.",
            },
        },
        "required": ["action", "target"],
        "if": {"properties": {"action": {"enum": ["replace", "remove"]}}},
        "then": {"required": ["action", "target", "old_text"]},
    },
}
