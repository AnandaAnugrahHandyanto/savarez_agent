...
Set background=true for long-running processes (pairs with notify_on_complete=true).
Set pty=true for interactive CLI tools (Codex, Claude Code, Python REPL).
Use workdir for per-command working directory.

IMPORTANT: Load the terminal-best-practices skill for background processes, PTY mode, server lifecycle, and environment management.
"""
