...
def get_active_environments_info() -> Dict[str, Any]:
    """Return a lightweight snapshot of active environments and their disk usage."""
    scratch_dir = _get_scratch_dir()
    total_bytes = 0
    with _env_lock:
    ...
    for task_id in task_ids:
        ...
        if not prefix:
            continue
        for env_dir in Path(scratch_dir).glob(f'hermes-sandbox-{prefix}*'):
    ...
    return {
        ...
        'task_ids': task_ids,
        'total_disk_usage_mb': total_bytes / (1024 ** 2),
    }
...
def terminal_tool(
    ...
    try:
        ...
        # Wrap command with RTK if available and applicable. RTK is optional;
        # terminal execution must fail open when the adapter is not installed.
        try:
            from lib.rtk_adapter import get_rtk_adapter
            rtk = get_rtk_adapter()
        except Exception:
            rtk = None
        effective_command = command
        if rtk is not None and rtk.is_available() and rtk.should_use_rtk(command):
            effective_command = rtk.wrap_command(command)
            logger.debug("RTK wrapping command: %s -> %s", command[:100], effective_command[:100])
        ...
        if background:
            ...
            while retry_count <= max_retries:
                try:
                    ...
                    result = env.execute(effective_command, **execute_kwargs)
                except Exception as e:
            ...
            # Apply RTK filtering to reduce token consumption (60-90% savings)
            try:
                from lib.rtk_adapter import get_rtk_adapter
                rtk = get_rtk_adapter()
            except Exception:
            ...
            if rtk is not None and rtk.is_available() and rtk.should_use_rtk(command):
                ...
                # For now, RTK filtering is done at command execution time
                # Post-processing filters will be added in future iterations
                # output = rtk.filter_output(command, output)
                # filtered_length = len(output)
                # stats = rtk.get_stats(original_length, filtered_length)
                # logger.debug("RTK filtered output: %s", stats)
