...
MINIMAX_IMAGE_GENERATE_SCHEMA = {
    ...
    "description": "Generate images from text prompts using MiniMax's Image-01 model. Uses the same MINIMAX_API_KEY as text/chat models (or MINIMAX_CN_API_KEY for China region). Returns a local file path to the generated image. Display it using markdown: ![description](file_path) or send it directly to the user. Aspect ratios: 1:1 (square), 16:9 (wide), 9:16 (tall), 4:3, 3:4.",
    "parameters": {
        "type": "object",
        "properties": {
            "prompt": {
...
registry.register(
    ...
    handler=lambda args, **kw: minimax_image_generate_tool(
    ...
    requires_env=["MINIMAX_API_KEY", "MINIMAX_CN_API_KEY"],
    is_async=False,
    emoji="🎨",
)
