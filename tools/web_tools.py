...
- LLMLayer: https://llmlayer.ai (search, extract)
...
def _get_backend() -> str:
    ...
    if configured in {"parallel", "firecrawl", "tavily", "exa", "llmlayer", "searxng", "brave-free", "ddgs", "xai"}:
        return configured
    ...
    backend_candidates = (
        ...
        ("llmlayer", _has_env("LLMLAYER_API_KEY")),
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "llmlayer":
        return _has_env("LLMLAYER_API_KEY")
    if backend == "parallel":
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        # (brave-free, ddgs, searxng, exa, llmlayer, parallel, tavily, firecrawl)
...
async def web_extract_tool(
    ...
    try:
        ...
        if not safe_urls:
            ...
            # All bundled providers (brave-free, ddgs, searxng, exa, llmlayer,
                    ...
                    return json.dumps(
                        {
                            ...
                            "error": (
                                ...
                                "tavily, exa, parallel, or llmlayer."
                            ),
                        },
                        ensure_ascii=False,
                    )
...
def check_web_api_key() -> bool:
    ...
    if configured in {"exa", "llmlayer", "parallel", "firecrawl", "tavily", "searxng", "brave-free", "ddgs", "xai"}:
        return _is_backend_available(configured)
    return any(
        _is_backend_available(backend)
        for backend in ("exa", "llmlayer", "parallel", "firecrawl", "tavily", "searxng", "brave-free", "ddgs", "xai")
    )
...
if __name__ == "__main__":
    ...
    if web_available:
        ...
        elif backend == "llmlayer":
            print("   Using LLMLayer API (https://llmlayer.ai)")
        ...
        print(
            "Set EXA_API_KEY, LLMLAYER_API_KEY, PARALLEL_API_KEY, TAVILY_API_KEY, FIRECRAWL_API_KEY, FIRECRAWL_API_URL"
