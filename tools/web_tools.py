...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        # now live as plugins. The dispatcher tries the fallback chain:
        # if the active provider fails mid-call (rate limit, empty
        # results, etc.), the next provider in the chain is tried.
        ...
        if not chain:
            ...
            if provider is None:
                ...
                # Single provider, no chain — call directly
                logger.info(
                    "Web search via %s: '%s' (limit: %d)",
                    provider.name, query, limit,
                )
                response_data = provider.search(query, limit)
                if _is_provider_failure(response_data):
                    logger.warning(
                        "Web search provider %s failed: %s",
                        provider.name,
                        response_data.get("error", "empty results")[:120],
                    )
        else:
            # Walk the chain until one succeeds
            response_data = {"success": False, "error": "No search provider available"}
...
async def web_extract_tool(
    ...
    try:
        ...
        if not safe_urls:
            ...
            if not chain:
                # No extract providers available — fall back to old resolution
            ...
            for i, provider in enumerate(chain):
                ...
                # List-shaped: check if all entries errored
                if isinstance(results, list) and results and all(
                    ...
                    logger.info(
                        "Web extract %s all pages errored, trying next: %s",
                        provider.name,
                        str(last_error)[:80],
                    )
                    continue
