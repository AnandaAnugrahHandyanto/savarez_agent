...
Backend is selected during ``savarez tools`` setup (web.backend in config.yaml).
When available, Savarez can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Savarez config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Savarez' config/.env layer (``hermes config set``, ``hermes tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.savarez/config.yaml."""
    try:
...
def _get_backend(capability: str = "search") -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``savarez tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `savarez tools` repaint.
        try:
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `savarez tools` to set one up."
                ),
            }
        else:
            logger.info(
