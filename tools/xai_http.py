...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``savarez tools`` repaint, tool-registration scans,
    ...
    2. ``~/.savarez/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.savarez/.env`` first, then ``os.environ``.
...
def hermes_xai_user_agent() -> str:
    """Return a stable Savarez-specific User-Agent for xAI HTTP calls."""
    try:
        from hermes_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Savarez-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Savarez-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``hermes_cli.config.get_env_value`` so keys
    stored in ``~/.savarez/.env`` (the standard Savarez location) are honored —
