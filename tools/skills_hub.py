...
# ---------------------------------------------------------------------------
# Paths — resolved dynamically so that profile / HERMES_HOME changes
# within a process lifetime are reflected without a module reload.
#
# Each path constant is available as a module attribute for backward
# compatibility (resolved via __getattr__).  Internal code should prefer
# the private getter functions below.
# ---------------------------------------------------------------------------
...
def _get_hub_dir() -> Path:
    if "HUB_DIR" in globals():
        return globals()["HUB_DIR"]
    return _get_skills_dir() / ".hub"


def _get_lock_file() -> Path:
    if "LOCK_FILE" in globals():
        return globals()["LOCK_FILE"]
    return _get_hub_dir() / "lock.json"


def _get_quarantine_dir() -> Path:
    if "QUARANTINE_DIR" in globals():
        return globals()["QUARANTINE_DIR"]
    return _get_hub_dir() / "quarantine"


def _get_audit_log() -> Path:
    if "AUDIT_LOG" in globals():
        return globals()["AUDIT_LOG"]
    return _get_hub_dir() / "audit.log"


def _get_taps_file() -> Path:
    if "TAPS_FILE" in globals():
        return globals()["TAPS_FILE"]
    return _get_hub_dir() / "taps.json"


def _get_index_cache_dir() -> Path:
    if "INDEX_CACHE_DIR" in globals():
        return globals()["INDEX_CACHE_DIR"]
    return _get_hub_dir() / "index-cache"


def __getattr__(name: str):
    """Backward-compatible lazy resolution of path constants.
    Module-level __getattr__ (PEP 562) fires when ``module.NAME``
    is accessed and *name* is not in the module's __dict__."""
    _map = {
        ...
        "TAPS_FILE": _get_taps_file,
        "INDEX_CACHE_DIR": _get_index_cache_dir,
        "HERMES_INDEX_CACHE_FILE": lambda: _get_index_cache_dir() / "hermes-index.json",
    }
    if name in _map:
        return _map[name]()
...
def _resolve_lock_install_path(install_path: str, skill_name: str) -> Path:
    ...
    2. After resolve(), reject not just escape-out but also ``resolved == _get_skills_dir()``
       — an empty/``"."``/``""`` install_path resolves to the skills root itself,
       and ``rmtree(_get_skills_dir())`` would wipe every installed skill.
    """
    normalized = _normalize_lock_install_path(install_path, skill_name)
    skills_root = _get_skills_dir().resolve()

    target = _get_skills_dir()
...
class GitHubSource(SkillSource):
    ...
    def _read_cache(self, key: str) -> Optional[list]:
        ...
        cache_file = _get_index_cache_dir() / f"{key}.json"
    ...
    def _write_cache(self, key: str, data: list) -> None:
        ...
        _get_index_cache_dir().mkdir(parents=True, exist_ok=True)
        cache_file = _get_index_cache_dir() / f"{key}.json"
        try:
...
def _write_index_cache(key: str, data: Any) -> None:
    ...
    ignore_file = _get_hub_dir() / ".ignore"
...
class HubLockFile:
    ...
    def __init__(self, path: Path | None = None):
        if path is None:
            # Resolve lazily via module __getattr__ so that a
            # HERMES_HOME change between module import and first
            # instantiation is picked up.
            path = _get_lock_file()
        self.path: Path = path  # type: ignore[assignment]
...
def append_audit_log(action: str, skill_name: str, source: str,
    ...
    _get_audit_log().parent.mkdir(parents=True, exist_ok=True)
    ...
    line = " ".join(parts) + "\n"
    try:
        with open(_get_audit_log(), "a", encoding="utf-8") as f:
            f.write(line)
    except OSError as e:
...
def ensure_hub_dirs() -> None:
    ...
    _get_hub_dir().mkdir(parents=True, exist_ok=True)
    _get_quarantine_dir().mkdir(exist_ok=True)
    _get_index_cache_dir().mkdir(exist_ok=True)
    if not _get_lock_file().exists():
        _get_lock_file().write_text('{"version": 1, "installed": {}}\n')
    if not _get_audit_log().exists():
        _get_audit_log().touch()
    if not _get_taps_file().exists():
        _get_taps_file().write_text('{"taps": []}\n')
...
def quarantine_bundle(bundle: SkillBundle) -> Path:
    ...
    dest = _get_quarantine_dir() / skill_name
    if dest.exists():
        shutil.rmtree(dest)
...
def install_from_quarantine(
    ...
    quarantine_root = _get_quarantine_dir().resolve()
    ...
    lock.record_install(
        ...
        install_path=str(install_dir.relative_to(_get_skills_dir())),
...
# HERMES_INDEX_CACHE_FILE is resolved lazily via module __getattr__
...
def _load_hermes_index() -> Optional[dict]:
    ...
    """
    # Check local cache
    if (_get_index_cache_dir() / 'hermes-index.json').exists():
        try:
            age = time.time() - (_get_index_cache_dir() / 'hermes-index.json').stat().st_mtime
            if age < HERMES_INDEX_TTL:
                return json.loads((_get_index_cache_dir() / 'hermes-index.json').read_text())
        except (OSError, json.JSONDecodeError):
            pass
    ...
    # Cache locally
    try:
        (_get_index_cache_dir() / 'hermes-index.json').parent.mkdir(parents=True, exist_ok=True)
        (_get_index_cache_dir() / 'hermes-index.json').write_text(json.dumps(data))
    except OSError:
        pass
