"""
Lazy dependency installer for opt-in Savarez AI Agent backends.

Many Savarez features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``savarez-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``savarez tools`` or the manual pip command.
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``savarez update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def active_features() -> list[str]:
    ...
    Used by ``savarez update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``savarez update``. Never raises; lazy-install failures
