...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None):
    ...
    # --- Non-Telegram/Discord/Matrix/Weixin/SIGNAL platforms without native media support (WhatsApp, Slack, etc.) ---
    if platform not in (Platform.TELEGRAM, Platform.DISCORD, Platform.MATRIX, Platform.WEIXIN, Platform.SIGNAL) and media_files and not message.strip():
        return {
            "error": (
    ...
    # Warn for platforms that don't accept media_files in their send function
    warning = None
    if platform not in (Platform.TELEGRAM, Platform.DISCORD, Platform.MATRIX, Platform.WEIXIN, Platform.SIGNAL) and media_files:
        warning = (
    ...
    for chunk in chunks:
        if platform == Platform.SLACK:
            ...
            result = await _send_signal(pconfig.extra, chat_id, chunk, media_files)
