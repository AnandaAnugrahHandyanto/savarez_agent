...
# U... user IDs are also accepted: _send_slack will call conversations.open
# automatically to resolve them to a D... channel before posting.
...
SEND_MESSAGE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "target": {
                "type": "string",
                "description": "Delivery target. Format: 'platform' (current conversation if active, else home channel), 'platform:#channel-name', 'platform:chat_id', or 'platform:chat_id:thread_id' for Telegram topics and Discord threads. Examples: 'telegram', 'telegram:-1001234567890:17585', 'discord:999888777:555444333', 'discord:#bot-home', 'slack:#engineering', 'slack:U08XXXXXXX' (DM a user by Slack user ID — the DM is opened automatically, so this works even if the user is not in the action='list' output), 'signal:+155****4567', 'matrix:!roomid:server.org', 'matrix:@user:server.org', 'yuanbao:direct:<account_id>' (DM), 'yuanbao:group:<group_code>' (group chat)"
            },
            "message": {
                "type": "string",
...
def _handle_send(args):
    ...
    # ai-skills-library-send-message-user-prefix-patch: accept
    # `user:<slack_id>` as a generic alias and route it to Slack. The
    # bs-flow-progressing skill (and other催办-style flows) document this
    # format for DMing arbitrary users by their NS-recorded slack_id.
    # Without this rewrite, `user` is parsed as the platform name and the
    # call fails channel-name resolution before reaching _send_slack,
    # which already handles U-IDs via conversations.open.
    if platform_name == "user" and target_ref and _SLACK_TARGET_RE.fullmatch(target_ref):
        platform_name = "slack"
    chat_id = None
    thread_id = None
