...
SESSION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "FTS5-backed retrieval over the SQLite message store.\n\n"
        "FOUR SHAPES:\n"
        "1) DISCOVERY: pass `query` (keywords/boolean/phrases) → top N sessions with excerpts\n"
        "2) SCROLL: pass `session_id` + `around_message_id` → window of ±N messages around anchor\n"
        "3) READ: pass `session_id` only → dumps whole session (first 20 + last 10)\n"
        "4) BROWSE: no args → recent sessions chronologically\n\n"
        "FTS5: AND is default, use OR/NOT/quotes/prefix wildcards explicitly.\n\n"
        "IMPORTANT: Load the session-search-guide skill for full calling shapes, "
        "field reference, scroll patterns, and query syntax."
    ),
    "parameters": {
        "type": "object",
        "properties": {
