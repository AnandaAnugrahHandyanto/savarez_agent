...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "preferred for fixes), edit (full SKILL.md rewrite — major overhauls only), delete, "
        "write_file, remove_file.\n\n"
        "On delete, pass `absorbed_into=<umbrella>` to merge into another skill, or "
        "`absorbed_into=''` to prune with no forwarding target.\n\n"
        "Create when: complex task succeeded (5+ calls), errors overcome, user-corrected "
        "approach worked, or non-trivial workflow discovered.\n"
        "Update when: instructions stale/wrong, pitfalls found during use. "
        "Patch immediately if you used a skill and hit issues.\n\n"
        "After difficult/iterative tasks, offer to save as a skill. "
        "Skip for simple one-offs. Pinned skills are protected from deletion only."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            ...
            "absorbed_into": {
                "type": "string",
                "description": ("For 'delete' only — 'umbrella skill name' to merge this skill into an existing one (target must exist), or '' to prune with no target.")
            },
        },
