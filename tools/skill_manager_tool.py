...
~/.savarez/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `savarez config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.savarez/skills/ (single source of truth)
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`savarez curator unpin {name}` if they want to delete it. "
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.savarez/skills/) first, then any
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER Savarez profiles.
    ...
    # Default profile (~/.savarez/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.savarez/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`savarez -p "
            ...
            base += (
                ...
                f"Switch profiles (`savarez -p <name>`) to edit there, or "
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "will refuse with a message pointing the user to `savarez curator unpin <name>`. "
