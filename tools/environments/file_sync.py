...
def iter_sync_files(container_base: str = "/root/.savarez") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.savarez to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync_back(self, hermes_home: Path | None = None) -> None:
        ...
        Downloads the remote ``.savarez/`` directory as a tar archive,
        ...
        # against an uninitialized remote .savarez/ directory.
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="savarez-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.savarez/skills/a.md`` →
        ``~/.savarez/skills/a.md``, a new remote file at
        ``/root/.savarez/skills/b.md`` maps to ``~/.savarez/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
