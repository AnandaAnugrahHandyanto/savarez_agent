...
def _load_hermes_env_vars() -> dict[str, str]:
    """Load ~/.savarez/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Savarez profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale savarez-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=savarez-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Savarez process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=savarez-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a savarez process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Savarez
    process leaves its container behind permanently because there's no
    subsequent Savarez process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=savarez-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=savarez-profile={_sanitize_label_value(profile_filter)}"])
...
#   SETUID/SETGID - the image's init drops from root to the 'savarez'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``savarez-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.savarez/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. savarez-agent:latest) already use /init as PID 1
        ...
        container_name = f"savarez-{uuid.uuid4().hex[:8]}"
        # Labels make savarez-created containers identifiable to:
        #   * the orphan reaper (`savarez-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`savarez-task-id`, `savarez-profile`)
        #   * operators running `docker ps --filter label=savarez-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Savarez profile is captured at
        ...
        label_args = [
            "--label", "savarez-agent=1",
            "--label", f"savarez-task-id={task_label}",
            "--label", f"savarez-profile={profile_name}",
        ]
        self._labels = {
            "savarez-agent": "1",
            "savarez-task-id": task_label,
            "savarez-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Savarez process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Savarez secret blocklist. Only implicit passthrough
        # keys are filtered.
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``savarez-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=savarez-agent=1",
                    "--filter", f"label=savarez-task-id={task_label}",
                    "--filter", f"label=savarez-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
        ...
        # container) but can happen if a previous Savarez process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Savarez exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Savarez process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Savarez startup. That covers the
        ...
        # outlives Savarez processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"savarez-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``savarez /quit``
