...
Provides speech-to-text transcription with multiple providers:
  ...
  - **elevenlabs** — ElevenLabs Speech-to-Text API, requires ``ELEVENLABS_API_KEY``.
...
def _get_provider(stt_config: dict) -> str:
    ...
    if get_env_value("XAI_API_KEY"):
        ...
        return "xai"
    if get_env_value("ELEVENLABS_API_KEY"):
        logger.info("No local STT available, using ElevenLabs Speech-to-Text API")
        return "elevenlabs"
    return "none"
...
def _transcribe_elevenlabs(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    Uses the ``POST /v1/speech-to-text`` REST endpoint with multipart/form-data.
    Requires ``ELEVENLABS_API_KEY``, the same token used by ElevenLabs TTS.
    """
    ...
    data: Dict[str, str] = {"model_id": model_name or DEFAULT_ELEVENLABS_STT_MODEL}

    language_code = str(elevenlabs_config.get("language_code") or "").strip()
    if language_code:
        data["language_code"] = language_code

    if "tag_audio_events" in elevenlabs_config:
        data["tag_audio_events"] = (
            "true" if is_truthy_value(elevenlabs_config.get("tag_audio_events")) else "false"
        )
    if "diarize" in elevenlabs_config:
        data["diarize"] = (
            "true" if is_truthy_value(elevenlabs_config.get("diarize")) else "false"
        )
    timestamps_granularity = str(elevenlabs_config.get("timestamps_granularity") or "").strip()
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            try:
                ...
                if isinstance(err_body, dict):
                    ...
                    detail = (
                        str(err_body.get("detail") or err_body.get("message") or "")
                        or (str(error_obj.get("message")) if isinstance(error_obj, dict) and error_obj.get("message") else "")
        ...
        logger.info(
            "Transcribed %s via ElevenLabs STT (%s, %d chars)",
            Path(file_path).name,
            model_name,
            len(transcript_text),
        )
...
def transcribe_audio(file_path: str, model: Optional[str] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "error": (
            ...
            "for ElevenLabs Speech-to-Text, or set VOICE_TOOLS_OPENAI_KEY "
