...
Lets the LLM write a Python script that calls Savarez tools via RPC,
...
# available on every platform Savarez itself runs on.
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Savarez-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Savarez-profile soft guard."""',
...
"""Auto-generated Savarez tools RPC stubs."""
...
"""Auto-generated Savarez tools RPC stubs (file-based transport)."""
...
def execute_code(
    ...
    try:
        ...
        # Ensure the savarez-agent root is importable in the sandbox so
        ...
        # HERMES_TIMEZONE is an internal Savarez setting and must not leak
        ...
        # {SAVAREZ_HOME}/home/ when that directory exists.
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.savarez/.env')).
...
def _load_config() -> dict:
    ...
    ``savarez --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (savarez-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``savarez tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and savarez-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.savarez/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Savarez tools programmatically. "
