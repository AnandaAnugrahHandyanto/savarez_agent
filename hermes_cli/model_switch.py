...
def list_authenticated_providers(
    ...
    try:
        ...
        curated["openai-codex"] = get_codex_model_ids()
    ...
    def _env_vars_for_provider(hermes_id: str, mdev_id: str, pdata: dict) -> list[str]:
        """Merge models.dev env names with Hermes-specific provider env names."""
        envs: list[str] = []
        raw_envs = pdata.get("env", [])
        if isinstance(raw_envs, list):
            envs.extend(ev for ev in raw_envs if isinstance(ev, str))

        try:
            from hermes_cli.auth import PROVIDER_REGISTRY

            for pid in (hermes_id, mdev_id):
                pconfig = PROVIDER_REGISTRY.get(pid)
                if pconfig:
                    envs.extend(pconfig.api_key_env_vars)
        except Exception:
            pass

        for pid in (hermes_id, mdev_id):
            overlay = HERMES_OVERLAYS.get(pid)
            if overlay:
                envs.extend(overlay.extra_env_vars)

        return list(dict.fromkeys(ev for ev in envs if ev))

    def _models_for_provider(hermes_id: str) -> list[str]:
        """Merge dynamic models.dev catalogs with static fallback lists.

        For alibaba, always merges dynamic (from models.dev) and static (hardcoded)
        lists to ensure third-party models like glm-5, kimi-k2.5, MiniMax-M2.5 appear
        in the picker alongside native alibaba/qwen models.
        """
        static = curated.get(hermes_id, [])
        if hermes_id not in {"openrouter", "nous"}:
            try:
                dynamic = list_agentic_models(hermes_id)
                if dynamic:
                    if hermes_id == "alibaba":
                        # Merge: dedupe, preserve dynamic order, append static extras
                        seen = set()
                        merged = []
                        for m in dynamic:
                            if m not in seen:
                        ...
                        for m in static:
                            if m not in seen:
                                ...
                                merged.append(m)
                        return merged
                    elif len(dynamic) > len(static):
                        return dynamic
            except Exception as exc:
                logger.debug("models.dev model list failed for %s: %s", hermes_id, exc)
        return static
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        env_vars = _env_vars_for_provider(hermes_id, mdev_id, pdata)
        ...
        model_ids = _models_for_provider(hermes_id)
        total = len(model_ids)
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        model_ids = _models_for_provider(pid)
        total = len(model_ids)
        top = model_ids[:max_models]
