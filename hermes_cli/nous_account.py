    ...
    *,
    ...
    if account_info is None:
        return (
            ...
            f"is unavailable. Run `savarez model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Nous inference credentials are configured, but Savarez cannot verify "
                f"your Nous Portal paid access for {capability}. Log in with "
                f"`savarez model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to Nous Portal to use {capability}: run `savarez model`. "
    ...
    if account_info.paid_service_access is None:
        ...
        if include_refresh_hint:
            detail += " Run `savarez model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Savarez could not find a Nous Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `savarez model` to "
