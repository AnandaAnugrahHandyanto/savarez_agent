"""
Backup and import commands for savarez CLI.

`savarez backup` creates a zip archive of the entire ~/.savarez/ directory
(excluding the savarez-agent repo and transient files).

`savarez import` restores from a backup zip, overlaying onto the current
SAVAREZ_HOME root.
"""
...
_EXCLUDED_DIRS = {
    "savarez-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to savarez root) should be skipped."""
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside SAVAREZ_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Savarez home directory."""
    hermes_root = get_default_hermes_root()

    if not hermes_root.is_dir():
        print(f"Error: Savarez home directory not found at {hermes_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"savarez-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"savarez-backup-{stamp}.zip"
    ...
    print(f"\nRestore with: savarez import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Savarez backup.
    ...
    # Look for telltale files that a savarez home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Savarez backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.savarez/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a savarez dir name
        if prefix in {".savarez", "savarez"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Savarez configuration.")
        ...
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print(f"  Run: savarez profile list  (after installing savarez)")

        # Guidance
        print()
        if not (hermes_root / "savarez-agent").is_dir():
            print("Note: The savarez-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: savarez update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  savarez -p {pname} gateway install")

        print("Done. Your Savarez configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and savarez backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to SAVAREZ_HOME).
...
# — `savarez update` snapshots this set before pulling so approved-user lists
...
# Relative path of the cron job database inside SAVAREZ_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``savarez update``.
    ...
    Args:
        ...
        hermes_home: Override for the Savarez home directory (tests).
...
def run_quick_backup(args) -> None:
    """CLI entry point for savarez backup --quick."""
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of SAVAREZ_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<SAVAREZ_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``savarez update``) should continue even if the backup fails.
    """
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `savarez claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of SAVAREZ_HOME under ``backups/`` before a
    ``savarez claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``savarez import <archive>``.  Writes to
    ``<SAVAREZ_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `savarez import` and the
