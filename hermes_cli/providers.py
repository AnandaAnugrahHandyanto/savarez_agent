"""
Single source of truth for provider identity in Savarez AI Agent.
...
2. **Savarez overlays** — transport type, auth patterns, aggregator flags,
...
# -- Savarez overlay ----------------------------------------------------------
# Savarez-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class SavarezOverlay:
    """Savarez-specific provider metadata layered on top of models.dev."""
...
HERMES_OVERLAYS: Dict[str, SavarezOverlay] = {
    "openrouter": SavarezOverlay(
    ...
    "openai-codex": SavarezOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": SavarezOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": SavarezOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": SavarezOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="HERMES_QWEN_BASE_URL",
    ),
    "google-gemini-cli": SavarezOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": SavarezOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": SavarezOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": SavarezOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": SavarezOverlay(
    ...
    "kimi-for-coding": SavarezOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": SavarezOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": SavarezOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": SavarezOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": SavarezOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": SavarezOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": SavarezOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": SavarezOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "opencode": SavarezOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": SavarezOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "huggingface": SavarezOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": SavarezOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": SavarezOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": SavarezOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": SavarezOverlay(
        transport="openai_chat",
        base_url_env_var="TOKENHUB_BASE_URL",
    ),
    "arcee": SavarezOverlay(
        transport="openai_chat",
    ...
    "azure-foundry": SavarezOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": SavarezOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "savarez", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Savarez overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Savarez overlay
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + savarez extra
    ...
    if overlay is not None:
        # Savarez-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
