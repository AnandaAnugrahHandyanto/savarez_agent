"""savarez hooks — inspect and manage shell-script hooks.

Usage::

    savarez hooks list
    savarez hooks test <event> [--for-tool X] [--payload-file F]
    savarez hooks revoke <command>
    savarez hooks doctor

Consent records live under ``~/.savarez/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.savarez/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``savarez hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: savarez hooks {list|test|revoke|doctor}")
        print("Run 'savarez hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.savarez/config.yaml.")
        print("See `savarez hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `savarez hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `savarez hooks test` and `savarez hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Savarez wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `savarez hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `savarez hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `savarez hooks doctor`.")
