...
on Japanese locales, etc.).  Savarez's banners, tool output feed, and slash
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Savarez is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the Savarez-managed Git install directories so
    ...
    # already-running shells don't see the change — which means savarez
    ...
    # install without Savarez-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Savarez's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Savarez's
    ``savarez config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Savarez (User env var in Windows Settings, or
    export in a PowerShell profile) and Savarez picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Savarez-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\savarez\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``savarez`` from right after install) retain their old PATH.

    Any subprocess Savarez spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Savarez-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "savarez", "git", "cmd"),
        os.path.join(local_appdata, "savarez", "git", "bin"),
        os.path.join(local_appdata, "savarez", "git", "usr", "bin"),
        # Savarez venv Scripts directory — host of the savarez.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes savarez, but harmless to include.
        os.path.join(local_appdata, "savarez", "savarez-agent", "venv", "Scripts"),
