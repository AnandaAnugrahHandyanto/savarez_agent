...
Pure logic: walks a Savarez config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `savarez doctor` and a future `savarez migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Savarez config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Savarez config and return retirement issues.
