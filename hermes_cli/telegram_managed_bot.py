...
manual BotFather token copy-paste. Savarez talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.savarez-agent.nousresearch.com"
...
DEFAULT_MANAGER_BOT = "SavarezSetupBot"

DEFAULT_BOT_NAME = "Savarez Agent"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Savarez Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Savarez Telegram onboarding service.")
