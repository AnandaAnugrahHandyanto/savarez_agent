...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Savarez'
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "savarez"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Savarez' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Savarez for
        ...
        try:
            ...
            # Tools/MCP servers (excluding the savarez-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "savarez-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + Savarez tool callback are always-on production
            ...
            if "savarez-tools" in mig_report.migrated:
                msg_lines.append(
                    "Savarez tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Savarez runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Savarez tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Savarez runtime.")
