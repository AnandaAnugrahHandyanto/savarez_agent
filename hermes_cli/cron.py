"""
Cron subcommand for savarez CLI.
...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(savarez\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*savarez)"
    r"|(systemctl\s+(restart|stop|start)\s+.*savarez)"
    r"|(p?kill\s+.*savarez.*gateway)"
)
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'savarez cron create ...' or the /cron command in chat.", Colors.DIM))
        return
    ...
    if not find_gateway_pids():
        ...
        print(color("     Start it with: savarez gateway install", Colors.DIM))
        print(color("                    sudo savarez gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print("    savarez gateway install    # Install as a user service")
        print("    sudo savarez gateway install --system  # Linux servers: boot-time system service")
        print("    savarez gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `savarez gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
...
def cron_command(args):
    ...
    print("Usage: savarez cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
