...
function renderSafety(){
  ...
  $('#safetyPanel').innerHTML='<div class="panel accent"><h3>Approval risk status</h3>'+badge(ar.status||'ok')+'</div>'+riskCards.map(([k,v])=>`<div class="panel ${v? 'risk-card':''}"><h3>${k}</h3><strong>${escapeHtml(v)}</strong></div>`).join('')+base.map(([k,v])=>`<div class="panel"><h3>${k}</h3>${badge(v)}</div>`).join('');
}
