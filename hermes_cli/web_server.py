...
async def get_logs_summary():
    """Return error/warning counts from the last 500 lines of each log file.

    Used by the Logs page to show at-a-glance health indicators without loading
    full log content.  Fails-open: missing or unreadable log files return zero
    counts instead of a 500.
    """
    from hermes_cli.logs import _read_tail, LOG_FILES

    summary: dict = {}
    for file_key, log_name in LOG_FILES.items():
        log_path = get_hermes_home() / "logs" / log_name
        counts: dict = {"errors": 0, "warnings": 0, "recent_errors": []}
        if log_path.exists():
            try:
                tail = _read_tail(log_path, 500, has_filters=False)
                err_lines = [
                    line for line in tail
                    if any(kw in line.upper() for kw in ("ERROR", "CRITICAL", "FATAL"))
                ]
                warn_lines = [
                    line for line in tail
                    if ("WARNING" in line.upper() or " WARN " in line.upper())
                    and not any(kw in line.upper() for kw in ("ERROR", "CRITICAL", "FATAL"))
                ]
                counts = {
                    "errors": len(err_lines),
                    "warnings": len(warn_lines),
                    "recent_errors": err_lines[-5:],
                }
            except Exception:
                _log.debug("get_logs_summary failed for %s", file_key, exc_info=True)
        summary[file_key] = counts
