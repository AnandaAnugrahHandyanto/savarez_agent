...
async def get_sessions(limit: int = 20, offset: int = 0, min_messages: int = 0):
    ...
    try:
        ...
        try:
            if min_messages > 0:
                # Custom query with message_count filter — mirrors list_sessions_rich
                # but adds the min_messages constraint to both data and count queries
                sessions_rows = db._conn.execute("""
                    SELECT s.*,
                        COALESCE(
                    ...
                    FROM sessions s
                    WHERE s.parent_session_id IS NULL AND s.message_count >= ?
                    ORDER BY s.started_at DESC
                    LIMIT ? OFFSET ?
                """, (min_messages, limit, offset)).fetchall()
                sessions = [dict(r) for r in sessions_rows]
                count_row = db._conn.execute(
                    "SELECT COUNT(*) FROM sessions WHERE parent_session_id IS NULL AND message_count >= ?",
                    (min_messages,),
                ).fetchone()
