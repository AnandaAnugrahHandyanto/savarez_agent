"""
Savarez AI Agent — Web UI server.
...
try:
    ...
    # running `savarez dashboard` needs fastapi+uvicorn; lazy install keeps
...
app = FastAPI(title="Savarez AI Agent", version=__version__, lifespan=_lifespan)
...
_SESSION_HEADER_NAME = "X-Savarez-Session-Token"
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Savarez (no terminal prompt), "
...
async def get_status():
    ...
    # and which providers are registered so ``savarez status`` and the
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the savarez home root).
    """
...
# streamed to a per-action log file under ``~/.savarez/logs/<action>.log`` so
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "savarez-update": "savarez-update.log",
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``savarez <subcommand>`` detached and record the Popen handle.
...
async def restart_gateway():
    """Kick off a ``savarez gateway restart`` in the background."""
    try:
...
@app.post("/api/savarez/update")
async def update_hermes():
    """Kick off ``savarez update`` in the background."""
    install_method = detect_install_method(PROJECT_ROOT)
    if install_method == "docker":
        message = format_docker_update_message()
        _record_completed_action("savarez-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "savarez-update",
    ...
    try:
        proc = _spawn_hermes_action(["update"], "savarez-update")
    except Exception as exc:
        _log.exception("Failed to spawn savarez update")
...
async def check_hermes_update(force: bool = False):
    """Report whether a Savarez update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/savarez/update`` actually runs ``savarez update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Savarez version string
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.savarez/config.yaml`` under ``tts.``.
    """
...
async def search_sessions(q: str = "", limit: int = 20):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Savarez surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Savarez supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def get_model_options():
    ...
    try:
        ...
        # onboarding picker) sees the SAME full provider universe `savarez model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `savarez model` does so GUI onboarding lands on a
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.savarez/config.yaml`` — applies to **new** sessions only.
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Savarez from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Savarez to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Savarez from Slack via Socket Mode.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Savarez to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Savarez in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Savarez through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Savarez via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Savarez through an IMAP/SMTP mailbox.",
        "docs_url": "https://savarez-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Savarez to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Savarez inside Feishu / Lark.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Savarez through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Savarez to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Savarez to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Savarez as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://savarez-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://savarez-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Savarez Agent").strip() or "Savarez Agent"
...
# the canonical ``savarez auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Savarez resolves Anthropic creds in this order at runtime:
    1. ``~/.savarez/.anthropic_oauth.json`` — Savarez-managed PKCE flow
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"Savarez PKCE ({_HERMES_OAUTH_FILE})",
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Savarez even
    when they also have a separate Savarez-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "savarez auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "savarez auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "savarez auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "savarez auth add minimax-oauth",
    ...
    },
    {
        ...
        "flow": "loopback",
        "cli_command": "savarez auth add xai-oauth",
        "docs_url": "https://savarez-agent.nousresearch.com/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "savarez auth add anthropic",
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Savarez-managed PKCE file
...
#          → persists to ~/.savarez/.anthropic_oauth.json AND credential pool
...
# Guarded so savarez web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Savarez file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``savarez auth add anthropic``.
    """
    from agent.anthropic_adapter import _HERMES_OAUTH_FILE
    payload = {
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "savarez-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
# the tokens exactly like `savarez auth add xai-oauth`.
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `savarez auth add xai-oauth`'s credential-pool insert.
...
async def get_session_stats():
    """Session-store statistics for the Sessions page (mirrors `savarez sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `savarez sessions prune`)."""
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, SAVAREZ_HOME)."""
    from hermes_cli import profiles as profiles_mod
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process SAVAREZ_HOME at import time. The dashboard is a single
...
# servers managed here show up under `savarez mcp list` and vice versa.  Secrets
...
async def list_mcp_catalog():
    ...
    `savarez mcp catalog` / `savarez mcp install` read.
    """
    try:
...
# spawn the real `savarez gateway <verb>` so behaviour matches the CLI exactly.
...
# `savarez memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `savarez memory setup` to configure a new one.",
            )
...
def _profile_setup_command(name: str) -> str:
    ...
    return "savarez setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local SAVAREZ_HOME override so the write lands in the target
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``savarez profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from SAVAREZ_HOME).
    """
    from hermes_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``savarez profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the SAVAREZ_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``savarez profile describe
    <name> --auto``.
...
async def toggle_toolset(name: str, body: ToolsetToggle):
    ...
    helper the CLI ``savarez tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str):
    ...
    Surfaces the same provider rows the CLI ``savarez tools`` picker shows
...
async def select_toolset_provider(name: str, body: ToolsetProviderSelect):
    ...
    core extracted from the CLI configurator — so the GUI and ``savarez tools``
...
#
# The endpoint spawns the same ``savarez --tui`` binary the CLI uses, behind
...
def _resolve_chat_argv(
    ...
    Default: whatever ``savarez --tui`` would run.  Tests monkeypatch this
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Savarez inside WSL2 to use the dashboard's /chat "
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/savarez/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /savarez`` on every request. We
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/savarez``)
    ...
    # under ``/savarez`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Savarez backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Savarez Teal",         "description": "Classic dark teal — the canonical Savarez look"},
    {"name": "default-large", "label": "Savarez Teal (Large)", "description": "Savarez Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.savarez/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.savarez/dashboard-themes/*.yaml for user-created themes.
...
async def get_dashboard_themes():
    ...
    from `~/.savarez/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    1. User plugins:    ~/.savarez/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.savarez/plugins/  (only if HERMES_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()
    ...
    if env_var_enabled("HERMES_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".savarez" / "plugins", "project"))
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"savarez auth {name}"
                        break
            except Exception:
                pass
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.savarez/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.savarez/plugins/ if you trust it)",
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # Each provider plugin that ships with Savarez AI Agent exposes a
    ...
    print(f"  Savarez Web UI → http://{host}:{port}")
