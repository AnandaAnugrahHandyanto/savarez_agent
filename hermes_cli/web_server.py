...
async def get_voice_realtime_config():
...
async def create_xai_realtime_client_secret():
    ...
    body = json.dumps({"expires_after": {"seconds": _voice_realtime_ttl_seconds(cfg)}}).encode("utf-8")
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"Unable to reach xAI realtime token service: {type(exc).__name__}") from exc
...
def _voice_active_run_count() -> int:
    return sum(
        1
...
def _persist_voice_run_event(delegate_id: str, event: Dict[str, Any]) -> None:
    if not delegate_id:
        return
    event = dict(event)
...
def _load_voice_run_events(delegate_id: str, limit: int = 20) -> List[Dict[str, Any]]:
    with _VOICE_DELEGATE_LEDGER_LOCK:
        try:
            with _voice_delegate_connect() as conn:
                rows = conn.execute(
...
def _load_voice_run_status(delegate_id: str) -> Optional[Dict[str, Any]]:
    with _VOICE_DELEGATE_LEDGER_LOCK:
        try:
            with _voice_delegate_connect() as conn:
                row = conn.execute(
    ...
    status: Dict[str, Any] = {
        ...
        "events": _load_voice_run_events(row["delegate_id"]),
    }
...
def _get_voice_run_status(delegate_id: str) -> Optional[Dict[str, Any]]:
    status = _VOICE_RUN_STATUSES.get(delegate_id)
    loaded_from_ledger = False
    if status is None:
        status = _load_voice_run_status(delegate_id)
...
def _append_voice_run_event(run_id: str, event: Dict[str, Any]) -> None:
    status = _VOICE_RUN_STATUSES.get(run_id) or _load_voice_run_status(run_id)
    if status is None:
        return
    ...
    _persist_voice_run_event(run_id, event)
...
async def _run_voice_dispatch(run_id: str, body: VoiceRunCreate) -> None:
...
async def _start_voice_delegate(body: VoiceRunCreate):
    cfg = _voice_realtime_config()
    if not bool(cfg.get("enabled", False)):
        raise HTTPException(status_code=400, detail="Realtime voice dispatch is disabled")
    ...
    if _voice_active_run_count() >= max_active:
    ...
    _append_voice_run_event(
        delegate_id,
        {"event": "delegate.queued", "delegate_id": delegate_id, "run_id": delegate_id, "timestamp": time.time()},
    )
    task = asyncio.create_task(_run_voice_dispatch(delegate_id, body))
    _VOICE_ACTIVE_RUN_TASKS[delegate_id] = task
    return JSONResponse(
        status_code=202,
        content={"object": "hermes.voice.delegate", "delegate_id": delegate_id, "run_id": delegate_id, "status": "started"},
    )


@app.post("/api/voice/delegates")
async def start_voice_delegate(body: VoiceRunCreate):
    return await _start_voice_delegate(body)


@app.post("/api/voice/runs")
async def start_voice_run(body: VoiceRunCreate):
    """Backward-compatible alias for the delegate launcher."""
    return await _start_voice_delegate(body)


async def _get_voice_delegate_or_404(delegate_id: str) -> Dict[str, Any]:
    status = _get_voice_run_status(delegate_id)
    if status is None:
...
async def get_voice_delegate(delegate_id: str):
    return await _get_voice_delegate_or_404(delegate_id)


@app.get("/api/voice/delegates/{delegate_id}/events")
async def get_voice_delegate_events(delegate_id: str):
    status = await _get_voice_delegate_or_404(delegate_id)
...
async def get_voice_run_events(run_id: str):
    """Backward-compatible alias for delegate event history."""
    status = await _get_voice_delegate_or_404(run_id)
...
async def get_voice_run(run_id: str):
    """Backward-compatible alias for delegate status."""
    return await _get_voice_delegate_or_404(run_id)


async def _approve_voice_delegate(delegate_id: str, body: VoiceRunApproval):
    if _get_voice_run_status(delegate_id) is None:
        raise HTTPException(status_code=404, detail=f"Voice delegate not found: {delegate_id}")
...
async def approve_voice_delegate(delegate_id: str, body: VoiceRunApproval):
    return await _approve_voice_delegate(delegate_id, body)


@app.post("/api/voice/runs/{run_id}/approval")
async def approve_voice_run(run_id: str, body: VoiceRunApproval):
    """Backward-compatible alias for delegate approval."""
    return await _approve_voice_delegate(run_id, body)


async def _stop_voice_delegate(delegate_id: str):
    status = _get_voice_run_status(delegate_id)
...
async def stop_voice_delegate(delegate_id: str):
    return await _stop_voice_delegate(delegate_id)


@app.post("/api/voice/runs/{run_id}/stop")
async def stop_voice_run(run_id: str):
    """Backward-compatible alias for delegate stop."""
    return await _stop_voice_delegate(run_id)
