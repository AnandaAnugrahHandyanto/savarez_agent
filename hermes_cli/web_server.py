...
def _workspace_preview_target(raw_target: str, base_dir: str | None = None) -> dict[str, Any] | None:
    ...
    elif not Path(raw).is_absolute() and base_dir:
        path_text = str((Path(base_dir).expanduser() / raw).resolve())
...
def _workspace_read_file_text(file_path: str) -> dict[str, Any]:
    target = Path(str(file_path or "")).expanduser().resolve()
    if not target.is_file():
        raise HTTPException(status_code=404, detail="File not found")
    size = target.stat().st_size
    raw = target.read_bytes()[: _WORKSPACE_TEXT_PREVIEW_MAX_BYTES + 1]
...
def _workspace_read_file_data_url(file_path: str) -> dict[str, str]:
    target = Path(str(file_path or "")).expanduser().resolve()
    if not target.is_file():
        raise HTTPException(status_code=404, detail="File not found")
...
def _workspace_read_dir(dir_path: str) -> dict[str, Any]:
    target = Path(str(dir_path or "")).expanduser().resolve()
    if not target.is_dir():
    ...
    try:
        for item in target.iterdir():
            try:
                ...
                continue
            entries.append({"name": item.name, "path": str(item), "isDirectory": is_dir})
    except OSError as exc:
...
async def read_workspace_file_text(path: str):
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(None, lambda: _workspace_read_file_text(path))


@app.get("/api/workspace/file/data-url")
async def read_workspace_file_data_url(path: str):
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(None, lambda: _workspace_read_file_data_url(path))


@app.get("/api/workspace/fs/read-dir")
async def read_workspace_dir(path: str):
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(None, lambda: _workspace_read_dir(path))
...
def _write_file_change(args: dict[str, Any], result: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for raw_path in paths:
        ...
        if content is not None:
            ...
            added = len(lines)
            diff = [
                {"type": "meta", "text": f"--- before/{path}"},
                {"type": "meta", "text": f"+++ after/{path}"},
                {"type": "hunk", "text": "@@ write_file content @@"},
            ]
            diff.extend({"type": "added", "text": f"+{line}"} for line in lines[:_SESSION_CHANGE_CONTENT_LINE_LIMIT])
            if len(lines) > _SESSION_CHANGE_CONTENT_LINE_LIMIT:
                diff.append({"type": "meta", "text": f"… truncated {len(lines) - _SESSION_CHANGE_CONTENT_LINE_LIMIT} lines"})
        out.append({
            "path": path,
...
def _extract_tool_changes(tool_msg: dict[str, Any], call_args: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    if isinstance(result.get("diff"), str) and result["diff"].strip():
        changes.extend(_parse_unified_diff(result["diff"], fallback_paths))
    elif tool_name == "write_file" or result.get("bytes_written") is not None:
