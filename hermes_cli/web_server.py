...
async def get_workflow_events_endpoint(workflow_id: str, limit: int = 100):
    try:
        with workflow_connect() as conn:
            return workflow_get_workflow_events(conn, workflow_id, limit=limit)
    except ValueError as exc:
        message = str(exc)
        if message.startswith("workflow not found:"):
            raise HTTPException(status_code=404, detail=message)
...
async def get_workflow_artifacts_endpoint(workflow_id: str, kind: Optional[str] = None, limit: int = 100):
    try:
        with workflow_connect() as conn:
            return workflow_get_workflow_artifacts(conn, workflow_id, kind=kind, limit=limit)
    except ValueError as exc:
        message = str(exc)
        if message.startswith("workflow not found:"):
            raise HTTPException(status_code=404, detail=message)
