"""CLI handlers for ``savarez migrate ...``.

Currently exposes only ``savarez migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``savarez migrate <subtype>``."""
    sub = getattr(args, "migrate_type", None)
    if sub == "xai":
        return cmd_migrate_xai(args)

    print("usage: savarez migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `savarez migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `savarez doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0
