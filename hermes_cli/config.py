...
def _read_env_lines(env_path: Path) -> list[str]:
    """Read ``env_path``, transparently decrypting SOPS-managed files when needed.

    arifOS contribution: lets operators store ``~/.hermes/.env`` as a
    SOPS-encrypted file (AES256_GCM via ``age`` key) instead of plaintext.
    When the file contains any line with a SOPS ``ENC[`` marker and the
    ``sops`` binary is on ``$PATH``, this function shells out to
    ``sops -d`` and returns the decrypted lines. Plaintext files and
    sops-missing installs are a pass-through (no behavior change for
    existing users). The caller treats the returned lines exactly like
    a direct ``open(...).readlines()`` — sanitizer + parser unchanged.
    """
    open_kw = {"encoding": "utf-8-sig", "errors": "replace"}
    with open(env_path, **open_kw) as f:
        raw_lines = f.readlines()

    if not any("ENC[" in line for line in raw_lines):
        return raw_lines
    ...
    try:
        ...
        logger.warning(
            "SOPS decrypt failed for %s: %s — using raw lines", env_path, exc
        )
        return raw_lines
...
def load_env() -> Dict[str, str]:
    ...
    if env_path.exists():
        ...
        # Route through _read_env_lines() so SOPS-encrypted .env files
        # (AES256_GCM age encryption) are transparently decrypted when
        # the `sops` binary is on PATH. Plaintext .env is unchanged.
