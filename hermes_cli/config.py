...
def show_config():
    ...
    elif terminal.get('backend') == 'novita':
        print(f"  Novita template: {terminal.get('novita_image', 'code-interpreter-v1') or '(default)'}")
        novita_key = get_env_value('NOVITA_API_KEY')
        print(f"  API key:      {'configured' if novita_key else '(not set)'}")
...
def set_config_value(key: str, value: str):
    ...
    _config_to_env_sync = {
        ...
        "terminal.novita_image": "TERMINAL_NOVITA_IMAGE",
