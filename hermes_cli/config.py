...
def show_config():
    ...
    print(f"  Show cost:    {'on' if display.get('show_cost', False) else 'off'}")
    print(f"  Bell:         {'on' if display.get('bell_on_complete', False) else 'off'}")
    print(f"  Streaming:    {'on' if display.get('streaming', False) else 'off'}")
    print(f"  Timestamps:   {'on' if display.get('timestamps', False) else 'off'}")
    print(f"  Inline diffs: {'on' if display.get('inline_diffs', True) else 'off'}")
    tp = display.get('tool_progress')
    tp_str = tp if tp else 'off'
    print(f"  Tool progress: {tp_str}")
    print(f"  Tool progress cmd: {'on' if display.get('tool_progress_command', False) else 'off'}")
    tpl = display.get('tool_preview_length', 0)
    print(f"  Tool preview: {'full' if tpl == 0 else f'{tpl} chars'}")
