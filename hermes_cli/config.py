"""
Configuration management for Savarez AI Agent.

Config files are stored in ~/.savarez/ for easy access:
- ~/.savarez/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.savarez/.env         - API keys and secrets

This module provides:
- savarez config          - Show current configuration
- savarez config edit     - Open config in editor
- savarez config set      - Set a specific value
- savarez config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``savarez config set`` (which rewrites
    ...
    ``config.yaml`` in place: savarez never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.savarez/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``savarez logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  savarez config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Savarez makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Savarez itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``savarez update``, the TUI build.
...
#   on every plugin install / ``savarez update``.
...
# * ``SAVAREZ_HOME`` / ``HERMES_PROFILE`` / ``HERMES_CONFIG`` /
#   ``HERMES_ENV`` — Savarez runtime location flags. Writing these into
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Savarez runtime location — never via dashboard env writer.
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Savarez runtime location "
            "(SAVAREZ_HOME, HERMES_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.savarez/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `savarez plugins enable
    # observability/langfuse` or `savarez tools → Langfuse`); credentials gate
...
def is_managed() -> bool:
    """Check if Savarez is running in package-manager-managed mode.

    Two signals: the HERMES_MANAGED env var (set by the systemd service),
    or a .managed marker file in SAVAREZ_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade savarez-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Savarez was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.savarez/.install_method`` file (written by installers)
    ...
    container detection here:
      ...
      - the published ``AnandaAnugrahHandyanto/savarez-agent`` image stamps ``docker``
    ...
    so ``savarez update`` bailed with "doesn't apply inside the Docker
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.savarez/.install_method."""
    stamp = get_hermes_home() / ".install_method"
    try:
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Savarez lives in a ``uv tool`` layout.

    ``uv tool install savarez-agent`` places the install at
    ``.../uv/tools/savarez-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``savarez-agent`` happens to be uv-tool-installed on the machine while
    the *active* Savarez is a regular pip/venv install, causing
    ``savarez update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/savarez-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade savarez-agent"
    if method == "docker":
        return "docker pull AnandaAnugrahHandyanto/savarez-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade savarez-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade savarez-agent"
        return "pip install --upgrade savarez-agent"
    return "savarez update"
...
# Long-form text for ``savarez update`` / ``--check`` when running inside the
...
#     — that script installs a *new* host-side Savarez, it doesn't update
...
✗ ``savarez update`` doesn't apply inside the Docker container.

Savarez AI Agent runs as a published image (AnandaAnugrahHandyanto/savarez-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull AnandaAnugrahHandyanto/savarez-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate savarez-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm AnandaAnugrahHandyanto/savarez-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/AnandaAnugrahHandyanto/savarez-agent/tags
  • Your config and session history live under ``$SAVAREZ_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``savarez update`` inside Docker.
...
def format_managed_message(action: str = "modify this Savarez installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this Savarez installation is managed by NixOS "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Edit services.savarez-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Savarez installation is managed by Homebrew "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade savarez-agent"
        )

    return (
        f"Cannot {action}: this Savarez installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Savarez."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from SAVAREZ_HOME/.container-mode.
    ...
    container_name = info.get("container_name", "savarez-agent")
    exec_user = info.get("exec_user", "savarez")
    hermes_bin = info.get("hermes_bin", "/data/current-package/bin/savarez")
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    Docker containers running Savarez commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level SAVAREZ_HOME once, but
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the savarez group can
    share state with the gateway service.

    The mode can be overridden via the SAVAREZ_HOME_MODE environment variable
    (e.g. SAVAREZ_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse SAVAREZ_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("SAVAREZ_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Savarez runs in a container with volume-mounted config files, forcing
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into SAVAREZ_HOME if the user doesn't have one yet."""
...
def ensure_hermes_home():
    """Ensure ~/.savarez directory structure exists with secure permissions.
...
def _ensure_hermes_home_managed(home: Path):
    ...
    if not home.is_dir():
        raise RuntimeError(
            f"SAVAREZ_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Savarez-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Savarez
    ...
    "terminal": {
        ...
        # skipped. When empty, Savarez auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Savarez sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Savarez
        ...
        # bundled Savarez image, which drops to the `savarez` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Savarez sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Savarez should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``savarez chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.savarez/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: savarez sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Savarez truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``savarez kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``savarez kanban decompose`` and the kanban
        ...
        # ``savarez profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `savarez model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `savarez` (and `savarez chat`) launches by default:
        ...
        "interface": "cli",
        # When true, `savarez --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `savarez -c` muscle memory.  Default off so existing
        # users aren't surprised.  HERMES_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `savarez --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/savarez`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # optional path prefix (e.g. ``https://example.com/savarez``) —
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.savarez/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.savarez/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Savarez feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Savarez auto-pauses the goal and
    ...
    # cron daemon.
    #
    # See `savarez curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # normally restored on every `savarez update`, so pruning them only
        ...
        # skipped), snapshot ~/.savarez/skills/ into
        # ~/.savarez/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `savarez curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for savarez-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Savarez AI Agent*" header.
    ...
    # stored approval from ~/.savarez/shell-hooks-allowlist.json.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `savarez doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``hermes_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Savarez to lazy-install opt-in backend packages from PyPI
    ...
    # todos to ready, and fires `savarez -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `savarez` launches with
        ...
        # decomposition is manual via `savarez kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     savarez-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Savarez tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.savarez/logs/.
    ...
    # update model picker lists without shipping a savarez-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://savarez-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `savarez model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.savarez/.env,
        ...
        # behavior: files must live under the Savarez cache, under
        ...
        # uploaded as native gateway attachments. Files inside the Savarez
        # cache (~/.savarez/cache/{documents,images,audio,video,screenshots})
    ...
    # Session storage — controls automatic cleanup of ~/.savarez/state.db.
    ...
    "sessions": {
        ...
        # default of ``savarez sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.savarez/sessions/session_{sid}.json`` on every turn
    ...
    # ``savarez update`` behaviour.
    "updates": {
        # Run a full ``savarez backup``-style zip of SAVAREZ_HOME before every
        # ``savarez update``.  Backups land in ``<SAVAREZ_HOME>/backups/`` and
        # can be restored with ``savarez import <path>``.  Off by default —
        # on large SAVAREZ_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<SAVAREZ_HOME>/lsp/bin/`` on first use.
    ...
    # `savarez tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.savarez/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.savarez/.env (or your shell) and never in config.yaml.
            ...
            # ~/.savarez/bin/ on first use.  When False you must install
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'savarez model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Savarez auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @savarez:example.org)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #savarez)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: savarez-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'savarez-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Savarez API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Savarez API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Savarez API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.savarez.config`` entries, then checks
    ...
    try:
        ...
        # should never break `savarez update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Savarez versions) write ``models`` as
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'savarez setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Savarez won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'savarez doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("SAVAREZ_HOME", "~/.savarez")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$SAVAREZ_HOME/plugins/`` for currently installed user plugins.
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `savarez plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `savarez curator status` has no
    ...
    #   3. Creates `~/.savarez/logs/curator/` if missing (belt-and-suspenders
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: savarez config set <key> <value>")
    ...
    # metadata.savarez.config in their SKILL.md frontmatter.
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.savarez/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.savarez/config.yaml.
    ...
    (which change ``SAVAREZ_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — savarez auth) — OpenAI Codex
#   nous         (OAuth — savarez auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.savarez/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.savarez/.env.
    ...
    interactive menu render (`savarez tools`, `savarez setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `savarez tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Savarez env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.savarez/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.savarez/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.savarez/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.savarez/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Savarez — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Savarez vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.savarez/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Savarez Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                ...
                f"(run 'savarez doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  savarez config edit     # Edit config file", Colors.DIM))
    print(color("  savarez config set <key> <value>", Colors.DIM))
    print(color("  savarez setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: savarez config set <key> <value>")
            print()
            print("Examples:")
            print("  savarez config set model anthropic/claude-sonnet-4")
            print("  savarez config set terminal.backend docker")
            print("  savarez config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'savarez config migrate' to add them")
        ...
        print("  savarez config           Show current configuration")
        print("  savarez config edit      Open config in editor")
        print("  savarez config set <key> <value>   Set a config value")
        print("  savarez config check     Check for missing/outdated config")
        print("  savarez config migrate   Update config with new options")
        print("  savarez config path      Show config file path")
        print("  savarez config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``savarez config`` UI so users
