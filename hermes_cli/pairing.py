...
Usage:
    savarez pairing list              # Show all pending + approved users
    savarez pairing approve <platform> <code>  # Approve a pairing code
    savarez pairing revoke <platform> <user_id> # Revoke user access
    savarez pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle savarez pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: savarez pairing {list|approve|revoke|clear-pending}")
        print("Run 'savarez pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.savarez/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'savarez pairing list' to see pending codes.\n")
