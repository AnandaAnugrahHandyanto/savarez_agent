...
def _whatsapp_bridge_need_npm_install(root: Path) -> bool:
    """True when the WhatsApp bridge install is missing, partial, or stale."""
    baileys = root / "node_modules" / "@whiskeysockets" / "baileys" / "package.json"
    if not baileys.is_file():
        return True
    lock = root / "package-lock.json"
    if not lock.is_file():
        return False
...
def cmd_whatsapp(args):
    ...
    if _whatsapp_bridge_need_npm_install(bridge_dir):
