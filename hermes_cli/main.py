...
Usage:
    ...
    hermes uninstall           Uninstall Secure Safer Marketing Assistant
...
def _print_fast_version_info() -> None:
    ...
    print(f"Secure Safer Marketing Assistant v{__version__} ({__release_date__})")
...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Secure Safer Marketing Assistant'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def cmd_uninstall(args):
    """Uninstall Secure Safer Marketing Assistant (or just the Chat GUI with --gui)."""
...
def _update_via_zip(args):
    """Update Secure Safer Marketing Assistant by downloading a ZIP archive.
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# Secure Safer Marketing Assistant — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def cmd_update(args):
    """Update Secure Safer Marketing Assistant to the latest version.
    ...
    if is_managed():
        managed_error("update Secure Safer Marketing Assistant")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Secure Safer Marketing Assistant...")
    print()
...
def cmd_acp(args):
    """Launch Secure Safer Marketing Assistant as an ACP server."""
    try:
