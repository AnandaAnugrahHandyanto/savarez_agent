...
def select_provider_and_model(args=None):
    ...
    extended_providers = [
        ...
        ("gemini", "Google AI Studio (Gemini models — API key)"),
        ("gemini-oauth", "Google AI Studio (Gemini models — OAuth browser login)"),
...
def _model_flow_gemini_oauth(config, current_model=""):
    """Google AI Studio OAuth provider: trigger browser OAuth, then pick model."""
    import asyncio
    from hermes_cli.auth import (
        PROVIDER_REGISTRY, _prompt_model_selection, _save_model_choice,
        deactivate_provider, resolve_gemini_oauth_runtime_credentials,
        AuthError, format_auth_error,
    )
    from hermes_cli.config import load_config, save_config
    from hermes_cli.google_oauth import (
        start_gemini_oauth_flow,
    ...
    if has_stored_credentials():
        try:
            access_token, expires_at = get_valid_access_token()
            print(f"  Google AI Studio OAuth: ✓ (credentials active)")
            print()
        except Exception as exc:
            print(f"  Stored credentials may be expired: {exc}")
            print("  Re-running OAuth flow...")
            print()
            try:
                asyncio.run(start_gemini_oauth_flow())
                print("  ✓ OAuth flow completed successfully.")
                print()
            except Exception as oauth_exc:
                print(f"  OAuth failed: {oauth_exc}")
                return
    else:
        # Run OAuth flow
        print("  Google AI Studio OAuth requires browser authorization.")
    ...
    # Model selection — use curated Gemini list
    curated = _PROVIDER_MODELS.get("gemini", [])
    model_list = curated or [
        "gemini-2.5-flash",
        "gemini-2.5-pro",
        "gemini-2.0-flash",
        "gemini-1.5-flash",
    ...
    if selected:
        ...
        if not isinstance(model, dict):
            ...
            cfg["model"] = model
        model["provider"] = provider_id
        model["base_url"] = PROVIDER_REGISTRY[provider_id].inference_base_url
        model["api_mode"] = "chat_completions"
        save_config(cfg)
        deactivate_provider()

        print(f"Default model set to: {selected} (via Google AI Studio OAuth)")
    else:
        print("No change.")
