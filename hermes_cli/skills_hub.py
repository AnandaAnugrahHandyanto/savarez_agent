...
def _write_bundle_to_temp(bundle, root: Path) -> Path:
    ...
    skill_name = bundle.name if _is_valid_installed_skill_name(getattr(bundle, "name", "")) else "skill"
...
def do_scan(identifier: str, source: str = "local", console: Optional[Console] = None) -> None:
    ...
    if Path(identifier).expanduser().exists():
        c.print("[bold red]Error:[/] Local scan target must be a skill directory containing SKILL.md or a SKILL.md file.\n")
        return
...
def skills_command(args) -> None:
    ...
    if action == "browse":
        ...
        _console.print("Usage: hermes skills [browse|search|install|scan|inspect|list|check|update|audit|uninstall|reset|publish|snapshot|tap]\n")
...
def handle_skills_slash(cmd: str, console: Optional[Console] = None) -> None:
    ...
    if action == "browse":
        ...
        if not args:
            c.print("[bold red]Usage:[/] /skills scan <identifier-or-path> [--source <source>]\n")
            return
        identifier = args[0]
        source = "local"
        for i, a in enumerate(args):
