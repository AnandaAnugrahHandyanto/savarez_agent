#!/usr/bin/env python3
"""GitHub App integration primitives for Hermes Code Mode.

This module intentionally keeps GitHub secrets out of persisted state:
private keys are read from a configured path only when signing an app JWT,
and installation access tokens are cached in memory with expiry metadata.
"""
...
_REDACT_RULES: list[tuple[re.Pattern[str], str]] = [
    ...
    (re.compile(r"(?i)((?:token|private_key|webhook_secret|access_token|refresh_token|client_secret)\s*[:=]\s*)\S+"), r"\1[REDACTED]"),
...
def redact_github_secrets(text: str) -> str:
    """Redact GitHub-looking credentials from *text*."""
    result = str(text)
...
class GitHubAPIClient:
    """Small REST wrapper with redaction and rate-limit metadata."""
    ...
    def request(
        ...
        *,
        ...
        return {"data": data, "rate_limit": rate_limit, "status_code": response.status_code}

    def list_paginated(self, path: str, params: Optional[dict] = None) -> List[Any]:
        """Fetch list resources using conservative page-number pagination."""
        page = 1
        per_page = 100
        merged_params = dict(params or {})
        items: List[Any] = []
        while True:
            merged_params.update({"page": page, "per_page": per_page})
            result = self.request("GET", path, params=merged_params)
            data = result["data"]
            if isinstance(data, dict) and isinstance(data.get("repositories"), list):
                page_items = data["repositories"]
...
class GitHubIntegrationDB:
    """SQLite persistence for GitHub metadata."""
    ...
    def _row(row: Optional[sqlite3.Row]) -> Optional[Dict[str, Any]]:
        ...
        for key in (
            ...
            if key in result:
                result[key.removesuffix("_json")] = _json_loads(result.pop(key), [] if key.endswith("s_json") else {})
        if "private" in result:
            result["private"] = bool(result["private"])
        for key in ("archived", "disabled", "mergeable", "draft", "protected"):
    ...
    def upsert_installation(self, installation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        permissions = _json_dumps(installation.get("permissions") or {})
        events = _json_dumps(installation.get("events") or [])
        status = installation.get("status") or "active"

        def _do(conn):
            conn.execute(
                """INSERT INTO github_app_installations
                   (id, installation_id, account_login, account_type, app_id,
                    permissions_json, events_json, status, created_at, updated_at, last_synced_at)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        def _do(conn):
            conn.execute(
                """INSERT INTO github_repositories
                   (id, installation_id, github_repo_id, owner, name, full_name,
                    default_branch, private, html_url, clone_url, ssh_url, archived,
                    disabled, pushed_at, created_at, updated_at, last_synced_at)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        def _do(conn):
            conn.execute(
                """INSERT INTO github_issues
                   (id, repo_full_name, github_issue_id, number, title, state,
                    author_login, labels_json, assignees_json, milestone_json,
                    html_url, created_at, updated_at, closed_at, last_synced_at)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ...
                (
                    ...
                    issue.get("title"),
                    issue.get("state"),
                    (issue.get("user") or {}).get("login") or issue.get("author_login"),
        ...
        def _do(conn):
            conn.execute(
                """INSERT INTO github_pull_requests
                   (id, repo_full_name, github_pr_id, number, title, state,
                    author_login, base_branch, head_branch, head_sha, mergeable,
                    draft, html_url, created_at, updated_at, closed_at, merged_at, last_synced_at)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                   ON CONFLICT(repo_full_name, number) DO UPDATE SET
                ...
                (
                    ...
                    pr.get("title"),
                    pr.get("state"),
                    (pr.get("user") or {}).get("login") or pr.get("author_login"),
                    (pr.get("base") or {}).get("ref") or pr.get("base_branch"),
                    (pr.get("head") or {}).get("ref") or pr.get("head_branch"),
                    (pr.get("head") or {}).get("sha") or pr.get("head_sha"),
    ...
    def upsert_branch(self, repo_full_name: str, branch: Dict[str, Any]) -> Dict[str, Any]:
        now = _utc_now()
        sha = (branch.get("commit") or {}).get("sha") or branch.get("sha")
        ...
        def _do(conn):
            conn.execute(
                """INSERT INTO github_webhook_deliveries
                   (id, delivery_id, event, action, repo_full_name, sender_login,
        ...
        def _do(conn):
            conn.execute(
                """INSERT INTO github_chatops_commands
                   (id, delivery_id, repo_full_name, issue_number, pr_number,
                    comment_id, sender_login, command, args, status,
                    orchestrated_run_id, code_session_id, created_at, updated_at)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (
                    command_id,
                    delivery_id,
    ...
    def update_chatops_command(self, command_id: str, **updates: Any) -> Optional[Dict[str, Any]]:
        allowed = {"status", "orchestrated_run_id", "code_session_id"}
        ...
        def _do(conn):
            conn.execute(f"UPDATE github_chatops_commands SET {clause} WHERE id = ?", values)
    ...
    def create_status_report(self, report: Dict[str, Any]) -> Dict[str, Any]:
        ...
        def _do(conn):
            conn.execute(
                """INSERT INTO github_status_reports
                   (id, repo_full_name, sha, context, status, conclusion,
                    details_url, external_id, created_at, updated_at)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (
                    report_id,
        ...
        self._execute_write(_do)
        return self._row(
            self._conn.execute("SELECT * FROM github_status_reports WHERE id = ?", (report_id,)).fetchone()
        ) or {}
...
class GitHubIntegrationService:
    ...
    def status(self) -> Dict[str, Any]:
        ...
        db = self._db()
        try:
            installations = db.list_installations(limit=100)
            repos = db.list_repositories(limit=500)
        finally:
            db.close()
        private_key_exists = False
        if config.private_key_path:
            try:
                private_key_exists = Path(config.private_key_path).expanduser().is_file()
            except Exception:
                private_key_exists = False
        return {
            ...
            "private_key_available": private_key_exists,
    ...
    def get_installation_token(self, installation_id: int) -> str:
        ...
        if config.mode == "pat_dev":
            return _env_value("HERMES_GITHUB_DEV_PAT").strip()
        cached = self._token_cache.get(int(installation_id))
        if cached:
            expires_at = cached.get("expires_at_dt")
            if isinstance(expires_at, datetime) and expires_at > datetime.now(timezone.utc) + timedelta(seconds=60):
        ...
        if not token or not expires_at:
            raise GitHubAPIError("GitHub installation token response was incomplete")
    ...
    def api_client(self, installation_id: Optional[int] = None) -> GitHubAPIClient:
        ...
        if config.mode == "pat_dev":
            return GitHubAPIClient(
                lambda: _env_value("HERMES_GITHUB_DEV_PAT").strip(),
                http_client=self._http_client,
            )
        if installation_id is None:
            db = self._db()
            try:
                installations = db.list_installations(limit=1)
            finally:
                db.close()
...
GITHUB_TABLE_DDL: tuple[str, ...] = (
    ...
    "CREATE INDEX IF NOT EXISTS idx_github_pull_requests_repo ON github_pull_requests(repo_full_name)",
    ...
    "CREATE INDEX IF NOT EXISTS idx_github_branches_repo ON github_branches(repo_full_name)",
    ...
    "CREATE INDEX IF NOT EXISTS idx_github_webhooks_repo ON github_webhook_deliveries(repo_full_name)",
    ...
    "CREATE INDEX IF NOT EXISTS idx_github_chatops_status ON github_chatops_commands(status)",
    "CREATE INDEX IF NOT EXISTS idx_github_chatops_repo ON github_chatops_commands(repo_full_name)",
    ...
    "CREATE INDEX IF NOT EXISTS idx_github_status_reports_repo_sha ON github_status_reports(repo_full_name, sha)",
)
