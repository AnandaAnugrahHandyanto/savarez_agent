...
def _xai_wait_for_callback(
    ...
    *,
    ...
    del timeout_seconds  # indefinite wait; user presses Ctrl+C to cancel
    try:
        callback_event.wait()
    except KeyboardInterrupt:
        print("\nCancelled by user.")
        return {"code": None, "state": None, "error": "cancelled", "error_description": None}
    finally:
        server.shutdown()
        server.server_close()
        thread.join(timeout=1.0)
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 0):
