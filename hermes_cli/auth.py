...
def _write_codex_cli_tokens(access_token: str, refresh_token: str, last_refresh: str = None) -> None:
    """Write refreshed Codex tokens to CODEX_HOME/auth.json for CLI compatibility."""
...
def _refresh_codex_auth_tokens(
    ...
    last_refresh = str(refreshed.get("last_refresh") or datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"))

    _save_codex_tokens(updated_tokens, last_refresh)
    _write_codex_cli_tokens(updated_tokens["access_token"], updated_tokens["refresh_token"], last_refresh=last_refresh)
