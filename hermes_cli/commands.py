...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("whereami", "Show current chat/topic routing diagnostics", "Info",
               gateway_only=True, aliases=("where",)),
    CommandDef("guide", "Show the multi-agent workspace guide", "Info",
               gateway_only=True),
    CommandDef("agent_status", "Show registered agent roster and status", "Info",
               gateway_only=True, aliases=("roster",)),
    CommandDef("summon", "Invoke a specific agent with a task", "Session",
               gateway_only=True, args_hint="@agent [task]"),
    CommandDef("swarm", "Dispatch a task to multiple agents simultaneously", "Session",
               gateway_only=True, args_hint="@agent1 [@agent2 ...] [task]"),
    CommandDef("route", "Show routing guidance for directing work to an agent", "Info",
               gateway_only=True, args_hint="@agent [task]"),
