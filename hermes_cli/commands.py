...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("resume", "Resume an existing session by ID or title", "Session",
               args_hint="[session]"),
    CommandDef("today", "Show today's planner summary", "Session",
               gateway_only=True),
    CommandDef("inbox", "Show planner inbox items", "Session",
               gateway_only=True),
    CommandDef("done", "Mark a planner item as done", "Session",
               args_hint="<item_id>", gateway_only=True),
    CommandDef("review", "Show daily planner review", "Session",
               gateway_only=True),
...
_TELEGRAM_RU_DESCRIPTIONS: dict[str, str] = {
    ...
    "resume": "Продолжить сессию по ID или названию",
    "today": "Показать план на сегодня",
    "inbox": "Показать входящие planner-задачи",
    "done": "Отметить элемент planner как выполненный",
    "review": "Показать обзор дня по planner",
