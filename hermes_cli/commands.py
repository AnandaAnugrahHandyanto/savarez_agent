...
COMMAND_REGISTRY: list[CommandDef] = [
    # Session
    CommandDef("new", "开始新会话（新会话 ID + 历史记录）", "Session",
               aliases=("reset",), args_hint="[name]"),
    CommandDef("topic", "启用或查看 Telegram DM 话题会话", "Session",
               gateway_only=True, args_hint="[off|help|session-id]"),
    CommandDef("clear", "清屏并开始新会话", "Session",
               cli_only=True),
    CommandDef("redraw", "强制刷新 UI（从终端漂移中恢复）", "Session",
               cli_only=True),
    ...
    CommandDef("undo", "删除上一轮用户/助手对话", "Session"),
    CommandDef("title", "设置当前会话标题", "Session",
               args_hint="[name]"),
    CommandDef("handoff", "将会话移交给消息平台（Telegram/Discord 等）", "Session",
    ...
    CommandDef("rollback", "列出或恢复文件系统检查点", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "创建或恢复 Hermes 配置/状态快照", "Session",
               cli_only=True, aliases=("snap",), args_hint="[create|restore <id>|prune]"),
    CommandDef("stop", "终止所有后台进程", "Session"),
    ...
    CommandDef("agents", "显示活跃代理和运行中的任务", "Session",
               aliases=("tasks",)),
    CommandDef("queue", "排队提示给下一轮（不打断当前）", "Session",
               aliases=("q",), args_hint="<prompt>"),
    CommandDef("steer", "在下一次工具调用后注入消息", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "设置长期目标，多轮持续推进", "Session",
               args_hint="[text | pause | resume | clear | status]"),
    CommandDef("subgoal", "管理活跃目标的额外条件", "Session",
               args_hint="[text | remove N | clear]"),
    CommandDef("status", "显示会话信息", "Session"),
    CommandDef("whoami", "显示斜杠命令权限（管理员/用户）", "Info"),
    CommandDef("profile", "显示配置文件名和主目录", "Info"),
    CommandDef("sethome", "将此聊天设为主频道", "Session",
               gateway_only=True, aliases=("set-home",)),
    CommandDef("resume", "恢复已命名的会话", "Session",
               args_hint="[name]"),

    # Configuration
    CommandDef("sessions", "浏览和恢复之前的会话", "Session"),

    # Configuration
    CommandDef("config", "查看当前配置", "Configuration",
               cli_only=True),
    CommandDef("model", "切换当前会话模型", "Configuration",
               aliases=("provider",), args_hint="[model] [--provider name] [--global]"),
    CommandDef("codex-runtime", "切换 Codex 应用服务器运行时",
               "Configuration", aliases=("codex_runtime",),
               args_hint="[auto|codex_app_server]"),
    CommandDef("gquota", "查看 Gemini 配额使用情况", "Info",
               cli_only=True),
    ...
    CommandDef("verbose", "切换工具进度显示模式",
               "Configuration", cli_only=True,
               gateway_config_gate="display.tool_progress_command"),
    CommandDef("footer", "切换最终回复的运行时页脚",
               "Configuration", args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("yolo", "切换 YOLO 模式（跳过批准）",
               "Configuration"),
    CommandDef("reasoning", "管理推理强度和显示", "Configuration",
               args_hint="[level|show|hide]",
               subcommands=("none", "minimal", "low", "medium", "high", "xhigh", "show", "hide", "on", "off")),
    CommandDef("fast", "切换快速模式", "Configuration",
               args_hint="[normal|fast|status]",
               subcommands=("normal", "fast", "status", "on", "off")),
    CommandDef("skin", "显示或更改显示主题", "Configuration",
    ...
    CommandDef("busy", "控制 Enter 键行为", "Configuration",
               cli_only=True, args_hint="[queue|steer|interrupt|status]",
               subcommands=("queue", "steer", "interrupt", "status")),

    # Tools & Skills
    CommandDef("tools", "管理工具列表", "Tools & Skills",
               args_hint="[list|disable|enable] [name...]", cli_only=True),
    CommandDef("toolsets", "列出可用工具集", "Tools & Skills",
               cli_only=True),
    CommandDef("skills", "搜索/安装/检查/管理技能",
               "Tools & Skills", cli_only=True,
               subcommands=("search", "browse", "inspect", "install", "audit")),
    CommandDef("bundles", "列出技能包",
               "Tools & Skills"),
    CommandDef("cron", "管理定时任务", "Tools & Skills",
               cli_only=True, args_hint="[subcommand]",
               subcommands=("list", "add", "create", "edit", "pause", "resume", "run", "remove")),
    CommandDef("curator", "后台技能维护",
               "Tools & Skills", args_hint="[subcommand]",
               subcommands=("status", "run", "pause", "resume", "pin", "unpin", "restore", "list-archived")),
    CommandDef("kanban", "多配置协作面板（任务/链接/评论）",
               "Tools & Skills", args_hint="[subcommand]",
    ...
    CommandDef("reload-mcp", "从配置重新加载 MCP 服务器", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "重新扫描技能目录",
               "Tools & Skills", aliases=("reload_skills",)),
    CommandDef("browser", "通过 CDP 连接浏览器工具", "Tools & Skills",
    ...
    CommandDef("restart", "排空活跃运行后重启网关", "Session",
               gateway_only=True),
    CommandDef("usage", "显示令牌使用和速率限制", "Info"),
    CommandDef("insights", "显示使用洞察分析", "Info",
               args_hint="[days]"),
    ...
    CommandDef("copy", "复制最后回复到剪贴板", "Info",
               cli_only=True, args_hint="[number]"),
    CommandDef("paste", "从剪贴板附加图片", "Info",
               cli_only=True),
    CommandDef("image", "为下一次提示附加本地图片", "Info",
               cli_only=True, args_hint="<path>"),
    CommandDef("update", "更新 Hermes 到最新版本", "Info"),
    CommandDef("debug", "上传调试报告并获取分享链接", "Info"),

    # Exit
    CommandDef("quit", "退出 CLI（使用 --delete 可删除会话历史）", "Exit",
