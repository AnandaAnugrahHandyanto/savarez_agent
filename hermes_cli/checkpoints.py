"""`savarez checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.savarez/checkpoints/``.  Actions:

    savarez checkpoints               # same as `status`
    savarez checkpoints status        # total size, project count, breakdown
    savarez checkpoints list          # per-project checkpoint counts + workdir
    savarez checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    savarez checkpoints clear [-f]    # nuke the entire base (asks first)
    savarez checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    savarez checkpoints
    savarez checkpoints prune --retention-days 3 --max-size-mb 200
    savarez checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: savarez checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``savarez checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `savarez checkpoints` → status
