"""Shell completion script generation for savarez CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# Savarez AI Agent bash completion
# Add to ~/.bashrc:
#   eval "$(savarez completion bash)"

_hermes_profiles() {{
    local profiles_dir="$HOME/.savarez/profiles"
...
complete -F _hermes_completion savarez
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef savarez
# Savarez AI Agent zsh completion
# Add to ~/.zshrc:
...
_hermes_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.savarez/profiles" ]]; then
        profiles+=($HOME/.savarez/profiles/*(N/:t))
    fi
...
{top_cmds_str}
            ...
            _describe 'savarez command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Savarez AI Agent fish completion",
        "# Add to your config:",
        "#   savarez completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __hermes_profiles",
        "    echo default",
        "    if test -d $HOME/.savarez/profiles",
        "        for d in $HOME/.savarez/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c savarez -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c savarez -f -s p -l profile"
