"""
savarez fallback — manage the fallback provider chain.
...
Subcommands:
  savarez fallback [list]   Show the current fallback chain (default when no subcommand)
  savarez fallback add      Pick provider + model via the same picker as `savarez model`,
                           then append the selection to the chain
  savarez fallback remove   Pick an entry to delete from the chain
  savarez fallback clear    Remove all fallback entries
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  savarez fallback add")
        print()
        return
    ...
    print("  Docs: https://savarez-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    ...
    print()
    print("  Run `savarez fallback list` to view, or `savarez fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``savarez fallback [subcommand]``."""
