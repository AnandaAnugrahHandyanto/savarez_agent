...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    "nvidia": [
        ...
        "z-ai/glm-5.1",
        "glm-5.1",  # bare alias so /model glm-5.1 stays on nvidia
        "openai/gpt-oss-120b",
    ],
    "kimi-coding": [
        "kimi-k2.6",
...
PROVIDER_GROUPS: dict[str, tuple[str, str, list[str]]] = {
    ...
    "google": (
        "Google Gemini",
        "AI Studio API or OAuth + Code Assist",
...
def validate_requested_model(
    ...
    *,
    ...
    if api_models is not None:
        ...
        # NVIDIA NIM uses vendor/model format (e.g. "z-ai/glm-5.1") but
        # users often type bare model names (e.g. "glm-5.1"). Build a
        # lookup that maps bare names to their full vendor/model IDs so
        # /model glm-5.1 resolves correctly on NVIDIA.
        _nvidia_bare_to_full: dict[str, str] = {}
        if normalized == "nvidia":
            for mid in api_models:
                if isinstance(mid, str) and "/" in mid:
                    _, bare = mid.split("/", 1)
                    _nvidia_bare_to_full[bare.lower()] = mid
        if requested_for_lookup in set(api_models):
            ...
            # NVIDIA: check if bare model name matches a vendor/model ID
            # (e.g. "glm-5.1" → "z-ai/glm-5.1")
            if normalized == "nvidia" and _nvidia_bare_to_full:
                bare_lower = requested_for_lookup.lower()
                if bare_lower in _nvidia_bare_to_full:
                    corrected = _nvidia_bare_to_full[bare_lower]
                    return {
                        "accepted": True,
                        "persist": True,
                        "recognized": True,
