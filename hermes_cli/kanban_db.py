...
def release_stale_claims(
    ...
    *extended* (with a ``claim_extended`` event) instead of being
    ...
    This TTL path only proves that the claim lease expired. It must not
    terminate a host-local live PID just because a heartbeat is old: the
    dispatcher-level stale detector owns that decision and requires both
    the configured task-age floor and heartbeat gap. ``enforce_max_runtime``,
    ``detect_stale_running`` and ``detect_crashed_workers`` remain the upper
