...
``<root>`` is the **shared Savarez root** (the parent of any active
...
``savarez -p <profile>`` joins the same board as the dispatcher that
...
the profile's Savarez home but are otherwise isolated: a worker spawned
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``savarez kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.savarez``. In Docker / custom
deployments where ``SAVAREZ_HOME`` points outside ``~/.savarez`` (e.g.
``/opt/savarez``), ``<root>`` is ``SAVAREZ_HOME``. Legacy env-var
...
coding workloads.  See ``docs/savarez-kanban-v1-spec.pdf`` for the full
...
# enough that kebab-case names like ``atm10-server`` or ``savarez-agent``
...
def kanban_home() -> Path:
    """Return the shared Savarez root that anchors the kanban board.
    ...
    2. ``get_default_hermes_root()``, which already returns ``<root>``
       when ``SAVAREZ_HOME`` is ``<root>/profiles/<name>``, and returns
       ``SAVAREZ_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``SAVAREZ_HOME`` would silently fork the board per profile,
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``savarez kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    2. ``<root>/kanban/current`` on disk (set by ``savarez kanban boards
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``savarez kanban boards switch <typo>`` returns an error
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``savarez kanban log`` unambiguous even when multiple
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Savarez treats as user-controlled-but-trusted on the
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``savarez kanban init`` and the
...
def create_task(
    ...
    *,
    ...
    each name to ``savarez --skills ...`` alongside the built-in
    ...
    # `savarez --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``savarez kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `savarez kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``savarez kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``savarez kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``savarez kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``savarez kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Savarez' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Savarez-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``savarez kanban show <id>`` and the dashboard.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Savarez
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``savarez kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Savarez profile.
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Savarez profile.
        # `_default_spawn` invokes ``savarez -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Savarez
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `savarez kanban tail` — without
...
def _module_hermes_argv() -> list[str]:
    """Return the interpreter-bound Savarez CLI invocation."""
    # ``hermes_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``savarez`` package — there is no
    # ``savarez`` package to import.
    return [sys.executable, "-m", "hermes_cli.main"]


def _absolute_hermes_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Savarez shim."""
...
def _hermes_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Savarez executable path.
...
def _resolve_hermes_argv() -> list[str]:
    """Resolve the ``savarez`` invocation as argv parts for ``Popen``.
    ...
    2. ``shutil.which("savarez")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\savarez.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m hermes_cli.main`` — fallback for setups where
       Savarez is launched from a venv and the ``savarez`` shim is not on
    ...
    hermes_bin = _safe_which_no_cwd("savarez") if _IS_WINDOWS else shutil.which("savarez")
    if hermes_bin:
...
def _kanban_worker_skill_available(hermes_home: Optional[str]) -> bool:
    ...
    the worker activates a profile (``savarez -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset SAVAREZ_HOME means the worker falls back to the default root
    # home (``~/.savarez``), which ships the bundled skill.
    base = _Path(hermes_home) if hermes_home else (_Path.home() / ".savarez")
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``savarez -p <profile> chat -q ...`` subprocess.
    ...
    # Inject SAVAREZ_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `savarez -p <name>` the
    # _apply_profile_override() runs *before* hermes_constants is imported.
    # If SAVAREZ_HOME is absent from the child's env, get_hermes_home() falls
    # back to Path.home() / ".savarez" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from hermes_cli.profiles import resolve_profile_env
    try:
        env["SAVAREZ_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via HERMES_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # SAVAREZ_HOME never had profiles created.
        pass
    if task.tenant:
    ...
    # that even when the worker activates a profile (`savarez -p <name>`
    # rewrites SAVAREZ_HOME), its kanban paths still match the
    ...
    # `savarez -p <assignee>` activates the profile, but the env var is
    ...
    cmd = [
        *_resolve_hermes_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped SAVAREZ_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("SAVAREZ_HOME")):
    ...
    # `savarez kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`savarez` executable not found on PATH. "
            "Install Savarez AI Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``savarez kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled HERMES_PROFILE values like "savarez-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Savarez root exists
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``savarez kanban assignees`` for the terminal.
