...
prefix (e.g. ``mission-control.tilos.com/savarez/*`` -> dashboard on
:9119), injecting ``X-Forwarded-Prefix: /savarez`` so the backend can
...
def normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/savarez"`` (no trailing slash) or ``""``
