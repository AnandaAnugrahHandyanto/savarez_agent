...
def _setup_provider_model_selection(config, provider_id, current_model, prompt_choice, prompt_fn):
    ...
    if provider_id == "gemini-oauth":
        import asyncio
        from hermes_cli.google_oauth import start_gemini_oauth_flow, has_stored_credentials
        from hermes_cli.models import _PROVIDER_MODELS

        if not has_stored_credentials():
            asyncio.run(start_gemini_oauth_flow())

        provider_models = _PROVIDER_MODELS.get("gemini-oauth", [])
        ...
        if model_idx < len(provider_models):
            selected = provider_models[model_idx]
            _set_default_model(config, selected)
        return
