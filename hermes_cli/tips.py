...
def get_random_tip(exclude_recent: int = 0) -> str:
    ...
    Returns:
        A tip string in the language determined by HERMES_LANGUAGE
        or config.yaml display.language setting.
    """
    # Detect language
    lang = None

    # 1. Check environment variable
    lang = os.environ.get("HERMES_LANGUAGE")

    # 2. Check config.yaml if env var not set
    if not lang:
        try:
            from hermes_cli.config import load_config
            config = load_config()
            lang = config.get("display", {}).get("language", "en")
        except Exception:
            # Fallback to English if config fails to load
            lang = "en"

    # 3. Fallback to English
    if not lang or lang not in ("zh", "zh-hant"):
        lang = "en"

    # Select tip corpus based on language
    if lang in ("zh", "zh-hant"):
        tips_corpus = TIPS_ZH
    else:
        tips_corpus = TIPS

    return random.choice(tips_corpus)
