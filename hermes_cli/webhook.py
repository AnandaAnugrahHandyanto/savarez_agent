"""savarez webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    savarez webhook subscribe <name> [options]
    savarez webhook list
    savarez webhook remove <name>
    savarez webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.savarez/webhook_subscriptions.json and are
...
def _setup_hint() -> str:
  ...
  Then start the gateway: savarez gateway run
"""
...
def webhook_command(args):
    """Entry point for 'savarez webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: savarez webhook {subscribe|list|remove|test}")
        print("Run 'savarez webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (savarez gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: savarez webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from savarez webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (savarez gateway run)")
