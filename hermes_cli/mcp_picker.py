"""MCP picker — interactive `savarez mcp picker` (also the default `savarez mcp`).
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `savarez mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: savarez mcp install <name>    Picker: savarez mcp",
        Colors.DIM,
    ))
...
def show_catalog() -> None:
    """`savarez mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`savarez mcp picker` (and default `savarez mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`savarez mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `savarez mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
