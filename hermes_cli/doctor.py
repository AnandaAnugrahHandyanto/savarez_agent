...
def run_doctor(args):
    ...
    # Check hot-path Hermes memory hygiene
    print()
    print(color("◆ Memory Hygiene", Colors.CYAN, Colors.BOLD))
    memory_lint = HERMES_HOME / "scripts" / "memory_lint.py"
    if memory_lint.exists():
        try:
            lint = subprocess.run(
                [sys.executable, str(memory_lint), "--json"],
                capture_output=True,
                text=True,
                timeout=30,
            )
            if lint.returncode == 0:
                check_ok("Hot memory lint clean")
            else:
                status = "warning" if lint.returncode == 1 else "critical"
                check_warn(f"Hot memory lint reports {status}", "(run python ~/.hermes/scripts/memory_lint.py --json)")
                issues.append("Run python ~/.hermes/scripts/memory_lint.py --json and route findings to memory/skill/Obsidian cleanup")
        except Exception as e:
            check_warn("Could not run hot memory lint", f"({e})")
    else:
        check_warn("Memory lint script not found", f"({_DHH}/scripts/memory_lint.py)")
