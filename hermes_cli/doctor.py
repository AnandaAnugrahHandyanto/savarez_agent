...
def _show_verbose_fallback_chain() -> None:
    ...
    try:
        from hermes_cli.config import load_config
        cfg = load_config()
        fallback_providers = cfg.get("fallback_providers") if isinstance(cfg, dict) else []
        legacy_fallback = cfg.get("fallback_model") if isinstance(cfg, dict) else ""
        ...
        if isinstance(fallback_providers, list) and fallback_providers:
            ...
            check_info(f"legacy fallback_model: {legacy_fallback}")
        else:
...
def _show_verbose_auxiliary_config() -> None:
    ...
    try:
        ...
        _aux_tasks = [
            ...
            "skills_hub", "approval", "mcp", "triage_specifier",
            "kanban_decomposer", "profile_describer", "curator",
        ]
        for task in _aux_tasks:
