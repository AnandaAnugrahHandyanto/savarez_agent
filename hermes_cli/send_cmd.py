"""CLI subcommand: ``savarez send`` — pipe text from shell scripts to any
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"savarez send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"savarez send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"savarez send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"savarez send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `savarez gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.savarez/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_hermes_env() -> None:
    """Populate ``os.environ`` from ``~/.savarez/.env`` AND bridge top-level
    ...
    startup that ``savarez send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.savarez/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.savarez/config.yaml`` into
       ...
       via ``savarez config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``savarez send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.savarez/.env and ~/.savarez/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "savarez send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  savarez send --to telegram \"hello\"\n"
            "  savarez send --to discord:#ops --file report.md\n"
            "  savarez send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "savarez send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `savarez send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Savarez "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.savarez/.env + ~/.savarez/config.yaml) — no LLM, "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `savarez send --list telegram`.",
    )
