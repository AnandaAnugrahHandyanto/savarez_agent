"""
Savarez Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Savarez.app`` or ``~/Applications/Savarez.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Savarez`` (NSIS per-user)
...
the app name ("Savarez"), separate from ``$HERMES_HOME``:
  - macOS:   ``~/Library/Application Support/Savarez``
  - Windows: ``%APPDATA%\\Savarez``
  - Linux:   ``$XDG_CONFIG_HOME/Savarez`` (default ``~/.config/Savarez``)
...
def desktop_userdata_dir() -> Path:
    ...
    Mirrors Electron's ``app.getPath('userData')`` for an app named "Savarez"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Savarez"
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Savarez" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Savarez.app"),
            home / "Applications" / "Savarez.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Savarez).
            local_base / "Programs" / "Savarez",
        ...
        if program_files:
            ...
            paths.append(Path(program_files) / "Savarez")
    else:
        ...
        paths += [
            ...
            data_base / "applications" / "Savarez.desktop",
        ]
    return paths
