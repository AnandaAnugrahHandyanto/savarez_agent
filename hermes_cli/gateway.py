...
def _get_service_pids() -> set:
    ...
    if is_macos():
        try:
            loaded, _output, launchd_pids = _launchd_print_job()
            if loaded:
                pids.update(launchd_pids)
        except (FileNotFoundError, subprocess.TimeoutExpired):
            pass
...
def _launchd_print_job() -> tuple[bool, str, set[int]]:
...
{result.stderr}" if result.stderr else "")
    pids = {int(pid) for pid in re.findall(r"\bpid = (\d+)\b", output)}
    return result.returncode == 0, output, pids
...
def _canonicalize_launchd_path_value(value: str) -> str:
    return ":".join(os.path.normpath(part) for part in value.split(":") if part)


def _canonicalize_launchd_plist_value(value):
    if isinstance(value, dict):
        normalized = {}
        for key, item in value.items():
            if key == "PATH" and isinstance(item, str):
                normalized[key] = _canonicalize_launchd_path_value(item)
            else:
                normalized[key] = _canonicalize_launchd_plist_value(item)
        return normalized
    if isinstance(value, list):
        return [_canonicalize_launchd_plist_value(item) for item in value]
    return value


def _load_launchd_plist(text: str):
...
def launchd_plist_is_current() -> bool:
    ...
    try:
        installed = _load_launchd_plist(plist_path.read_text(encoding="utf-8"))
        expected = _load_launchd_plist(generate_launchd_plist())
    except Exception:
        return False

    return _canonicalize_launchd_plist_value(installed) == _canonicalize_launchd_plist_value(expected)
...
def launchd_status(deep: bool = False):
    plist_path = get_launchd_plist_path()
    loaded, loaded_output, launchd_pids = _launchd_print_job()
    ...
    if loaded:
        if launchd_pids:
            pid_text = ", ".join(str(pid) for pid in sorted(launchd_pids))
            print(f"✓ Gateway service is running (PID: {pid_text})")
        else:
            print("⚠ Gateway service is loaded but not running")
        if loaded_output.strip():
            print(loaded_output.strip())
    else:
...
def _is_service_running() -> bool:
    ...
    if is_linux():
        ...
        try:
            loaded, _output, launchd_pids = _launchd_print_job()
            return loaded and bool(launchd_pids)
        except subprocess.TimeoutExpired:
            return False
