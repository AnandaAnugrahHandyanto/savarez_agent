...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Create an Azure Bot resource and copy the App ID, client secret, and tenant ID",
            "2. Expose Hermes webhook publicly (e.g. https://<host>/api/messages) with ngrok or Tailscale Funnel for local dev",
            "3. Set the Azure Bot Messaging endpoint to that public /api/messages URL",
            "4. Install the Teams app package with personal/team/groupChat scopes and add the bot to Teams",
            "5. Restrict access with MSTEAMS_ALLOWED_USERS for production use",
        ],
        "vars": [
            {"name": "MSTEAMS_APP_ID", "prompt": "Azure Bot App ID", "password": False,
             "help": "Microsoft App ID from the Azure Bot resource."},
            {"name": "MSTEAMS_APP_PASSWORD", "prompt": "Azure Bot App Password", "password": True,
             "help": "Client secret for the Azure Bot app registration."},
            {"name": "MSTEAMS_TENANT_ID", "prompt": "Tenant ID", "password": False,
             "help": "Microsoft Entra tenant ID for the Azure Bot."},
            {"name": "MSTEAMS_ALLOWED_USERS", "prompt": "Allowed Microsoft Teams user IDs (comma-separated, or empty)", "password": False,
             "is_allowlist": True,
             "help": "Restrict which Microsoft Teams users can interact with the bot."},
            {"name": "MSTEAMS_HOME_CHANNEL", "prompt": "Home conversation ID (optional, for cron/notifications)", "password": False,
             "help": "Conversation ID where scheduled results and notifications should be delivered."},
        ],
    },
    {
        "key": "feishu",
