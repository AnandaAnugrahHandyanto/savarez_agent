...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    subprocess.run([_launchctl_bin(), "bootout", f"{_launchd_domain()}/{label}"], check=False, timeout=90)
    subprocess.run([_launchctl_bin(), "bootstrap", _launchd_domain(), str(plist_path)], check=False, timeout=30)
...
def launchd_install(force: bool = False):
    ...
    subprocess.run([_launchctl_bin(), "bootstrap", _launchd_domain(), str(plist_path)], check=True, timeout=30)
...
def launchd_start():
    ...
    if not plist_path.exists():
        ...
        plist_path.write_text(generate_launchd_plist(), encoding="utf-8")
        subprocess.run([_launchctl_bin(), "bootstrap", _launchd_domain(), str(plist_path)], check=True, timeout=30)
        subprocess.run([_launchctl_bin(), "kickstart", f"{_launchd_domain()}/{label}"], check=True, timeout=30)
