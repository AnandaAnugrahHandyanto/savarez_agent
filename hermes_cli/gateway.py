...
_PLATFORMS = [
    ...
    },
    {
        ...
        "label": "Rocket.Chat",
        "emoji": "🚀",
        "token_var": "ROCKETCHAT_TOKEN",
        "setup_instructions": [
            "1. In Rocket.Chat admin: Users → New (create a bot user, e.g. 'hermes-bot')",
            "   Assign it the 'bot' role and add it to the rooms you want it to see.",
            "2. Sign in as the bot → Account → Personal Access Tokens → Create new",
            "   ⚠ Check 'Ignore Two Factor Authentication' so unattended REST calls work",
            "   on 2FA-enabled workspaces.",
            "3. Copy BOTH the User ID and the Token shown at creation time —",
            "   they're only displayed once.",
            "4. To get a room ID: open the room → kebab menu → copy the roomId",
            "   (or use GET /api/v1/channels.info?roomName=<name>).",
        ],
        "vars": [
            {"name": "ROCKETCHAT_URL", "prompt": "Server URL (e.g. https://rc.example.com)", "password": False,
             "help": "Your Rocket.Chat server URL. Works with any self-hosted instance."},
            {"name": "ROCKETCHAT_TOKEN", "prompt": "Personal Access Token", "password": True,
             "help": "The token half of the PAT shown at step 3."},
            {"name": "ROCKETCHAT_USER_ID", "prompt": "Bot user ID", "password": False,
             "help": "The user ID half of the PAT shown at step 3 (NOT the username)."},
            {"name": "ROCKETCHAT_ALLOWED_USERS", "prompt": "Allowed user IDs (comma-separated)", "password": False,
             "is_allowlist": True,
             "help": "Rocket.Chat user IDs who can interact with the bot. Find yours under Profile."},
            {"name": "ROCKETCHAT_HOME_CHANNEL", "prompt": "Home room ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Room ID where Hermes delivers cron results and notifications."},
            {"name": "ROCKETCHAT_REPLY_MODE", "prompt": "Reply mode — 'off' for flat messages, 'thread' for threaded replies (default: off)", "password": False,
             "help": "off = flat room messages, thread = replies nest under your message via tmid."},
        ],
    },
    {
        "key": "whatsapp",
