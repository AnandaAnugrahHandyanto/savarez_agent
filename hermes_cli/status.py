...
def show_status(args):
    ...
    if sys.platform.startswith('linux'):
        ...
        try:
            result = subprocess.run(
                ["launchctl", "print", f"gui/{os.getuid()}/{get_launchd_label()}"],
                capture_output=True,
                text=True,
                timeout=5
            )
            is_loaded = result.returncode == 0
            is_running = bool(re.search(r"\bpid = \d+\b", result.stdout or ""))
        except subprocess.TimeoutExpired:
            is_loaded = False
            is_running = False
        state = "running" if is_running else ("loaded" if is_loaded else "not loaded")
        print(f"  Status:       {check_mark(is_loaded)} {state}")
