...
The Savarez docs site hosts a JSON manifest of curated models for providers
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.savarez/cache/model_catalog.json``.
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/AnandaAnugrahHandyanto/savarez_agent/main/website/static/api/model-catalog.json",
)
DEFAULT_TTL_HOURS = 1
DEFAULT_FETCH_TIMEOUT = 8.0
SUPPORTED_SCHEMA_VERSION = 1

_HERMES_USER_AGENT = f"savarez-cli/{_HERMES_VERSION}"
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``savarez model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
