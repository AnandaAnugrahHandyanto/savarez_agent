...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        # Install the langfuse SDK and activate the bundled plugin.
        try:
            __import__("langfuse")
        ...
        # Enable the bundled plugin in config.yaml (not .env — it's a feature flag, not a secret).
        cfg = load_config()
        plugins_cfg = cfg.setdefault("plugins", {})
        enabled_plugins = plugins_cfg.get("enabled", [])
        if "langfuse_tracing" not in enabled_plugins:
            enabled_plugins.append("langfuse_tracing")
            plugins_cfg["enabled"] = enabled_plugins
        # Write plugins.langfuse_tracing.enabled: true to config.yaml.
        lt_cfg = plugins_cfg.setdefault("langfuse_tracing", {})
        lt_cfg["enabled"] = True
        save_config(cfg)
        _print_success("    langfuse_tracing plugin enabled")
