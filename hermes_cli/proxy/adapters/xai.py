...
# xAI's public API is OpenAI-compatible for the endpoints Savarez commonly
# uses. The Responses endpoint is included because Savarez' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Savarez-managed OAuth credentials."""

    auth_hint = "savarez auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`savarez auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`savarez auth reset xai-oauth` or re-authenticate with "
                    "`savarez auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `savarez auth add xai-oauth --type oauth`."
            )
