"""CLI handlers for the ``savarez proxy`` subcommand."""
...
def _print_aiohttp_missing() -> None:
    print(
        "savarez proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"savarez auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Savarez proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Savarez proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: savarez proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``savarez proxy <subcommand>``."""
    ...
    print(
        "savarez proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  savarez proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
