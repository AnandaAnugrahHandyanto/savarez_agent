...
def _discover_all_plugins() -> list:
    ...
    for base, source in ((repo_plugins, "bundled"), (_plugins_dir(), "user")):
        ...
        for d in sorted(base.iterdir()):
            ...
            if not manifest_file.exists():
                # Recurse one level into dirs without plugin.yaml (e.g. monorepos)
                for sub in sorted(d.iterdir()):
                    if not sub.is_dir():
                        continue
                    sub_manifest = sub / "plugin.yaml"
                    if not sub_manifest.exists():
                        sub_manifest = sub / "plugin.yml"
                    if sub_manifest.exists():
                        # Temporarily set d to sub for the rest of the loop
                        # We'll handle this with a nested block
                        pass
                # Handle nested plugins via a helper
                for sub in sorted(d.iterdir()):
                    if not sub.is_dir():
                        continue
                    sub_manifest = sub / "plugin.yaml"
                    ...
                    if yaml:
                        try:
                            ...
                            sub_name = sub_data.get("name", sub.name)
                            sub_version = sub_data.get("version", "")
                            sub_description = sub_data.get("description", "")
                        except Exception:
                            pass
                    if sub_name in seen and source == "bundled":
                        continue
                    sub_src = source
                    if source == "user" and (sub / ".git").exists():
                        sub_src = "git"
                    elif source == "user" and (d / ".git").exists():
                        sub_src = "git"
                    seen[sub_name] = (sub_name, sub_version, sub_description, sub_src, sub)
                continue
            name = d.name
            version = ""
            description = ""
...
def _discover_remote_plugins() -> list:
    """Return a list of (name, version, description, source, dir_path) for
    plugins that were cloned into ``~/.hermes/plugin-repos/`` by the remote
    GitHub source loader (plugins.remote config key).

    Each entry is tagged with source="github" so the dashboard can
    distinguish remote-sourced plugins from local/bundled ones and offer
    a "Pull" button to re-fetch updates.
    """
    try:
        import yaml
    except ImportError:
    ...
    results: list = []
    plugin_repos = get_hermes_home() / "plugin-repos"
    if not plugin_repos.is_dir():
        return results

    for repo_dir in sorted(plugin_repos.iterdir()):
        if not repo_dir.is_dir():
            continue
        # Each repo_dir is a hashed cache directory; scan it for plugin.yaml
        # either at the root or one level deep (category layout).
        manifest_file = repo_dir / "plugin.yaml"
        if not manifest_file.exists():
            manifest_file = repo_dir / "plugin.yml"
        if not manifest_file.exists():
            # Check one subdirectory level (category/<plugin>/)
                ...
                if m.exists():
                    ...
                    if yaml:
                        try:
                            ...
                            pass
                    results.append((name, version, description, "github", sub))
            continue
        ...
        if yaml:
            try:
                ...
                pass
        results.append((name, version, description, "github", repo_dir))
...
def dashboard_update_remote_plugin(name: str) -> dict[str, Any]:
    """Re-fetch and update a remote (GitHub-sourced) plugin from plugin-repos.

    Finds the plugin's cache directory under ~/.hermes/plugin-repos/ by
    scanning for a directory that contains a plugin.yaml with matching name,
    then runs git fetch + git checkout + git pull to update it.
    """
    try:
        import yaml
    except ImportError:
    ...
    plugin_repos = get_hermes_home() / "plugin-repos"
    if not plugin_repos.is_dir():
        return {"ok": False, "error": "No plugin-repos directory found."}

    # Find the cache dir for this plugin by scanning for matching name.
    cache_dir = None
    for repo_dir in sorted(plugin_repos.iterdir()):
        if not repo_dir.is_dir():
            continue
        # Check root-level plugin.yaml
        for mf in ("plugin.yaml", "plugin.yml"):
            manifest_file = repo_dir / mf
    ...
    if cache_dir is None:
        return {
            "ok": False,
            "error": f"Remote plugin '{name}' not found in plugin-repos cache.",
        }

    if not (cache_dir / ".git").exists():
        return {
            "ok": False,
            "error": f"Remote plugin '{name}' cache is not a git repo; cannot pull.",
        }
    ...
    try:
        ...
        fetch_result = subprocess.run(
            [git_exe, "fetch", "--quiet", "--tags", "--force", "origin"],
            capture_output=True,
            text=True,
            timeout=60,
            cwd=str(cache_dir),
        ...
        if fetch_result.returncode != 0:
            err = (fetch_result.stderr or "").strip() or "git fetch failed."
            return {"ok": False, "error": err}

        # Pull (fast-forward only).
        pull_result = subprocess.run(
            [git_exe, "pull", "--quiet", "--ff-only"],
            capture_output=True,
            text=True,
            timeout=60,
            cwd=str(cache_dir),
        ...
        if pull_result.returncode != 0:
            err = (pull_result.stderr or "").strip() or "git pull failed."
            return {"ok": False, "error": err}

        msg = pull_result.stdout.strip()
        unchanged = "Already up to date" in msg
        return {"ok": True, "name": name, "output": msg, "unchanged": unchanged}
    except subprocess.TimeoutExpired:
        return {"ok": False, "error": "Git pull timed out after 60 seconds."}
    except FileNotFoundError:
        return {"ok": False, "error": "git is not installed or not in PATH."}
