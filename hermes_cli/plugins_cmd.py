...
import json as _json_autogen
import re as _re_autogen
from datetime import datetime as _datetime_autogen

from agent.skill_utils import NAMESPACE_PATTERN as _NS_PATTERN_AUTOGEN

from hermes_constants import get_hermes_home

logger = logging.getLogger(__name__)

_SHIM_INIT_PY_TEMPLATE_V1 = '''\
"""Auto-generated hermes plugin shim for skill bundle.

This file was generated by `hermes plugins install` because the upstream
bundle did not provide a hermes-aware __init__.py but did ship a skills/
directory.

This file is tracked in .hermes-autogen/shim.lock and may be regenerated
by `hermes plugins update`. To take ownership and customize loading:

  rm -rf .hermes-autogen/

Then edit this file freely — future updates will preserve your version.
"""
from pathlib import Path
...
def register(ctx):
    _auto_register_skills_from_dir_v1(ctx, Path(__file__).parent / "skills")
'''


def _iso_now() -> str:
    return _datetime_autogen.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")


def _extract_first_paragraph(text: str) -> str:
    """从 Markdown 文本中提取第一个非标题段落。"""
    for block in text.split("\n\n"):
        ...
        if block.startswith("#"):
            continue
        # 去除行首的 Markdown 列表标记
        block = _re_autogen.sub(r"^[\*\-]\s*", "", block)
        return block.replace("\n", " ").strip()
    return ""


def _generate_minimal_manifest(plugin_dir: Path) -> str:
    """为仅包含 skills 的 bundle 生成最小化的 plugin.yaml。

    使用目录名作为插件名称（校验 namespace pattern）。
    尝试从 README.md 的第一段提取 description；若不存在则使用通用描述。
    """
    name = plugin_dir.name
    if not _NS_PATTERN_AUTOGEN.match(name):
        raise ValueError(
            f"Plugin directory name '{name}' contains invalid characters. "
            f"Allowed: [a-zA-Z0-9_-]"
        )

    description = "Auto-generated manifest for skill bundle."
    ...
    if readme.exists():
        try:
            ...
            pass  # README 读取失败不影响主流程

    return (
        "# Auto-generated by hermes plugins install\n"
        "# HERMES_AUTOGEN_MANIFEST_V1\n"
        f"name: {name}\n"
        "version: 0.0.0\n"
