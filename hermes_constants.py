"""Shared constants for Savarez AI Agent.
...
_UNSET = object()
_SAVAREZ_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_SAVAREZ_HOME_OVERRIDE", default=_UNSET
)
...
def set_hermes_home_override(path: str | Path | None) -> Token:
    ...
    return _SAVAREZ_HOME_OVERRIDE.set(value)
...
def get_hermes_home() -> Path:
    ...
    When ``SAVAREZ_HOME`` is unset but an ``active_profile`` file indicates
    ...
    ``hermes_cli/kanban_db.py``).  See https://github.com/AnandaAnugrahHandyanto/savarez_agent/issues/18594.
    """
    ...
    if not _profile_fallback_warned:
        ...
        if active and active != "default":
            ...
            msg = (
                f"[SAVAREZ_HOME fallback] SAVAREZ_HOME is unset but active "
                f"profile is {active!r}. Falling back to {fallback_home}, which "
                f"is the DEFAULT profile — not {active!r}. Any data this "
                f"process writes will land in the wrong profile. The "
                f"subprocess spawner should pass SAVAREZ_HOME explicitly "
                f"(see issue #18594)."
            )
            try:
...
def get_default_hermes_root() -> Path:
    ...
    In profile mode where ``SAVAREZ_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.savarez/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_hermes_home()
    env_home = os.environ.get("SAVAREZ_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # SAVAREZ_HOME is under ~/.savarez (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — SAVAREZ_HOME itself is the root
    return env_path
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        ...
        4. ``<SAVAREZ_HOME>/skills`` last-resort
    """
...
def get_hermes_dir(new_subpath: str, old_name: str) -> Path:
    ...
    Args:
        new_subpath: Preferred path relative to SAVAREZ_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to SAVAREZ_HOME (e.g. ``"image_cache"``).
...
def display_hermes_home() -> str:
    """Return a user-friendly display string for the current SAVAREZ_HOME.
...
def secure_parent_dir(path: Path) -> None:
    ...
    See https://github.com/AnandaAnugrahHandyanto/savarez_agent/issues/25821.
    """
...
def get_subprocess_home() -> str | None:
    ...
    **never** modified — only subprocess environments should inject this value.
    ...
    """
    hermes_home = get_hermes_home_override() or os.getenv("SAVAREZ_HOME")
    if not hermes_home:
        return None
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under SAVAREZ_HOME.
...
def get_skills_dir() -> Path:
    """Return the path to the skills directory under SAVAREZ_HOME."""
    return get_hermes_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under SAVAREZ_HOME."""
