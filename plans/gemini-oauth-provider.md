...
## Client ID
- Need to register a "Desktop app" OAuth client on a Ananda Anugrah Handyanto GCP project
...
## Token Lifecycle
- Store at `~/.savarez/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
## Prerequisites
- Ananda Anugrah Handyanto GCP project with Desktop OAuth client registered
...
- savarez-agent Copilot OAuth: `hermes_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
