...
Google Ads skills are in `.claude/skills/`. Load them with `skill_view(name)` when relevant:

| Category | Skills |
|----------|--------|
| Auditing | account-auditor, bidding-auditor, budget-auditor, keyword-auditor, search-term-auditor, placement-auditor, lp-auditor, quality-score-auditor, geo-schedule-auditor, offer-auditor, tracking-specialist |
| Optimization | bidding-optimizer, budget-optimizer, keyword-optimizer, search-term-optimizer, placement-optimizer, geo-schedule-optimizer, lp-optimizer |
| Creation | rsa-maker, landing-page-builder, ecom-page-builder, offer-maker |
| Context | gads-context, ads-context-gatherer, business-context-gatherer, account-changelog, competitive-analyst, competitor-scraper, strategy-specialist |

## Before Any Task

Read `context/business.md` first. Know the targets, constraints, and priorities before doing anything.
Read `context/account-changelog.md` for recent changes. If the "Last updated" timestamp is older than 5 days, tell the user to refresh it with `/skill account-changelog`.
...
- Before recommending anything, check: do I have the data? If not, tell the user what to pull and which skill to load
...
- Check `context/account-changelog.md` for recent budget changes before recommending budget or CPA/ROAS changes
...
All context lives in `context/`. Google Ads data in `context/google-ads/data/`, competitor ads in `context/competitor-ads/`, analysis reports in `context/analysis/`.

If data isn't available use the query.js scripts in the skills to get the data using the API and put it into the correct place inside `context/`.

Operational work logs go in `context/memory/YYYY-MM-DD.md`.

All outputs go to `created/` (importable files) or `context/analysis/` (reports).
