...
- For `barge_in=True`: real turn detection on a single fixed wav cannot deterministically produce a second mid-TTS onset, so the **barge-in variant injects a scripted `FakeTurnDetection`** (ENDPOINT → USER_SPEECH_STARTED → USER_SPEECH_STOPPED, as the fake sim does) while keeping **real STT + real TTS**. This still validates the real-TTS flush/abandon path deterministically. Documented as an explicit, intentional narrowing.
