...
class PlacementCandidate:
    ...
    tail_entry_edge: str | None
    tail_cross_ratio: float
...
def resolve_anchor(panel_analysis: dict[str, Any], speaker: str, item_id: str) -> dict[str, Any] | None:
    ...
    tail_overrides = panel_analysis["render_hints"].get("tail_overrides", {}).get(item_id, {})
    forced_anchor_id = tail_overrides.get("speaker_anchor_id") or item_overrides.get("force_anchor_id")
...
def build_tail_points(
    box: tuple[int, int, int, int],
    speaker_point: tuple[int, int] | None,
    entry_edge: str | None,
) -> list[tuple[int, int]] | None:
    if not speaker_point or not entry_edge:
        return None
    x1, y1, x2, y2 = box
    target_x, target_y = speaker_point
    dx = target_x - (x1 + x2) / 2
    dy = target_y - (y1 + y2) / 2
    tail_length = math.hypot(dx, dy)
    base_half = int(max(18, min(32, tail_length * 0.09)))
    if entry_edge == "top":
        base_y = y1 + 6
        center_x = min(max(x1 + 30, target_x), x2 - 30)
        return [(center_x - base_half, base_y), (center_x + base_half, base_y), (target_x, target_y)]
    ...
    if entry_edge == "left":
        base_x = x1 + 6
        center_y = min(max(y1 + 30, target_y), y2 - 30)
        return [(base_x, center_y - base_half), (base_x, center_y + base_half), (target_x, target_y)]
    base_x = x2 - 6
    center_y = min(max(y1 + 30, target_y), y2 - 30)
    return [(base_x, center_y - base_half), (base_x, center_y + base_half), (target_x, target_y)]
...
def render_shape(base: Image.Image, placement: PlacementCandidate, font_path: str | None) -> None:
    ...
    if should_draw_tail(placement):
        draw.polygon(placement.tail_points, fill=style.fill, outline=style.outline)
        draw.line([placement.tail_points[0], placement.tail_points[2], placement.tail_points[1]], fill=style.outline, width=3)
...
def compute_tail_cross_ratio(
    ...
    if not tail_points:
        return 0.0
    target = tail_points[2]
    ratios: list[float] = []
    sample_count = 24
    for point in tail_points[:2]:
        hits = 0
        considered = 0
        for step in range(1, sample_count + 1):
            t = step / sample_count
            px = point[0] + (target[0] - point[0]) * t
            py = point[1] + (target[1] - point[1]) * t
            if anchor_point and math.hypot(px - anchor_point[0], py - anchor_point[1]) < 14:
                continue
            considered += 1
            for zone in forbidden_abs:
                if zone["kind"] == "focal_center":
                    continue
                if anchor_point and zone["kind"] == "face":
                    zx1, zy1, zx2, zy2 = zone["box"]
                    if zx1 <= anchor_point[0] <= zx2 and zy1 <= anchor_point[1] <= zy2:
                        continue
                if zone["kind"] == "focal_center":
                    continue
                zx1, zy1, zx2, zy2 = zone["box"]
                if zx1 <= px <= zx2 and zy1 <= py <= zy2:
                    hits += 1
                    break
        ratios.append(hits / max(1, considered))
    return round(max(ratios, default=0.0), 4)
...
def edge_midpoint(box: tuple[int, int, int, int], edge: str) -> tuple[float, float]:
    x1, y1, x2, y2 = box
    if edge == "top":
    ...
    if edge == "left":
        return (float(x1), (y1 + y2) / 2.0)
    return (float(x2), (y1 + y2) / 2.0)


def speaker_facing_edges(box: tuple[int, int, int, int], anchor_point: tuple[int, int] | None) -> list[str]:
    edges = ["top", "bottom", "left", "right"]
    if anchor_point is None:
        return edges
    cx, cy = box_center(box)
    ax = anchor_point[0] - cx
    ay = anchor_point[1] - cy
    preferred: list[tuple[float, str]] = []
    fallback: list[tuple[float, str]] = []
    for edge in edges:
        mx, my = edge_midpoint(box, edge)
        ex = mx - cx
        ey = my - cy
        distance = math.hypot(mx - anchor_point[0], my - anchor_point[1])
        if ex * ax + ey * ay >= 0:
            preferred.append((distance, edge))
        else:
            fallback.append((distance, edge))
    return [edge for _, edge in sorted(preferred)] + [edge for _, edge in sorted(fallback)]


def resolve_tail_route(
    box: tuple[int, int, int, int],
    speaker_point: tuple[int, int] | None,
    tail_policy: dict[str, Any],
    tail_override: dict[str, Any],
    forbidden_abs: list[dict[str, Any]],
) -> tuple[str | None, list[tuple[int, int]] | None, float, list[str]]:
    if speaker_point is None:
        return None, None, 0.0, ["missing_speaker_anchor"]
    preferred_edge = tail_override.get("entry_edge")
    candidate_edges = speaker_facing_edges(box, speaker_point)
    if preferred_edge in candidate_edges:
        candidate_edges.remove(preferred_edge)
        candidate_edges.insert(0, preferred_edge)
    max_cross_ratio = float(tail_policy.get("max_cross_ratio", 1.0))
    best_route: tuple[str, list[tuple[int, int]], float] | None = None
    for edge in candidate_edges:
        tail_points = build_tail_points(box, speaker_point, edge)
        tail_cross_ratio = compute_tail_cross_ratio(tail_points, forbidden_abs, speaker_point)
        route = (edge, tail_points or [], tail_cross_ratio)
        if best_route is None or route[2] < best_route[2]:
            best_route = route
        if route[2] <= max_cross_ratio:
            return route[0], route[1], route[2], []
    if best_route is None:
        return None, None, 0.0, ["missing_speaker_anchor"]
    return best_route[0], best_route[1], best_route[2], ["tail_crosses_forbidden_region"]
...
def attachment_score_breakdown(
    ...
    return {
        ...
        "tail_cross_penalty": round(tail_cross_ratio * 120.0, 3),
...
def choose_placement(
    ...
    panel_overrides = panel_analysis.get("panel_overrides", {})
    tail_policy = panel_analysis["render_hints"].get("tail_policy", {})
    tail_override = panel_analysis["render_hints"].get("tail_overrides", {}).get(item["id"], {})
    rejected: list[dict[str, Any]] = []
    candidates_scored: list[PlacementCandidate] = []
    warned_fallback: PlacementCandidate | None = None
    anchor = None
    speaker_point = None
    if template == "speech" and panel_analysis["render_hints"].get("allow_tail"):
    ...
    for zone in candidates:
        ...
        if template == "speech":
            entry_edge, tail_points, tail_cross_ratio, review_reasons = resolve_tail_route(
                outer_box,
                speaker_point,
                tail_policy,
                tail_override,
            ...
            entry_edge, tail_points, tail_cross_ratio, review_reasons = None, None, 0.0, []
    ...
    if candidates_scored:
        ...
        def tie_break_key(candidate: PlacementCandidate) -> tuple[float, float, float, int, str]:
            return (
                ...
                candidate.score_breakdown.get("tail_cross_penalty", 0.0),
        ...
        "placements": [
            {
                ...
                "tail_points": [list(point) for point in placement.tail_points] if placement.tail_points else None,
                "tail_entry_edge": placement.tail_entry_edge,
                "tail_cross_ratio": placement.tail_cross_ratio,
