...
| 35-57s | Measured gains | Five metric cards animate in: 1378x, 24.8x, 9.25x, 5.55x, 2-3x | Ticks emphasize each metric reveal |
...
- Metadata cache: old path can wait on a 10s network probe; fresh disk cache
  lookup measured around 0.0073s per lookup in the benchmark scenario.
...
- Endpoint startup: dead numeric loopback probe fast-fail, around 9.25x for the
  measured scenario.
- Parallel tools: independent I/O-bound batch around 5.55x.
