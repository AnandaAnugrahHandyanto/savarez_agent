# Savarez Observer Hooks

Savarez observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Savarez' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Savarez catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Savarez only builds
