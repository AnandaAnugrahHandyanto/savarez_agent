...
- ในโค้ด Hermes วันนี้ ทั้งสองบทบาท **อยู่ในโปรเซสเดียวกัน**: reasoning ทั้งหมดคือคลาส `AIAgent` ที่ถูก instantiate ซ้ำ **~20+ จุด (สองโหล)** ทั่วทั้ง codebase การจะแยกจริงต้องค่อย ๆ ทำตาม phase ใน §11 — **ไม่เริ่มจากงานเสี่ยงสุด (ยก Core เป็น service)**
...
> ที่มา: การสำรวจโค้ดจริง ความจริงสำคัญคือ **วันนี้ทั้งสองบทบาทอยู่ในโปรเซสเดียวกัน** — `AIAgent` ถูก instantiate ใหม่ ~20+ จุด ไม่ใช่ service กลางตัวเดียว
...
| `tui_gateway/server.py` (~8,498, 74 `@method`) | 🦾 **Action Runtime** (แต่ถือ agent lifecycle ด้วย) | JSON-RPC dispatcher; แต่ยังทำ `_make_agent`, `session.steer`, `agent.switch_model` เอง |
...
- **เคส side-effect (เช่น `/model`)** ที่ `tui_gateway/server.py:7451-7467`: คืน `_ok` เสมอ แต่ใส่ `payload["error"]` *เฉพาะ*ตอนที่ `_mirror_slash_side_effects` คืน warning ที่ `_slash_sync_warning_is_failure` ตีว่าเป็น hard failure คอมเมนต์ในโค้ด (`server.py:7462-7464`) เขียนเองว่า *"Existing slash.exec consumers (TUI, web slashExec) read only output/warning"* → field นี้ inert สำหรับ client เก่า
- **เคส plugin command error** ที่ `server.py:7438`: ใช้ convention **คนละแบบเลย** — คืน `_ok` แล้วยัด error ลง `output` เป็น string (`f"Plugin command error: {e}"`) → วันนี้ plugin error **หน้าตาเหมือน success สนิท**
- ฝั่ง client ต้องเขียน defensive: `apps/desktop/src/app/session/hooks/use-model-controls.ts:77-84` แกะ `result.error` เองแล้ว `throw` เพื่อ rollback optimistic update
...
| in-memory ต่อโปรเซส | credential pool, tool schemas, context cache | ⚠️ **rebuilt ใหม่ในทุก AIAgent site (~20+)** |
...
> **ปัญหาเชิงโครงสร้างที่กระทบ Phase 3:** lock ระดับ session (`history_lock`, `agent_build_lock`) ฝังอยู่*ใน* dict `_sessions` (อ้าง 58 ครั้ง) บวก module-level lock อีก 5 ตัว (`_sessions_lock`, `_prompt_lock`, `_session_resume_lock`, `_cfg_lock`, `_stdout_lock`) → **ย้าย agent lifecycle ออกไม่ได้สะอาดถ้าไม่แตก `_sessions` เป็น `SessionState` ที่ถือ lock ของตัวเองก่อน** (ดู Phase 3)
...
1. **ไม่มี Core process เดียว — `AIAgent` ถูกสร้างใหม่ ~20+ จุด.** (เช่น `cli.py` ×3, `gateway/run.py` ×4, `tui_gateway/server.py` ×3 ที่ `2609/5135/5246`, `cron/scheduler.py`, `acp_adapter/session.py`, `batch_runner.py`, `hermes_cli/oneshot.py`, `agent/background_review.py`, `tools/delegate_tool.py`, `gateway/platforms/…`) → Core เป็น library ฝังในแต่ละ frontend ไม่ใช่ service
...
- **Acceptance:** (1) plugin error (`server.py:7438`) + model-switch failure (`:7451`) คืน `status`/`error` structured · (2) client เก่า (TUI, web slashExec) **ยังอ่าน `output`/`warning` ได้เหมือนเดิม ไม่พัง** · (3) `tests/tui_gateway/test_protocol.py` ครอบ plugin-error path + model-switch-failure path · (4) desktop ไม่ rollback เพราะ stale
...
1. **Additive-first compatibility** — ห้ามทำ client 3 ตัว (TUI/web/desktop) พัง: เพิ่ม field ใหม่ข้างของเดิม → migrate client → ค่อยลบของเก่า (โค้ดเตือนเองที่ `server.py:7462-7464`)
...
| `tui_gateway/server.py` | JSON-RPC dispatcher + agent lifecycle (74 handler); `slash.exec` ที่ `:7438`/`:7451` |
