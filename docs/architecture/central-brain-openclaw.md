...
| 2026-06-11 | **ทุก branch เข้าระบบ PR ครบ + nudge reviewer.** Verify 3 branch ด้วย agent ก่อนเปิด (เจอ+แก้ docstring ค้างจาก cutover merge 1 จุด) แล้วเปิด Draft PR ตามลำดับ stack: #44233 (trace-events: trace_id + task.* events + Q5 parity + parent linking + desktop board) → #44235 (Step 7 cutover, **มี merge gate ใน title/body ชัดเจน**) · #44236 (brainhost cache ~3.8x, stacked บน #43422). ทั้งหมดเป็น Draft จนกว่า parent จะ merge — เหตุผล: คอขวดจริงคือคิว review ของ maintainer (repo มี fix/* ค้างเป็นร้อย) การเปิด Ready พร้อมกันคือ noise. Nudge สุภาพบน #43039 พร้อม triage signals (additive/byte-identical/rebased/MERGEABLE + track record #43895 approved). สถานะ: PR เปิดแล้ว 8 อัน (5 ready + 3 draft) ครอบทุกงานในแผน — เหลือแค่รอ review จริงๆ แล้วตอนนี้ |
| 2026-06-11 | **Track 2 ปิดครบ: 2.3 + 2.5 + 2.6.** 2.3 (`trace-events`): `delegate_task` รับ `registry_parent_task_id` (additive, default=พฤติกรรมเดิม) — children ของ `task.submit intent="delegate"` link กับ caller task_id ตาม Step 5 ruling; legacy dict คง engine id (pre-cutover) / wrapper สะท้อนค่าเดียว (post-cutover, assertion แยกตาม branch). 2.5 (`trace-events`): desktop task board ใน Agents view — store แบบ session-keyed seed จาก `task.list` ตอน gateway open (เป็น reconnect reconciliation ในตัว) + live จาก `task.started/completed` (snapshot session_id เป็น authoritative, ไม่มี id = drop ไม่ผูก focused chat), terminal linger 60s. 2.6 (branch ใหม่ `feat/brainhost-schema-cache`): **measure-first พลิก design** — tool definitions มี memo อยู่แล้ว (keyed `registry._generation`); ของแพงจริงคือ context-length probe (335-788ms ทุก construction บน non-Ollama) → memo บน BrainHost singleton ติดตั้งผ่าน `install_context_length_cache` (flag off = passthrough byte-identical), exclusions lmstudio/nous, TTL 1h/fallback 60s/cap 64. **วัดได้จริง: repeat construction 423→110ms (~3.8x)**, 11 tests ใหม่, 310 เขียว. หมายเหตุ: งาน 2.6 ตายกลาง session limit หนึ่งรอบ — agent ตัวจบรับช่วงจาก diff ที่ค้างไว้สำเร็จ |
| 2026-06-11 | **Overnight batch: 3 standalone PRs + observability branch + Step 7 prep.** PR ใหม่เข้า upstream: #43709 (browser test hermetic — root cause: Darwin fallback ใน `manual_chrome_debug_command` คืน `open -a` โดยไม่เช็คว่ามี Chrome → test พังทุก macOS), #43710 (ws flake — session ไร้ timestamp ถูก idle reaper เก็บกลางคัน; probe 199/200→0/200), #43895 (ui-tui lint 34 errors → 0, zero behavior change). Branch `feat/phase5-trace-events` (stacked): trace_id ครบ contract→wire→record→ledger (absent-when-unset, byte-compat pinned) + `task.started`/`task.completed` events ผ่าน observer seam (registry ไม่ import transport) + Q5 parity fields (label/tools/error; field ที่เหลือ TUI-only ระบุในเอกสาร). Branch `feat/phase5-step7-cutover-prep` (**prep-only ห้าม merge จน 5-6 stable**): globals เก่าถูกลบ, ชื่อ public เป็น thin wrappers, `is_subagent` discriminator, R2 untouched — พิสูจน์ 0 regressions ด้วย stash-diff ของ failure set ทั้ง scope (37 pre-existing เหมือนกันเป๊ะสองฝั่ง). Reviewer guide โพสต์บน #43422 |
