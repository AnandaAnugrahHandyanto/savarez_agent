...
Status: **design draft** (2026-06-10) — pending the owner decisions in the final section before any code lands. Owner: operator + desktop lane.
...
These are PRODUCT/SECURITY calls — **not invented here.** Each lists options + a recommendation.

- **D1 — Channel messages server-visible or E2E?** Options: (a) server-visible plaintext (enables cloud backfill/relay/search/preview/future cloud agent); (b) E2E (cloud relays ciphertext only — loses search/preview/cloud-agent, adds key-distribution + revocation-rekey); (c) hybrid (plaintext within an org, E2E across `peer_meshes` boundaries). **Rec: (a) for 4.0–4.2** (matches the whole platform), design `content` so a future `ciphertext`/`enc_alg` column is additive; **revisit (c) at 4.3** if cross-org customers demand it.
- **D2 — Can a new member read pre-join history?** (a) full; (b) join-forward only (schema default); (c) owner-chooses per invite. **Rec: (c) defaulting to (b)** — invite takes optional admin-only `include_history`; join-forward is the privacy-safe default.
- **D3 — Sharing identity: per-device, per-account, or per-team?** (a) per-account (as written); (b) per-team (coarser); (c) both. **Rec: (a) for 4.2** (finest-grained, maps onto bearer model), **add (b) at 4.3** that *expands* to per-account rows at grant time so revocation stays per-account.
- **D4 — When a remote member posts, agent runs on host or cloud lane?** (a) always the owner's host (Phase-3 invariant; cloud is pure relay); (b) cloud-lane fallback when host offline (different model/tools, bills owner). **Rec: (a)** — preserves single-writer ordering, the user's model/tools/keys, and the zero-dep story; "host offline ⇒ read-only" is the honest behavior.
- **D5 — Cross-org sharing: require a signed `peer_meshes` handshake, or is an email invite enough?** (a) require an `active` edge for any cross-org invite (explicit/signed/revocable/auditable); (b) email alone (lower friction, any admin can pull an external account in); (c) gate by plan tier. **Rec: (a)** — cross-org data flow should be an org-admin-level signed decision; within one org, plain invites are fine.
- **D6 — Cloud message-log retention/deletion?** (a) retain until channel deleted; (b) TTL/cap; (c) owner-configurable with a hard max. **Rec: (a) for 4.0** + an explicit `DELETE /v1/channels/{id}` hard-erase; add **(c)** before any paid org tier; flag for legal/compliance review before cross-org GA.
