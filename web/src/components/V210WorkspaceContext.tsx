    ...
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center gap-2 text-base">
          ...
          {t.v210?.contextTitle ?? "v2.10 Context"}
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-2 text-sm">
        <div className="flex items-center justify-between">
          <span className="text-muted-foreground">{t.v210?.mode ?? "Mode"}</span>
          <Badge tone={health?.mode === "multi_entry" ? "success" : "secondary"}>
            {health?.mode === "multi_entry" ? t.v210?.modeMultiEntry ?? "Multi-Entry" : t.v210?.modeCliLegacy ?? "CLI Legacy"}
          </Badge>
        </div>
        {workspaces.length > 0 && (
          <div className="flex items-center justify-between">
            <span className="text-muted-foreground">{t.v210?.workspaces ?? "Workspaces"}</span>
            <span className="font-medium">{workspaces.length}</span>
          </div>
        )}
        {sessions.length > 0 && (
          <div className="flex items-center justify-between">
            <span className="text-muted-foreground">{t.v210?.sessions ?? "Sessions"}</span>
            <span className="font-medium">{sessions.length}</span>
          </div>
        )}
        {health && health.registered_entrypoints.length > 0 && (
          <div className="flex items-center justify-between">
            <span className="text-muted-foreground">{t.v210?.adapters ?? "Adapters"}</span>
