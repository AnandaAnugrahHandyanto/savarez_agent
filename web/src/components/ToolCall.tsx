    ...
    >
      ...
      >
        ...
        {tool.status === "running" && (
          <span
            ...
            title={td.toolCall.running}
          />
        )}
        {tool.status === "error" && (
          <AlertCircle
            className="h-3 w-3 shrink-0 text-destructive"
            aria-label={td.toolCall.error}
          />
        )}
        {tool.status === "done" && (
          <Check
            className="h-3 w-3 shrink-0 text-primary/80"
            aria-label={td.toolCall.done}
          />
        )}
      ...
      {open && hasBody && (
        <div className="border-t border-border/60 px-3 py-2 space-y-2 text-xs font-mono">
          {tool.context && <Section label={td.toolCall.context}>{tool.context}</Section>}

          {tool.preview && tool.status === "running" && (
            <Section label={td.toolCall.streaming}>
              {tool.preview}
          ...
          {tool.inline_diff && (
            <Section label={td.toolCall.diff}>
          ...
          {tool.summary && (
            <Section label={td.toolCall.result}>
          ...
          {tool.error && (
            <Section label={td.toolCall.error} tone="error">
