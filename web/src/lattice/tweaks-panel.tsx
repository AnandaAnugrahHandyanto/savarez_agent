// tweaks-panel.tsx — Reusable Tweaks shell + form-control helpers
...
// ── useTweaks ───────────────────────────────────────────────────────────────
export function useTweaks<T extends Record<string, any>>(defaults: T): [T, (keyOrEdits: keyof T | Partial<T>, val?: any) => void] {
...
type TweakOption = string | number | boolean | { value: any; label: string };
...
export function TweakRadio({ label, value, options, onChange }: TweakRadioProps) {
  ...
  const opts = options.map((o) => (typeof o === 'object' ? o as { value: any; label: string } : { value: o, label: String(o) }));
...
export function TweakColor({ label, value, options, onChange }: TweakColorProps) {
  if (!options || !options.length) {
    return (
      <div className="twk-row twk-row-h">
        <div className="twk-lbl"><span>{label}</span></div>
        <input type="color" className="twk-swatch" value={Array.isArray(value) ? value[0] : value}
               onChange={(e) => onChange(e.target.value)} />
      </div>
    );
  }
