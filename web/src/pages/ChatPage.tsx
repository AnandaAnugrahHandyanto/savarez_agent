...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__HERMES_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `savarez dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      ...
      // even when the inner Savarez TUI has enabled xterm mouse-events
    ...
    // forwarding them into Savarez. Keyboard input, paste, and resize still
    // behave normally.
