...
export const zh: Translations = {
  ...
  cron: {
    ...
    prompt: "提示词",
    promptScheduleRequired: "提示词和调度表达式必填",
  ...
  dashboard: {
    ...
    modelPicker: {
      ...
      persistGlobally: "全局持久化（否则仅对当前会话生效）",
      switchModel: "切换",
      loading: "加载中…",
      noMatches: "无匹配结果",
    ...
    models: {
      ...
      resetAuxiliaryModels: "重置辅助模型",
      resetEveryAuxiliaryTask:
        "将所有辅助任务重置为 auto？这会覆盖已设置的单任务模型。",
      resetAll: "全部重置",
      resetAllToAuto: "全部重置为 auto",
      auxDescription:
        "辅助任务用于处理视觉、会话搜索、上下文压缩等后台工作。auto 表示“使用主模型”。当某个任务需要更低成本或更快的模型时，可以按任务单独覆盖。",
      autoUseMainModel: "auto（使用主模型）",
      providerDefault: "（提供方默认值）",
      unset: "（未设置）",
      override: "项已覆盖",
      overrides: "项已覆盖",
      tasksAllAuto: "{count} 个任务 · 全部 auto",
      tasksAuto: "{count} 个任务 · {auto} 个 auto",
      auxiliarySummaryWithOverrides: "{overrides} 项已覆盖 · {auto} 个 auto",
      useAs: "用作",
      auxiliaryTask: "辅助任务",
      ...
      tokenAnalyticsShort:
        "Token 与费用统计已隐藏，因为本地统计不包含辅助调用和提供方重试。可在“配置”中启用 dashboard.show_token_analytics 来显示本地调试估算。",
      tokenAnalyticsHiddenTitle: "Token 分析已隐藏",
      tokenAnalyticsHiddenP1:
        "本页的 Token、费用和日均分析只是本地调试估算。它只统计带有可用 usage 字段的成功主代理响应，并会静默排除辅助调用、提供方重试和回退尝试。缓存写入也完全没有计入。",
      tokenAnalyticsHiddenP2:
        "对于辅助流量较重的模型，本地总量可能比提供方账单低 10 到 100 倍。隐藏这些数字比让它们看起来像权威数据更安全。",
      tokenAnalyticsHiddenP3:
        "请以提供方控制台的实际用量和账单为准。如仍需显示本地调试估算，请在配置中设置 dashboard.show_token_analytics: true。",
      configLink: "配置",
      capabilityTools: "工具",
      capabilityVision: "视觉",
      ...
      auxTasks: {
        ...
        triage_specifier: { label: "分诊规格制定", hint: "完善看板规格" },
        kanban_decomposer: { label: "看板拆解", hint: "任务拆解" },
        profile_describer: { label: "多Agent配置描述", hint: "自动生成配置描述" },
        curator: { label: "维护器", hint: "技能使用回顾" },
      },
    },
    modelInfo: {
      loading: "正在加载模型信息…",
    ...
    oauth: {
      loadFailed: "加载提供方失败：{error}",
      disconnectFailed: "断开连接失败：{error}",
      disconnected: "{provider} 已断开连接",
      openDocs: "打开 {provider} 文档",
      disconnectConfirmTitle: "断开 {provider}？",
      disconnectConfirmDescription:
        "这会删除 {provider} 已保存的 OAuth 令牌。之后需要重新认证才能继续使用。",
      token: "令牌",
    },
    analytics: {
      hiddenTitle: "Token 分析已隐藏",
      hiddenP1:
        "本页的 Token、费用和日均分析只是本地调试估算。它只统计带有可用 usage 字段的成功主代理响应，并会静默排除辅助调用（上下文压缩、标题生成、视觉、会话搜索、网页提取、智能审批、MCP 路由、插件 LLM 访问）、提供方重试和回退尝试。缓存写入也完全没有计入。",
      hiddenP2:
        "对于辅助流量较重的模型（Kimi K2.6、MiniMax M2.7），本地总量可能比提供方账单低 10 到 100 倍。隐藏这些数字比让它们看起来像权威数据更安全。",
      hiddenP3:
        "请以提供方控制台（OpenRouter、Anthropic 等）的实际用量和账单为准。如仍需显示本地调试估算，请在配置中设置 dashboard.show_token_analytics: true。",
      configLink: "配置",
    },
    cron: {
      profile: "多Agent配置",
      allProfiles: "全部多Agent配置",
    },
  },
  ...
  kanban: {
    ...
    moveFailed: "移动失败：",
    bulkFailed: "批量操作：",
    completionBlockedHallucination: "⚠ 完成被阻止 — 不存在的卡片 ID",
    suspectedHallucinatedReferences: "⚠ 文本引用了不存在的卡片 ID",
    pickProfileFirst: "请先选择一个多Agent配置。",
    ...
    confirmBlocked:
      "将此任务标记为阻塞？工作者将被释放。",
    confirmScheduled:
      "将此任务移至已调度？这适用于明确的时间延迟，而不是需要人工处理的阻塞。",
    completionSummary:
