"""langfuse — Savarez plugin for Langfuse observability.

Traces Savarez conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Savarez plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``savarez plugins enable
observability/langfuse`` or ``savarez tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``savarez tools`` or ~/.savarez/.env):
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Savarez plugin system —
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="Savarez turn",
                tags=["savarez", "langfuse"],
            ):
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Savarez branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Savarez also has a turn-scoped
    ...
    # messages is a list only for legacy Savarez branches that fired
    # pre_llm_call with API messages directly. Current Savarez fires
...
def register(ctx) -> None:
    ...
    # Savarez versions.  pre_api_request / post_api_request fire per API
