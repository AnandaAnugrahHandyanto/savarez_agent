...
def on_post_llm_call(*, task_id: str = "", session_id: str = "", provider: str = "", base_url: str = "",
    ...
    try:
        ...
        if trace_id:
            logger.info("Langfuse scoring: trace_id=%s api_duration=%.3f tool_count=%d",
                        trace_id, api_duration, tool_count)
            _score_turn(
                client,
                trace_id=trace_id,
            ...
            logger.info("Langfuse scoring: completed for trace_id=%s", trace_id)
    except Exception as exc:
        logger.warning("Langfuse scoring failed: %s", exc)
