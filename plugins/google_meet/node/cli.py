"""`savarez meet node ...` subcommand tree.

Wired into the existing ``savarez meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``savarez meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="savarez-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``savarez meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             savarez meet node approve <name> ws://<host>:{args.port} {token}")
        try:
