...
Let the savarez agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where savarez runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `savarez meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `savarez meet` CLI |
...
| `cli.py` | `savarez meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$SAVAREZ_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `savarez meet node {run,list,approve,remove,status,ping}` |
...
savarez meet install                                      # pip + Chromium
savarez meet setup                                        # preflight
savarez meet auth                                         # optional
savarez meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
savarez meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.savarez/.env
savarez meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
savarez meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
savarez meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.savarez/.env
savarez meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, savarez will **not** switch your system audio input automatically — the
...
savarez plugins enable google_meet
savarez meet node run --display-name my-mac --host 0.0.0.0 --port 18789
# prints the bearer token on first run; copy it
```

On the gateway:
```bash
savarez meet node approve my-mac ws://<mac-ip>:18789 <token>
savarez meet node ping my-mac
