...
Demonstrates the host-owned structured LLM lane that landed alongside
this plugin. Registers a ``/receipt-extract`` slash command that takes
either:

* a path to a text file containing a receipt, or
* a path to a receipt image (PNG / JPEG),

and asks the user's active model to return a structured JSON record::
...
The plugin never sees an OAuth token or API key — model selection,
auth, fallback, and timeout are all owned by the host. The trust gate
defaults are fully restrictive: the plugin runs against whatever
provider+model the user has active. Operators who want to pin this
plugin to a specific model add::
...
This file is also the canonical worked example linked from the docs
page at ``website/docs/developer-guide/plugin-llm-access.md``.
"""
