...
```bash
savarez memory setup    # select "hindsight"
```
...
```bash
savarez config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.savarez/.env
```
...
Savarez spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.savarez/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p savarez ui start
```
...
Config file: `~/.savarez/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `savarez` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `savarez-{profile}` isolates memory per active Savarez profile. Empty placeholders collapse cleanly (e.g. `savarez-{user}` with no user becomes `savarez`). |
...
| `retain_context` | `conversation between Savarez AI Agent and the User` | Context label for retained memories |
...
The LLM API key is stored in `~/.savarez/.env` as `HINDSIGHT_LLM_API_KEY`.
