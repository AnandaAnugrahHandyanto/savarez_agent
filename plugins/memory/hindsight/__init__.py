...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "retain_tool_calls", "description": "Include tool calls and results in retained content", "default": True},
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._retain_tool_calls = self._config.get("retain_tool_calls", True)
        ...
        logger.debug("Hindsight config: auto_retain=%s, auto_recall=%s, retain_every_n=%d, "
                     "retain_tool_calls=%s, retain_async=%s, retain_context=%s, "
                     "recall_max_tokens=%d, recall_max_input_chars=%d, tags=%s, recall_tags=%s",
                     self._auto_retain, self._auto_recall, self._retain_every_n_turns,
                     self._retain_tool_calls, self._retain_async, self._retain_context,
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "", turn_messages: list | None = None) -> None:
        ...
        When retain_tool_calls is True and turn_messages is provided,
        includes tool calls/results in the retained content.
        """
        ...
        if self._retain_tool_calls and turn_messages:
            # Build rich content from the full turn messages
            entries = []
            for msg in turn_messages:
                ...
                if role == "user":
                    entries.append({"role": "user", "content": content if isinstance(content, str) else str(content), "timestamp": ts})
                elif role == "assistant":
                    if isinstance(content, str) and content:
                        entries.append({"role": "assistant", "content": content, "timestamp": ts})
                    elif isinstance(content, list):
                        for block in content:
                            if isinstance(block, dict):
                                if block.get("type") == "text" and block.get("text"):
                                    entries.append({"role": "assistant", "content": block["text"], "timestamp": ts})
