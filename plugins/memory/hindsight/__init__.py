...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._recall_tags = None if configured_recall_tags == "auto" else (_normalize_retain_tags(configured_recall_tags) or None)
    ...
    def handle_tool_call(self, tool_name: str, args: dict, **kwargs) -> str:
        if tool_name == "hindsight_retain":
            ...
            try:
                ...
                # The manual retain tool should acknowledge durable queueing, not
                # wait for local LLM extraction. On local LM Studio backends, a
                # successful extraction can take several minutes while Hindsight
                # continues processing after the client-side timeout.
                ...
                logger.warning("hindsight_retain enqueue timed out; backend may still be processing: %s", e, exc_info=True)
                return tool_error("Timed out while queueing memory; Hindsight may still be processing it in the background.")
            except Exception as e:
