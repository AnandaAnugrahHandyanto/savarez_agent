...
|-----|---------|-------------|
| `container_tag` | `savarez` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `savarez-{identity}` → `savarez-coder`). |
...
All Supermemory API calls send `x-sm-source: savarez`, and document writes stamp
`metadata.sm_source: savarez`. This is a **functional routing key, not telemetry**:
it groups Savarez-written memories into a dedicated "Savarez" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Savarez profile:

```json
{
  "container_tag": "savarez-{identity}"
}
```

For a profile named `coder`, this resolves to `savarez-coder`. The default profile resolves to `savarez-default`. Without `{identity}`, all profiles share the same container.
