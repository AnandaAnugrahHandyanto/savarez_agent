...
Requires the memos-local-plugin Node.js runtime — run install.sh first.
"""
...
class MemTensorProvider(MemoryProvider):
    ...
    def queue_prefetch(self, query: str, *, session_id: str = "") -> None:
        ...
        Hermes calls sync_all() BEFORE queue_prefetch_all(), so we search
        first, then flush the deferred ingest.  This prevents the current
        turn from appearing in its own search results.
        """
    ...
    def sync_turn(
        ...
        if _is_trivial(user_content) and _is_trivial(assistant_content):
            logger.debug("sync_turn: skipping trivial turn")
            return
        ...
        return [
            {
                ...
                "description": "Embedding provider (openai, cohere, voyage, gemini, mistral, or local)",
                "default": "local",
                "env_var": "MEMOS_EMBEDDING_PROVIDER",
            },
            {
                "key": "embedding_api_key",
                "description": "API key for embedding provider (not needed for local)",
                "secret": True,
                "env_var": "MEMOS_EMBEDDING_API_KEY",
            },
        ]

    def save_config(self, values: Dict[str, Any], hermes_home: str) -> None:
        config_path = Path(hermes_home) / "memtensor.json"
        existing = {}
