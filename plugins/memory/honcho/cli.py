...
Handles: savarez honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "savarez" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Savarez profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'savarez honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'savarez honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `savarez update` -- no output, no exceptions.
    """
    try:
        from hermes_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Savarez profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $SAVAREZ_HOME/honcho.json so each profile/instance gets
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Savarez persistent cross-session memory.")
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'savarez honcho setup' again once you have a key.\n")
            return
    ...
    current_ai = hermes_host.get("aiPeer") or cfg.get("aiPeer", "savarez")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        hermes_host["aiPeer"] = new_ai

    current_workspace = hermes_host.get("workspace") or cfg.get("workspace", "savarez")
    ...
    if new_shape == "single":
        ...
        print("  Multi-user mode: each runtime ID → own peer. Use 'savarez honcho status' to inspect.")
    ...
    try:
        ...
        print("  Run: savarez config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    savarez honcho status     -- show full config")
    print("    savarez honcho mode       -- change recall/observation mode")
    print("    savarez honcho tokens     -- tune context and dialectic budgets")
    print("    savarez honcho peer       -- update peer names")
    print("    savarez honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Savarez profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: savarez honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'savarez honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: savarez honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        savarez = hosts.get(_host_key(), {})
        user = savarez.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = savarez.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = savarez.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = savarez.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Savarez' identity in Honcho. Seed with 'savarez honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: savarez honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: savarez honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = savarez.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = savarez.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = savarez.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Savarez asks Honcho's AI peer a question")
        ...
        print("\n  Set with: savarez honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'savarez honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'savarez honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    savarez honcho identity --show        — show both peer representations")
        print("    savarez honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Savarez + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Savarez\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Savarez persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  savarez honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'savarez honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'savarez honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  savarez honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'savarez'")
        print("  with Honcho configured and no prior session history.")
        print("  (Savarez calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    savarez honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'savarez honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Savarez, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'savarez honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    savarez honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  savarez honcho identity <path/to/SOUL.md>")
    ...
    print("    Savarez:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Savarez:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Savarez:   Honcho observes every message and updates representations")
    ...
    print("    Savarez:   per-session by default — each run gets its own session")
    print("              Map a custom name:  savarez honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. savarez honcho setup              — configure API key (required)")
        print("  2. savarez honcho migrate            — re-run this walkthrough")
    else:
        print("  1. savarez honcho status             — verify Honcho connection")
        print("  2. savarez                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. savarez honcho identity --show    — verify AI peer representation")
        print("  4. savarez honcho tokens             — tune context and dialectic budgets")
        print("  5. savarez honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'savarez memory setup'...\n")
...
def register_cli(subparser) -> None:
    """Build the ``savarez honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``savarez honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to savarez memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Savarez Honcho",
    )
