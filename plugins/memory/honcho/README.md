...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/savarez>
...
```bash
savarez memory setup honcho   # configure Honcho directly (works on a fresh install)
savarez memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
savarez config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.savarez/.env
```

> `savarez honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `savarez memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$SAVAREZ_HOME/honcho.json` | Profile-local (isolated Savarez instances) |
| 2 | `~/.savarez/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Savarez profile: `savarez` (default) or `hermes_<profile>`.
...
**Deployment shapes** (`savarez memory setup honcho` asks one prompt to set these):

- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect Savarez to your own Telegram/Discord/etc.
...
| 4 | `per-session` strategy | Savarez session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`savarez-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`savarez`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-savarez-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening savarez in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Savarez session ID (timestamp + hex). Every `savarez` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Savarez profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`savarez`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Savarez profile: `savarez` (default) or `hermes_<profile>` (e.g. `savarez -p coder` -> host key `hermes_coder`). Older `savarez.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
|---------|-------------|
| `savarez memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `savarez honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `savarez memory setup`) |
| `savarez honcho status` | Show resolved config for active profile |
| `savarez honcho enable` / `disable` | Toggle Honcho for active profile |
| `savarez honcho mode <mode>` | Change recall or observation mode |
| `savarez honcho peer --user <name>` | Update user peer name |
| `savarez honcho peer --ai <name>` | Update AI peer name |
| `savarez honcho tokens --context <N>` | Set context token budget |
| `savarez honcho tokens --dialectic <N>` | Set dialectic max chars |
| `savarez honcho map <name>` | Map current directory to a session name |
| `savarez honcho sync` | Create host blocks for all Savarez profiles |
