...
CONTROL_STATUS_QUERIES = {"현재모드", "현재 모드", "지금모드", "지금 모드", "모드 뭐야", "모드뭐야"}


def _normalized_control_text(text: str) -> str:
    return " ".join(str(text or "").strip().strip("`'\"“”‘’ 。.!?\t\n").lower().split())
...
def classify_request(text: str, *, explicit_project: Optional[str] = None, active_project: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    control_text = _normalized_control_text(raw)
    if control_text in CONTROL_STATUS_QUERIES:
        mode, reason, confidence = "chat", "control_status_query", 1.0
    elif any(t in lower for t in COUNCIL_TRIGGERS):
...
def apply_route(text: str, *, project: Optional[str] = None, source: str = "wave_route_request", emit_notes: bool = False) -> Dict[str, Any]:
    ...
    ctx = _load_context()
    if route.get("reason") == "control_status_query":
...
def register(ctx) -> None:
    ...
    # Do not auto-spam Wave T2 with raw tool/session hooks. Sangkun's
    # feedback: panes full of read_file/search_files/terminal snippets are
    # not useful. Keep the explicit tools available so Hermes can write
    # deliberate, high-signal role notes only when they matter.
