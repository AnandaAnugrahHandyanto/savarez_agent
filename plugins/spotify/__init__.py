...
``_check_spotify_available()`` — when the user has not run ``savarez auth
spotify``, the tools remain registered (so they appear in ``savarez
...
The Spotify auth flow (``savarez auth spotify``), CLI plumbing, and docs
