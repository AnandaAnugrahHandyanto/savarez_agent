...
# ``model_tools`` import time, so ``savarez status``, ``savarez chat``, etc.
...
#     fresh "Savarez is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a savarez MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${SAVAREZ_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _hermes_home = _Path.home() / ".savarez"
        ...
        # they don't sit in the chat forever as "Savarez is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("SAVAREZ_HOME", str(_Path.home() / ".savarez"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${SAVAREZ_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a savarez MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Savarez is thinking…' marker message.
        ...
        "Savarez is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Savarez is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Savarez is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Savarez is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Savarez is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``savarez setup``.
    ...
    prompt for env vars, persist them to ``~/.savarez/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. savarez-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.savarez/.env")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'savarez-agent[google_chat]'",
        ...
        # the "Savarez is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Savarez is thinking…' marker message "
