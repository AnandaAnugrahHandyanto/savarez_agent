...
- Per-user tokens (keyed by sender email):
    ``${SAVAREZ_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${SAVAREZ_HOME}/google_chat_user_token.json``
...
def _hermes_home() -> Path:
    """Resolve SAVAREZ_HOME at call time (NOT module import).
...
# ``ls ~/.savarez/google_chat_user_tokens/`` trivial.
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install savarez-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'savarez-agent[google_chat]'")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into SAVAREZ_HOME."""
