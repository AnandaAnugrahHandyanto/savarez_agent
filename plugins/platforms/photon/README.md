...
This plugin connects Savarez to iMessage (and other Spectrum
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Savarez-Sidecar-Token`.
...
2. **Find or create** the `Savarez` project on the Photon dashboard.
