// Savarez — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Savarez): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Savarez -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Savarez-Sidecar-Token: ${TOKEN}`):
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Savarez supplied them so a known-good
