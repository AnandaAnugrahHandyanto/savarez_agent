...
Small Node helper that bridges Savarez to Photon's Spectrum SDK
(`spectrum-ts`).  Savarez is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Savarez-Sidecar-Token`)
...
The Savarez plugin's `hermes photon setup` command runs `npm install`
here automatically.
