...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.savarez/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Savarez gateway
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Savarez manages."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Savarez session")
        ...
        @tree.command(name="status", description="Show Savarez session status")
        ...
        @tree.command(name="stop", description="Stop the running Savarez AI agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.savarez/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update Savarez AI Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Savarez gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Savarez session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Savarez in the thread",
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a Savarez skill",
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Savarez session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Savarez: **{name}**"
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else "Savarez"
        if len(content) > 80:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by Savarez: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
    ...
    async def create_handoff_thread(
        ...
        reason = "Savarez session handoff"
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Savarez handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_clarify(
        ...
        try:
            ...
            embed = discord.Embed(
                title="❓ Savarez needs your input",
                description=body,
                color=discord.Color.orange(),
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``savarez update --gateway``
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``savarez update`` prompts.
...
# is not in this process (e.g. ``savarez cron`` running standalone) and no live
...
def interactive_setup() -> None:
    ...
    print()
    print_info("📬 Home Channel: where Savarez delivers cron job results,")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'savarez-agent[messaging]'",
