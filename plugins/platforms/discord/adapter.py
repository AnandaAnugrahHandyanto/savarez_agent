...
class ClarifyChoiceView(discord.ui.View):
    ...
    def __init__(self, choices: List[str], clarify_id: str, allowed_user_ids: set, allowed_role_ids: Optional[set] = None):
        ...
        # 1. Numaralı seçim butonlarını oluştur
    ...
    async def on_timeout(self):
        """Zaman aşımı olduğunda tüm butonları kilitle."""
        self.resolved = True
        ...
        if hasattr(self, "message") and self.message:
            try:
                await self.message.edit(content="⚠️ [Interaction timed out]", view=self)
            except Exception:
                pass
    ...
    async def _resolve_choice(self, interaction: "discord.Interaction", index: int, choice: str) -> None:
        ...
        try:
            ...
            resolve_gateway_clarify(self.clarify_id, resolved_text or choice)
        except Exception as exc:
            logger.error("Discord clarify resolve failed: %s", exc)
    ...
    async def _on_other(self, interaction: "discord.Interaction") -> None:
        ...
        try:
            ...
            logger.warning("Marking text failed: %s", exc)
