"""langfuse_tracing — Hermes plugin for Langfuse observability.
...
def _is_enabled() -> bool:
    if Langfuse is None:
        return False
    # Primary activation path: config.yaml plugins.langfuse_tracing.enabled
    try:
        from hermes_cli.config import load_config
        _cfg = load_config()
        _plugin_cfg = _cfg.get("plugins", {})
        if isinstance(_plugin_cfg, dict):
            _lt_cfg = _plugin_cfg.get("langfuse_tracing", {})
