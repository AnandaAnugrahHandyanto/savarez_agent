(function () {
  ...
  const TIER_CN_MAP = {Copper:"铜",Silver:"银",Gold:"金",Diamond:"钻石",Olympian:"奥金"};
  function tierLabel(tier, locale) { return locale === "zh" ? (TIER_CN_MAP[tier] || tier) : tier; }
  const TIER_CN = new Proxy({}, { get: function(_, tier) {
    var _l = "en";
    try { var _i = SDK.useI18n; if (_i) _l = _i().locale; } catch(e) {}
    return tierLabel(tier, _l);
  } });
  ...
  function ShareDialog({ achievement, onClose }) {
    ...
    function tweetText() {
    function formatTier(ach) {
      return TIER_CN[ach.tier || ach.next_tier] || ach.tier || "";
    }
    const tierPart = formatTier(achievement) ? "【" + formatTier(achievement) + "】" : "";
    const tmpl = tx(t, "share.tweet_text", "刚刚解锁 {tier_part}\"{name}\" - Hermes Agent ☤", {
        tier_part: tierPart,
        name: tx(t, "achievementData." + achievement.id + ".name", achievement.name),
      });
  ...
  function AchievementCard({ achievement }) {
    ...
    let tierLabel;
    if (achievement.tier) {
      tierLabel = TIER_CN[achievement.tier] || achievement.tier;
    } else if (targetTier) {
      tierLabel = tx(t, "tier.target", "Target {tier}", { tier: TIER_CN[targetTier] || targetTier });
      ...
      React.createElement(C.CardContent, { className: "ha-card-content" },
        React.createElement("div", { className: "ha-card-head" },
          ...
          React.createElement("div", { className: "ha-card-title-wrap" },
            React.createElement("div", { className: "ha-card-title" }, tx(t, "achievementData." + achievement.id + ".name", achievement.name)),
            React.createElement("div", { className: "ha-card-category" }, tx(t, "categories." + achievement.category, achievement.category))
          ),
          React.createElement("div", { className: "ha-badges" },
            ...
            state === "unlocked" && React.createElement("button", {
              ...
              "aria-label": tx(t, "card.share_label", "Share {name}", { name: tx(t, "achievementData." + achievement.id + ".name", achievement.name) }),
            }, tx(t, "card.share_text", "Share"))
          )
        ),
        React.createElement("p", { className: "ha-description" }, tx(t, "achievementData." + achievement.id + ".description", achievement.description)),
  ...
  function AchievementsPage() {
    ...
    const highest = ["Olympian", "Diamond", "Gold", "Silver", "Copper"].find(function (tier) { return unlocked.some(function (a) { return a.tier === tier; }); });
    const highestDisplay = highest ? (TIER_CN[highest] || highest) : tx(t, "stats.none_yet", "None yet");
    ...
    return React.createElement("div", { className: "ha-page" },
      ...
      React.createElement("div", { className: "ha-stats" },
        ...
        React.createElement(StatCard, { label: tx(t, "stats.highest_tier", "Highest tier"), value: highestDisplay, hint: tx(t, "stats.highest_tier_hint", "Copper \u2192 Silver \u2192 Gold \u2192 Diamond \u2192 Olympian") }),
        React.createElement(StatCard, { label: tx(t, "stats.latest", "Latest"), value: latest[0] ? tx(t, "achievementData." + latest[0].id + ".name", latest[0].name) : tx(t, "stats.none_yet", "None yet"), hint: latest[0] ? tx(t, "categories." + latest[0].category, latest[0].category) : tx(t, "stats.latest_hint_empty", "run Hermes more") })
      ),
      ...
      React.createElement("div", { className: "ha-toolbar" },
        React.createElement("div", { className: "ha-pills" }, categories.map(function (cat) {
          ...
          const pillLabel = cat === "All" ? allCategoryLabel : tx(t, "categories." + cat, cat);
      ...
      latest.length > 0 && React.createElement("section", { className: "ha-latest" },
        ...
        React.createElement("div", { className: "ha-latest-row" }, latest.map(function (a) {
          return React.createElement("div", { key: a.id, className: cn("ha-chip", tierClass(a.tier)) },
            ...
            tx(t, "achievementData." + a.id + ".name", a.name)
          );
        }))
      ),
