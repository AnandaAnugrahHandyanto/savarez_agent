(function () {
  ...
  async function buildShareImage(achievement, t) {
    const W = 1200;
    const H = 630;
    ...
    const nameLines = wrapText(ctx, achievement.name || t.achievements.achievementFallback, rMaxWidth).slice(0, 2);
    let cursorY = 150;
    ...
    // Tier badge pill
    const badgeLabel = tier.toUpperCase() + t.achievements.tierSuffix;
  ...
  function ShareDialog({ achievement, onClose }) {
    ...
    hooks.useEffect(function () {
      let cancelled = false;
      let createdUrl = null;
      buildShareImage(achievement, t).then(function (blob) {
        if (cancelled) return;
    ...
    async function copyToClipboard() {
      ...
      try {
        if (!navigator.clipboard || !window.ClipboardItem) {
          throw new Error(t.achievements.clipboardNotSupported);
        }
    ...
    function tweetText() {
      ...
      return t.achievements.tweetText
        .replace("{tier}", tierPart)
    ...
    return React.createElement("div", {
      ...
      React.createElement("div", { className: "ha-share-dialog", role: "dialog", "aria-label": t.achievements.shareTitle.replace("{name}", achievement.name) },
        React.createElement("div", { className: "ha-share-head" },
          React.createElement("strong", null, t.achievements.shareTitle.replace("{name}", achievement.name)),
          React.createElement("button", { className: "ha-share-close", onClick: onClose, "aria-label": t.achievements.close }, "×")
        ),
        React.createElement("div", { className: "ha-share-preview" },
          status === "rendering" && React.createElement("div", { className: "ha-share-placeholder" }, t.achievements.rendering),
          previewUrl && React.createElement("img", { src: previewUrl, alt: achievement.name + " share card" })
        ),
        status === "error" && React.createElement("div", { className: "ha-share-error" }, errorMsg || t.achievements.somethingWentWrong),
        React.createElement("div", { className: "ha-share-actions" },
          React.createElement("button", {
            className: "ha-share-btn ha-share-btn-primary",
            onClick: shareOnX,
            title: t.achievements.shareOnXTitle,
          }, t.achievements.shareOnX),
          React.createElement("button", {
            className: "ha-share-btn",
            onClick: copyToClipboard,
            disabled: status !== "ready" && status !== "copied",
            title: t.achievements.copyImageTitle,
          }, status === "copied" ? t.achievements.copied : t.achievements.copyImage),
          React.createElement("button", {
            className: "ha-share-btn",
            onClick: download,
            disabled: status !== "ready" && status !== "copied",
  ...
  function TierLegend() {
    const { t } = useI18n();
    const tiers = [
      { key: "copper", label: t.achievements.tierCopper },
      { key: "silver", label: t.achievements.tierSilver },
      { key: "gold", label: t.achievements.tierGold },
      { key: "diamond", label: t.achievements.tierDiamond },
      { key: "olympian", label: t.achievements.tierOlympian },
    ];
    return React.createElement("div", { className: "ha-tier-legend" },
      tiers.map(function (tier, index, arr) {
        return React.createElement(React.Fragment, { key: tier.key },
          React.createElement("span", { className: "ha-tier-step ha-tier-" + tier.key },
    ...
    return React.createElement("div", { className: "ha-page ha-page-loading" },
      React.createElement("section", { className: "ha-hero ha-loading-hero" },
        React.createElement("div", null,
          React.createElement("div", { className: "ha-kicker" }, t.achievements.kicker),
          React.createElement("h1", null, t.achievements.title),
          React.createElement("p", null, t.achievements.loadingSubtitle)
        ),
        React.createElement("div", { className: "ha-scan-status", role: "status", "aria-live": "polite" },
          ...
          React.createElement("div", null,
            React.createElement("strong", null, t.achievements.buildingProfile),
            React.createElement("p", null, t.achievements.readingData)
          )
        )
      ),
      React.createElement("div", { className: "ha-stats" },
        [t.achievements.unlocked, t.achievements.discovered, t.achievements.secrets, t.achievements.highestTier, t.achievements.latest].map(function (label) {
      ...
      React.createElement("section", { className: "ha-guide ha-loading-guide" },
        React.createElement("div", null,
          React.createElement("strong", null, t.achievements.scanStatus),
          React.createElement("p", null, t.achievements.scanStatusDescription)
        ),
        React.createElement("div", null,
          React.createElement("strong", null, t.achievements.whatIsScanned),
          React.createElement("p", null, t.achievements.whatIsScannedDescription)
        )
      ),
  ...
  function AchievementCard({ achievement }) {
    ...
    const stateLabel = state === "unlocked" ? t.achievements.stateUnlocked : (state === "secret" ? t.achievements.stateSecret : t.achievements.stateDiscovered);
    const targetTier = achievement.next_tier || achievement.tier;
    const tierNameMap = {
      "Copper": t.achievements.tierCopper,
      "Silver": t.achievements.tierSilver,
      "Gold": t.achievements.tierGold,
      "Diamond": t.achievements.tierDiamond,
      "Olympian": t.achievements.tierOlympian,
    };
    const tierLabel = achievement.tier ? (tierNameMap[achievement.tier] || achievement.tier) : (targetTier ? t.achievements.targetTier.replace("{tier}", tierNameMap[targetTier] || targetTier) : (state === "secret" ? t.achievements.hidden : (unlocked ? t.achievements.complete : t.achievements.objective)));
    const progressText = state === "secret" ? t.achievements.hiddenProgress : (progress + (achievement.next_threshold ? " / " + achievement.next_threshold : ""));
    ...
    return React.createElement(C.Card, { className: cn("ha-card", "ha-state-" + state, tierClass(achievement.tier || achievement.next_tier)) },
      React.createElement(C.CardContent, { className: "ha-card-content" },
        React.createElement("div", { className: "ha-card-head" },
          ...
          React.createElement("div", { className: "ha-badges" },
            ...
            state === "unlocked" && React.createElement("button", {
              ...
              title: t.achievements.shareTitle.replace(/\s*[:：]\s*\{name\}/, ""),
              "aria-label": "Share " + achievement.name,
            }, t.achievements.shareTitle.replace(/\s*[:：]\s*\{name\}/, ""))
          )
        ),
        ...
        achievement.criteria && React.createElement("details", { className: "ha-criteria" },
          React.createElement("summary", null, state === "secret" ? t.achievements.howToReveal : t.achievements.whatCounts),
        ...
        React.createElement("div", { className: "ha-evidence-slot" },
          achievement.evidence ? React.createElement("div", { className: "ha-evidence" },
            React.createElement("span", { className: "ha-evidence-label" }, t.achievements.evidence),
            React.createElement("span", { className: "ha-evidence-title" }, achievement.evidence.title || achievement.evidence.session_id || t.achievements.session)
          ) : React.createElement("div", { className: "ha-evidence ha-evidence-empty", "aria-hidden": "true" }, t.achievements.noEvidenceYet)
        ),
        React.createElement("div", { className: "ha-progress-row" },
  ...
  function AchievementsPage() {
    ...
    const highestTierKey = ["Olympian", "Diamond", "Gold", "Silver", "Copper"].find(function (tier) { return unlocked.some(function (a) { return a.tier === tier; }); });
    const tierNameMap = {
      "Copper": t.achievements.tierCopper,
      "Silver": t.achievements.tierSilver,
      "Gold": t.achievements.tierGold,
    ...
    const highest = highestTierKey ? tierNameMap[highestTierKey] : t.achievements.noneYet;
    ...
    if (scanInFlight) {
      ...
      const headline = scanMode === "pending"
        ? t.achievements.startingScan
        : t.achievements.buildingAchievementProfile;
      const detail = total > 0
        ? t.achievements.scannedSessions.replace("{scanned}", scanned.toLocaleString()).replace("{total}", total.toLocaleString()).replace("{pct}", pct)
        : t.achievements.scanDetailNoTotal;
    ...
    return React.createElement("div", { className: "ha-page" },
      React.createElement("section", { className: "ha-hero" },
        React.createElement("div", null,
          React.createElement("div", { className: "ha-kicker" }, t.achievements.kicker),
          React.createElement("h1", null, t.achievements.title),
          React.createElement("p", null, t.achievements.subtitle)
        ),
        React.createElement(C.Button, { onClick: load, className: "ha-refresh" }, t.achievements.rescan)
      ),
      scanBanner,
      error && React.createElement(C.Card, { className: "ha-error" }, React.createElement(C.CardContent, null, String(error))),
      React.createElement("div", { className: "ha-stats" },
        React.createElement(StatCard, { label: t.achievements.unlocked, value: (data ? data.unlocked_count : 0) + " / " + (data ? data.total_count : 0), hint: t.achievements.earnedBadges }),
        React.createElement(StatCard, { label: t.achievements.discovered, value: discovered.length, hint: t.achievements.knownNotEarned }),
        React.createElement(StatCard, { label: t.achievements.secrets, value: secret.length, hint: t.achievements.hiddenUntilSignal }),
        React.createElement(StatCard, { label: t.achievements.highestTier, value: highest, hint: t.achievements.tierOrder }),
        React.createElement(StatCard, { label: t.achievements.latest, value: latest[0] ? latest[0].name : t.achievements.noneYet, hint: latest[0] ? latest[0].category : t.achievements.runHermesMore })
      ),
      React.createElement("section", { className: "ha-guide" },
        React.createElement("div", null,
          React.createElement("strong", null, t.achievements.tiers),
          React.createElement(TierLegend, null)
        ),
        React.createElement("div", null,
          React.createElement("strong", null, t.achievements.secretAchievements),
          React.createElement("p", null, t.achievements.secretDescription)
        )
      ),
      React.createElement("div", { className: "ha-toolbar" },
        React.createElement("div", { className: "ha-pills" }, categories.map(function (cat) {
          return React.createElement("button", { key: cat, onClick: function () { setCategory(cat); }, className: cat === category ? "active" : "" }, cat === "All" ? t.achievements.all : cat);
        })),
        React.createElement("div", { className: "ha-pills" }, ["all", "unlocked", "discovered", "secret"].map(function (v) {
          const filterLabels = {
            all: t.achievements.filterAll,
            unlocked: t.achievements.filterUnlocked,
            discovered: t.achievements.filterDiscovered,
            secret: t.achievements.filterSecret,
          };
          return React.createElement("button", { key: v, onClick: function () { setVisibility(v); }, className: v === visibility ? "active" : "" }, filterLabels[v]);
        }))
      ),
      latest.length > 0 && React.createElement("section", { className: "ha-latest" },
        React.createElement("h2", null, t.achievements.recentUnlocks),
      ...
      visibility === "secret" && visible.length === 0 && React.createElement(C.Card, { className: "ha-secret-empty" },
        React.createElement(C.CardContent, { className: "ha-secret-empty-content" },
          React.createElement("strong", null, t.achievements.noHiddenSecrets),
          React.createElement("p", null, t.achievements.secretClue)
        )
      ),
