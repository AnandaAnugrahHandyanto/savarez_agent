# Savarez Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: savarez-achievements plugin
...
### `GET /api/plugins/savarez-achievements/achievements`
...
### `POST /api/plugins/savarez-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/savarez-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/savarez-achievements/overview`
...
- `~/.savarez/plugins/savarez-achievements/`
