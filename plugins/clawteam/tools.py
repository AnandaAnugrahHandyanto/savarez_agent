...
Six verbs an LLM agent inside Hermes can call to drive a ClawTeam:
discover, status, spawn, cleanup, inbox-send, inbox-peek. Each handler shells
...
def _handle_team_cleanup(args: dict, **_kw: Any) -> str:
    try:
        team = validate_name(require_str(args.get("team"), field="team"), field="team")
        result = run_clawteam_json("team", "cleanup", team, "--force")
    except Exception as exc:
        return _err(exc)
    return tool_result({"cleaned": result, "team": team})
...
_TEAM_CLEANUP_SCHEMA = {
    "name": "clawteam_team_cleanup",
    "description": "Clean up a temporary ClawTeam team after a PR ships or review verdict lands. Uses force cleanup and returns the cleanup result.",
    "parameters": {
        "type": "object",
        "properties": {
            "team": {"type": "string", "description": "Temporary team name to remove, e.g. issue-54-fix"},
        },
        "required": ["team"],
    },
}
...
TOOLS = [
    ...
    (_TEAM_CLEANUP_SCHEMA["name"], _TEAM_CLEANUP_SCHEMA, _handle_team_cleanup),
