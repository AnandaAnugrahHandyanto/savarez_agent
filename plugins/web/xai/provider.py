...
``{title, url, description, position}`` rows every other Savarez web
provider produces.
...
prefers Savarez-managed xAI Grok OAuth (via ``savarez auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.savarez/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``savarez tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `savarez auth` to sign in with "
