/**
 * Savarez Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.savarez/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
...
(function () {
  ...
  const COLUMN_DOT = {
    triage: "savarez-kanban-dot-triage",
    todo: "savarez-kanban-dot-todo",
    ready: "savarez-kanban-dot-ready",
    running: "savarez-kanban-dot-running",
    blocked: "savarez-kanban-dot-blocked",
    done: "savarez-kanban-dot-done",
    archived: "savarez-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-savarez-task";
  ...
  const DOCS_URL = "https://savarez-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://savarez-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "savarez.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="savarez-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "savarez-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "savarez-kanban-md",
  ...
  // standard `drop` event and our `savarez-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("savarez-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("savarez-kanban-column--drop");
          if (target) target.classList.add("savarez-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("savarez-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("savarez-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("savarez-kanban:drop", {
  ...
  function KanbanPage() {
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "savarez-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "savarez-kanban-attention",
        "savarez-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "savarez-kanban-attention-bar" },
        h("span", { className: "savarez-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "savarez-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "savarez-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "savarez-kanban-attention-row",
                  "savarez-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "savarez-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "savarez-kanban-attention-row-id" }, task.id),
                h("span", { className: "savarez-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "savarez-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "savarez-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "savarez-kanban-diag-action-btn",
      action.suggested ? "savarez-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " savarez-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".savarez-kanban-drawer-comment-row input, .savarez-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "savarez-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("savarez-kanban-diag", sevClass) },
      h("div", { className: "savarez-kanban-diag-header" },
        h("span", { className: "savarez-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "savarez-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "savarez-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "savarez-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "savarez-kanban-diag-data-row" },
                  h("span", { className: "savarez-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "savarez-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "savarez-kanban-diag-data-row" },
                h("span", { className: "savarez-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "savarez-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "savarez-kanban-diag-reassign-row" },
            h("span", { className: "savarez-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "savarez-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "savarez-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "savarez-kanban-diag-msg",
              msg.ok ? "savarez-kanban-diag-msg--ok" : "savarez-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "savarez-kanban-section" },
      h("div", { className: "savarez-kanban-section-head-row" },
        h("span", { className: "savarez-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "savarez-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "savarez-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "savarez-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "savarez-kanban-docs-link",
      title: "Open Savarez Kanban docs in a new tab",
      "aria-label": "Savarez Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `savarez kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "savarez-kanban-msg-ok" : "savarez-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "savarez-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "savarez-kanban-boardswitcher" },
      h("div", { className: "savarez-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "savarez-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "savarez-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "savarez-kanban-dialog-title" },
        ...
        h("div", { className: "savarez-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Savarez profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "savarez-kanban-bulk" },
      h("span", { className: "savarez-kanban-bulk-count" },
      ...
      h("div", { className: "savarez-kanban-bulk-priority",
      ...
      h("div", { className: "savarez-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Savarez profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "savarez-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("savarez-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("savarez-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "savarez-kanban-trash",
        dragOver ? "savarez-kanban-trash--drop" : "",
        props.draggingTaskId ? "savarez-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "savarez-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "savarez-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".savarez-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "savarez-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("savarez-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("savarez-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "savarez-kanban-column",
        dragOver ? "savarez-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "savarez-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "savarez-kanban-col-check",
        ...
        h("span", { className: cn("savarez-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "savarez-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "savarez-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "savarez-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "savarez-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "savarez-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "savarez-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "savarez-kanban-lane" },
                  h("div", { className: "savarez-kanban-lane-head" },
                    h("span", { className: "savarez-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "savarez-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "savarez-kanban-card--stale-red";
    if (age >= tier.amber) return "savarez-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".savarez-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "savarez-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "savarez-kanban-card",
        props.selected ? "savarez-kanban-card--selected" : "",
        props.failed ? "savarez-kanban-card--failed" : "",
        props.draggingSource ? "savarez-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "savarez-kanban-card-content" },
          h("div", { className: "savarez-kanban-card-row" },
            h("label", {
              className: "savarez-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "savarez-kanban-card-check",
            ...
            h("span", { className: "savarez-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or savarez kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "savarez-kanban-warning-badge",
                    "savarez-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "savarez-kanban-needs-assignee",
          ...
          h("div", { className: "savarez-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "savarez-kanban-card-row savarez-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "savarez-kanban-assignee",
                            title: `Assigned to Savarez profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "savarez-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "savarez-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "savarez-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "savarez-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Savarez profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Savarez profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "savarez-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "savarez-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "savarez-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "savarez-kanban-drawer-close",
        ...
        data ? h("div", { className: "savarez-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "savarez-kanban-section" },
      h("div", { className: "savarez-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "savarez-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "savarez-kanban-drawer-body" },
      h("div", { className: "savarez-kanban-drawer-title" },
        h("span", { className: cn("savarez-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "savarez-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "savarez-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "savarez-kanban-section" },
        h("div", { className: "savarez-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "savarez-kanban-section" },
        h("div", { className: "savarez-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "savarez-kanban-comment" },
                h("div", { className: "savarez-kanban-comment-head" },
                  h("span", { className: "savarez-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "savarez-kanban-comment-ago" },
      ...
      h("div", { className: "savarez-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "savarez-kanban-event",
              isDiag ? "savarez-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "savarez-kanban-event-header" },
                  h("span", { className: "savarez-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "savarez-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "savarez-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "savarez-kanban-event-header-plain" },
                  h("span", { className: "savarez-kanban-event-kind" }, e.kind),
                  h("span", { className: "savarez-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "savarez-kanban-event-phantom-row" },
                  h("span", { className: "savarez-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "savarez-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "savarez-kanban-event-payload" },
    ...
    return h("div", { className: "savarez-kanban-section" },
      h("div", { className: "savarez-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "savarez-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `savarez-kanban-run--${r.outcome || r.status || "ended"}`
          : "savarez-kanban-run--active";
        return h("div", { key: r.id, className: cn("savarez-kanban-run", outcomeClass) },
          h("div", { className: "savarez-kanban-run-head" },
            h("span", { className: "savarez-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "savarez-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "savarez-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "savarez-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "savarez-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "savarez-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "savarez-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "savarez-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "savarez-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "savarez-kanban-pre savarez-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "savarez-kanban-meta-row" },
      h("span", { className: "savarez-kanban-meta-label" }, props.label),
      h("span", { className: "savarez-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "savarez-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "savarez-kanban-meta-row" },
        h("span", { className: "savarez-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "savarez-kanban-meta-value savarez-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "savarez-kanban-meta-row" },
        h("span", { className: "savarez-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "savarez-kanban-meta-value savarez-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "savarez-kanban-section" },
      h("div", { className: "savarez-kanban-section-head-row" },
        h("span", { className: "savarez-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "savarez-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "savarez-kanban-section" },
      h("div", { className: "savarez-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "savarez-kanban-deps-row" },
        h("span", { className: "savarez-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "savarez-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "savarez-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "savarez-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "savarez-kanban-dep-chip-x",
      ...
      h("div", { className: "savarez-kanban-deps-row" },
        h("span", { className: "savarez-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "savarez-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "savarez-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "savarez-kanban-msg-ok"
          : "savarez-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "savarez-kanban-section" },
      ...
      h("div", { className: "savarez-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "savarez-kanban-home-sub savarez-kanban-home-sub--on"
              : "savarez-kanban-home-sub",
          }, label);
        })
      )
    );
