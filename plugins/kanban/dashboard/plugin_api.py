...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``savarez kanban
...
def list_diagnostics(
    ...
    AND for ``savarez kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``savarez kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``savarez kanban specify <task_id>``.
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``savarez kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def _active_profile_name() -> str:
    """Return the current Savarez profile name for notify-sub ownership."""
    try:
...
def decompose_task_endpoint(
    ...
    1:1 to ``savarez kanban decompose <task_id>``.
