...
# shellcheck shell=sh
# /opt/savarez/docker/main-wrapper.sh — wraps the container's CMD with
...
# what s6-supervised services use too (see main-savarez/run).
#
# Routing:
#   no args                       → exec `savarez` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `savarez <args>` (subcommand passthrough)
#
# Drop to savarez via s6-setuidgid, but skip it when already non-root.
set -e

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid savarez "$@"; exec "$@"; }
...
# an arbitrary non-root, non-savarez UID, the bootstrap was skipped and the
# baked image dirs (owned by the savarez build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u savarez)" ]; then
    cat >&2 <<EOF
[savarez] ERROR: container started with --user $cur_uid (an arbitrary, non-savarez UID) — not supported.
...
# to the savarez user's home before dropping privileges so libraries that
...
# Savarez subcommand pass-through.
drop savarez "$@"
