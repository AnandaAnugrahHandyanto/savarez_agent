...
# by /etc/cont-init.d/01-savarez-setup (installed by the Dockerfile). This
...
# on the pre-s6 contract "entrypoint.sh sets up state then execs savarez"
...
echo "[savarez] WARNING: docker/entrypoint.sh is a deprecated shim under " \
...
exec /opt/savarez/docker/stage2-hook.sh "$@"
