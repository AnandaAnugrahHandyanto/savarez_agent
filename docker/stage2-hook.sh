...
#
# Wired into the image as /etc/cont-init.d/01-savarez-setup by the
...
SAVAREZ_HOME="${SAVAREZ_HOME:-/opt/data}"
INSTALL_DIR="/opt/savarez"

# Drop to savarez via s6-setuidgid, but skip it when already non-root.
as_hermes() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid savarez "$@"; }
...
# the container starts non-root. The baked image trees (/opt/data, /opt/savarez/
# .venv, ui-tui, node_modules) stay owned by the savarez build UID (10000), so an
...
# into $SAVAREZ_HOME). See #34837 for the supervision-tree side of this.
#
...
# `id -u savarez` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# savarez UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u savarez)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-savarez UID).
...
and the baked image directories are owned by the savarez user (UID $(id -u savarez)),
...
The image remaps the savarez user to that UID/GID at boot and chowns the data
...
# --- Bootstrap SAVAREZ_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid savarez mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom SAVAREZ_HOME paths whose parents only
# root can create (e.g. `SAVAREZ_HOME=/home/savarez/.savarez` in a Compose
...
if [ -n "${HERMES_UID:-}" ] && validate_uid_gid "$HERMES_UID" && [ "$HERMES_UID" != "$(id -u savarez)" ]; then
    echo "[stage2] Changing savarez UID to $HERMES_UID"
    usermod -u "$HERMES_UID" savarez
fi
if [ -n "${HERMES_GID:-}" ] && validate_uid_gid "$HERMES_GID" && [ "$HERMES_GID" != "$(id -g savarez)" ]; then
    echo "[stage2] Changing savarez GID to $HERMES_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$HERMES_GID" savarez 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised savarez user (UID 10000)
...
# savarez process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes savarez. Idempotent across container
...
#   - savarez is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G savarez 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] savarez already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" savarez 2>/dev/null; then
        echo "[stage2] Added savarez to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group savarez failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When HERMES_UID is remapped or the top-level $SAVAREZ_HOME isn't owned by
# the runtime savarez UID, restore ownership to savarez — but ONLY for the
# directories savarez actually writes to. The full $SAVAREZ_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of savarez-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_hermes_uid=$(id -u savarez)
needs_chown=false
if [ "$(stat -c %u "$SAVAREZ_HOME" 2>/dev/null)" != "$actual_hermes_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $SAVAREZ_HOME (targeted) to savarez ($actual_hermes_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $SAVAREZ_HOME: chown the directory itself (not its contents)
    # so savarez can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    chown savarez:savarez "$SAVAREZ_HOME" 2>/dev/null || \
        echo "[stage2] Warning: chown $SAVAREZ_HOME failed (rootless container?) — continuing"
    # Savarez-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by savarez (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing; do
        if [ -e "$SAVAREZ_HOME/$sub" ]; then
            chown -R savarez:savarez "$SAVAREZ_HOME/$sub" 2>/dev/null || \
                echo "[stage2] Warning: chown $SAVAREZ_HOME/$sub failed (rootless container?) — continuing"
        fi
    done
fi

# --- Fix ownership of build trees under $INSTALL_DIR ---
# Savarez-owned trees under $INSTALL_DIR must be re-chowned whenever the
# runtime savarez UID no longer owns them — otherwise:
...
#     savarez UID can't write the build output (#28851).
...
# The set mirrors the build-time `chown -R savarez:savarez` line in the
# Dockerfile — keep them in sync if the Dockerfile chown set changes.
# These are under $INSTALL_DIR (not $SAVAREZ_HOME), so the bind-mount
# concern doesn't apply — recursive is fine.
#
# This MUST be gated independently of the $SAVAREZ_HOME ownership check
# above. `usermod -u <new> savarez` re-chowns the savarez home dir
# ($SAVAREZ_HOME == /opt/data) to the new UID as a side effect, so after a
# HERMES_UID/PUID remap `stat $SAVAREZ_HOME` always already matches the new
# UID and `needs_chown` is false — but the build trees under /opt/savarez
# are NOT touched by usermod and remain owned by the build-time UID
# (10000). Gating them on $SAVAREZ_HOME ownership (as #35027 did) silently
...
# when the venv is not already owned by the runtime savarez UID. Idempotent
...
if [ -n "$venv_owner" ] && [ "$venv_owner" != "$actual_hermes_uid" ]; then
    echo "[stage2] Fixing ownership of build trees under $INSTALL_DIR to savarez ($actual_hermes_uid)"
    chown -R savarez:savarez \
        "$INSTALL_DIR/.venv" \
        "$INSTALL_DIR/ui-tui" \
        "$INSTALL_DIR/gateway" \
...
# Always reset ownership of $SAVAREZ_HOME/profiles to savarez on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> savarez …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as savarez and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$SAVAREZ_HOME/profiles" ]; then
...
# sweep so host-owned files in a bind-mounted $SAVAREZ_HOME are never
...
for f in \
    ...
    active_profile; do
    if [ -e "$SAVAREZ_HOME/$f" ]; then
        chown savarez:savarez "$SAVAREZ_HOME/$f" 2>/dev/null || true
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the savarez runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$SAVAREZ_HOME/config.yaml" ]; then
    chown savarez:savarez "$SAVAREZ_HOME/config.yaml" 2>/dev/null || true
    chmod 640 "$SAVAREZ_HOME/config.yaml" 2>/dev/null || true
fi

# --- Seed directory structure as savarez user ---
# Run as savarez via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $SAVAREZ_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_hermes mkdir -p \
    "$SAVAREZ_HOME/cron" \
    "$SAVAREZ_HOME/sessions" \
    "$SAVAREZ_HOME/logs" \
    "$SAVAREZ_HOME/hooks" \
    "$SAVAREZ_HOME/memories" \
    "$SAVAREZ_HOME/skills" \
    "$SAVAREZ_HOME/skins" \
    "$SAVAREZ_HOME/plans" \
    "$SAVAREZ_HOME/workspace" \
    "$SAVAREZ_HOME/home" \
    "$SAVAREZ_HOME/pairing" \
    "$SAVAREZ_HOME/platforms/pairing"

# --- Install-method stamp (read by detect_install_method() in savarez status) ---
# Preserved from the tini-era entrypoint (PR #27843). Must be written as
# the savarez user so ownership matches the file's documented owner.
# tee is invoked directly via s6-setuidgid (no `sh -c` wrapper) for the
# same shell-metacharacter safety described above.
printf 'docker\n' | as_hermes tee "$SAVAREZ_HOME/.install_method" >/dev/null \
    || true
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$SAVAREZ_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        as_hermes cp "$INSTALL_DIR/$src" "$SAVAREZ_HOME/$dest"
    fi
}
...
if [ -f "$SAVAREZ_HOME/.env" ]; then
    chown savarez:savarez "$SAVAREZ_HOME/.env" 2>/dev/null || true
    chmod 600 "$SAVAREZ_HOME/.env" 2>/dev/null || true
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $SAVAREZ_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `savarez update` runs for non-Docker installs,
...
if [ -f "$SAVAREZ_HOME/config.yaml" ]; then
    s6-setuidgid savarez "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$SAVAREZ_HOME/auth.json" ] && [ -n "${HERMES_AUTH_JSON_BOOTSTRAP:-}" ]; then
    printf '%s' "$HERMES_AUTH_JSON_BOOTSTRAP" > "$SAVAREZ_HOME/auth.json"
    chown savarez:savarez "$SAVAREZ_HOME/auth.json" 2>/dev/null || true
    chmod 600 "$SAVAREZ_HOME/auth.json"
fi
...
if [ ! -f "$SAVAREZ_HOME/gateway_state.json" ] && \
        [ "${HERMES_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    printf '{"gateway_state":"running"}\n' > "$SAVAREZ_HOME/gateway_state.json"
    chown savarez:savarez "$SAVAREZ_HOME/gateway_state.json" 2>/dev/null || true
    chmod 644 "$SAVAREZ_HOME/gateway_state.json"
fi
...
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/savarez/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Savarez spawns for the
...
# main-wrapper.sh propagates it into the supervised ``savarez`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-savarez, dashboard, etc.).
