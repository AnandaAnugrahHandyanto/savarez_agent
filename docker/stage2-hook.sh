...
# root can create (e.g. `SAVAREZ_HOME=/home/hermes/.hermes` in a Compose
...
# directories hermes actually writes to. The full $SAVAREZ_HOME may be a
...
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $SAVAREZ_HOME (targeted) to hermes ($actual_hermes_uid)"
    ...
    chown hermes:hermes "$SAVAREZ_HOME" 2>/dev/null || \
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing; do
        if [ -e "$SAVAREZ_HOME/$sub" ]; then
            chown -R hermes:hermes "$SAVAREZ_HOME/$sub" 2>/dev/null || \
...
# Always reset ownership of $SAVAREZ_HOME/profiles to hermes on every
...
if [ -d "$SAVAREZ_HOME/profiles" ]; then
    chown -R hermes:hermes "$SAVAREZ_HOME/profiles" 2>/dev/null || true
fi
...
for f in \
    ...
    if [ -e "$SAVAREZ_HOME/$f" ]; then
        chown hermes:hermes "$SAVAREZ_HOME/$f" 2>/dev/null || true
    fi
done
...
if [ -f "$SAVAREZ_HOME/config.yaml" ]; then
    chown hermes:hermes "$SAVAREZ_HOME/config.yaml" 2>/dev/null || true
...
if [ -f "$SAVAREZ_HOME/.env" ]; then
    chown hermes:hermes "$SAVAREZ_HOME/.env" 2>/dev/null || true
...
if [ ! -f "$SAVAREZ_HOME/auth.json" ] && [ -n "${HERMES_AUTH_JSON_BOOTSTRAP:-}" ]; then
    ...
    chown hermes:hermes "$SAVAREZ_HOME/auth.json" 2>/dev/null || true
...
if [ ! -f "$SAVAREZ_HOME/gateway_state.json" ] && \
    ...
    chown hermes:hermes "$SAVAREZ_HOME/gateway_state.json" 2>/dev/null || true
