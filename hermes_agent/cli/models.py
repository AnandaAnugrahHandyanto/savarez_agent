...
def _codex_curated_models() -> list[str]:
    ...
    """
    from hermes_agent.cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from hermes_agent.cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from hermes_agent.cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def detect_provider_for_model(
    ...
    if direct_match:
        ...
        if not has_creds:
            try:
                from hermes_agent.cli.auth import _load_auth_store
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    try:
        from hermes_agent.cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from hermes_agent.cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from hermes_agent.cli.copilot_auth import copilot_request_headers
