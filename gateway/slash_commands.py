...
def _run_mod():
    """Return gateway.run lazily to avoid import cycles and preserve monkeypatches."""
    import gateway.run as _run
    return _run


class _RunPathProxy:
    """Path-like proxy for gateway.run._hermes_home resolved at call time."""

    def _value(self):
        return getattr(_run_mod(), "_hermes_home")
...
_hermes_home = _RunPathProxy()


def _run_file() -> str:
    return getattr(_run_mod(), "__file__")


def _agent_pending_sentinel():
    return getattr(_run_mod(), "_AGENT_PENDING_SENTINEL")


def _interrupt_reason_stop() -> str:
    return getattr(_run_mod(), "_INTERRUPT_REASON_STOP")


def _home_target_env_var(*args, **kwargs):
    return getattr(_run_mod(), "_home_target_env_var")(*args, **kwargs)


def _home_thread_env_var(*args, **kwargs):
    return getattr(_run_mod(), "_home_thread_env_var")(*args, **kwargs)


def _load_gateway_config(*args, **kwargs):
    return getattr(_run_mod(), "_load_gateway_config")(*args, **kwargs)


def _platform_config_key(*args, **kwargs):
    return getattr(_run_mod(), "_platform_config_key")(*args, **kwargs)


def _resolve_gateway_model(*args, **kwargs):
    return getattr(_run_mod(), "_resolve_gateway_model")(*args, **kwargs)


def _resolve_hermes_bin(*args, **kwargs):
    return getattr(_run_mod(), "_resolve_hermes_bin")(*args, **kwargs)


def _telegramize_command_mentions(*args, **kwargs):
    return getattr(_run_mod(), "_telegramize_command_mentions")(*args, **kwargs)


def atomic_json_write(*args, **kwargs):
    return getattr(_run_mod(), "atomic_json_write")(*args, **kwargs)


def atomic_yaml_write(*args, **kwargs):
    return getattr(_run_mod(), "atomic_yaml_write")(*args, **kwargs)


def fetch_account_usage(*args, **kwargs):
    return getattr(_run_mod(), "fetch_account_usage")(*args, **kwargs)


def render_account_usage_lines(*args, **kwargs):
    return getattr(_run_mod(), "render_account_usage_lines")(*args, **kwargs)
...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        project_root = Path(_run_file()).parent.parent.resolve()
