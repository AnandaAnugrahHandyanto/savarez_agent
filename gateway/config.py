...
@dataclass
class IncidentMonitorConfig:
    """
    Configuration for automated incident channel monitoring and RCA orchestration.

    Loaded from config.yaml under gateway_config.incident_monitor.
    ...
    Example YAML::
        ...
        gateway_config:
          incident_monitor:
            enabled: true
            match_pattern: "^inc-.*$"
            grant_channel_members: true
            admin_whitelist: ["U12345", "U67890"]
            auto_investigate: ["P0", "P1"]
            manual_investigate: ["P2", "escalation"]
            thread_title: "RCA Incident Investigation"
            report_format: "html"
    """
    enabled: bool = False
    # Regex applied to channel names — channels that match are auto-joined.
    match_pattern: str = r"^inc-.*$"
    # When True, any member of a matched incident channel is granted temporary
    # Operator permissions for the bot (channel-member RBAC).
    grant_channel_members: bool = False
    # Hard admin allowlist that always bypasses channel-member RBAC.
    # Corresponds to SLACK_ALLOWED_USERS in .env — stored here as an overlay.
    admin_whitelist: List[str] = field(default_factory=list)
    # Tiers that trigger automatic RCA on channel join.
    auto_investigate: List[str] = field(default_factory=lambda: ["P0", "P1"])
    # Tiers that require an explicit /hermes investigate command.
    manual_investigate: List[str] = field(default_factory=lambda: ["P2", "ESCALATION"])
    # Title posted as the parent thread for all RCA activity.
    thread_title: str = "RCA Incident Investigation"
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "grant_channel_members": self.grant_channel_members,
            "admin_whitelist": self.admin_whitelist,
            "auto_investigate": self.auto_investigate,
            "manual_investigate": self.manual_investigate,
            "thread_title": self.thread_title,
            "report_format": self.report_format,
        }

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "IncidentMonitorConfig":
        if not data:
            return cls()
        raw_auto = data.get("auto_investigate", ["P0", "P1"])
        raw_manual = data.get("manual_investigate", ["P2", "escalation"])
        raw_whitelist = data.get("admin_whitelist", [])
        return cls(
            enabled=_coerce_bool(data.get("enabled"), False),
            match_pattern=data.get("match_pattern", r"^inc-.*$"),
            grant_channel_members=_coerce_bool(data.get("grant_channel_members"), False),
            admin_whitelist=list(raw_whitelist) if isinstance(raw_whitelist, (list, tuple)) else [],
            auto_investigate=[str(t).upper() for t in raw_auto] if raw_auto else ["P0", "P1"],
            manual_investigate=[str(t).upper() for t in raw_manual] if raw_manual else ["P2", "ESCALATION"],
            thread_title=data.get("thread_title", "RCA Incident Investigation"),
...
class GatewayConfig:
    ...
    # Incident channel monitoring + automated RCA orchestration (Slack).
    incident_monitor: IncidentMonitorConfig = field(default_factory=IncidentMonitorConfig)
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        return cls(
            ...
            incident_monitor=IncidentMonitorConfig.from_dict(data.get("incident_monitor", {})),
        )
...
def load_gateway_config() -> GatewayConfig:
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # Incident monitor config — nested under gateway_config.incident_monitor
            # or at the top-level incident_monitor key.
            incident_monitor_cfg = yaml_cfg.get("incident_monitor")
            if isinstance(incident_monitor_cfg, dict):
                gw_data["incident_monitor"] = incident_monitor_cfg
