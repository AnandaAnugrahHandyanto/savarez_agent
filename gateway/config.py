        ...
        for entry in platform_registry.plugin_entries():
            ...
            if existing_cfg is not None:
                enabled_was_explicit = bool(
                    existing_cfg.extra.pop("_enabled_explicit", False)
                )
                if enabled_was_explicit and not existing_cfg.enabled:
                    # Respect explicit YAML disables such as ``discord.enabled: false``.
                    # Some plugin check_fn implementations only verify Python deps, not
                    # credentials, so blindly enabling here can make an intentionally
                    # disabled platform reconnect forever without a token.
                    continue
