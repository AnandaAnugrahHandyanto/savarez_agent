...
def _cleanup_invalid_pid_path(pid_path: Path) -> None:
    """Force-unlink stale PID + lock metadata after liveness checks fail."""
    try:
...
def remove_pid_file() -> None:
    ...
    try:
        ...
        pass
    try:
        _get_gateway_lock_path().unlink(missing_ok=True)
    except Exception:
        pass
...
def get_running_pid(pid_path: Optional[Path] = None) -> Optional[int]:
    ...
    if not is_gateway_runtime_lock_active(resolved_lock_path):
        _cleanup_invalid_pid_path(resolved_pid_path)
        return None
