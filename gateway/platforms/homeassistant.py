...
class HomeAssistantAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if not self._notifications_enabled:
                logger.info("[%s] Outbound persistent notifications disabled "
                            "(notifications=false in config)", self.name)
            return True
    ...
    async def send(
        ...
        """
        # Honor the `notifications` config flag (default: true).
        # When false, persistent_notification.create is skipped but
        # event listening continues unaffected.
        if not self._notifications_enabled:
            logger.debug("[%s] notifications disabled — skip send()", self.name)
            return SendResult(success=True, message_id=f"silent_{uuid.uuid4().hex[:12]}")
