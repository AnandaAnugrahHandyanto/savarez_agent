...
class QQAdapter(BasePlatformAdapter):
    ...
    async def _listen_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                for fut in self._pending_responses.values():
                    if not fut.done():
                        fut.set_exception(RuntimeError("Connection interrupted"))
                self._pending_responses.clear()
    ...
    async def _send_identify(self) -> None:
        ...
        identify_payload = {
            ...
            "d": {
                "token": f"QQBot {token}",
                "intents": 1 << 25 | 1 << 30,  # GUILD_MESSAGES + C2C_GROUP_AT_MESSAGES
                "shard": [0, 1],
                "properties": {
                    "$os": "macOS",
    ...
    def _dispatch_payload(self, payload: Dict[str, Any]) -> None:
        ...
        if op == 10:
            ...
            interval = d_data.get("heartbeat_interval", 30000)
            logger.debug("[%s] Hello received, heartbeat_interval=%dms", self.name, interval)
    ...
    def _guess_chat_type(chat_id: str) -> str:
        """Heuristic to determine the chat type from chat_id format."""
        # QQ openids are typically hex strings (32 chars) or UUIDs
        # Group openids are similar but come from group context
        # We rely on metadata stored by the adapter during inbound routing
        return "c2c"  # Default; overridden by send() when metadata provides chat_type
