...
class FeishuAdapter(BasePlatformAdapter):
    ...
    async def _process_inbound_message(
        ...
        *,
        ...
        thread_id = self._message_thread_id(message, chat_type=chat_type)
        reply_to_message_id = (
    ...
    def _message_thread_id(message: Any, *, chat_type: str) -> Optional[str]:
        """Return Feishu thread/topic id for session routing.

        Feishu P2P replies can carry root_id/parent_id metadata even though the
        user still expects one continuous DM conversation. Treating that root_id
        as a Hermes thread splits the private chat into hidden per-reply
        sessions, so only group/channel messages contribute thread_id.
        """
        if chat_type == "p2p":
            return None
        return getattr(message, "thread_id", None) or getattr(message, "root_id", None) or None
