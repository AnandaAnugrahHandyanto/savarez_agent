...
class FeishuAdapter(BasePlatformAdapter):
    ...
    async def _add_ack_reaction(self, message_id: str) -> Optional[str]:
        """Add the standard OK acknowledgement reaction used by Hermes."""
        reaction_id = await self._add_reaction(message_id, 'OK')
        if reaction_id is None:
            logger.warning('Failed to add ack reaction to %s', message_id)
        return reaction_id
    ...
    async def _hydrate_bot_identity(self) -> None:
        ...
        try:
            ...
            if not response or not response.success():
                # Optional endpoint: missing application-info permission should
                # not become a persistent gateway warning. Operators can avoid
                # this probe entirely by setting FEISHU_BOT_NAME, and precise
                # mention gating still works with FEISHU_BOT_OPEN_ID.
                logger.debug(
                    "[Feishu] Unable to hydrate bot name from application info",
                    extra={"code": getattr(response, "code", None)},
                )
                return
