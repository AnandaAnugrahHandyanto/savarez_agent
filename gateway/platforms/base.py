...
# Audio file extensions Savarez recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Savarez' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Savarez-created Telegram private-chat
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.savarez/.env manually."
)
...
# Default location: {SAVAREZ_HOME}/cache/images/ (legacy: image_cache/)
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; SavarezAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _SAVAREZ_HOME / "image_cache",
    _SAVAREZ_HOME / "audio_cache",
    _SAVAREZ_HOME / "video_cache",
    _SAVAREZ_HOME / "document_cache",
    _SAVAREZ_HOME / "browser_screenshots",
    ...
    _SAVAREZ_HOME / "cache" / "images",
    _SAVAREZ_HOME / "cache" / "audio",
    _SAVAREZ_HOME / "cache" / "videos",
    _SAVAREZ_HOME / "cache" / "documents",
    _SAVAREZ_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Savarez profile and shared Savarez root both contain control
    # files and credentials. Only cache subdirectories under them are
    # explicitly allowlisted above.
    for hermes_root in (_SAVAREZ_HOME, _HERMES_ROOT):
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Savarez
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    Savarez-managed cache, under an operator-allowlisted root
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.savarez/.env,
    # ~/.savarez/auth.json, etc. — so the obvious prompt-injection sites
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?savarez\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+savarez[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # Savarez commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.savarez/media/generated/stale.png"}
