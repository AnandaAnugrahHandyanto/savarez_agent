...
class TelegramAdapter(BasePlatformAdapter):
        ...
        **kwargs,
        ...
        try:
            ...
            if not os.path.exists(audio_path):
                error_msg = self._missing_media_path_error("Audio", audio_path)
            ...
            # Validate response
            if msg is None or not hasattr(msg, "message_id") or msg.message_id is None:
                logger.error(
                    "[%s] send_voice returned invalid response for chat=%s",
                    self.name,
                    chat_id,
                )
                return SendResult(
                    success=False, error="Telegram returned invalid response"
                )
            logger.info(
                "[%s] Voice/audio sent successfully: chat=%s message_id=%s",
                self.name,
                chat_id,
                msg.message_id,
            )
            ...
            error_msg = f"Failed to send voice/audio to chat={chat_id}: {e}"
            logger.error("[%s] %s", self.name, error_msg, exc_info=True)
            return SendResult(success=False, error=error_msg)
        ...
        **kwargs,
        ...
        try:
            ...
            if not os.path.exists(image_path):
                error_msg = self._missing_media_path_error("Image", image_path)
                logger.error("[%s] %s", self.name, error_msg)
                return SendResult(success=False, error=error_msg)
            ...
            if msg is None or not hasattr(msg, "message_id") or msg.message_id is None:
                logger.error(
                    "[%s] send_photo returned invalid response for chat=%s",
                    self.name,
                    chat_id,
                )
                return SendResult(
            ...
            logger.info(
                "[%s] Image sent successfully: chat=%s message_id=%s",
                self.name,
                chat_id,
                msg.message_id,
            )
            ...
            error_msg = f"Failed to send image to chat={chat_id}: {e}"
            logger.error("[%s] %s", self.name, error_msg, exc_info=True)
            return SendResult(success=False, error=error_msg)
        ...
        **kwargs,
        ...
        try:
            if not os.path.exists(file_path):
                error_msg = self._missing_media_path_error("File", file_path)
                logger.error("[%s] %s", self.name, error_msg)
                return SendResult(success=False, error=error_msg)
            ...
            # Validate response - msg must exist and have message_id
            if msg is None or not hasattr(msg, "message_id") or msg.message_id is None:
                logger.error(
                    "[%s] send_document returned invalid response for chat=%s",
                    self.name,
                    chat_id,
                )
                return SendResult(
            ...
            logger.info(
                "[%s] Document sent successfully: chat=%s message_id=%s",
                self.name,
                chat_id,
                msg.message_id,
            )
            ...
            error_msg = f"Failed to send document to chat={chat_id}: {e}"
            logger.error("[%s] %s", self.name, error_msg, exc_info=True)
            return SendResult(success=False, error=error_msg)
        ...
        **kwargs,
        ...
        try:
            if not os.path.exists(video_path):
                error_msg = self._missing_media_path_error("Video", video_path)
                logger.error("[%s] %s", self.name, error_msg)
                return SendResult(success=False, error=error_msg)
            ...
            if msg is None or not hasattr(msg, "message_id") or msg.message_id is None:
                logger.error(
                    "[%s] send_video returned invalid response for chat=%s",
                    self.name,
                    chat_id,
                )
                return SendResult(
            ...
            logger.info(
                "[%s] Video sent successfully: chat=%s message_id=%s",
                self.name,
                chat_id,
                msg.message_id,
            )
            ...
            error_msg = f"Failed to send video to chat={chat_id}: {e}"
            logger.error("[%s] %s", self.name, error_msg, exc_info=True)
            return SendResult(success=False, error=error_msg)
            ...
            try:
                ...
                try:
                    ...
                    if (
                        ...
                        and self._is_thread_not_found_error(e)
                    ):
                        await self._bot.send_chat_action(
                            chat_id=int(chat_id),
                            action="typing",
    ...
    def _build_message_event(
        ...
        source = self.build_source(
            ...
            user_id=str(user.id)
            if user
            else (str(chat.id) if chat_type == "dm" else None),
