...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def _auto_rename_thread(self, thread_id: str, content: str, *, already_titled: bool = False) -> None:
        """Rename a Discord thread to a short topic title.

        When *already_titled* is True, *content* is treated as the final
        title string (generated by maybe_auto_title / the session title
        pipeline) and is used directly after light sanitisation.

        When *already_titled* is False (legacy path), *content* is the raw
        first user message — Discord mention syntax is stripped and the
        result is truncated to 80 chars.

        Silently swallows errors (permission denied, rate-limit) since
        renaming is a best-effort quality-of-life feature.
        """
        import re as _re

        if already_titled:
            # Title already generated by LLM — just sanitise length
            name = content.strip()
            name = name[:77] + "..." if len(name) > 80 else name
        else:
            # Raw user message — strip Discord mention syntax
            name = _re.sub(r"<@[!&]?\d+>", "", content)
            name = _re.sub(r"<#\d+>", "", name)
            name = _re.sub(r"\s+", " ", name).strip()
            name = name[:77] + "..." if len(name) > 80 else name[:80]
        ...
        try:
            ...
            req = _urlreq.Request(url, data=data, method="PATCH")
            req.add_header("Authorization", f"Bot {token}")
            req.add_header("Content-Type", "application/json")
            req.add_header("User-Agent", "HermesBot/1.0")
            _urlreq.urlopen(req, timeout=10)
            logger.debug("[%s] Auto-renamed thread %s → %r", self.name, thread_id, name)
        except Exception as exc:
            logger.debug("[%s] Auto-rename thread %s failed: %s", self.name, thread_id, exc)
