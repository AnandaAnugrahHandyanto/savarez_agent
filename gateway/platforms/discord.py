...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def _handle_message(self, message: DiscordMessage) -> None:
        ...
        # Discord autocomplete can surface an app/bot role with the same display
        # name as the bot.  Those mentions arrive as <@&role_id> in
        # message.role_mentions, not message.mentions, so the normal bot-user
        # mention gate above would ignore them.  Allow specific role IDs to act
        # as aliases for this bot mention via discord.mention_role_ids or
        # DISCORD_MENTION_ROLE_IDS (comma-separated).  Keep this opt-in so
        # arbitrary role pings do not wake the bot.
        role_mention_ids_raw = self.config.extra.get("mention_role_ids")
        if role_mention_ids_raw is None:
            role_mention_ids_raw = os.getenv("DISCORD_MENTION_ROLE_IDS", "")
        if isinstance(role_mention_ids_raw, (list, tuple, set)):
            mention_role_ids = {str(v).strip() for v in role_mention_ids_raw if str(v).strip()}
        else:
            mention_role_ids = {
                part.strip()
                for part in str(role_mention_ids_raw or "").split(",")
                if part.strip()
            }
        if mention_role_ids:
            role_mentions = getattr(message, "role_mentions", []) or []
            matched_role_ids = {str(getattr(role, "id", "")) for role in role_mentions}
            matched_role_ids &= mention_role_ids
            if matched_role_ids:
                mention_prefix = True
