...
class IMessageAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._db_path = extra.get("database_path", _DEFAULT_DB_PATH)
    ...
    async def send_image(self, chat_id: str, image_url: str, caption: Optional[str] = None,
        ...
        if caption:
            await self.send(chat_id, caption)
        elif not self._local and image_url:
    ...
    async def send_document(self, chat_id: str, file_path: str, caption: Optional[str] = None) -> SendResult:
        if self._local:
    ...
    async def _handle_local_message(self, row: Dict[str, Any]):
        ...
        for att in row.get("attachments") or []:
            ...
            if ext in _AUDIO_EXTENSIONS or mime.startswith("audio/"):
                cached = cache_audio_from_bytes(Path(file_path).read_bytes(), ext or ".m4a")
                content = f"{content}\n[Voice message: {cached}]" if content else f"[Voice message: {cached}]"
                continue

            if mime.startswith("image/"):
                cached = cache_image_from_bytes(Path(file_path).read_bytes(), ext or ".jpg")
                image_paths.append(cached)
            else:
                cached = cache_document_from_bytes(Path(file_path).read_bytes(), ext or ".bin")
