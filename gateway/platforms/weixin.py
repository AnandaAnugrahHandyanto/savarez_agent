...
class WeixinAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._enable_markdown: bool = extra.get("enable_markdown", True)
    ...
    async def send(
        ...
        plain = content.strip() if self._enable_markdown else _markdown_to_plain(content).strip()
        if not plain:
    ...
    async def _send_media_file(
        ...
        item_list = []
        if caption:
            item_list.append({"type": WX_ITEM_TEXT, "text_item": {"text": caption if self._enable_markdown else _markdown_to_plain(caption)}})
    ...
    def format_message(self, content: str) -> str:
        return content if self._enable_markdown else _markdown_to_plain(content)
