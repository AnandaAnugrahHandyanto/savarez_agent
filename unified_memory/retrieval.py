...
from unified_memory.types import MemoryFact, ScoredFact, FactType, METABOLIC_RATES
from unified_memory.config import UnifiedMemoryConfig
from unified_memory.links import cosine_similarity, build_link_map_and_embeddings
...
def score_candidates(
    ...
    for c in candidates:
        ...
        # Equalize base_level for batch-stored facts (within 1 second)
        # This prevents insertion order from dominating when facts are
        # stored simultaneously (e.g. MEMORY_SPEC notation parsing)
        age_seconds = now - c.get("created_at", now)
        if age_seconds < 1.0:
            base_level = max(base_level, 2.0)  # Normalize very recent facts
...
def expand_query_from_targets(conn: sqlite3.Connection, query: str) -> str:
    """Expand a query with related terms from stored fact targets.

    When TF-IDF embeddings give zero signal, FTS5 target column can provide
    the missing keyword bridge. E.g. query "authentication method" + target
    "auth.method" → expanded query includes "auth method authentication".
    
    Inspired by Icarus topic-change detection: proactively find relevant
    context when the current signal is weak.
    """
    ...
    # Find targets whose words overlap with query words
    rows = conn.execute(
        "SELECT DISTINCT target FROM um_facts WHERE status='active' AND target != 'general'"
    ).fetchall()

    extra_words = set()
    for r in rows:
        target = r["target"]
        target_words = {w.lower() for w in target.replace(".", " ").replace("_", " ").split() if len(w) > 2}
        # Check if any query word is a prefix/substring of a target word or vice versa
        for qw in query_words:
            for tw in target_words:
                if len(qw) >= 3 and (tw.startswith(qw) or qw.startswith(tw)):
                    extra_words |= target_words
                    break

    if extra_words:
        return query + " " + " ".join(extra_words - query_words)
    return query
...
def fts5_search(
    ...
    """
    # Expand query with target metadata for better keyword coverage
    expanded = expand_query_from_targets(conn, query)
    tokens = [t for t in expanded.strip().split() if t.lower() not in STOP_WORDS and len(t) > 1]
    if not tokens:
...
def apply_dampening(
    ...
    # ── 2. HUB DAMPENING ──
    from unified_memory.links import get_all_links
...
def _row_to_fact(row: dict, embedding: Optional[np.ndarray], access_times: List[float]) -> MemoryFact:
    ...
    from unified_memory.types import FACT_TYPE_FROM_NOTATION
