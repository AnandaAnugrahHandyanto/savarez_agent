...
class UnifiedMemoryStore:
    ...
    def store(
        ...
        if parsed:
            ...
            # Prepend target keywords to content for searchability
            # "C[auth.mfa]: Multi-factor auth..." → "auth mfa: Multi-factor auth..."
            # This ensures queries matching the target domain find the fact
            target_words = tgt.replace(".", " ").replace("_", " ")
            content = f"{target_words}: {raw_content}"
        ...
        if (self._config.enable_supersession
            ...
            if superseded_id:
                # Inherit from superseded fact: activation, embedding signal, links
                old_row = self._conn.execute(
                    "SELECT activation, embedding, content FROM um_facts WHERE id = ?",
                    (superseded_id,)
                ).fetchone()
                if old_row:
                    superseded_activation = old_row["activation"] * self._config.activation_transfer_ratio

                    # Blend embeddings: new fact inherits retrieval signal from old
                    # This ensures queries that found the old fact will find the new one
                    if embedding is not None and old_row["embedding"] is not None:
                        import numpy as np
                        old_emb = np.frombuffer(old_row["embedding"], dtype=np.float32)
                        if old_emb.shape[0] == embedding.shape[0]:
                            # 70% new + 30% old: new meaning dominates but old signal preserved
                            embedding = 0.7 * embedding + 0.3 * old_emb
                        elif old_emb.shape[0] < embedding.shape[0]:
                            # Pad old embedding and blend
                            padded = np.pad(old_emb, (0, embedding.shape[0] - old_emb.shape[0]))
                            embedding = 0.7 * embedding + 0.3 * padded

                    # Mark superseded_id for post-INSERT link+access transfer
                    self._pending_superseded_id = superseded_id
        ...
        # Complete supersession inheritance: transfer access times and links
        pending_sup = getattr(self, '_pending_superseded_id', None)
        if pending_sup:
            # Transfer access history (gives new fact the old fact's retrieval boost)
            self._conn.execute(
                "UPDATE um_access_times SET fact_id = ? WHERE fact_id = ?",
                (fact_id, pending_sup)
            )
            # Transfer Hebbian links
            self._conn.execute(
            ...
            self._pending_superseded_id = None
        ...
        if self._config.enable_interaction_log:
            self._mark_recent_interactions_followed_by_store(now)
    ...
    def recall(
        ...
        if self._config.enable_interaction_log and results:
            self._log_interaction(query, results, scope, now)
    ...
    def reward_memory(self, memory_id: str, signal: float) -> None:
        ...
        if self._qvalue_store:
            ...
            q_value = self._qvalue_store.get_q(memory_id)
            now = self._now()
            self._conn.execute(
                "UPDATE um_facts SET q_value=?, updated_at=? WHERE id=?",
                (q_value, now, memory_id)
            )
            self._conn.execute(
                """INSERT INTO um_qvalues
                (memory_id, q_value, update_count, total_retrievals, last_updated)
                VALUES (?, ?, 1, 0, ?)
                ON CONFLICT(memory_id) DO UPDATE SET
                    q_value=excluded.q_value,
                    update_count=um_qvalues.update_count + 1,
                    last_updated=excluded.last_updated
                """,
                (memory_id, q_value, now)
            )
            self._conn.commit()
    ...
    def _log_interaction(
        ...
        """Persist a recall interaction for later training-data export."""
        interaction_id = str(uuid.uuid4())
        recalled_ids = json.dumps([r.fact.id for r in results])
        self._conn.execute(
            """INSERT INTO um_interactions
            (id, query, scope, recalled_ids, followed_by_store, created_at)
            VALUES (?, ?, ?, ?, 0, ?)""",
            (interaction_id, query, scope, recalled_ids, now)
        )
        self._conn.commit()

    def _mark_recent_interactions_followed_by_store(self, now: float) -> None:
        """Mark recent recall interactions as successful when a store follows them."""
        credit_window = 300.0
        if self._reward_tracker is not None:
            credit_window = self._reward_tracker.credit_window
        self._conn.execute(
            """UPDATE um_interactions
            SET followed_by_store = 1
            WHERE followed_by_store = 0
              AND (? - created_at) <= ?
              AND (? - created_at) >= 0
            """,
            (now, credit_window, now)
        )
        self._conn.commit()
