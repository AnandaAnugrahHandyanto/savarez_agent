...
-- ------------------------------------------------------------ um_interactions ---
CREATE TABLE IF NOT EXISTS um_interactions (
    id                TEXT    PRIMARY KEY,
    query             TEXT    NOT NULL,
    scope             TEXT,
    recalled_ids      TEXT    NOT NULL,
    followed_by_store INTEGER NOT NULL DEFAULT 0,
    created_at        REAL    NOT NULL
);

CREATE INDEX IF NOT EXISTS idx_um_interactions_created_at ON um_interactions(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_um_interactions_followed_store
    ON um_interactions(followed_by_store, created_at DESC);
