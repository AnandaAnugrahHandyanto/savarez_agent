...
def run_conversation(
    ...
    # Hermes 2.8: Build Task Card + Event Log. Best-effort; never fail
    # the user turn if local lifecycle tracking has a problem.
    try:
        ...
        agent._event_log.log_status_changed(
            task_id=task_card.task_id,
            session_id=agent.session_id,
            from_status="pending",
            to_status="running",
        ...
        agent._event_log.log_execution_started(
            task_id=task_card.task_id,
            session_id=agent.session_id,
        )
    except Exception:
    ...
    if getattr(agent, "_current_task_card", None) is not None:
        try:
            ...
            agent._event_log.log_dispatch_decision(
                task_id=tc.task_id,
                session_id=agent.session_id,
                mode=routing.mode,
    ...
    if tc is not None and completed and final_response:
        try:
            ...
            if memory_store is not None:
                try:
                    ...
                    if mem_entries:
                        ...
                        llm_results = agent._review_gate.populate_llm_checks_from_memory(
            ...
            review_result = agent._review_gate.check(
                task_card=tc,
            ...
            agent._event_log.log_quality_check(
                task_id=tc.task_id,
                session_id=agent.session_id,
                quality_score=review_result.quality_score,
                passed=not agent._review_gate.is_blocked(review_result),
                risks=review_result.risks,
            )

            if agent._review_gate.is_blocked(review_result):
                ...
                tc.status = "reviewing"
                save_task_card(tc)
            elif agent._review_gate.allows_degraded_delivery(review_result):
                _review_risks = review_result.risks
        except Exception:
    ...
    if tc is not None and not _review_blocked:
        try:
            if completed and not interrupted:
                agent._event_log.log_execution_completed(
                    task_id=tc.task_id,
                    session_id=agent.session_id,
