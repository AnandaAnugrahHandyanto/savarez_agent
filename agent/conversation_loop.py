...
from agent.event_bus import HermesEvent, HermesEventType
...
def run_conversation(
    ...
    # Phase 1: Emit task started on the event bus
    try:
        agent._event_bus.emit(HermesEvent(type=HermesEventType.TASK_STARTED))
    except Exception:
        pass  # Never let event bus errors affect the turn
    ...
    while (
        ...
        if agent._budget_grace_call:
            ...
            # Phase 1: Emit budget exhaustion event
            try:
                agent._event_bus.emit(
                    HermesEvent(type=HermesEventType.ITERATION_EXHAUSTED)
                )
            except Exception:
                pass
            try:
                agent._op_state.on_budget_update(0.0)
            except Exception:
                pass
            break
        else:
            # Normal iteration — update operational state with current budget fraction
            try:
                frac = (
                    agent.iteration_budget.remaining / agent.iteration_budget.max_total
                )
                agent._op_state.on_budget_update(frac)
            except Exception:
                pass
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if response_invalid:
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}⚠️  Invalid API response (attempt {retry_count}/{max_retries}): {', '.join(error_details)}",
                        force=True,
                    )
                    agent._vprint(
                        f"{agent.log_prefix}   🏢 Provider: {provider_name}", force=True
                    )
                    cleaned_provider_error = agent._clean_error_message(error_msg)
                    agent._vprint(
                        f"{agent.log_prefix}   📝 Provider message: {cleaned_provider_error}",
                        force=True,
                    )
                    agent._vprint(
                        f"{agent.log_prefix}   ⏱️  {_failure_hint}", force=True
                    )
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}⏳ Retrying in {wait_time:.1f}s ({_failure_hint})...",
                        force=True,
                    )
                    logging.warning(
                ...
                if (
                    ...
                    if agent._try_refresh_codex_client_credentials(force=True):
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}🔐 {_label} auth refreshed after 401. Retrying request..."
                        )
                        continue
                if (
                    ...
                    print(
                        f"{agent.log_prefix}     • Re-authenticate: hermes login --provider nous"
                    )
                    print(
                ...
                if (
                    ...
                    if agent._try_refresh_copilot_client_credentials():
                        agent._vprint(
                            f"{agent.log_prefix}🔐 Copilot credentials refreshed after 401. Retrying request..."
                        )
                        continue
                if (
                ...
                agent._vprint(
                    f"{agent.log_prefix}⚠️  API call failed (attempt {retry_count}/{max_retries}): {error_type}{_status_code_str}",
                    force=True,
                )
                agent._vprint(
                ...
                agent._vprint(
                    f"{agent.log_prefix}   📝 Error: {_error_summary}", force=True
                )
                if status_code and status_code < 500:
                    ...
                    if _err_body_str:
                        agent._vprint(
                            f"{agent.log_prefix}   📋 Details: {_err_body_str}",
                            force=True,
                        )
                agent._vprint(
                    f"{agent.log_prefix}   ⏱️  Elapsed: {elapsed_time:.2f}s  Context: {len(api_messages)} msgs, ~{approx_tokens:,} tokens"
                )
                ...
                if is_context_length_error:
                    ...
                    if parsed_limit and parsed_limit < old_ctx:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}Context limit detected from API: {new_ctx:,} tokens (was {old_ctx:,})",
                            force=True,
                        )
                    ...
                    if new_ctx and new_ctx < old_ctx:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}⚠️  Context length exceeded — stepping down: {old_ctx:,} → {new_ctx:,} tokens",
                            force=True,
                        )
                    else:
                        agent._vprint(
                            f"{agent.log_prefix}⚠️  Context length exceeded at minimum tier — attempting compression...",
                            force=True,
                        )
                    ...
                    if (
                        ...
                        if (
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(
                                    f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok Subscription) from `hermes model`.",
                                    force=True,
                                )
                        else:
                            agent._vprint(
        ...
        try:
            ...
            if has_incomplete_scratchpad(assistant_message.content or ""):
                ...
                agent._vprint(
                    f"{agent.log_prefix}⚠️  Incomplete <REASONING_SCRATCHPAD> detected (opened but never closed)"
                )

                if agent._incomplete_scratchpad_retries <= 2:
                    agent._vprint(
                        f"{agent.log_prefix}🔄 Retrying API call ({agent._incomplete_scratchpad_retries}/2)..."
                    )
            ...
            if assistant_message.tool_calls:
                ...
                if invalid_tool_calls:
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}⚠️  Unknown tool '{invalid_preview}' — sending error to model for agent-correction ({agent._invalid_tool_retries}/3)"
                    )
                ...
                if invalid_json_args:
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}⚠️  Invalid JSON in tool call arguments for '{tool_name}': {error_msg}"
                    )

                    if agent._invalid_json_retries < 3:
                        agent._vprint(
                            f"{agent.log_prefix}🔄 Retrying API call ({agent._invalid_json_retries}/3)..."
                        )
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}⚠️  Injecting recovery tool results for invalid JSON..."
                        )
    ...
    # Phase 1: Emit task ended + record turn outcome in operational state
    _turn_ok = (
        completed
        and not interrupted
        and _turn_exit_reason
        in (
            "normal",
    ...
    try:
        agent._op_state.on_turn_complete(ok=_turn_ok)
        agent._op_state.on_task_ended()
        agent._event_bus.emit(HermesEvent(type=HermesEventType.TASK_ENDED))
    except Exception:
        pass
