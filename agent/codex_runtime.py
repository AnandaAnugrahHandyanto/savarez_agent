...
def _codex_stream_parts_response(
    *,
    output_items: list,
    text_parts: list,
    has_tool_calls: bool,
    ...
    """Build a minimal completed response from already-streamed content."""
    if output_items:
        logger.debug(
            "%s: recovered %d output items from stream events",
            log_prefix,
            len(output_items),
        )
...
def _backfill_codex_response_output(
    ...
    *,
    ...
    if isinstance(_out, list) and _out:
        return response
    recovered = _codex_stream_parts_response(
    ...
    if recovered is not None:
        ...
        if getattr(recovered, "output_text", None) and not getattr(response, "output_text", None):
            response.output_text = recovered.output_text
        if not getattr(response, "status", None):
            response.status = "completed"
    return response


def _is_responses_output_none_type_error(exc: TypeError) -> bool:
    err_text = str(exc)
    ...
    for attempt in range(max_stream_retries + 1):
        ...
        try:
            with active_client.responses.stream(**api_kwargs) as stream:
                ...
                # but get_final_response() can return empty/null output.
            ...
            if not _is_responses_output_none_type_error(exc):
                raise
            recovered = _codex_stream_parts_response(
                output_items=collected_output_items,
                text_parts=agent._codex_streamed_text_parts,
                has_tool_calls=has_tool_calls,
                log_prefix="Codex stream parser recovery",
            )
            if recovered is not None:
                logger.warning(
                    "Codex Responses stream parser failed on response.output=None; "
                    "recovered from %d output items and %d text deltas. %s",
            ...
            if attempt < max_stream_retries:
                ...
                continue
            logger.warning(
                "Codex Responses stream parser failed on response.output=None and "
                "no streamed content was available; falling back to create(stream=True). %s",
                agent._client_log_context(),
            )
            return agent._run_codex_create_stream_fallback(api_kwargs, client=active_client)
