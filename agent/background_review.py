...
_COMBINED_REVIEW_PROMPT = (
    "Review the conversation above and consider saving learnings. "
    "Three stores are available — pick exactly one per fact, in "
    "priority order (#30220):\n\n"
    "  1. **SKILL** (highest priority for procedural knowledge): how "
    "to do this class of task. If the lesson is 'next time I face this "
    "kind of work, do it this way', it belongs in a skill, NOT in "
    "memory. The skill ladder below governs which skill and which "
    "update form.\n"
    "  2. **USER.md** (memory tool, target='user'): who the user is. "
    "Communication style, tone/format/verbosity preferences, work "
    "style, expectations about how you should behave, personal "
    "details the user revealed. Use this for cross-cutting preferences "
    "that don't belong to any single skill.\n"
    "  3. **MEMORY.md** (memory tool, target='memory'): ENVIRONMENT "
    "facts. Tool quirks, project conventions, recurring config "
    "gotchas, paths/endpoints that matter, things learned about the "
    "system you operate in.\n\n"
    "One fact → one store, never duplicated (#30220). If a user "
    "complaint about how you handled a task can be embedded in a "
    "skill, put it in the skill and NOT in USER.md — the skill is the "
    "operational record; USER.md is for cross-cutting profile data. "
    "If it can't be embedded in any skill (e.g. 'I prefer Vietnamese "
    "in casual replies'), then USER.md is the right place. Never both.\n\n"
    "**Skills detail** — how to update if signal fires:\n\n"
    ...
    "Memory says 'who the user is and what the current situation and "
    "state of operations are'; skills say 'how to do this class of "
    "task for this user'. A user-preference lesson belongs in EXACTLY "
    "ONE of {skill, USER.md} — pick by the priority order above and do "
    "not write it to both (#30220).\n\n"
    ...
    "troubleshooting skill — never 'this tool does not work' as a "
    "standalone constraint.\n\n"
    "Act on whichever store has real signal. If nothing stands out "
    "in any of {skill, USER.md, MEMORY.md}, say 'Nothing to save.' "
    "and stop. 'Nothing to save.' is a valid and frequent outcome "
    "(#30220) — many sessions don't produce anything durable, and "
    "saving low-signal entries pollutes both the skill library and "
    "the system-prompt memory snapshot."
)
