...
def _safe_response_output_text(response: Any) -> str:
    """Best-effort access to response.output_text.

    Some OpenAI SDK builds synthesize ``output_text`` by iterating
    ``response.output``.  When a backend returns ``output=None`` that property
    can raise ``TypeError`` instead of returning an empty string.
    """
    try:
        out_text = getattr(response, "output_text", None)
    except Exception:
        logger.debug("Codex response.output_text accessor failed", exc_info=True)
        return ""
