...
def _chat_messages_to_responses_input(
    ...
    *,
    ...
    state. xAI's OAuth/SuperGrok ``/v1/responses`` surface rejects encrypted
    reasoning blobs minted by prior turns, so ``is_xai_responses=True`` drops
    opaque replay while preserving visible assistant/tool messages.
    """
    ...
    for msg in messages:
        ...
        if role in {"user", "assistant"}:
            ...
            if role == "assistant":
                ...
                # previous turns so native Codex can maintain coherent state.
                # xAI rejects encrypted replay blobs minted by prior turns, so
                # skip opaque replay for that provider surface.
