...
def compress_context(
    ...
    *,
    ...
    if todo_snapshot:
        # The todo snapshot is generated by Hermes, not typed by the user.  Keep
        # it out of the user role so the next model does not mistake the capsule
        # itself for the latest user request after compaction.  If the
        # compressor already ended on an assistant summary, fold the capsule into
        # that message instead of appending a second assistant turn and breaking
        # provider role-alternation invariants.
        if compressed and compressed[-1].get("role") == "assistant" and not compressed[-1].get("tool_calls"):
            existing_content = compressed[-1].get("content", "")
            if isinstance(existing_content, str):
                compressed[-1]["content"] = (
                    f"{existing_content}\n\n{todo_snapshot}" if existing_content else todo_snapshot
                )
            else:
