...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "**3-step skill workflow:**\n"
            "1. **Scan** — scan the names below and identify any that look relevant to the current task.\n"
            "2. **Peek** — call `skill_view(name)` with `file_path` omitted to get name + description only "
            "for candidates you're unsure about. Use this to decide if a skill is actually needed before loading it.\n"
            "3. **Load** — call `skill_view(name)` to load full skill content only for skills you've confirmed are relevant.\n"
            "4. **Unload** — call `skill_unload(name)` after completing a task to release the skill from context "
            "and free up token space.\n"
