...
KANBAN_GUIDANCE = (
    ...
    "3. **Heartbeat for visibility.** Call `kanban_heartbeat(note=...)` after "
    "orientation and before/after each major phase when a task has multiple "
    "steps. For Vitaliy's team, write all human-facing heartbeat notes, summaries, "
    "handoffs, comments, reports, and documents in Russian by default unless the task "
    "explicitly requests another language. English is acceptable only for commands, "
    "paths, model/tool names, API terms, and raw error text. Examples: `требования прочитаны`, "
    "`файлы созданы`, `тесты запущены`. During long subprocesses (training, encoding, "
    "crawling, installs, large test runs), call it every 2–5 minutes with a short "
    "human-readable progress note. **If your task may run longer than 1 hour, you "
    "MUST call `kanban_heartbeat` at least once an hour** — the dispatcher reclaims "
    "tasks running past `kanban.dispatch_stale_timeout_seconds` (default 4 hours) "
    "when no heartbeat has arrived in the last hour. A reclaim re-queues the task "
    "as `ready` without penalty, but you lose your current run's progress. Skip "
    "only truly tiny tasks that finish in one or two tool calls.\n"
    "4. **Block on genuine ambiguity.** If you need a human decision you cannot "
    "infer (missing credentials, UX choice, paywalled source, peer output you "
    "need first), call `kanban_block(reason=\"...\")` and stop. Don't guess. "
    "The user will unblock with context and the dispatcher will respawn you.\n"
    "5. **Complete with structured handoff.** Call `kanban_complete(summary=..., "
    "metadata=...)`. `summary` is 1–3 human-readable Russian sentences naming concrete "
