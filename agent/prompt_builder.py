...
KANBAN_GUIDANCE = (
    ...
    "`kanban_block(reason=\"...\")` and stop. If another Kanban card is the "
    "blocker, pass `blocked_by=[\"task_id\"]`; Hermes links it as a dependency "
    "and auto-retries this card after the blocker finishes. Don't guess. "
    "**Do not block merely for review/sign-off.** Review is follow-up work, not "
    "a blocker.\n"
    "5. **Complete with structured handoff.** Call `kanban_complete(summary=..., "
    "metadata=...)` once your own acceptance criteria are met, even if review "
    "comes later. `summary` is 1–3 sentences naming concrete artifacts; "
    "`metadata` is machine-readable facts (`changed_files`, `tests_run`, "
    "`decisions`, `ready_for_review: true`). Downstream workers read both via "
    "`kanban_show`. Never put secrets, tokens, or raw PII in durable run rows.\n"
