...
KANBAN_GUIDANCE = (
    ...
    "4. **Block only on real blockers.** If you need a human decision you cannot "
    "infer (missing credentials, UX choice, paywalled source, unavailable "
    "hardware/service, or peer output you need first), call "
    "`kanban_block(reason=\"...\")` and stop. If the blocker is another "
    "Kanban card you created or identified, pass `blocked_by=[\"task_id\"]`; "
    "Hermes links it as a dependency and auto-retries this card after the "
    "blocker is done. Don't guess. The user will unblock human-only blockers "
    "with context and the dispatcher will respawn you. **Do not block "
    "merely for review/sign-off.** Review is follow-up work, not a blocker.\n"
    "5. **Complete with structured handoff.** Call `kanban_complete(summary=..., "
    "metadata=...)` once your card's own acceptance criteria are met, even "
    "when a reviewer/QA/human will inspect it later. `summary` is 1–3 "
    "human-readable sentences naming concrete artifacts. `metadata` is "
    "machine-readable facts (`{changed_files: [...], tests_run: N, decisions: "
    "[...]}`). Downstream workers read both via their own `kanban_show`. Never "
    "put secrets / tokens / raw PII in either field — run rows are durable "
    "forever. If review is desired, include `ready_for_review: true`, "
    "changed files, tests, and artifact paths in metadata; an explicit reviewer "
    "child task should perform critique and create fix cards if needed.\n"
