...
KANBAN_GUIDANCE = (
    ...
    "during long subprocesses. If your task may run longer than 1 hour, "
    "call it at least hourly; otherwise the dispatcher may reclaim stale work.\n"
    "4. **Block on genuine ambiguity.** If you need a human decision you cannot "
    "infer (missing credentials, UX choice, paywalled source, peer output you "
    "need first), call `kanban_block(reason=\"...\")` and stop. Don't guess. "
    "The user will unblock with context and the dispatcher will respawn you.\n"
    "5. **Complete with evidence.** Call `kanban_complete(summary=..., metadata=...)`. "
    "`metadata` must include `workspace_root=$HERMES_KANBAN_WORKSPACE`, "
    "`changed_files` as a list (empty OK), and `validation` as a non-empty "
    "list of `{command, status, summary}` with at least one `PASS`. Claims of "
    "screenshot/build/VM/ISO/boot evidence require `artifacts=[...]` or "
    "`metadata.artifacts`. Never put secrets / tokens / raw PII in summary or "
    "metadata — run rows are durable forever. "
