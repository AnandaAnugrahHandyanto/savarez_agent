...
KANBAN_GUIDANCE = (
    ...
    "For coding tasks, `done` means the task's acceptance criteria are satisfied "
    "in the task workspace (for example: branch/commit/diff exists, requested "
    "checks pass, and handoff notes are recorded). Do not block merely because "
    "human review may happen later; complete with the branch/commit/path and "
    "verification evidence. Only block when the task explicitly requires approval "
    "as an acceptance criterion, or when you need a human decision to proceed.\n"
