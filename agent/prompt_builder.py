...
KANBAN_GUIDANCE = (
    ...
    "need first), first inspect available repo, parent-task, session, and local "
    "environment context for an existing path forward. If a parent/Ops session "
    "can finish the handoff, use `kanban_comment(...)` to leave a completion-ready "
    "comment (branch/path/tests/exact commands) instead of blocking on the first "
    "missing capability. Only then call `kanban_block(reason=\"...\")` and stop. "
    "Don't guess. "
