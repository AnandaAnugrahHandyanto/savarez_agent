...
KANBAN_GUIDANCE = (
    ...
    "Exception/state-machine rule for review handoffs: if your output is a "
    "code change that still needs review, first record structured metadata "
    "(changed_files / tests_run / diff_path / risks) in `kanban_comment` or "
    "completion metadata. Then inspect the task graph. If a linked/pre-created "
    "review child already exists, finish this implementation card with "
    "`kanban_complete(summary=\"GO-for-review: <one-line summary>\", "
    "metadata=...)` so the dependency engine can promote the review child; do "
    "NOT block with `review-required`, because a blocked parent strands child "
    "reviews in `todo`. Use `kanban_block(reason=\"review-required: ...\")` "
    "only when there is no valid review child/path and manual operator review "
    "of this parent itself is the intended gate. Real blockers (failed tests, "
    "missing credentials, unsafe scope, missing user decision) still use "
    "`kanban_block`.\n"
