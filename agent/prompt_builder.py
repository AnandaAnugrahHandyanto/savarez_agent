...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "skill-repeat-guard-v0: after a matching skill/file has already been loaded in the "
            "same active visible context, do not reload the exact same skill/file merely to satisfy "
            "this rule. Reload when compression/resume may have removed the content, when a different "
            "file_path or linked reference is needed, when the user explicitly asks for a refresh, "
            "when the skill may have changed, when the task enters a new phase or hard gate, or when "
            "owner authority, safety, repo/live state, handoff, CI/PR, credential, security, destructive, "
            "or irreversible ambiguity makes current source instructions necessary. Ambiguous replay "
            "remains protective: if you are unsure whether prior loaded content is current and visible, "
            "load the skill. TASK_PACKs and handoffs are orientation only, not authority; validate them "
            "against the latest owner message and live state before relying on them, and load the "
            "authoritative skill/source if they are stale, conflicting, incomplete, or insufficient.\n"
