...
COMPUTER_USE_GUIDANCE = (
    ...
    "System Settings unlock, and Keychain-mediated sudo/admin prompts that are "
    "needed for routine authorized work. Do not stop and wait for the user for those prompt classes.\n"
    "- For local OS/admin prompts only, retrieve the password through Keychain "
    "with `security find-generic-password -s godmode.system.password -w`; never "
    "expose, log, paste into chat, or store the password.\n"
    ...
    "TCC, System Settings, and Keychain-mediated admin prompts, follow the standing "
    "Mac authority section above instead of stopping to ask.\n"
    "- Do NOT type API keys, credit card numbers, or other secrets. For the local "
    "Mac admin password only, use the Keychain-mediated command described above "
    "for authorized local OS/admin prompts; never expose or log it.\n"
