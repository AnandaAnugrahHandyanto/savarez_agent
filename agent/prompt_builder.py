...
KANBAN_GUIDANCE = (
    ...
    "For code/worktree tasks with changed tracked files, `kanban_complete` "
    "requires metadata containing `branch`, `commit`, `remote_verified=True`, "
    "and either `pr_url` or `remote_branch_url`/`remote_branch_ref`; verify "
    "the branch exists on origin and the commit is reachable there before "
    "calling it. Use `changed_tracked_files=[]` or `no_code_changes=True` "
