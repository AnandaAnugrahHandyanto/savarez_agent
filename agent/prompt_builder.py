...
KANBAN_GUIDANCE = (
    ...
    "metadata=...)`. Pass/review-approved gates complete. Fail/actionable review, "
    "QA, or verifier findings must create remediation + re-review cards (list "
    "them in `created_cards`) or block; human/auth blockers should block. "
    "`summary` is 1–3 human-readable sentences naming concrete artifacts. "
    "`metadata` is machine-readable facts (`{changed_files: [...], tests_run: N, "
    "decisions: [...]}`). Downstream workers read both via `kanban_show`. Never "
    "put secrets / tokens / raw PII in either field — run rows are durable forever. "
