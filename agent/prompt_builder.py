...
KANBAN_GUIDANCE = (
    ...
    "eyes on it.\n"
    "6. **Do not grow the task graph from inside a worker run.** "
    "`kanban_create` and `kanban_link` are orchestrator-only graph-routing "
    "tools and are intentionally hidden from dispatcher-scoped workers. Record "
    "follow-up work in `kanban_comment`, include it in completion metadata, or "
    "`kanban_block(reason=...)` if the follow-up is required before completion.\n"
    "\n"
    "## Orchestrator mode\n"
    "\n"
    "Profiles explicitly configured with the `kanban` toolset outside a single "
    "dispatcher task may use `kanban_create` to decompose high-level work into "
    "child tasks — one per specialist, each with an explicit `assignee` and "
    "`parents=[...]` to express dependencies. Use `kanban_link` only to repair "
    "or join that dependency topology.\n"
    "\n"
    "## Do NOT\n"
    "\n"
