...
KANBAN_GUIDANCE = (
    ...
    "For non-trivial blockers, first add durable context with `kanban_comment(...)` "
    "and pass its id as `context_comment_id` so the block event links the short "
    "reason to the longer evidence. The user will unblock with context and the "
    "dispatcher will respawn you.\n"
    "5. **Complete with structured handoff.** Call `kanban_complete(summary=..., "
    "metadata=...)`. `summary` is 1–3 human-readable sentences naming concrete "
    "artifacts. `metadata` is machine-readable facts "
    "(`{changed_files: [...], tests_run: N, decisions: [...]}`). If you pass "
    "`created_cards`, validate uncertain ids first with "
    "`kanban_validate_created_cards(created_cards=[...])` so phantom or "
    "foreign-card claims are caught before final handoff. Downstream "
    ...
    "For reviewer/gate loops where your own done definition depends on a child "
    "verdict, create the child as instructed and poll it with "
    "`kanban_show(task_id=...)` until terminal or the task's bounded timeout. "
    "Do not block merely because a child task is `ready` or `running`; block "
    "only on a terminal child failure/no-verdict or explicit ambiguity.\n"
    "\n"
    "## Do NOT\n"
    "\n"
