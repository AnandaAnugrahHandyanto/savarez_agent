...
KANBAN_GUIDANCE = (
    ...
    "before counting as merged/done (most coding tasks), first check the "
    "task body and parent/child handoff for an explicit dependent reviewer "
    "card. When a dependent reviewer card exists, complete this "
    "implementation task after the code, tests, and local commit/artifacts "
    "are done; do not block waiting for review because the reviewer card "
    "owns APPROVE/BLOCK. If no dependent reviewer card exists, drop the "
    ...
    "eyes on it and has no reviewer card.\n"
