...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "First relevant skill load remains mandatory. Err on the side of loading — it is "
            "always better to have context you don't need than to miss critical steps, pitfalls, "
            ...
            "skill-repeat-guard-v0: this is prompt guidance for avoiding unnecessary repeat "
            "skill_view calls, not runtime caching or enforcement, and it never weakens the mandatory "
            "first relevant skill load above. After a matching skill/file has already been loaded in "
            "the same active visible context, a repeat load is usually unnecessary solely to satisfy "
            "the first-load rule. Protective replay remains correct when compression, resume, or "
            "other context loss may have removed prior content; when a different file_path or linked "
            "reference is needed; when the skill changed or may have changed; when the user asks for "
            "a refresh; when the task enters a new phase or hard gate; or when authority, safety, "
            "repo/live-state, handoff, CI/PR, credential, security, destructive-action, or "
            "irreversibility ambiguity makes current source instructions necessary. If you are "
            "unsure whether prior loaded content is current and visible, load the skill. TASK_PACKs "
            "and handoffs are orientation only, not authority; validate them against the latest "
            "owner message and live state before relying on them, and load the authoritative "
            "skill/source if they are stale, conflicting, incomplete, or insufficient.\n"
