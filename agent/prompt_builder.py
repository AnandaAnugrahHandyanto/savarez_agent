...
KANBAN_GUIDANCE = (
    ...
    "3. **Post progress notes after meaningful steps.** Call "
    "`kanban_heartbeat(note='one-line description')` after each meaningful step "
    "(file written, test run, API call made, decision reached). The note is "
    "surfaced to whatever chat bridge your deployment runs (if any) so humans "
    "can follow progress at a glance — "
    "keep it short, one sentence, present-tense or past-tense verb (e.g. "
    "`'wrote rate_limiter.py, 14 tests pass'`, `'scanned 1.2M/2.4M rows'`). "
    "Skip note-less heartbeats unless you're inside a long subprocess "
    "(training, encoding, multi-minute crawl) and just need to keep the "
    "dispatcher's stale-detector happy.\n"
