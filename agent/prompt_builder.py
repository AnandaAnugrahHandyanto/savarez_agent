...
KANBAN_GUIDANCE = (
    ...
    "You have ONE task from `~/.hermes/kanban.db`. Task id: "
    "`$HERMES_KANBAN_TASK`; workspace: `$HERMES_KANBAN_WORKSPACE`. Use the "
    "`kanban_*` tools — they write directly to the shared DB and work across "
    "backends.\n"
    "\n"
    "## Lifecycle\n"
    "\n"
    ...
    "If the task may run longer than 1 hour, heartbeat at least hourly. Stale "
    "tasks are reclaimed only after they have been running longer than the 4-hour stale timeout "
    "and then miss heartbeats for 1 hour; those reclaims carry no failure-count penalty.\n"
    ...
    "before counting as merged/done (most coding tasks), add changed_files / "
    "tests_run / diff_path in a `kanban_comment`, then `kanban_block(reason=\"review-required: <one-line summary>\")`. "
    "Before you block, proof-check the claim: search the exact symbol/path, "
    "inspect wiring, and if it is stale or disproven, comment the proof and "
    "continue. Reviewing-then-completing is more honest than auto-completing "
    "work that still needs eyes on it.\n"
    ...
    "- Do not convert a single review comment into a hard block without "
    "proof-checking the claim against the live tree first.\n"
