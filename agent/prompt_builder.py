...
_HERMES_MD_NAMES = (".savarez.md", "HERMES.md")


def _find_hermes_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.savarez.md`` or ``HERMES.md``.
...
HERMES_AGENT_HELP_GUIDANCE = (
    ...
    "information. Load the `savarez-agent` skill with skill_view(name='savarez-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.savarez/kanban.db`. Your task id is in "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `savarez kanban <verb>` for board operations. Use "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Savarez "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Savarez WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Savarez itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Savarez runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Savarez itself is running. The host OS, home, and cwd "
                f"of the Savarez process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Savarez "
    ...
    # Savarez desktop GUI — any agent running under the desktop app should know
    ...
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Savarez desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
    ...
    # Embedder-supplied environment description. Lets a host that wraps Savarez
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.savarez/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Savarez AI Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `savarez-agent` skill "
            "first. It has the actual commands (e.g. `savarez config set …`, `savarez tools`, "
            "`savarez setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: savarez setup, savarez setup tools, savarez setup terminal, savarez status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from SAVAREZ_HOME and return its content, or None.
    ...
    try:
        ...
        logger.debug("Could not ensure SAVAREZ_HOME before loading SOUL.md: %s", e)
...
def _load_hermes_md(cwd_path: Path) -> str:
    """.savarez.md / HERMES.md — walk to git root."""
    ...
    try:
        ...
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".savarez.md")
    except Exception as e:
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .savarez.md / HERMES.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from SAVAREZ_HOME is independent and always included when present.
    ...
    # SOUL.md from SAVAREZ_HOME only — skip when already loaded as identity
    if not skip_soul:
