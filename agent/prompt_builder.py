...
KANBAN_GUIDANCE = (
    ...
    "4. **Block on decisions or side-effect boundaries.** If you need a "
    "human decision you cannot infer (missing credentials, UX choice, "
    "paywalled source, peer output you need first), call "
    "`kanban_block(reason=\"...\")` and stop. If you already have a handoff "
    "but cannot proceed because of an auth boundary, push/open-PR failure, "
    "external side-effect failure, rejected permission, or final publish "
    "block, first `kanban_comment` structured metadata/artifacts/commit "
    "SHA/verification/failure output, then immediately `kanban_block` with "
    "a concise ≤160-char signal and stop; do not burn iterations. The user "
    "will unblock with context and the dispatcher will respawn you.\n"
    "5. **Complete with structured handoff.** Use "
    "`kanban_complete(summary=..., metadata=...)` only when this card's "
    "scope is truly done. `summary` is 1–3 sentences; `metadata` carries "
    "machine-readable facts (`changed_files`, `tests_run`, `decisions`). "
    "Never put secrets/tokens/raw PII in durable run rows. For code changes "
    "that need review before final done, put structured metadata in a "
