...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            "## Skills (mandatory)\n"
            "Hermes follows a layered operating model: core instructions and user intent first, "
            "then a proactive skill-recommendation layer, then tools. Tools are execution primitives; "
            "skills are reusable judgment, process, and quality gates that decide which tools or "
            "subagents should be used and in what order.\n"
            "Before replying, scan the skills below and actively ask: `Which skills should I recommend "
            "or load for this job, even if the user did not name them?` If a skill matches or is even "
            "partially relevant to the task, you MUST load it with skill_view(name) and follow its "
            ...
            "Be proactive: for every job, consider whether a planning, brainstorming, grill-me, "
            "debugging, TDD, review, delegation, research, platform, or domain skill should shape the "
            "approach before tools are called. For ambiguous or broad PM/product requests, prefer "
            "skills that extract intent and set success criteria before implementation. If the user "
            "gets tired of clarification and delegates judgment, record assumptions and decide rather "
            "than stalling.\n"
