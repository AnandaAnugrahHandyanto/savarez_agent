...
def init_agent(
    ...
    # Hard assertion: NVIDIA (and other OpenAI-compatible /v1 providers)
    # MUST use chat_completions, NEVER codex_responses. These providers
    # do not implement the OpenAI Responses API endpoint (/v1/responses).
    # If stale session overrides or config.yaml defaults carry codex_responses
    # into a NVIDIA session, force-correct here with a warning.
    _OPENAI_COMPAT_CHAT_PROVIDERS = {"nvidia", "openrouter", "deepseek", "groq"}
    if agent.api_mode == "codex_responses" and (
        agent.provider in _OPENAI_COMPAT_CHAT_PROVIDERS
        or (
            str(agent.base_url or "").lower().rstrip("/").endswith("/v1")
            and agent.provider not in ("openai", "openai-codex")
        )
    ):
        logger.warning(
            "Normalization guard: forcing api_mode from codex_responses to "
            "chat_completions for provider=%s base_url=%s (these providers "
