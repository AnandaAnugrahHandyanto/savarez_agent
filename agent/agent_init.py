...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.savarez/SOUL.md as the primary
    ...
    # both live under ~/.savarez/logs/.  Idempotent, so gateway mode
                ...
                if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                    ...
                    if not _fb_resolved:
                        raise RuntimeError(
                            ...
                            f"variable, or switch to a different provider with `savarez model`."
                        )
                if not getattr(agent, "_fallback_activated", False):
                    # No provider configured — reject with a clear message.
                    raise RuntimeError(
                        "No LLM provider configured. Run `savarez model` to "
                        "select a provider, or run `savarez setup` for first-time "
                        "configuration."
                    )
    ...
    # Session logs go into ~/.savarez/sessions/ alongside gateway sessions
    hermes_home = get_hermes_home()
    agent.logs_dir = hermes_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.savarez/sessions/session_{sid}.json)
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "savarez"
                    except Exception:
                        pass
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Savarez AI Agent.  Choose a model with at least "
