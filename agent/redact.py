...
# cli.py) or `HERMES_REDACT_SECRETS=false` in ~/.savarez/.env. An opt-out
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across Savarez — used by
    ``savarez config``, ``savarez status``, ``savarez dump``, and anywhere
...
def redact_sensitive_text(text: str, *, force: bool = False, code_file: bool = False) -> str:
    ...
    for URLs, ``"eyJ" in text`` for JWTs). On a typical savarez log line
