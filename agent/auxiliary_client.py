...
def _try_openrouter(explicit_api_key: str = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    or_key = (explicit_api_key or "").strip() or os.getenv("OPENROUTER_API_KEY")
    if not or_key:
        return None, None
...
def resolve_provider_client(
    ...
    if provider == "custom":
        if explicit_base_url:
            # Only rewrite /anthropic → /v1 when the caller is NOT using
            # api_mode=anthropic_messages, which sends Anthropic-native request
            # bodies and expects an /anthropic-compatible endpoint.
