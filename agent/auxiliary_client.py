...
def _resolve_task_provider_model(
    ...
    cfg_api_key = None
    cfg_api_mode = None
    compression_cfg_provider = None
    compression_cfg_model = None
    compression_cfg_base_url = None
    ...
    if task:
        ...
        if task == "compression":
            ...
            if isinstance(comp, dict):
                compression_cfg_provider = str(comp.get("summary_provider", "")).strip() or None
                compression_cfg_model = str(comp.get("summary_model", "")).strip() or None
                compression_cfg_base_url = str(comp.get("summary_base_url", "")).strip() or None
                if cfg_provider == "main" and compression_cfg_provider and compression_cfg_provider != "main":
                    cfg_provider = compression_cfg_provider

    resolved_model = model or cfg_model or compression_cfg_model
    resolved_api_mode = cfg_api_mode

    def _resolve_provider_tuple(raw_provider: Optional[str]) -> Tuple[Optional[str], Optional[str], Optional[str]]:
        candidate = str(raw_provider or "").strip()
        if not candidate:
            return None, None, None

        normalized = candidate.lower()
        if normalized == "main":
            main_provider = _read_main_provider() or "custom"
            main_provider = str(main_provider).strip()
            custom_base, custom_key = _resolve_custom_runtime()
            if custom_base:
                return main_provider, custom_base, custom_key
            return main_provider, None, None
        ...
        if normalized.startswith("custom:"):
            ...
            if suffix:
                try:
                    from hermes_cli.runtime_provider import _get_named_custom_provider
                    custom_entry = _get_named_custom_provider(suffix)
                except Exception:
                    custom_entry = None
                if custom_entry:
                    custom_base = str(custom_entry.get("base_url", "")).strip() or None
                    custom_key = str(custom_entry.get("api_key", "")).strip() or None
                    return candidate, custom_base, custom_key
            custom_base, custom_key = _resolve_custom_runtime()
            return "custom", custom_base, custom_key

        if _is_named_custom_provider(candidate):
            try:
                from hermes_cli.runtime_provider import _get_named_custom_provider
    ...
    if provider:
        resolved_provider, provider_base_url, provider_api_key = _resolve_provider_tuple(provider)
        if provider_base_url:
            return resolved_provider or "custom", resolved_model, provider_base_url, provider_api_key or api_key, resolved_api_mode
        return resolved_provider or provider, resolved_model, base_url, api_key, resolved_api_mode

    if task:
        if cfg_base_url:
            return "custom", resolved_model, cfg_base_url, cfg_api_key, resolved_api_mode
        if cfg_provider and cfg_provider != "auto":
            resolved_provider, provider_base_url, provider_api_key = _resolve_provider_tuple(cfg_provider)
            if provider_base_url:
                return resolved_provider or "custom", resolved_model, provider_base_url, provider_api_key or cfg_api_key, resolved_api_mode
            return resolved_provider or cfg_provider, resolved_model, None, None, resolved_api_mode
        if compression_cfg_base_url:
            return "custom", resolved_model, compression_cfg_base_url, cfg_api_key, resolved_api_mode
        if compression_cfg_provider and compression_cfg_provider != "auto":
            resolved_provider, provider_base_url, provider_api_key = _resolve_provider_tuple(compression_cfg_provider)
            if provider_base_url:
                return resolved_provider or "custom", resolved_model, provider_base_url, provider_api_key or cfg_api_key, resolved_api_mode
            return resolved_provider or compression_cfg_provider, resolved_model, None, None, resolved_api_mode
