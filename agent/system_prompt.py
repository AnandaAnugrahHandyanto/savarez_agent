...
upstream prefix cache warm.  See ``savarez-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Savarez never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want SAVAREZ_HOME persona while keeping
    ...
    # Pointer to the savarez-agent skill + docs for user questions about Savarez itself.
    ...
    # Coding posture (base Savarez, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Savarez profile the agent is running
    # under so it doesn't conflate ~/.savarez/skills/ (default profile) with
    # ~/.savarez/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Savarez profile: default. Other profiles (if any) live "
            "under ~/.savarez/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Savarez profile: {active_profile}. This session reads "
            f"and writes ~/.savarez/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.savarez/skills/, ~/.savarez/plugins/, "
            f"~/.savarez/cron/, ~/.savarez/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Savarez never rebuilds or reinjects parts of it
