...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Savarez starts and finds ``~/.openclaw/``.

    Points users at ``savarez claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``savarez claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Savarez, run "
        "`savarez claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `savarez claw cleanup` (renames it to ~/.openclaw.pre-migration — "
