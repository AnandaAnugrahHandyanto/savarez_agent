...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Savarez with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.savarez/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Savarez
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``savarez tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``savarez setup`` for
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``savarez tools`` picker.
