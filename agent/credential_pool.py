...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``savarez model`` / ``savarez auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Savarez process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``savarez`` invocation that defaults to the active provider
        (e.g. setup wizard, ``savarez auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        try:
            if self.provider == "anthropic":
                ...
                # refresh_token — single-use tokens consumed by another Savarez
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For openai-codex: same race as xAI/nous — another Savarez process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Savarez profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `savarez model` / `savarez auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `savarez model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``savarez auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `savarez auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `savarez auth remove <provider> <N>` is stable across all source types.
    try:
    ...
    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Savarez PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `savarez setup` ("Claude
        ...
        # that masquerade.  Prefer ~/.savarez/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `savarez setup` writes.
        _env_file = load_env()
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `savarez auth add nous --label <name>`).  Fall back to the
        ...
        # Savarez auth store or env vars, so resolve them here.
        ...
        # MiniMax OAuth tokens live in ~/.savarez/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `savarez auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `savarez auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Savarez auth store.  Without this gate the removal is instantly
        ...
        # Savarez owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``savarez model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``savarez auth list`` reflects the logged-in state and so the pool
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.savarez/.env over os.environ — the user's config file is the
    # authoritative source for Savarez credentials. Stale env vars from parent
    ...
    # Honour user suppression — `savarez auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.savarez/.env.
    ...
    if provider == "openrouter":
        # Prefer ~/.savarez/.env over os.environ
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            # Savarez PKCE is Savarez-owned/persistable while present, but it is
