...
class MemoryProvider(ABC):
    ...
    def on_post_compress(self, summary: str) -> None:
        """Called after context compression generates a structured summary.

        Use to extract key sections from the compression summary and persist
        them to durable memory.  The summary follows a structured template
        with sections like ``## Active Task``, ``## Critical Context``,
        ``## Key Decisions``, ``## Constraints & Preferences``,
        ``## Pending User Asks``.

        summary: the full structured summary text generated by the compressor.
        Default is no-op.  Implementations should parse the summary sections
        and persist actionable facts (API endpoints, account states, etc.)
