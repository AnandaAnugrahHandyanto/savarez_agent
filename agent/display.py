...
# Module-level aliases — each access calls _diff_ansi() so they
# pick up the skin colors lazily.  This is fine for the handful of
# call-sites in _render_inline_diff / _render_inline_diff_summary.
def _ANSI_DIM():   return _diff_ansi()["dim"]
def _ANSI_FILE():  return _diff_ansi()["file"]
def _ANSI_HUNK():  return _diff_ansi()["hunk"]
def _ANSI_MINUS(): return _diff_ansi()["minus"]
def _ANSI_PLUS():  return _diff_ansi()["plus"]
