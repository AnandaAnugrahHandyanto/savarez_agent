...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    # Inject current time into the summary prompt so the agent knows the real
    # wall-clock time at max-iterations (session may have run for hours).
    try:
        from hermes_time import format_current_time_context
        _time_block = format_current_time_context()
        if _time_block:
            summary_request = f"{_time_block}\n\n{summary_request}"
    except Exception:
        pass
