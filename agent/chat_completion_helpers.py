...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    try:
        ...
        # Determine api_mode from explicit fallback config first, then provider / base URL / model.
        # ``fallback_providers`` preserves extra keys like ``api_mode``; honoring it here lets
        # third-party Anthropic-compatible endpoints use the native Messages transport even when
        # their URL does not end in /anthropic.
        fb_api_mode = fb_api_mode_hint if fb_api_mode_hint in {
            "chat_completions",
            "codex_responses",
            "anthropic_messages",
            "bedrock_converse",
