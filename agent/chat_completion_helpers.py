...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    try:
        ...
        # Normalization guard for fallback: if the fallback provider is an
        # OpenAI-compatible endpoint that doesn't support the Responses API
        # (e.g. NVIDIA, OpenRouter, DeepSeek, Groq), force chat_completions.
        # This prevents fallback api_mode from incorrectly becoming
        # codex_responses and causing HTTP 404 on /v1/responses.
        _OPENAI_COMPAT_CHAT_PROVIDERS = {"nvidia", "openrouter", "deepseek", "groq"}
        if fb_api_mode == "codex_responses":
            _needs_correction = False
            if fb_provider in _OPENAI_COMPAT_CHAT_PROVIDERS:
                _needs_correction = True
            elif fb_base_url.rstrip("/").endswith("/v1") and fb_provider not in (
                "openai",
                "openai-codex",
            ):
                _needs_correction = True
            if _needs_correction:
                logger.warning(
                    "Fallback api_mode corrected: provider=%s base_url=%s "
                    "had api_mode=codex_responses but provider does not "
                    "support Responses API — rewriting to chat_completions",
                    fb_provider,
                    fb_base_url,
                )
                fb_api_mode = "chat_completions"
