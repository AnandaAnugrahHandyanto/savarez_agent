"""Projects codex app-server events into Savarez' messages list.

The translator that lets Savarez' memory/skill review keep working under the
...
Savarez' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Savarez
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Savarez' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Savarez stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Savarez already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Savarez' messages store text only.
        text_parts: list[str] = []
