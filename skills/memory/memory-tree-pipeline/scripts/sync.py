...
def push_to_obsidian():
    ...
    entities = conn.execute(
        "SELECT * FROM mem_tree_entity_index WHERE hotness > 1.0 ORDER BY hotness DESC LIMIT 50"
    ).fetchall()
...
def push_to_wiki():
    ...
    entities = conn.execute(
        "SELECT * FROM mem_tree_entity_index WHERE hotness > 2.0 ORDER BY hotness DESC LIMIT 30"
    ).fetchall()
