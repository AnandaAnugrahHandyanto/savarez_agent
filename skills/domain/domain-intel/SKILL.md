...
metadata:
  hermes:
    tags: [domain, osint, dns, ssl, whois, recon]
...
- `python3` available in the active runtime
- outbound HTTPS access for `crt.sh` and Google DNS-over-HTTPS queries
- outbound TCP port 43 if WHOIS lookups are required

Preflight:

```bash
python3 --version
python3 optional-skills/research/domain-intel/scripts/domain_intel.py whois example.com
```

If WHOIS port 43 is blocked, continue with `dns`, `ssl`, and `subdomains`, and explicitly tell the user that WHOIS data may be unavailable due to network policy.
...
Use these checks before claiming the skill is working:

```bash
# 1) WHOIS graceful handling
python3 optional-skills/research/domain-intel/scripts/domain_intel.py whois example.com

# 2) DNS output shape
python3 optional-skills/research/domain-intel/scripts/domain_intel.py dns example.com

# 3) Subdomain enumeration JSON
python3 optional-skills/research/domain-intel/scripts/domain_intel.py subdomains example.com
```

Success criteria:
- command exits successfully
- stdout is valid JSON
- payload contains the requested domain and command-specific keys (`records`, `subdomains`, `registrar`, etc.)
- on restricted networks, failures are explicit and structured instead of silent

## Pitfalls

- WHOIS is the most fragile path because port 43 is often blocked in cloud or enterprise networks.
- Availability is heuristic, not registrar-authoritative. Do not present it as a guaranteed registration result.
- Large domains can return many certificate entries; keep expectations realistic and cap analysis where needed.
- Use `duckduckgo-search` or web tools for company/content research — this skill is for infrastructure intelligence, not business profiling.
