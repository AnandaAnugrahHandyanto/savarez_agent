...
Run these checks before claiming Honcho is correctly integrated:

```bash
# Config + connectivity + resolved peer info
hermes honcho status

# Backfill missing host blocks for existing profiles
hermes honcho sync
```

Success criteria:
- status command exits cleanly
- resolved config shows the expected provider / base URL / peer mapping
- connection test passes
- the intended profile has its own `aiPeer`

If `memory.provider` is not set to `honcho`, the setup is incomplete even if `honcho.json` exists.
...
- Do not assume `honcho.json` alone activates memory. The Hermes config must also set `memory.provider: honcho`.
- Multi-profile setups can look correct locally while still sharing the wrong `aiPeer`; always verify with `hermes honcho status`.
- Observation settings edited server-side win on next session init. Local assumptions can become stale.
- `writeFrequency: session` delays persistence until exit; this often looks like "memory is broken" when it is just deferred writes.
