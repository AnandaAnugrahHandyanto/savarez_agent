...
4. Registers each tool in the Savarez tool registry
...
After discovery, MCP tools are automatically injected into all `savarez-*` platform toolsets (CLI, Discord, Telegram, etc.). This means MCP tools are available in every conversation without any additional configuration.
...
The most common transport. Savarez launches the MCP server as a subprocess and communicates over stdin/stdout.
...
For stdio servers, Savarez does NOT pass your full shell environment to MCP subprocesses. Only safe baseline variables are inherited:
...
Savarez supports MCP's `sampling/createMessage` capability — MCP servers can request LLM completions through the agent during tool execution. This enables agent-in-the-loop workflows (data analysis, content generation, decision-making).
