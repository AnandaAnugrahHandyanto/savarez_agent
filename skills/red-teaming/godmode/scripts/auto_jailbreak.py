...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("SAVAREZ_HOME", os.path.expanduser("~/.savarez")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("SAVAREZ_HOME", Path.home() / ".savarez")) / "skills" / "red-teaming" / "godmode"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Savarez models — already uncensored, just needs clean prompt
    "savarez": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "savarez" in model_lower or "nous" in model_lower:
        return "savarez"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Savarez config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.savarez/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Savarez for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Savarez for changes to take effect.")
