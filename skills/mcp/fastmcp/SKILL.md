...
Before claiming a FastMCP workflow is complete, verify the actual server contract from the CLI:

```bash
# 1) CLI and package availability
fastmcp version

# 2) Server object can be imported and inspected
fastmcp inspect acme_server.py:mcp

# 3) Tool/resource registry is visible
fastmcp list acme_server.py --json

# 4) At least one real tool invocation works
fastmcp call acme_server.py your_tool_name --json
```

If you are testing HTTP transport, also verify the networked path instead of stopping at local import success:

```bash
fastmcp run acme_server.py:mcp --transport http --host 127.0.0.1 --port 8000
fastmcp list http://127.0.0.1:8000/mcp --json
fastmcp call http://127.0.0.1:8000/mcp your_tool_name --json
```

Success criteria:
- `fastmcp inspect` exits successfully
- `fastmcp list` returns the expected tool/resource surface
- at least one `fastmcp call` returns valid JSON or the expected structured output
- for HTTP mode, the same server works over the exposed endpoint, not just in-process

## Pitfalls

- Passing import success is not enough — a server can import cleanly and still fail `fastmcp list` or `fastmcp call`.
- Do not hand off a server without at least one real CLI call per new tool.
- Hermes integration is a separate step from server creation. A valid FastMCP server is not automatically visible to Hermes until `native-mcp` config is updated.
- Avoid starting with a huge tool surface. Over-broad first versions are harder to inspect, test, and maintain.
- If a tool returns Python-only objects or non-serializable payloads, CLI calls may fail even though local Python code looked fine.
