...
Always check daemon access before taking action:

```bash
docker info >/dev/null && echo DOCKER_READY || echo DOCKER_UNAVAILABLE
```
...
Before claiming a Docker action succeeded, verify the resulting state explicitly:
...
Success criteria depend on the task:
- container lifecycle: target container appears in `docker ps -a` with the expected status
- compose: target service appears in `docker compose ps`
- cleanup: `docker system df` or `docker ps -a` reflects the expected reduction
- debugging: logs or inspect output confirms the root cause, not just that a command ran

## Pitfalls

- `docker compose down -v` destroys named-volume data. Treat it as destructive.
- `docker rm -f` and `docker image prune -a` are broad-impact operations; verify scope before using them.
- A successful `docker build` does not prove the containerized app works — run or inspect the resulting container.
- Permission errors often come from daemon access, not the container itself; check `docker info` first.
