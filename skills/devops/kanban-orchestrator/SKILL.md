...
2. **Reassign** (or `hermes kanban reassign <task_id> <new-profile> --reclaim`) — switch the task to a different profile (one that exists on this setup) and let the dispatcher pick it up with a fresh worker. The claim-release and assignee-update are atomic: the dispatcher never observes a state where the claim was released but the assignee was unchanged. From an external process this terminates the holding worker (standard orphaned-claim contract). When the holder is invoking the command on itself (self-reclaim — detected via `HERMES_KANBAN_TASK` env or PPID match) the kill is suppressed so the DB writes commit cleanly; the worker exits on its own.
