...
## 5. Savarez Configuration

### Config location

`$SAVAREZ_HOME/config.yaml` (defaults to `~/.savarez/config.yaml` when `SAVAREZ_HOME` is unset)
...
Restart the Savarez session for changes to take effect. The MCP connection is
