...
metadata:
  savarez:
    ...
    related_skills: [native-mcp, ascii-video, manim-video, savarez-video]
...
```
Savarez AI Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${SAVAREZ_HOME:-$HOME/.savarez}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Savarez config (if missing)
...
3. **Restart Savarez session** to pick up the new MCP server
