...
Registry and ABC for every inference provider Savarez knows about.
...
`$SAVAREZ_HOME/plugins/model-providers/<name>/` (per-user overrides). The
...
under `$SAVAREZ_HOME/plugins/model-providers/` for a private plugin).
