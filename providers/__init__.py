...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with savarez-agent)
2. User plugins: ``$SAVAREZ_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$SAVAREZ_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$SAVAREZ_HOME/plugins/model-providers/`` if it exists."""
    try:
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple SAVAREZ_HOME profiles don't alias each other.
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$SAVAREZ_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with savarez-agent.
    ...
    # 2. User plugins — under $SAVAREZ_HOME/plugins/model-providers/<name>/.
