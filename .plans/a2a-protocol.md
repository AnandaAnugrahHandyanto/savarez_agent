...
third edge: **Hermes as a callable agent for other agents.**
...
2. `hermes-a2a --host 0.0.0.0 --port 9100` (or `python -m a2a_adapter`)
...
- `tasks/get` / `tasks/cancel` — provided by the SDK's `DefaultRequestHandler` +
  `InMemoryTaskStore`; `cancel` wired into Hermes interruption.
...
mirroring the ACP adapter, the agent turn runs in a worker thread
(`asyncio.to_thread`) and its callbacks marshal A2A events back onto the loop
...
| `__main__.py` | `python -m a2a_adapter`                                                        |
...
- `[project.scripts]`: `hermes-a2a = "a2a_adapter.entry:main"`.
- `[tool.setuptools.packages.find]` `include`: add `"a2a_adapter"`.
...
- In-memory task store: tasks are lost on restart (acceptable for the slice).
