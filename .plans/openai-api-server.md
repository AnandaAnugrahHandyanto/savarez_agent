# OpenAI-Compatible API Server for Savarez AI Agent
...
SSE streaming. By exposing this endpoint, savarez-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  savarez-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.savarez/.env`
2. Run `savarez gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with savarez-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns savarez-agent as a model) |
...
Authorization: Bearer savarez-api-key-here
...
But savarez-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "savarez-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured savarez-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of savarez-agent's core prompt
...
Once implemented, savarez-agent works as a drop-in backend for:
