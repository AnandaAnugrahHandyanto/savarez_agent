...
class HermesCLI:
    ...
    def show_help(self):
        ...
        if skill_commands:
            _cprint(f"{t('cli.5534', skill_count=len(skill_commands))}")
    ...
    if query or image:
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if single_query_images or single_query_image_urls://n                    # Honour the same image-routing decision used by the
                    ...
                    _img_mode = "text"
                    _build_parts = None
                    try://n                        from agent.image_routing import (
                    ...
                    except Exception://n                        _img_mode = "text"

                    if _img_mode == "native" and _build_parts is not None://n                        try://n                            _parts, _skipped = _build_parts(
                            ...
                            if any(p.get("type") == "image_url" for p in _parts):
                            ...
                            else://n                                # All images unreadable — text fallback.
                                ...
                                if single_query_images://n                                    effective_query = cli._preprocess_images_with_vision(
                                        query, single_query_images, announce=False,
                                    )
                        except Exception://n                            if single_query_images://n                                effective_query = cli._preprocess_images_with_vision(
                                    query, single_query_images, announce=False,
                                )
                    elif single_query_images://n                        effective_query = cli._preprocess_images_with_vision(
                            query,
                            single_query_images,
                            announce=False,
                        )
