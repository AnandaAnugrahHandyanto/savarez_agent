...
class HermesCLI:
    ...
    def _prompt_text_input(self, prompt_text: str) -> str | None:
        """Prompt for free-text input; returns None on cancel or empty input."""
        # Non-TUI path: built-in input() works when no Application is running.
        if not self._app:
            try:
        ...
        # Issue #22958: When the TUI Application is active, input() can't read
        # stdin because prompt_toolkit's reader intercepts keystrokes. Use
        # pt_prompt inside run_in_terminal so it cooperates with the event loop.
        from prompt_toolkit.application import run_in_terminal
