...
def _init_session(
    ...
    # Auto-refresh MCP tools if discovery landed late (same as _build path).
    try:
        from hermes_cli.mcp_startup import spawn_late_mcp_refresh
        from model_tools import get_tool_definitions as _gtd

        def _on_reset_mcp_refreshed(added: int, total: int) -> None:
            try:
                _emit("session.info", sid, _session_info(agent, _sessions.get(sid, {})))
            except Exception:
                pass
        ...
        spawn_late_mcp_refresh(
            ...
            logger=logger,
            get_tool_definitions_fn=_gtd,
            on_refreshed=_on_reset_mcp_refreshed,
        )
    except Exception:
        pass
