...
# appends every unhandled exception to ~/.savarez/logs/tui_gateway_crash.log
...
class _SlashWorker:
    """Persistent SavarezCLI subprocess for slash commands."""
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `savarez` shows the prompt before constructing AIAgent; the TUI used
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    # Coding posture (base Savarez): with no explicit pin, collapse to the
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Savarez profile home
    # (~/.savarez/sessions/saved/) rather than the project/workspace CWD, and
...
#
# Layout:  $SAVAREZ_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Savarez Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Savarez/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Savarez Desktop. If it serves Savarez/Desktop UI or another unrelated app, stop that process and report failure.",
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Savarez provider is configured.",
                },
            )
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.savarez/.env`` into the gateway process via
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `savarez` is interactive — use `/savarez chat -q …` or run `savarez` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`savarez setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`savarez gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`savarez sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`savarez config edit` needs $EDITOR in a real terminal"
    return None
    ...
    try:
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `savarez model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.savarez/.env
