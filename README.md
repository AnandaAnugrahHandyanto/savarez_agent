...
- **Opus 4.7 `max` reasoning, wired end-to-end** — Hermes now accepts a literal `max` reasoning effort in config, slash commands, batch runs, and MoA entries. Anthropic routes preserve `max` natively, while non-Anthropic providers are clamped to their strongest supported tier so the setting remains usable across Codex, Copilot, Kimi, OpenRouter, Nous, AI Gateway, and custom MoA rosters.
