...
def _iso_week_folder(dt: datetime, base_dir: Optional[Path] = None) -> Path:
    """Return {base_dir}/{year}-W{week}/ for a given datetime."""
    year, week, _ = dt.isocalendar()
    root = base_dir or JOURNAL_DIR
...
def _summarize_turns(turns: List[dict]) -> Tuple[str, List[dict]]:
    ...
    summary = (
        f"{user_turns} user {_pluralize(user_turns, 'turn')}, "
        f"{asst_turns} assistant {_pluralize(asst_turns, 'turn')}, "
        f"{len(tool_calls)} tool {_pluralize(len(tool_calls), 'call')}"
    )
    if duration_mins > 0:
...
def _build_markdown(
    ...
    yaml_tags = "[" + ", ".join(tag.strip("#") for tag in sorted(tags)) + "]" if tags else "[]"

    lines = [
        "---",
        f"session_id: {json.dumps(session_id)}",
        f"date: {json.dumps(date_str)}",
        f"platform: {json.dumps(platform or '')}",
        f"tags: {yaml_tags}",
...
def write_session_journal(
    ...
    now = datetime.now(timezone.utc)
    folder = _iso_week_folder(now, output_dir)
