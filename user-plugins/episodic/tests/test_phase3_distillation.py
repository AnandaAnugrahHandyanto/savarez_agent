...
class TestSessionJournal:
    ...
    def test_summarize_turns(self):
        ...
        assert "1 user turn" in summary
        assert "1 assistant turn" in summary
    ...
    def test_write_session_journal_creates_markdown(self, tmp_path):
        ...
        assert 'session_id: "test-sess-123"' in content
        assert 'platform: "telegram"' in content
        assert "tags: [terminal]" in content
    ...
    def test_write_session_journal_uses_weekly_folder(self, tmp_path):
        ...
        assert result is not None
        # Should be inside the provided output_dir under a year-Wxx folder
    ...
    def test_write_session_journal_counts_tool_calls_from_assistant_metadata(self, tmp_path):
        from memory.journal import write_session_journal
        turns = [
        ...
        result = write_session_journal("tool-meta-test", turns=turns, output_dir=tmp_path)
        assert result is not None
        content = result.read_text(encoding="utf-8")
        assert "2 tool calls" in content
        assert "`read_file` × 1" in content
        assert "`terminal` × 1" in content
