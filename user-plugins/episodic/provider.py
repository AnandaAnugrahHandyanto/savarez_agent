...
from .session_writer import append_turn_jsonl

logger = logging.getLogger(__name__)

_MISSING_JOURNAL_GRACE_SECONDS = 300
_MISSING_JOURNAL_SCAN_LIMIT = 10
...
class EpisodicMemoryProvider(MemoryProvider):
    ...
    def _failure_dir(self) -> Path:
        path = get_hermes_home() / "tmp" / "episodic-failures"
    ...
    def _write_failure_artifact(
        ...
        *,
        ...
        safe_sid = (session_id or "unknown-session").replace("/", "-").replace(":", "-")[:80]
        timestamp = time.strftime("%Y%m%d-%H%M%S", time.gmtime())
        path = self._failure_dir() / f"{alert_key}_{safe_sid}_{timestamp}.md"
        lines = [
            "---",
        ...
        if details:
            lines.extend(["", "## details", "```json", json.dumps(details, ensure_ascii=False, indent=2, default=str), "```"])
        path.write_text("\n".join(lines) + "\n", encoding="utf-8")
        return path

    def _send_telegram_alert(self, text: str) -> bool:
        token = (os.environ.get("TELEGRAM_BOT_TOKEN") or "").strip()
        allowed = (os.environ.get("TELEGRAM_ALLOWED_USERS") or "").strip()
        if not token or not allowed:
            return False
        chat_id = next((part.strip() for part in allowed.split(",") if part.strip()), "")
        if not chat_id:
            return False
        ...
        try:
            ...
            logger.error("Episodic memory alert send failed: %s", e)
            return False
    ...
    def _emit_failure_alert(
        ...
        *,
        ...
        alert_text = f"⚠️ Episodic memory {failure_type}: {session_id or 'unknown-session'}\n{message}\nArtifact: {artifact}"
        self._send_telegram_alert(alert_text)
        logger.error(alert_text)
        return artifact

    def _find_missing_journal_sessions(self, grace_seconds: int = _MISSING_JOURNAL_GRACE_SECONDS) -> List[Dict[str, Any]]:
        now = time.time()
        missing: List[Dict[str, Any]] = []
        for jsonl_path in sorted(SESSIONS_DIR.glob("*.jsonl"), key=lambda p: p.stat().st_mtime, reverse=True):
            if len(missing) >= _MISSING_JOURNAL_SCAN_LIMIT:
                break
            sid = jsonl_path.stem
            if sid == self._session_id:
                continue
            age_seconds = now - jsonl_path.stat().st_mtime
            if age_seconds < grace_seconds:
                continue
            matches = list(JOURNAL_DIR.glob(f"**/*{sid}.md"))
            if matches:
                continue
            missing.append({
                "session_id": sid,
    ...
    def _alert_on_missing_journals(self) -> None:
        for item in self._find_missing_journal_sessions():
            session_id = item["session_id"]
            if list(self._failure_dir().glob(f"missing-journal_{session_id}_*.md")):
                continue
            self._emit_failure_alert(
                alert_key="missing-journal",
                session_id=session_id,
                failure_type="missing_journal_artifact",
                message="Session JSONL exists but no matching journal artifact was found after the grace period.",
                details=item,
            )
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            self._alert_on_missing_journals()
        except Exception as e:
    ...
    def sync_turn(
        ...
        *,
        ...
        try:
            ...
            asst_tid = self._store.append_turn(
                sid,
                "assistant",
                assistant_content,
    ...
    def _tool_stale_facts(self, args: Dict[str, Any]) -> str:
        ...
        from .temporal import detect_stale_entities, detect_stale_facts
    ...
    def _tool_contradictions(self, args: Dict[str, Any]) -> str:
        ...
        from .temporal import detect_contradictions
    ...
    def on_session_end(self, messages: List[Dict[str, Any]]) -> None:
        ...
        if ENABLE_SESSION_JOURNAL:
            try:
                from .journal import write_session_journal
        ...
        if self._pending_extractions:
            try:
                from .merge import merge_session
        ...
        try:
            from .compress import compress_session_to_d0
