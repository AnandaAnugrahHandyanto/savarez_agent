...
`--clone-all` is a snapshot-style copy. When cloning from the default profile, it keeps most profile state but excludes default-profile infrastructure and bulky local runtime directories such as sibling profiles, source checkouts, worktrees, local backups, heap dumps, cache directories, Kanban runtime state, `bin`, `node`, `lsp`, and `node_modules`; it also strips runtime PID/process files.
