...
This page shows how to combine worktrees with Savarez so each session has a clean, isolated working directory.

## Why Use Worktrees with Savarez?

Savarez treats the **current working directory** as the project root:
...
git worktree add ../repo-feature feature/savarez-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/savarez-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Savarez there:

```bash
cd ../repo-feature

# Start Savarez in the worktree
savarez
```
...
git worktree add ../repo-experiment-a feature/savarez-a
git worktree add ../repo-experiment-b feature/savarez-b
```
...
Each Savarez process:

- Works on its own branch (`feature/savarez-a` vs `feature/savarez-b`).
...
- Savarez checkpoint data under `~/.savarez/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Savarez experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/savarez-checkpoints-docs`, `feature/savarez-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Savarez from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `savarez -w` (Automatic Worktree Mode)

Savarez has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `savarez/savarez-<hash>`).
...
```bash
savarez -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `savarez -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Savarez session its own clean checkout.
