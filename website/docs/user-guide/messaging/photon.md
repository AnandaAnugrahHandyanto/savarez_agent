...
Connect Savarez to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Savarez — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Savarez runs it in a
...
2. **Finds or creates** the `Savarez` project on your account.
...
Photon uses the same authorization model as every other Savarez
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Savarez replies with a pairing code. Approve it with:
...
By default Savarez responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Savarez` and
`@Savarez agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Savarez will reply.
...
assigned iMessage line Savarez uses. When a Photon token and dashboard project
...
- **Outbound attachments are supported.** Savarez sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Savarez wake words  | JSON list / comma / newline regex patterns for group mentions |
