...
:::info No new heavy dependencies
The adapter uses `httpx` (already a core Hermes dependency) plus
`python-socketio` (bundled in the `messaging` extra). Nothing else to install.
:::
...
- **Voice in/out:** inbound voice is transcribed by Carbon Voice before Hermes
  ...
  configure a TTS provider (`voice.auto_tts: true` in `config.yaml`).
