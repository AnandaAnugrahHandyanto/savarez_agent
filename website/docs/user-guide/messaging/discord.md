...
| **Free-response channels** | You can make specific channels mention-free with `DISCORD_FREE_RESPONSE_CHANNELS`, or disable mentions globally with `DISCORD_REQUIRE_MENTION=false`. With `DISCORD_AUTO_THREAD=true`, mention-free channel messages can still open isolated threads. Use `DISCORD_NO_THREAD_CHANNELS` for channels that should stay inline. |
...
Free-response channels still honor `discord.auto_thread`. With the default `auto_thread: true`, a new top-level message in a free-response text channel opens an isolated thread even though no `@mention` is required. Add the channel to `discord.no_thread_channels` if you want mention-free inline chat instead.

#### `discord.auto_thread`

**Type:** boolean — **Default:** `true`

When enabled, top-level messages that pass the Discord mention/free-response gate automatically create a new thread for the conversation. This keeps the main channel clean and gives each conversation its own isolated session history. Once a thread is created, subsequent messages in that thread don't require `@mention` — the bot knows it's already participating. Set [`thread_require_mention`](#discordthread_require_mention) to `true` to disable this in-thread shortcut for multi-bot setups.
