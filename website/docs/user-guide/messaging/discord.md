...
| **Free-response channels** | You can make specific channels mention-free with `DISCORD_FREE_RESPONSE_CHANNELS`, or disable mentions globally with `DISCORD_REQUIRE_MENTION=false`. Free-response controls the mention gate only; use `DISCORD_NO_THREAD_CHANNELS` when a channel should answer inline instead of creating threads. |
...
Free-response channels can still auto-thread when `discord.auto_thread` is enabled. Free-response only removes the `@mention` requirement. If a mention-free channel should stay inline, add it to `discord.no_thread_channels` or `DISCORD_NO_THREAD_CHANNELS`.

#### `discord.auto_thread`

**Type:** boolean — **Default:** `true`

When enabled, every triggering message in a regular text channel automatically creates a new thread for the conversation. A triggering message is normally an `@mention`, or any message in a free-response channel. This keeps the main channel clean and gives each conversation its own isolated session history. Once a thread is created, subsequent messages in that thread don't require `@mention` — the bot knows it's already participating. Set [`thread_require_mention`](#discordthread_require_mention) to `true` to disable this in-thread shortcut for multi-bot setups.
