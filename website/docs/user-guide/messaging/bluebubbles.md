...
Connect Savarez to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Savarez and the BlueBubbles server
...
Or set environment variables directly in `~/.savarez/.env`:
...
By default, Savarez responds to every authorized BlueBubbles/iMessage DM or group message. To make group chats opt-in, enable mention gating:
...
With `require_mention: true`, DMs still work normally, but group-chat messages are ignored unless they match a mention pattern. If you do not configure custom patterns, Savarez uses conservative defaults for `Savarez` and `@Savarez AI agent` variants.
...
When someone messages your iMessage, Savarez automatically sends them a pairing code. Approve it with:
```bash
savarez pairing approve bluebubbles <CODE>
```
Use `savarez pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.savarez/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.savarez/.env`):
```bash
...
Savarez will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.
...
- **Outbound:** Savarez sends messages via the BlueBubbles REST API.
...
| `BLUEBUBBLES_MENTION_PATTERNS` | No | Savarez wake words | JSON array, newline-separated, or comma-separated regex patterns for group mention matching |

Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.savarez/config.yaml` (default: `true`). There is no corresponding environment variable.
...
### Chat Addressing
You can address chats by email or phone number — Savarez resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `savarez logs gateway` for webhook errors (or `savarez logs -f` to follow in real-time)
