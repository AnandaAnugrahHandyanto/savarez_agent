...
Savarez connects to SMS through the [Twilio](https://www.twilio.com/) API. People text your Twilio phone number and get AI responses back — same conversational experience as Telegram or Discord, but over standard text messages.
...
- **aiohttp** — `pip install 'savarez-agent[sms]'`
...
If you're running Savarez locally, use a tunnel to expose the webhook:
...
Text your Twilio number — Savarez will respond via SMS.
...
Savarez validates that inbound webhooks genuinely originate from Twilio by verifying the `X-Twilio-Signature` header (HMAC-SHA1). This prevents attackers from injecting forged messages.
...
3. Check Savarez gateway logs for Twilio API errors
