...
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Savarez
...
download the file. Save it somewhere only Savarez can read (e.g.,
...
- Topic ID: `savarez-chat-events`
- Leave the defaults for everything else.

After creation, the topic's detail page has a **Subscriptions** tab. Create one:

- Subscription ID: `savarez-chat-events-sub`
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a savarez restart)
...
- Principal: `savarez-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Savarez calls
...
- **App name**: whatever you want users to see ("Savarez" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/savarez-chat-events`.
...
`ADDED_TO_SPACE` event that Savarez uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Savarez
...
GOOGLE_CHAT_SUBSCRIPTION_NAME=projects/my-chat-bot-123/subscriptions/savarez-chat-events-sub
GOOGLE_CHAT_SERVICE_ACCOUNT_JSON=/home/you/.savarez/google-chat-sa.json
...
Install the dependencies the Google Chat adapter needs (no Savarez extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Savarez is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Savarez detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Savarez session.
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Savarez isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
- **Attachment download protection**: Savarez will only attach the SA bearer
