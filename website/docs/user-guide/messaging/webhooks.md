...
sidebar_position: 13
title: "Webhooks"
description: "Receive events from GitHub, GitLab, and other services to trigger Savarez AI agent runs"
---

# Webhooks

Receive events from external services (GitHub, GitLab, JIRA, Stripe, etc.) and trigger Savarez AI agent runs automatically. The webhook adapter runs an HTTP server that accepts POST requests, validates HMAC signatures, transforms payloads into agent prompts, and routes responses back to the source or to another configured platform.
...
1. Enable via `savarez gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `savarez webhook subscribe`
...
Add the `github-pr` route to your `~/.savarez/config.yaml` as shown in the example above.
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `savarez webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.
...
- Subscriptions are stored in `~/.savarez/webhook_subscriptions.json`
...
The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `savarez webhook subscribe` command.
...
- Run the gateway in foreground to see logs: `savarez gateway run`
