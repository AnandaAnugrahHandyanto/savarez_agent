...
public HTTP endpoints, so your Savarez instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Savarez uses the modern
...
The fastest path is to paste a manifest Savarez generates for you. It
...
### Option A: From a Savarez-generated manifest (recommended)

1. Generate the manifest:
   ```bash
   savarez slack manifest --write
   ```
...
it will only work in DMs. Without `files:read`, Savarez can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `savarez-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Savarez configuration issue.
:::
...
Savarez uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
Every Savarez command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Savarez command with its description.

Under the hood: Savarez ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/AnandaAnugrahHandyanto/savarez_agent/blob/main/hermes_cli/commands.py)
...
When Savarez adds new commands (e.g. after `savarez update`), regenerate
...
### Legacy `/savarez <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/savarez btw run the tests` — Savarez routes it the same way as `/btw
run the tests`. Free-form questions also work: `/savarez what's the
...
Slack never delivers them to Savarez.

As a workaround, Savarez recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Savarez treats it identically to the slash form and replies in the same
thread.
...
Savarez falls back to a text prompt, the prompt instructs you to reply
...
```bash
savarez slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how Savarez behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Savarez AI Agent what time is it?`). In channels, Savarez replies in a thread attached to that message. |
| **Threads** | If you @mention Savarez inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Savarez in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Savarez will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Savarez will deliver scheduled messages,
...
Savarez can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Savarez also loads tokens from an **OAuth token file** at:
...
- When a message arrives, Savarez uses the correct workspace-specific client to respond.
...
Savarez supports voice on Slack:
...
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Savarez now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
- Audit who has access to your Savarez config directory
