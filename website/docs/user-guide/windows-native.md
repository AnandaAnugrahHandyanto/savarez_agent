...
5. **Clones the repo** to `%LOCALAPPDATA%\hermes\hermes-agent` and creates a `.venv` project environment inside it.
6. **Project dependency sync** — runs `uv sync --locked --extra all`, then falls back to base `uv sync --locked` if optional extras fail. If the dashboard deps are missing after fallback, it tries a targeted `uv sync --locked --extra web`.
