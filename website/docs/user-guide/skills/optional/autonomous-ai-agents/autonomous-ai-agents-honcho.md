...
Run `hermes honcho setup`. Ensure `memory.provider: honcho` is in `~/.savarez/config.yaml`.
...
Use `--clone` when creating: `savarez profile create <name> --clone`. For existing profiles: `hermes honcho sync`.
