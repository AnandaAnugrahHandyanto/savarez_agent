...
| | |
|---|---|
| Source | Optional — install with `savarez skills install official/devops/watchers` |
...
All three live in `$SAVAREZ_HOME/skills/devops/watchers/scripts/` once the skill is installed. Each reads `WATCHER_STATE_DIR` (defaults to `$SAVAREZ_HOME/watcher-state/`) for its state file, keyed by the `--name` argument.
...
Watch a GitHub repo (set `GITHUB_TOKEN` in `~/.savarez/.env` to avoid the 60 req/hr anonymous rate limit):
...
Every watcher writes `$SAVAREZ_HOME/watcher-state/<name>.json`. Inspect:
...
4. **Putting the state dir where the agent's sandbox can't write.** `$SAVAREZ_HOME/watcher-state/` is always writable. Docker/Modal backends may not see arbitrary host paths.
