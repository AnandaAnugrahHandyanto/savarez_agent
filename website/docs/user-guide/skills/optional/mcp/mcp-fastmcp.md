...
| | |
|---|---|
| Source | Optional — install with `savarez skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Savarez or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Savarez. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.savarez/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.savarez/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Savarez integration, either:

- configure the server in `~/.savarez/config.yaml` using the `native-mcp` skill, or
...
### Savarez cannot see the deployed server

The server-building part may be correct while the Savarez config is not. Load the `native-mcp` skill and configure the server in `~/.savarez/config.yaml`, then restart Savarez.
