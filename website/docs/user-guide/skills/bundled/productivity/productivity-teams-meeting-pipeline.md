...
description: "Operate the Teams meeting summary pipeline via Savarez CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions"
---
...
Operate the Teams meeting summary pipeline via Savarez CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions.
...
Everything operator-facing is a `savarez teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
```bash
savarez teams-pipeline validate              # config snapshot — run first after any change
savarez teams-pipeline token-health          # Graph token status
savarez teams-pipeline token-health --force-refresh   # force a fresh token acquisition
savarez teams-pipeline list                  # recent meeting jobs
savarez teams-pipeline list --status failed  # only failed jobs
savarez teams-pipeline show <job-id>         # full detail of one job
savarez teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
savarez teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
savarez teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
savarez teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
savarez teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

savarez teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
savarez teams-pipeline delete-subscription <sub-id>
savarez teams-pipeline maintain-subscriptions            # renew near-expiry ones
savarez teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `savarez teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
