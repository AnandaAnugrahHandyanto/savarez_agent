---
title: "Find Nearby — Find nearby places (restaurants, cafes, bars, pharmacies, etc"
sidebar_label: "Find Nearby"
description: "Find nearby places (restaurants, cafes, bars, pharmacies, etc"
---
...
| Path | `skills/leisure/find-nearby` |
| Version | `1.0.0` |
...
## Location and sparse-category pitfalls

- Do **not** imply you have the user's precise location unless they explicitly shared a pin/address. If needed, an IP lookup can provide only a rough network/server location; label it clearly as approximate and possibly wrong.
- For local service categories that OSM may under-cover (e.g. `driving_school`, tutors, repair shops, small local offices), try OSM/Overpass first, but if results are empty or weak, use Google Maps/browser search as a fallback and extract names, ratings, neighborhoods, hours, and phone numbers from the visible listing.
- For driving lessons specifically, search phrases like `driving school near <city/area>`, `driving lessons <city>`, plus local-language variants. Present a short ranked contact list and give the user a call/message script with key questions: price per lesson/package, automatic vs manual, number of lessons, test/license help, car availability for test.
- When the user says "near me" but no GPS is available, ask for city/neighborhood or a maps pin; if they give a city/central area, proceed using that as the origin instead of stalling.
