...
| Related skills | [`systematic-debugging`](/docs/user-guide/skills/bundled/software-development/software-development-systematic-debugging), [`node-inspect-debugger`](/docs/user-guide/skills/bundled/software-development/software-development-node-inspect-debugger), [`debugging-savarez-tui-commands`](/docs/user-guide/skills/bundled/software-development/software-development-debugging-savarez-tui-commands) |
...
The savarez test runner and pytest both support this:
...
For long-lived processes: Savarez gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.

### Setup

```bash
source /home/bb/savarez-agent/.venv/bin/activate
pip install debugpy
```
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Savarez you have two practical options:
...
```json
{
  "name": "Attach to Savarez",
  "type": "debugpy",
  "request": "attach",
  "connect": { "host": "127.0.0.1", "port": 5678 },
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "/home/bb/savarez-agent" }
  ]
}
```
...
## Debugging Savarez-specific Processes
...
### `tui_gateway` subprocess (spawned by `savarez --tui`)
...
debugpy.wait_for_client()
```
Start `savarez --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Savarez subagents, debug one process at a time.
