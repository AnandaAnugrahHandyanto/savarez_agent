...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Savarez AI Agent\",
    \"comments\": [
...
```bash
source "${SAVAREZ_HOME:-$HOME/.savarez}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Savarez AI Agent. Code looks clean — good test coverage, no security concerns."
