...
| `"xai-oauth"` | Force xAI Grok OAuth (browser login for SuperGrok and X Premium+ subscribers, no API key). Same OAuth token covers chat, TTS, image, video, and transcription. | `hermes model` → xAI Grok OAuth (SuperGrok / Premium+) |
