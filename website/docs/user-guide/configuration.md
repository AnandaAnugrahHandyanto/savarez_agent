...
discord:
  ...
  auto_thread_free_response_channels: false  # Also thread free-response messages (off by default)
```
...
- `auto_thread_free_response_channels` — when `true`, top-level messages in `free_response_channels` also create threads. Defaults to `false` to keep free-response channels inline unless explicitly opted in.
