...
2. Hermes stores that single token in `~/.savarez/.env` as `BWS_ACCESS_TOKEN`.
3. Every time `savarez` (or the gateway, or a cron job) starts, after `~/.savarez/.env` has loaded, Hermes calls `bws secret list <project_id>` and sets the returned keys into `os.environ`.
