...
- Savarez AI Agent installed and running.
...
### One-shot: `savarez tools` or first-run setup
...
4. Saves it to `~/.savarez/.env` so future runs skip this step
5. Continues straight into the OAuth consent flow

After you approve, tokens are written under `providers.spotify` in `~/.savarez/auth.json`. The active inference provider is NOT changed — Spotify auth is independent of your LLM provider.
...
- **The cron agent inherits your active toolsets.** Spotify must be enabled in `savarez tools` for the cron session to see the Spotify tools.
...
```bash
savarez auth logout spotify
```

Removes tokens from `~/.savarez/auth.json`. To also clear the app config, delete `HERMES_SPOTIFY_CLIENT_ID` (and `HERMES_SPOTIFY_REDIRECT_URI` if you set it) from `~/.savarez/.env`, or run the wizard again.
...
```bash
savarez auth spotify --client-id <id> --redirect-uri http://localhost:3000/callback
```

Or set them permanently in `~/.savarez/.env`:
...
| File | Contents |
|------|----------|
| `~/.savarez/auth.json` → `providers.spotify` | access token, refresh token, expiry, scope, redirect URI |
| `~/.savarez/.env` | `HERMES_SPOTIFY_CLIENT_ID`, optional `HERMES_SPOTIFY_REDIRECT_URI` |
