...
chat completions — use your Savarez-managed provider subscription as their
...
| Use case | "Use Savarez as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Savarez stores
the refresh token in `~/.savarez/auth.json` — the same place all Savarez
...
```
Starting Savarez proxy for Nous Portal
...
Model:      Savarez-4-70B    # or Savarez-4.3-36B, Savarez-4-405B
```
...
```bash
savarez proxy status
```

```
Savarez proxy upstream adapters

  [nous    ] Nous Portal — ready (bearer expires 2026-05-15T06:43:21Z)
```

If you see `not logged in`, run `savarez portal`. If you see
...
INFERENCE_TEXT_MODEL=Savarez-4-70B
```
...
```bash
savarez proxy start --host 0.0.0.0 --port 8645
```
