...
When Savarez AI Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.savarez/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.savarez/config.yaml` (switchable per session
via `/personality`).
...
If you use Savarez' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.savarez/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Savarez AI Agent's deliverable
