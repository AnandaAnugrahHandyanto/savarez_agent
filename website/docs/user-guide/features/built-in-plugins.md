...
description: "Plugins shipped with Savarez AI Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Savarez ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.savarez/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Savarez Plugin](/guides/build-a-savarez-plugin) to write your own.
...
2. **User** — `~/.savarez/plugins/<name>/`
3. **Project** — `./.savarez/plugins/<name>/` (requires `HERMES_ENABLE_PROJECT_PLUGINS=1`)
...
`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `savarez memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `savarez plugins list` and the interactive `savarez plugins` UI), but none load until you explicitly enable them:

```bash
savarez plugins enable disk-cleanup
```

Or via `~/.savarez/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Savarez. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
savarez plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `savarez plugins enable <name>`.
...
| `savarez-achievements` | dashboard tab | Steam-style collectible badges generated from your real Savarez session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `savarez memory` and `savarez plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `SAVAREZ_HOME` or `/tmp/savarez-*`, track it silently as `test` / `temp` / `cron-output`. |
...
**State** — everything lives at `$SAVAREZ_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `SAVAREZ_HOME` or `/tmp/savarez-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `savarez plugins enable disk-cleanup` (or check the box in `savarez plugins`).

**Disabling again:** `savarez plugins disable disk-cleanup`.
...
**Enabling:** `savarez plugins enable security-guidance` (or check the box in `savarez plugins`).

**Disabling again:** `savarez plugins disable security-guidance`.
...
Traces Savarez turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Savarez' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `savarez logs`.
...
```bash
savarez tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Savarez and the next turn ships a trace.

**Setup (manual):**

```bash
pip install langfuse
savarez plugins enable observability/langfuse
```

Then put the credentials in `~/.savarez/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Savarez turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
Session grouping keys off the Savarez session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `savarez chat` session lives under one Langfuse session.

**Verify:**

```bash
savarez plugins list                 # observability/langfuse should show "enabled"
savarez chat -q "hello"              # check the Langfuse UI for a "Savarez turn" trace
```
...
Savarez-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Savarez-prefixed wins when both are set.
...
**Disabling:** `savarez plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.savarez/cache/google_meet/<meeting_id>/`
...
**Disabling:** `savarez plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.savarez/cache/google_meet/` until you remove them.

### savarez-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Savarez session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Savarez feature changes.

**How it works:**

- Scans your entire `~/.savarez/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$SAVAREZ_HOME/plugins/savarez-achievements/state.json`
...
| Secret | Hidden until Savarez detects the first related signal in your history |

**API** — routes mount under `/api/plugins/savarez-achievements/`:
...
**State files** — live under `$SAVAREZ_HOME/plugins/savarez-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Savarez updates. |
...
**Enabling:** Nothing to enable — `savarez-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `savarez dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/savarez-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.savarez/plugins/savarez-achievements/` that ships no dashboard. The plugin's state files under `$SAVAREZ_HOME/plugins/savarez-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Savarez plugin — see [Build a Savarez Plugin](/guides/build-a-savarez-plugin). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.savarez/plugins/<name>/`
- Manifest source is reported as `bundled` in `savarez plugins list`
