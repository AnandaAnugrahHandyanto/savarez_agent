...
Hermes ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.savarez/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `hermes memory` and `savarez plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `SAVAREZ_HOME` or `/tmp/hermes-*`, track it silently as `test` / `temp` / `cron-output`. |
...
**Safety** — cleanup only ever touches paths under `SAVAREZ_HOME` or `/tmp/hermes-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.
...
Session grouping keys off the Hermes session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `savarez chat` session lives under one Langfuse session.

**Verify:**

```bash
savarez plugins list                 # observability/langfuse should show "enabled"
savarez chat -q "hello"              # check the Langfuse UI for a "Hermes turn" trace
```
...
- Unlock state is persisted to `$SAVAREZ_HOME/plugins/hermes-achievements/state.json`
...
**State files** — live under `$SAVAREZ_HOME/plugins/hermes-achievements/`:
...
**Opting out:** Delete or rename `plugins/hermes-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.savarez/plugins/hermes-achievements/` that ships no dashboard. The plugin's state files under `$SAVAREZ_HOME/plugins/hermes-achievements/` survive — reinstalling preserves your unlock history.
