...
**Tool Search** is Savarez' opt-in progressive-disclosure layer for that
...
:::info Built-in Savarez tools never defer
The tools that make up Savarez' core capability set (`terminal`,
...
When the model invokes `tool_call`, Savarez **unwraps the bridge** and
...
- **No JS sandbox.** Savarez uses the simpler "structured tools" mode
