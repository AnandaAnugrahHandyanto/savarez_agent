...
Use `savarez computer-use status` to verify the install.
...
window-focus bug for UTM workflows). Savarez refreshes the binary in two
places so you don't get stuck on a stale release:

- **`savarez update`** — when you update Savarez itself, if `cua-driver` is
...
- **`savarez computer-use install --upgrade`** — manual force-refresh.
...
`savarez computer-use status` shows the installed version next to the
binary path.
...
Savarez applies multi-layer guardrails:
...
Screenshots are expensive. Savarez applies four layers of optimisation:
...
`savarez computer-use install` to fetch the cua-driver binary, or run
...
- [Universal skill: `macos-computer-use`](https://github.com/AnandaAnugrahHandyanto/savarez_agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
