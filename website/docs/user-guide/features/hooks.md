...
Gateway hooks fire automatically during gateway operation (Telegram, Discord, Slack, Webex, WhatsApp, Teams) without blocking the main agent pipeline.
...
:::info
Gateway hooks only fire in the **gateway** (Telegram, Discord, Slack, Webex, WhatsApp, Teams). The CLI does not load gateway hooks. For hooks that work everywhere, use [plugin hooks](#plugin-hooks).
:::
