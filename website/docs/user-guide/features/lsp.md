...
Savarez runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Savarez
...
1. Savarez captures a baseline of current diagnostics for the file.
...
…). Savarez auto-detects the binary on PATH or in
...
`node_modules` tree — Savarez installs both packages together when you
run `savarez lsp install typescript` or auto-install fires on first
use.

## CLI

```
savarez lsp status          # service state + per-server install status
savarez lsp list            # registry, optionally --installed-only
savarez lsp install <id>    # eagerly install one server
savarez lsp install-all     # try every server with a known recipe
savarez lsp restart         # tear down running clients
savarez lsp which <id>      # print resolved binary path
```

`savarez lsp status` is the best starting point — it shows which
...
When `install_strategy: auto`, Savarez installs binaries into
...
shared location — the staging dir is fully Savarez-owned and is
...
Servers are kept alive for the life of the Savarez process. There's
...
**`savarez lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<SAVAREZ_HOME>/lsp/bin/`. Run
`savarez lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `savarez lsp status`**
...
When `savarez lsp status` shows a `Backend warnings` section, install
...
the rest of the session. Run `savarez lsp restart` to clear the set;
