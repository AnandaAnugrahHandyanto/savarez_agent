...
Savarez AI Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
savarez memory setup      # interactive picker + configuration
savarez memory status     # check what's active
savarez memory off        # disable external provider
```

You can also select the active memory provider via `savarez plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.savarez/config.yaml`:
...
When a memory provider is active, Savarez automatically:
...
**Setup Wizard:**
```bash
savarez memory setup        # select "honcho" — runs the Honcho-specific post-setup
```
...
:::tip Migrating from `savarez honcho`
If you previously used `savarez honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Savarez profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Savarez profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Savarez profile. Host key `savarez` → default; `savarez.<profile>` for others. |
...
`--clone` creates a `savarez.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Savarez profile, creates host blocks for any profile without one, inherits settings from the default `savarez` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
**Config:** `$SAVAREZ_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `savarez-user` | User identifier |
...
**Local mode UI:** `hindsight-embed -p savarez ui start`

**Config:** `$SAVAREZ_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `savarez` | Memory bank identifier |
...
# Or manually:
savarez config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.savarez-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$SAVAREZ_HOME/memory_store.db` | SQLite database path |
...
- Knowledge tree stored at `$SAVAREZ_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
savarez memory setup    # select "supermemory"
# Or manually:
savarez config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.savarez/.env
```

**Config:** `$SAVAREZ_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `savarez` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `savarez-{identity}` → `savarez-coder`) to isolate memories per Savarez profile
...
| **Requires** | `pip install savarez-memori` + `savarez-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install savarez-memori
savarez-memori install
savarez config set memory.provider memori
savarez memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `savarez-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$SAVAREZ_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$SAVAREZ_HOME/` so each profile has its own credentials
