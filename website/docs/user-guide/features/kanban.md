...
- **Model tier** — optional task-level routing hint (`cheap`, `standard`, or `strong`) set at creation time with `hermes kanban create ... --model-tier cheap` or `kanban_create(model_tier="cheap")`. If omitted, the dispatcher passes no `--provider` / `--model` flags and the assignee profile keeps its configured default. If set, the dispatcher resolves `kanban.model_tiers.<tier>.provider/model`, records the requested/effective tier on the run metadata, and spawns the same profile with explicit model overrides. Cheap-tier startup/provider/model launch failures may escalate once to the configured `kanban.cheap_startup_escalation.target_tier` (default `standard`); task-work failures, blocks, reviews, timeouts after meaningful work, and `standard` failures do not auto-escalate.
