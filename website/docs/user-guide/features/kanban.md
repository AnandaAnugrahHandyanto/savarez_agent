...
| `kanban_assign` | (Orchestrators) assign or reassign a task; use `assignee="none"` to unassign. Refuses actively claimed running tasks. | `task_id`, `assignee` |
| `kanban_unblock` | (Orchestrators) move a blocked task back to `ready`. | `task_id` |
| `kanban_archive` | (Orchestrators) archive a task so it leaves active board views. | `task_id` |
