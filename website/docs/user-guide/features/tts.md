...
| **Murf TTS** | Excellent | Paid | `MURF_API_KEY` |
...
tts:
  provider: "edge"              # "edge" | "elevenlabs" | "openai" | "minimax" | "mistral" | "gemini" | "murf" | "xai" | "neutts" | "kittentts" | "piper"
  ...
  murf:
    model: "GEN2"               # "GEN2" (studio quality) or "FALCON" (low-latency stream endpoint)
...
- **Murf TTS** outputs MP3/WAV/streamed audio and uses **ffmpeg** to encode Opus for Telegram voice bubbles
...
Without ffmpeg, Edge TTS, MiniMax TTS, Murf TTS, NeuTTS, KittenTTS, and Piper audio are sent as regular audio files (playable, but shown as a rectangular player instead of a voice bubble).
