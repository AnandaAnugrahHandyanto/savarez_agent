...
If the file is empty, whitespace-only, or cannot be read, Savarez falls back to a built-in default identity ("You are Savarez AI Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
