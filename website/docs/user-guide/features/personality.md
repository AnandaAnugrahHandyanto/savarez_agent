...
description: "Customize Savarez AI Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Savarez AI Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `SAVAREZ_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Savarez is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Savarez now seeds a default `SOUL.md` automatically in:

```text
~/.savarez/SOUL.md
```

More precisely, it uses the current instance's `SAVAREZ_HOME`, so if you run Savarez with a custom home directory, it will use:

```text
$SAVAREZ_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- Savarez creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Savarez loads `SOUL.md` only from `SAVAREZ_HOME`
- Savarez does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Savarez falls back to a built-in default identity
...
If Savarez loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `SAVAREZ_HOME`, the personality belongs to the Savarez instance itself.

That also makes it easier to teach users:
- "Edit `~/.savarez/SOUL.md` to change Savarez' default personality."
...
- how Savarez should handle uncertainty, disagreement, or ambiguity
...
## What Savarez injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Savarez falls back to a built-in default identity ("You are Savarez AI Agent, an intelligent AI assistant created by Ananda Anugrah Handyanto..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Savarez ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Savarez, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Savarez its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.savarez/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.savarez/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-savarez)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Savarez speaks
- `display.skin` and `/skin` affect how Savarez looks in the terminal
