...
| [`pre_api_request`](/docs/user-guide/features/hooks#pre_api_request) | Before each provider request — can return `{"context": "..."}` to inject request-scoped context into the current turn's user message |
| [`post_api_request`](/docs/user-guide/features/hooks#post_api_request) | After each provider response |
