...
The CA generated by `hermes egress setup` is a 10-year self-signed root. Two situations call for replacing it before it expires:

- **Compromise response** — if `ca.key` may have leaked (a backup left world-readable, a stolen laptop, a shared-host mishap), the old root must be retired immediately so a thief can't mint trusted leaf certs for your upstreams.
- **Annual hygiene** — even with no incident, rotating once a year keeps key material fresh and exercises the recovery path so it isn't first attempted under pressure.

```bash
hermes egress rotate-ca --reason "annual rotation"
```

What it does, in order:

1. Archives the current `ca.crt` to `~/.hermes/proxy/ca-archive/ca-<YYYYMMDD-HHMMSS>.crt` (the archive dir keeps the five most recent; older ones are pruned).
2. Mints a fresh CA + key with the same atomic, `0o600`-key write path as first-boot generation.
3. Restarts the managed daemon if it was running, so the live proxy signs leaf certs with the new key.
4. Appends a structured record to `~/.hermes/proxy/rotation-history.jsonl` (timestamp, old/new SHA-256 fingerprints, reason, operator, subject, validity).

:::warning Restart your running sandboxes
Containers mount `ca.crt` read-only at create time, so a sandbox started before the rotation keeps trusting the **old** CA until it's recreated. After rotating, restart any running sandboxes so they re-mount the new cert. `rotate-ca` lists the labeled `hermes.sandbox=true` containers it found and (unless you pass `--force`) prompts for confirmation when any are running.
:::
...
| Flag | Effect |
|---|---|
| `--dry-run` | Print the new subject, the archive path, and the sandboxes that would need restarting — without changing a single file. Use it first when you're unsure. |
| `--reason TEXT` | Free-text reason recorded in `rotation-history.jsonl` (defaults to `null`). |
| `--no-restart` | Mint + archive but leave the daemon untouched. Useful for staged rollouts where you'll restart the proxy and sandboxes together on your own schedule. |
| `--force` | Skip the interactive "you have running sandboxes" confirmation. |

Verifying that already-running sandboxes still trust the new CA after a rotation (a `verify-trust` subcommand), and a companion `doctor` check that warns on rotation age, are planned follow-ups that land once the broader `doctor` / `audit` work is merged upstream.
...
- The CA is a 10-year self-signed cert on first generation. Rotation is a first-class command now — see [CA rotation](#ca-rotation) (`hermes egress rotate-ca`). Verifying post-rotation trust inside live sandboxes (`verify-trust`) remains a follow-up.
