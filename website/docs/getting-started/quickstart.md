...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Savarez AI Agent — from install to chatting in under 5 minutes"
---
...
## 1. Install Savarez Agent
### With the Savarez Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the Savarez Desktop installer](https://savarez-agent.nousresearch.com/desktop) from our website and run it.

### Without Savarez Desktop:
For a command-line only install without Savarez Desktop, run:
...
The single most important setup step. Use `savarez model` to walk through the choice interactively:
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `savarez model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `savarez model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `savarez model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `hermes_cli/models.py` may change between releases) | `savarez model` → MiniMax (OAuth) |
...
| **Google Gemini (OAuth)** | Gemini via the `google-gemini-cli` OAuth flow — no key needed | `savarez model` → Google Gemini (OAuth) |
| **xAI** | Grok models via direct API | Set `XAI_API_KEY` |
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `savarez model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `savarez model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `savarez model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `savarez model` (requires `copilot` CLI + `copilot login`) |
...
Savarez AI Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `savarez model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::
...
```bash
savarez gateway setup    # Interactive platform configuration
```
...
- `savarez tools` — tune tool access per platform
- `savarez skills` — browse and install reusable workflows
...
```bash
savarez config set terminal.backend docker    # Docker isolation
savarez config set terminal.backend ssh       # Remote server
```
...
```bash
savarez skills browse                      # list everything available
savarez skills search kubernetes           # find skills by keyword
savarez skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `savarez skills browse` shows the exact slugs to use.
...
| `savarez tools` | Configure which tools are enabled per platform |
| `savarez setup` | Full setup wizard (configures everything at once) |
