...
|---------|---------|--------|---------|-------|
| **Perplexity** (default) | `PERPLEXITY_API_KEY` | ✔ | — | — |
...
```yaml
web:
  backend: perplexity    # perplexity | firecrawl | searxng | parallel | tavily | exa
```

If `web.backend` is not set, the backend is auto-detected from whichever API key is available. Perplexity is search-only — pair it with Firecrawl/Tavily/Exa/Parallel for `web_extract`. Self-hosted Firecrawl is also supported via `FIRECRAWL_API_URL`.
