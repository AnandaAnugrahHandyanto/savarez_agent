...
# Kimi / Moonshot AI (international default: api.moonshot.ai)
...
Base URLs can be overridden with `GLM_BASE_URL`, `KIMI_BASE_URL` (for `kimi-coding` only), `MINIMAX_BASE_URL`, `MINIMAX_CN_BASE_URL`, `DASHSCOPE_BASE_URL`, or `XIAOMI_BASE_URL` environment variables.

:::note Kimi: international vs China
- **`kimi-coding`** uses `KIMI_API_KEY`. Default endpoint is `https://api.moonshot.ai/v1`. Keys prefixed `sk-kimi-` are routed to the Kimi Coding API (`https://api.kimi.com/coding/v1`). Override with `KIMI_BASE_URL` if needed.
- **`kimi-coding-cn`** uses **`KIMI_CN_API_KEY`** (separate from `KIMI_API_KEY`) and defaults to **`https://api.moonshot.cn/v1`**. Use this when your key is issued for the China console. Shorthand aliases: `kimi-cn`, `moonshot-cn`.
:::
