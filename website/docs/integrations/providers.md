...
| **W&B Inference** | `WANDB_API_KEY` in `~/.hermes/.env` (provider: `wandb`; aliases: `weights-and-biases`, `wandb-inference`, `wnb`) |
...
# W&B Inference
hermes chat --provider wandb --model meta-llama/Llama-3.1-8B-Instruct
# Requires: WANDB_API_KEY in ~/.hermes/.env
```
...
Base URLs can be overridden with `NOVITA_BASE_URL`, `GLM_BASE_URL`, `KIMI_BASE_URL`, `MINIMAX_BASE_URL`, `MINIMAX_CN_BASE_URL`, `DASHSCOPE_BASE_URL`, `XIAOMI_BASE_URL`, `GMI_BASE_URL`, `WANDB_BASE_URL`, or `TOKENHUB_BASE_URL` environment variables.
...
Open models served via [W&B Inference](https://wandb.ai/inference) (Weights & Biases serverless inference, hosted on CoreWeave) — OpenAI-compatible API, API key authentication. The model catalog is fetched live from `/v1/models`.
...
Get your API key at [wandb.ai/settings](https://wandb.ai/settings). The base URL can be overridden with `WANDB_BASE_URL` (default: `https://api.inference.wandb.ai/v1`).

:::note Personal W&B accounts
**Team accounts** work as-is. **Personal accounts** whose default project lacks Inference access must supply W&B's `openai-project` header (`team/project`) — add it under `model.default_headers` in `config.yaml`:
