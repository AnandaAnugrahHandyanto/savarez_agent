...
3. xAI redirects back to Hermes and the tokens are saved to `~/.savarez/auth.json`.
4. From then on, Hermes refreshes the access token in the background — you stay signed in until you `savarez auth remove xai-oauth` or revoke access from your xAI account settings.
...
This is **not** a stale-token problem — re-running `savarez model` won't change it. xAI's backend has been seen to restrict OAuth API access to specific SuperGrok tiers despite the in-app subscription being active (issue [#26847](https://github.com/NousResearch/hermes-agent/issues/26847)).
