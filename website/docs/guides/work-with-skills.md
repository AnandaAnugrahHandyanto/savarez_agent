...
description: "Find, install, use, and create skills — on-demand knowledge that teaches Savarez new workflows"
---

# Working with Skills

Skills are on-demand knowledge documents that teach Savarez how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.
...
Every Savarez installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask Savarez to use a specific skill, and it will load it via the `skill_view` tool.
...
Official optional skills ship with Savarez but aren't active by default. Install them explicitly:
...
What happens:
1. The skill directory is copied to `~/.savarez/skills/`
...
For how to ship skills in your own plugin, see [Build a Savarez Plugin → Bundle skills](/guides/build-a-savarez-plugin#bundle-skills).
...
When a skill with config is first loaded, Savarez prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.
...
savarez config show | grep '^skills\.config'
```
...
The skill appears automatically — no registration needed. Drop it in `~/.savarez/skills/` and it's live.

:::info
The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, Savarez may offer to save the approach as a skill for next time.
:::
...
**Let the agent create skills.** After a complex multi-step task, Savarez will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.

**Use categories.** Organize skills into subdirectories (`~/.savarez/skills/devops/`, `~/.savarez/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.

**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell Savarez to update the skill with what you learned. Skills that aren't maintained become liabilities.
