...
| [`pre_api_request`](/docs/user-guide/features/hooks#pre_api_request) | Before each provider API request | `session_id: str, task_id: str, model: str, provider: str, api_call_count: int, approx_input_tokens: int` | `{"action": "block", "message": str}` to stop before sending |
