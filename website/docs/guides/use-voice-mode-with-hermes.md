---
sidebar_position: 8
title: "Use Voice Mode with Savarez"
description: "A practical guide to setting up and using Savarez voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Savarez
...
- you want Savarez sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Savarez.
...
## Step 1: make sure normal Savarez works first
...
Savarez supports both local and cloud speech stacks.
...
### If you use `savarez setup`

If you choose NeuTTS in the setup wizard, Savarez checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Savarez transcribes and responds
...
- asking Savarez to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Savarez loop.

## Tuning CLI behavior

### Silence threshold

If Savarez starts/stops too aggressively, tune:
...
Savarez stays a normal chat bot, but can speak replies.
...
- you want Savarez to function like a portable research or ops assistant
...
Savarez joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Savarez detects speech boundaries
- transcripts are posted in the associated text channel
- Savarez responds in text and audio
...
1. get text Savarez working
2. install `savarez-agent[voice]`
