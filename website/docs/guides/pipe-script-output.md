...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `savarez send`."
---

# Pipe Script Output to Messaging Platforms

`savarez send` is a small, scriptable CLI that pushes a message to any
messaging platform Savarez is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | savarez send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `savarez
...
Any platform Savarez ships adapters for works as a target:
...
`savarez send` resolves the message body in this order:

1. **Positional argument** — `savarez send --to telegram "hi"`
2. **`--file PATH`** — `savarez send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | savarez send --to telegram`

When stdin is a TTY (no pipe), Savarez does **not** wait for input — you'll
...
Because `savarez send` reuses your Savarez config, the same script works on
any host where Savarez is installed — no need to export bot tokens into
...
`savarez send`. If the Python interpreter can't load because the box is
...
## Does `savarez send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `savarez send` calls
the platform's REST endpoint directly using credentials from
`~/.savarez/.env` and `~/.savarez/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `savarez gateway start` and retry.
...
The listing is built from `~/.savarez/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`savarez gateway
...
| Approach | Multi-platform | Reuses Savarez creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `savarez send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`savarez send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `savarez send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `savarez send` shares with cron delivery.
