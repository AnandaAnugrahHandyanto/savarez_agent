...
Savarez AI Agent uses a SQLite database (`~/.savarez/state.db`) to persist session
...
Multiple savarez processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.savarez/state.db
...
Default path: `~/.savarez/state.db`

This is derived from `hermes_constants.get_hermes_home()` which resolves to
`~/.savarez/` by default, or the value of `SAVAREZ_HOME` environment variable.
