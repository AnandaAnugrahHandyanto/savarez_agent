...
The ACP adapter wraps Savarez' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
savarez acp /.savarez-acp / python -m acp_adapter
...
The Zed ACP Registry path launches the same adapter through `uvx --from 'savarez-agent[acp]==<version>' savarez-acp`, pointed at the `savarez-agent` PyPI release.
...
- `allow_once` -> Savarez `once`
- `allow_always` -> Savarez `always`
- reject options -> Savarez `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Savarez tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["savarez-acp"])
...
Instead it reuses Savarez' runtime resolver:

- `acp_adapter/auth.py`
- `hermes_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Savarez provider/credentials. It also always advertises a terminal setup auth method (`savarez-setup`, args `--setup`) so first-run registry clients can open Savarez' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.savarez/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `savarez-acp` toolset definition
- `hermes_cli/main.py` — `savarez acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `savarez-acp` script
