...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Savarez already has applies. The plugin never sees a
...
savarez-agent itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Savarez provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Savarez provider
        # (must be one Savarez already knows about — same names as
        # `savarez model` and config.yaml model.provider).
        ...
        # plugin sends — Savarez does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.savarez/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Savarez' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Savarez subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/AnandaAnugrahHandyanto/savarez_agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/AnandaAnugrahHandyanto/savarez_agent/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/AnandaAnugrahHandyanto/savarez-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/AnandaAnugrahHandyanto/savarez-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
