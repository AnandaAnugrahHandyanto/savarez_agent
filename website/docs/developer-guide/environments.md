...
Hermes Agent includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/SIUNCT/atropos) RL training framework. This enables three workflows:
...
Dataset: [SIUNCT/terminal-bench-2](https://huggingface.co/datasets/SIUNCT/terminal-bench-2) on HuggingFace.
...
TBLite is a thin subclass of TerminalBench2 — only the dataset and timeouts differ. Created by the OpenThoughts Agent team (Snorkel AI + Bespoke Labs). Dataset: [SIUNCT/openthoughts-tblite](https://huggingface.co/datasets/SIUNCT/openthoughts-tblite).
...
env:
  ...
  dataset_name: "SIUNCT/terminal-bench-2"
...
- Python >= 3.11
- `atroposlib`: `pip install git+https://github.com/SIUNCT/atropos.git`
