...
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.savarez/plugins/` — zero core code changes needed. See [Plugin Path](#plugin-path-recommended) below.
...
| `savarez config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `savarez status` | Shows plugin platforms with `(plugin)` tag |
| `savarez gateway setup` | Plugin platforms appear in setup menu |
| `savarez tools` / `savarez skills` | Plugin platforms in per-platform config |
...
Most users set up a platform by dropping env vars into `~/.savarez/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `savarez gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `savarez cron run` separate from `savarez gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `savarez config`
...
version: 1.0.0
description: >
  My Platform gateway adapter for Savarez AI Agent.
author: Your Name
requires_env:
