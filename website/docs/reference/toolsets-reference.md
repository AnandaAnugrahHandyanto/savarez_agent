...
description: "Reference for Savarez core, composite, platform, and dynamic toolsets"
---
...
- **Platform** — A complete tool configuration for a specific deployment context (e.g., `savarez-cli` is the default for interactive CLI sessions)
...
| `code_execution` | `execute_code` | Run Python scripts that call Savarez tools programmatically. |
| `cronjob` | `cronjob` | Schedule and manage recurring tasks. |
| `debugging` | composite (`file` + `terminal` + `web`) | Debug bundle — file, process/terminal, web extract/search. |
| `delegation` | `delegate_task` | Spawn isolated subagent instances for parallel work. |
| `discord` | `discord` | Core Discord text/embed/DM actions (gateway-only). Active on the `savarez-discord` toolset. |
| `discord_admin` | `discord_admin` | Discord moderation (bans, role changes, channel management). Active on the `savarez-discord` toolset; requires the bot to hold the relevant Discord permissions. |
...
| `feishu_drive` | `feishu_drive_add_comment`, `feishu_drive_list_comments`, `feishu_drive_list_comment_replies`, `feishu_drive_reply_comment` | Feishu/Lark drive comment operations. Scoped to the comment agent; not exposed on `savarez-cli` or other messaging toolsets. |
...
| `x_search` | `x_search` | Search X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool. Off by default; opt in via `savarez tools`. Schema only registered when xAI credentials (SuperGrok OAuth or `XAI_API_KEY`) are configured. |
| `yuanbao` | `yb_query_group_info`, `yb_query_group_members`, `yb_search_sticker`, `yb_send_dm`, `yb_send_sticker` | Yuanbao DM/group actions and sticker search. Registered only on `savarez-yuanbao`. |

## Platform Toolsets

Platform toolsets define the complete tool configuration for a deployment target. Most messaging platforms use the same set as `savarez-cli`:

| Toolset | Differences from `savarez-cli` |
|---------|-------------------------------|
| `savarez-cli` | Full toolset — the default for interactive CLI sessions. Includes file, terminal, web, browser, memory, skills, vision, image_gen, todo, tts, delegation, code_execution, cronjob, session_search, clarify, and `safe` (read-only) bundles plus the standard messaging tools. |
| `savarez-acp` | Drops `clarify`, `cronjob`, `image_generate`, `send_message`, `text_to_speech`, and all four Home Assistant tools. Focused on coding tasks in IDE context. |
| `savarez-api-server` | Drops `clarify`, `send_message`, and `text_to_speech`. Keeps everything else — suitable for programmatic access where user interaction isn't possible. |
| `savarez-cron` | Same as `savarez-cli`. |
| `savarez-telegram` | Same as `savarez-cli`. |
| `savarez-discord` | Adds `discord` and `discord_admin` on top of `savarez-cli`. |
| `savarez-slack` | Same as `savarez-cli`. |
| `savarez-whatsapp` | Same as `savarez-cli`. |
| `savarez-signal` | Same as `savarez-cli`. |
| `savarez-matrix` | Same as `savarez-cli`. |
| `savarez-mattermost` | Same as `savarez-cli`. |
| `savarez-email` | Same as `savarez-cli`. |
| `savarez-sms` | Same as `savarez-cli`. |
| `savarez-bluebubbles` | Same as `savarez-cli`. |
| `savarez-dingtalk` | Same as `savarez-cli`. |
| `savarez-feishu` | Adds the five `feishu_doc_*` / `feishu_drive_*` tools (only used by the document-comment handler, not the regular chat adapter). |
| `savarez-qqbot` | Same as `savarez-cli`. |
| `savarez-wecom` | Same as `savarez-cli`. |
| `savarez-wecom-callback` | Same as `savarez-cli`. |
| `savarez-weixin` | Same as `savarez-cli`. |
| `savarez-yuanbao` | Adds the five `yb_*` tools (DM/group/sticker) on top of `savarez-cli`. |
| `savarez-homeassistant` | Same as `savarez-cli` (the Home Assistant tools are already present by default and activate when `HASS_TOKEN` is set). |
| `savarez-webhook` | Same as `savarez-cli`. |
| `savarez-gateway` | Internal gateway orchestrator toolset — union of every `savarez-<platform>` toolset; used when the gateway needs to accept any message source. |
...
## Relationship to `savarez tools`

The `savarez tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.
