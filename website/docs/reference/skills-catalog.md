...
description: "Catalog of bundled skills that ship with Savarez AI Agent"
---

# Bundled Skills Catalog

Savarez ships with a large built-in skill library copied into `~/.savarez/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.

Savarez also syncs bundled skills on `savarez update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.savarez/skills/` tree, it is still shipped with Savarez; restore it with `savarez skills reset <name> --restore`.
...
| [`savarez-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-savarez-agent) | Configure, extend, or contribute to Savarez AI Agent. | `autonomous-ai-agents/savarez-agent` |
...
| [`teams-meeting-pipeline`](/docs/user-guide/skills/bundled/productivity/productivity-teams-meeting-pipeline) | Operate the Teams meeting summary pipeline via Savarez CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions. | `productivity/teams-meeting-pipeline` |
...
| Skill | Description | Path |
|-------|-------------|------|
| [`savarez-agent-skill-authoring`](/docs/user-guide/skills/bundled/software-development/software-development-savarez-agent-skill-authoring) | Author in-repo SKILL.md: frontmatter, validator, structure. | `software-development/savarez-agent-skill-authoring` |
...
| [`plan`](/docs/user-guide/skills/bundled/software-development/software-development-plan) | Plan mode: write an actionable markdown plan to .savarez/plans/, no execution. Bite-sized tasks, exact paths, complete code. | `software-development/plan` |
