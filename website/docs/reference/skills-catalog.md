...
Hermes ships with a large built-in skill library copied into `~/.savarez/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.

Hermes also syncs bundled skills on `savarez update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.savarez/skills/` tree, it is still shipped with Hermes; restore it with `savarez skills reset <name> --restore`.
...
| [`hermes-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-hermes-agent) | Configure, extend, or contribute to Savarez AI Agent. | `autonomous-ai-agents/hermes-agent` |
