...
| [`claude-code-sdk`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code-sdk) | Programmatic Claude Code via the official claude-agent-sdk Python package. Persistent multi-turn sessions via session resume, parallel orchestration across projects, and per-query cost tracking. Complements the existing claude-code skill for SDK-based use cases. | `autonomous-ai-agents/claude-code-sdk` |
