...
| `recursive_context` | RLM-style large-context substrate. Create external corpora from text/files, then `search`/`read` bounded source-line windows or `map` chunks into delegation prompts. File ingests preserve resolved `source` and `source_line` citations; search results include matched `text` plus structured citation-aware `context`. Use this instead of pasting massive documents into context. | — |
