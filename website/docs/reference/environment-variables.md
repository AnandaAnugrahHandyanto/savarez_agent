...
| `MESSAGING_CWD` | Deprecated backward-compat fallback for messaging working directory. Prefer `terminal.cwd` in `config.yaml`; the gateway bridges that to `TERMINAL_CWD` for child tools. |
