...
| `ABLITERATION_API_KEY` | abliteration.ai API key ([abliteration.ai](https://abliteration.ai/); the provider quickstart uses `ABLIT_KEY` as an example shell variable name) |
| `ABLITERATION_BASE_URL` | Override abliteration.ai base URL (default: `https://api.abliteration.ai/v1`) |
...
| Variable | Description |
|----------|-------------|
| `HERMES_INFERENCE_PROVIDER` | Override provider selection: `auto`, `openrouter`, `nous`, `openai-codex`, `copilot`, `copilot-acp`, `anthropic`, `huggingface`, `zai`, `kimi-coding`, `kimi-coding-cn`, `minimax`, `minimax-cn`, `kilocode`, `xiaomi`, `arcee`, `abliteration`, `alibaba`, `deepseek`, `nvidia`, `ollama-cloud`, `xai` (alias `grok`), `google-gemini-cli`, `qwen-oauth`, `bedrock`, `opencode-zen`, `opencode-go`, `ai-gateway` (default: `auto`) |
