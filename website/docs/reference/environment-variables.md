...
| `HERMES_PRESERVE_HOST_HOME` | Set to `1` to opt out of the per-profile main-process `HOME` alignment (issue #27250). By default, when `${HERMES_HOME}/home/` exists, Hermes aligns the main Python process's `HOME` with the same per-profile directory subprocesses use, so `~` resolves to the same place for both. Set this to `1` to keep the historical split behaviour (main process keeps the host-level `HOME` such as `/opt/data`; subprocesses still get `${HERMES_HOME}/home/`) |
