...
| `DISCORD_ALLOWED_ROLES` | Comma-separated Discord role IDs allowed to use the bot in the originating guild (OR with `DISCORD_ALLOWED_USERS`). Auto-enables the Members intent. DMs require `DISCORD_ALLOWED_USERS` unless `discord.dm_role_auth_guild` is set in `config.yaml`. |
