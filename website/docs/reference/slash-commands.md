...
| `/supergoal <text>` (alias: `/sg`) | Create a Kanban-backed orchestration task for a larger objective, subscribe the current chat to task updates, and queue Hermes to decompose the work into child tasks, spawn/review builders where useful, and require proof before completion. Use `/supergoal status` to list active supergoals. |
...
| `/supergoal <text>` (alias: `/sg`) | Create a Kanban-backed orchestration task for a larger objective, subscribe this chat/thread to terminal task updates, and queue Hermes to decompose, assign/spawn builders, review, and require proof before completion. `/supergoal status` is safe mid-agent; creating a new supergoal requires the agent to be idle or `/stop` first. |
...
- `/status`, `/background`, `/queue`, `/steer`, `/voice`, `/reload-mcp`, `/reload-skills`, `/rollback`, `/debug`, `/fast`, `/footer`, `/curator`, `/kanban`, `/supergoal`, `/sessions`, and `/yolo` work in **both** the CLI and the messaging gateway.
