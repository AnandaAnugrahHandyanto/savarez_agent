...
| `/issue <owner/repo\|issue-url> <number> [--workdir path]` | **Gateway only.** Queue a GitHub issue for the headless persistent issue-resolution lane. Requests are stored in `~/.hermes/issue_resolution.db`, processed FIFO through the local Aider/Guardian coder lane, and reviewed through the configured cloud reviewer path. Master Epics are detected by the `master-plan` label or `# Master Project Plan` body heading. See [Issue Resolution Lane](/user-guide/features/issue-resolution). |
