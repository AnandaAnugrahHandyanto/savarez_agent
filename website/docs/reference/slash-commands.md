...
| `/copilot_remote [launch\|list\|show]` | Manage Copilot remote jobs: launch a routed session, list tracked jobs, or show reconnect details for one job. |
...
| `/copilot_remote [launch\|list\|show]` | Launch, list, or inspect tracked Copilot remote jobs. See [Copilot Remote Jobs](/docs/user-guide/features/copilot-remote). |
...
- `/status`, `/background`, `/copilot_remote`, `/voice`, `/reload-mcp`, `/rollback`, `/debug`, `/fast`, and `/yolo` work in **both** the CLI and the messaging gateway.
