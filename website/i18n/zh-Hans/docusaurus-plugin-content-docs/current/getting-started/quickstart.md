...
| 目标 | 先做这步 | 再做这步 |
|---|---|---|
| 只想让 Hermes 在本机跑起来 | `savarez setup` | 运行一次真实对话并验证有响应 |
...
:::tip 选择你的界面
Hermes 提供两种终端界面：经典的 `prompt_toolkit` CLI，以及更新的 [TUI](../user-guide/tui.md)（支持模态覆盖层、鼠标选择和非阻塞输入）。两者共享相同的会话、斜杠命令和配置——分别用 `savarez` 和 `hermes --tui` 试试看。
:::
...
# ~/.savarez/hermes-agent，在 Windows 上为 %LOCALAPPDATA%\hermes\hermes-agent）：
...
（如果安装时未包含 `[all]`，请先运行 `cd ~/.savarez/hermes-agent && uv pip install -e ".[acp]"`。）
...
| 现象 | 可能原因 | 解决方法 |
|---|---|---|
| Hermes 启动但回复为空或异常 | Provider 认证或模型选择有误 | 重新运行 `savarez model`，确认 provider、模型和认证信息 |
...
| `hermes --continue` 找不到旧会话 | 切换了 profile 或会话从未保存 | 检查 `savarez sessions list`，确认你在正确的 profile 下 |
