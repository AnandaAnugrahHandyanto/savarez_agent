...
执行 `nix profile install` 后，`savarez`、`hermes-agent` 和 `hermes-acp` 将出现在你的 PATH 中。之后的工作流与[标准安装](./installation.md)完全相同——`savarez setup` 引导你完成提供商选择，`savarez gateway install` 设置 launchd（macOS）或 systemd 用户服务，配置存放在 `~/.savarez/`。
...
- 路由是透明的：`savarez chat`、`savarez sessions list`、`hermes version` 等命令都会在底层 exec 进容器
- 所有 CLI 参数原样转发
...
`environment`（非密钥变量）和 `environmentFiles`（密钥文件）在激活时（`nixos-rebuild switch`）都会合并到 `$SAVAREZ_HOME/.env` 中。Hermes 在每次启动时读取此文件，因此更改在 `systemctl restart hermes-agent` 后生效——无需重建容器。
...
Agent 身份文件是独立的：Hermes 从 `$SAVAREZ_HOME/SOUL.md` 加载其主要 `SOUL.md`，在 NixOS 模块中对应 `${services.hermes-agent.stateDir}/.hermes/SOUL.md`。将 `SOUL.md` 放入 `documents` 只会创建一个工作区文件，不会替换主角色文件。
...
# 原生模式
sudo -u hermes SAVAREZ_HOME=/var/lib/hermes/.hermes \
  savarez mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
2. **`.managed` 标记文件**，位于 `SAVAREZ_HOME` 中——由激活脚本设置，对交互式 shell 可见（例如 `docker exec -it hermes-agent savarez config set ...` 也会被屏蔽）
...
~/.savarez -> /var/lib/hermes/.hermes       （符号链接桥接，按 hostUsers）
...
插件在激活时以符号链接方式安装到 `$SAVAREZ_HOME/plugins/`。Hermes 通过其正常的目录扫描发现它们。从列表中移除插件并运行 `nixos-rebuild switch` 会删除符号链接。
...
| 检查 | 测试内容 |
|---|---|
| `package-contents` | `savarez` 和 `hermes-agent` 二进制文件存在且 `hermes version` 可运行 |
...
| `stateDir` | `str` | `"/var/lib/hermes"` | 状态目录（`SAVAREZ_HOME` 的父目录） |
