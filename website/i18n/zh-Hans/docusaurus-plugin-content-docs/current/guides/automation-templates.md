...
常见自动化模式的复制粘贴配方。每个模板使用 Savarez 内置的 [cron 调度器](/user-guide/features/cron) 实现基于时间的触发，使用 [webhook 平台](/user-guide/messaging/webhooks) 实现事件驱动触发。
...
| **GitHub 事件** | PR 开启、推送、issue、CI 结果时触发 | Webhook 平台（`savarez webhook subscribe`） |
| **API 调用** | 外部服务向你的端点 POST JSON | Webhook 平台（config.yaml 路由或 `savarez webhook subscribe`） |
...
savarez cron create "0 2 * * *" \
  "You are a project manager triaging the AnandaAnugrahHandyanto/savarez-agent GitHub repo.

1. Run: gh issue list --repo AnandaAnugrahHandyanto/savarez-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
savarez webhook subscribe github-pr-review \
  --events "pull_request" \
...
savarez cron create "0 9 * * 1" \
  "Scan the AnandaAnugrahHandyanto/savarez-agent repo for documentation drift.

1. Run: gh pr list --repo AnandaAnugrahHandyanto/savarez-agent --state merged --json number,title,files,mergedAt --limit 30
...
```bash
savarez cron create "0 6 * * *" \
  "Run a dependency security audit on the savarez-agent project.

1. cd ~/.savarez/savarez-agent && source .venv/bin/activate
...
```bash
savarez webhook subscribe deploy-verify \
  --events "deployment" \
...
```python title="~/.savarez/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Savarez-Monitor/1.0"})
...
savarez cron create "every 30m" \
  ...
  --script ~/.savarez/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
savarez cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Savarez AI Agent development." \
...
```bash
savarez webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
savarez cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the savarez-agent codebase.
