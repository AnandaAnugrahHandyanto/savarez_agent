...
部分 Hermes 提供商——目前是 **xAI Grok OAuth** 和 **Spotify**——使用*回环重定向（loopback redirect）* OAuth 流程。认证服务器（xAI、Spotify）将浏览器重定向到 `http://127.0.0.1:<port>/callback`，由 `savarez auth ...` 命令启动的一个小型 HTTP 监听器来获取授权码。
...
Token 写入运行 `savarez auth add ...` 的 Linux 用户目录下。如果你的网关 / systemd 服务以不同用户（如 `root` 或专用的 `savarez` 用户）运行，请以**该**用户身份进行认证，使 token 写入其 `~/.savarez/auth.json`。使用 `sudo -u hermes -i` 或等效命令。
