...
|--------|-------------|
| `--version`, `-V` | 显示版本并退出。 |
| `--profile <name>`, `-p <name>` | 选择本次调用使用的 Hermes profile（配置文件）。覆盖 `savarez profile use` 设置的粘性默认值。 |
...
| `hermes login` / `logout` | **已弃用** — 请改用 `savarez auth`。 |
...
| `hermes checkpoints` | 检查/修剪/清除 `~/.savarez/checkpoints/`（`/rollback` 使用的影子存储）。不带参数运行可查看状态概览。 |
...
| `savarez mcp` | 管理 MCP 服务器配置，并将 Hermes 作为 MCP 服务器运行。 |
...
| `savarez profile` | 管理 profile——多个隔离的 Hermes 实例。 |
...
| `savarez uninstall` | 从系统中删除 Hermes。 |
...
**`savarez model`**（从终端运行，在任何 Hermes 会话外部）是**完整的 provider 设置向导**。它可以添加新 provider、运行 OAuth 流程、提示输入 API 密钥并配置端点。

**`/model`**（在活跃的 Hermes 聊天会话中输入）只能**在已设置好的 provider 和模型之间切换**。它无法添加新 provider、运行 OAuth 或提示输入 API 密钥。

**如果需要添加新 provider：** 先退出 Hermes 会话（`Ctrl+C` 或 `/quit`），然后从终端提示符运行 `savarez model`。
:::
...
:::tip WSL 用户
使用 `savarez gateway run` 而非 `savarez gateway start`——WSL 的 systemd 支持不稳定。用 tmux 包裹以保持持久运行：`tmux new -s hermes 'savarez gateway run'`。详见 [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-hermes-gateway-start-fails)。
:::
...
hermes slack manifest --write      # 写入 ~/.savarez/slack-manifest.json
...
`savarez update` 后重新运行 `hermes slack manifest --write` 以获取新增命令。


## `hermes login` / `hermes logout` *（已弃用）*

:::caution
`hermes login` 已被移除。请使用 `savarez auth` 管理 OAuth 凭据，使用 `savarez model` 选择 provider，或使用 `savarez setup` 进行完整的交互式设置。
:::
...
savarez auth spotify                                      # 通过 PKCE 将 Hermes 与 Spotify 认证
```
...
:::tip
`hermes dump` 专为分享而设计。交互式诊断请使用 `savarez doctor`。可视化概览请使用 `savarez status`。
:::
...
查看、跟踪和过滤 Hermes 日志文件。所有日志存储在 `~/.savarez/logs/`（非默认 profile 存储在 `<profile>/logs/`）。
...
Hermes 使用 Python 的 `RotatingFileHandler`。旧日志会自动轮转——查找 `agent.log.1`、`agent.log.2` 等。`savarez logs list` 子命令显示所有日志文件，包括已轮转的。
...
在全新安装时，第一次计划运行会延迟一个完整的 `interval_hours`（默认 7 天）——gateway 不会在 `savarez update` 后的第一次 tick 时立即执行 curator。使用 `hermes curator run --dry-run` 在此之前预览。
...
:::info Provider 特定子命令
当外部 memory provider 处于活跃状态时，它可能会注册自己的顶级 `hermes <provider>` 命令用于 provider 特定管理（例如 Honcho 激活时的 `hermes honcho`）。未激活的 provider 不暴露其子命令。运行 `savarez --help` 查看当前已连接的命令。
:::
...
`hermes computer-use install` 是安装 `computer_use` toolset 使用的 [cua-driver](https://github.com/trycua/cua) 二进制文件的稳定入口。它运行与首次启用 Computer Use 时 `savarez tools` 调用的相同上游安装程序，因此如果 toolset 切换未触发安装（例如在已配置用户的设置中），可以安全地用于重新运行安装。

`savarez update` 在更新结束时，如果 cua-driver 在 PATH 中，会自动重新运行上游安装程序，因此大多数用户不需要手动调用 `--upgrade`。当上游发布了你现在就想要的修复，而不想等待下次 Hermes 更新时，使用此选项。
...
将 OpenClaw 设置迁移到 Hermes。从 `~/.openclaw`（或自定义路径）读取并写入 `~/.savarez`。自动检测旧版目录名（`~/.clawdbot`、`~/.moltbot`）和配置文件名（`clawdbot.json`、`moltbot.json`）。
...
| `--no-backup` | 跳过迁移前对 `~/.savarez/` 的 zip 快照（默认情况下，在应用前会将单个还原点归档写入 `~/.savarez/backups/pre-migration-*.zip`；可用 `hermes import` 恢复）。 |
...
**pip 安装：** `savarez update` 自动检测基于 pip 的安装——查询 PyPI 获取最新版本并运行 `pip install --upgrade hermes-agent`，而非 `git pull`。PyPI 发布跟踪标记版本（主要/次要版本），而非 `main` 上的每个 commit。使用 `--check` 查看是否有更新的 PyPI 版本可用，而不安装。
...
- **配对数据快照。** 即使 `--backup` 关闭，`savarez update` 也会在 `git pull` 前对 `~/.savarez/pairing/` 和 Feishu 评论规则进行轻量快照。如果拉取覆盖了你正在编辑的文件，可以用 `hermes backup restore --state pre-update` 回滚。
- **旧版 `savarez.service` 警告。** 如果 Hermes 检测到预重命名的 `savarez.service` systemd 单元（而非当前的 `hermes-gateway.service`），会打印一次性迁移提示，帮助你避免循环重启问题。
...
| `savarez uninstall [--full] [--yes]` | 删除 Hermes，可选择删除所有 config/数据。 |
