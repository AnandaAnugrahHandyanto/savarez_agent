...
```
savarez computer-use install
```

此命令会获取并运行上游 cua-driver 安装脚本：
`curl -fsSL https://raw.githubusercontent.com/trycua/cua/main/libs/cua-driver/scripts/install.sh`。
使用 `savarez computer-use status` 验证安装结果。
...
3. 在提示时授予 macOS 权限：
   - **系统设置 → 隐私与安全性 → 辅助功能** → 允许终端（或 Savarez 应用）。
   - **系统设置 → 隐私与安全性 → 屏幕录制** → 允许同一应用。
4. 启动启用了该工具集的会话：
   ```
   savarez -t computer_use chat
   ```
   或在 `~/.savarez/config.yaml` 中将 `computer_use` 添加到已启用的工具集列表。

## 保持 cua-driver 最新

cua-driver 项目会定期发布修复（例如 v0.1.6 修复了 UTM 工作流中的 Safari 窗口焦点问题）。Savarez 在两处刷新二进制文件，避免你停留在过时版本：

- **`savarez update`** — 更新 Savarez 本身时，如果 `cua-driver` 在 PATH 中，更新结束时会重新运行上游安装程序。对非 macOS 用户及未安装 cua-driver 的用户无操作。
- **`savarez computer-use install --upgrade`** — 手动强制刷新。无论 cua-driver 是否已安装，都会重新运行上游安装程序。在不等待下次 Agent 更新的情况下获取最新修复时使用此命令。

`savarez computer-use status` 会在二进制路径旁显示已安装的版本号。
...
**`computer_use backend unavailable: cua-driver is not installed`** — 运行 `savarez computer-use install` 获取 cua-driver 二进制文件，或运行 `savarez tools` 并启用 Computer Use 工具集。
...
- [通用技能：`macos-computer-use`](https://github.com/AnandaAnugrahHandyanto/savarez_agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
