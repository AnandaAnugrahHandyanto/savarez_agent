...
1. **已安装 Savarez AI Agent** — `pip install hermes-agent`（参见 [安装](/getting-started/installation)）
...
语音模式在**经典 CLI**（`savarez chat`）和 **TUI**（`hermes --tui`）中均可使用。两者行为完全一致 — 相同的斜杠命令、相同的 VAD（语音活动检测）静音检测、相同的流式 TTS、相同的幻觉过滤器。TUI 额外将崩溃诊断日志转发至 `~/.savarez/logs/`，以便在异常音频后端出现按键录音失败时提供完整堆栈跟踪，而非静默消失。
