...
# Discord + Telegram 消息（包含 Discord 语音频道依赖）
...
:::info
`messaging` 扩展包使用显式版本固定安装 Discord 语音频道支持：**discord.py**、**davey**（DAVE 端到端加密）、**PyNaCl**（语音加密）和 **aiohttp**。Hermes 在这里避免使用上游 voice extra，以便 PyNaCl 保持在已修复的 1.6.x 版本线。
:::
