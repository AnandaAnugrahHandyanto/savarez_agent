...
在 `~/.hermes/config.yaml`（或项目级 `cli-config.yaml` 回退）的 `hooks:` 块中声明 shell 脚本 hook，Hermes 会在对应的插件 hook 事件触发时将其作为子进程运行——在 CLI 和 gateway 会话中均适用。无需编写 Python 插件。
...
3. `~/.hermes/config.yaml` 中的 `hooks_auto_accept: true`（或项目级 `cli-config.yaml` 回退）
