...
**配置：** `$SAVAREZ_HOME/honcho.json`（profile 本地）或 `~/.honcho/config.json`（全局）。解析顺序：`$SAVAREZ_HOME/honcho.json` > `~/.savarez/honcho.json` > `~/.honcho/config.json`。参见[配置参考](https://github.com/hermes-ai/hermes-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/hermes)。
...
| **AI peer**（`aiPeer`） | 每个 Hermes profile 一个。host key `savarez` → 默认；其他 profile 使用 `hermes.<profile>`。 |
...
扫描所有 Hermes profile，为没有 host 块的 profile 创建 host 块，从默认 `savarez` 块继承设置，并提前创建新的 AI peer。幂等操作——跳过已有 host 块的 profile。
