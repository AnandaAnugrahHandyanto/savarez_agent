...
列出 API 服务器 agent 可见的已安装技能，使仪表板和控制平面无需读取私有 Python 内部实现即可枚举可用技能。

### GET /v1/toolsets

列出 toolsets 及其解析后的工具（即当前运行 agent 的有效可配置 toolsets）。
...
服务器在持久化的 Hermes session 之上暴露了一个 CRUD + chat 接口，使外部 UI 无需在客户端管理历史即可列出、分支和继续对话。所有端点均受同一 bearer 认证保护。

### GET /api/sessions

列出持久化的 Hermes session。

### POST /api/sessions

创建一个空的 Hermes session 记录。
...
通过当前的 SessionDB 原语将该 session 分支为一个新的 session。

### POST /api/sessions/\{session_id\}/chat

针对该 session 运行一次同步的 agent 轮次并返回结果。

### POST /api/sessions/\{session_id\}/chat/stream

`/chat` 的 Server-Sent Events 变体，在 agent 轮次运行时进行流式传输。
