...
4. 将其保存到 `~/.savarez/.env`，后续运行时跳过此步骤
5. 直接进入 OAuth 授权流程

授权完成后，token 将写入 `~/.savarez/auth.json` 的 `providers.spotify` 下。当前推理提供商不会改变——Spotify 认证与你的 LLM 提供商无关。
...
- **cron agent 继承你的活跃工具集。** Spotify 必须在 `savarez tools` 中启用，cron 会话才能看到 Spotify 工具。
...
从 `~/.savarez/auth.json` 中移除 token。若还需清除应用配置，请从 `~/.savarez/.env` 中删除 `HERMES_SPOTIFY_CLIENT_ID`（以及 `HERMES_SPOTIFY_REDIRECT_URI`，如果你设置了的话），或重新运行向导。
...
| 文件 | 内容 |
|------|----------|
| `~/.savarez/auth.json` → `providers.spotify` | access token、refresh token、过期时间、scope、redirect URI |
| `~/.savarez/.env` | `HERMES_SPOTIFY_CLIENT_ID`，可选 `HERMES_SPOTIFY_REDIRECT_URI` |
