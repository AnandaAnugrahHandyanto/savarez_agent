...
**富消息（Bot API 10.1）。** 最终回复通过 Telegram 原生的 [`sendRichMessage`](https://core.telegram.org/bots/api#sendrichmessage) 发送，使用 Agent 的**原始 markdown**，因此表格、任务列表、标题、嵌套引用块、可折叠的 `<details>`、脚注/引用、数学公式、下划线、上下标、高亮文本和锚点都能原生渲染——无需客户端展平。当启用原生草稿流式传输时（`streaming.transport: auto` 或 `draft`，仅限私聊，需 python-telegram-bot 22.6+），实时预览使用 `sendRichMessageDraft`，因此动画草稿与最终的富消息保持一致。基于编辑的流式传输（群组以及默认的 `edit` 传输方式）会让流式消息全程保持 MarkdownV2，确保格式不会在消息中途翻转。富消息发送**默认开启**；如需禁用（改用下方的旧版 MarkdownV2 路径），可按平台配置：
...
当内容超过 32,768 个字符的富文本上限时，富消息路径会自动跳过；Telegram 的任何拒绝（较旧的 Bot API 服务器缺少该端点、解析错误、块/列过多）都会**透明回退**到 MarkdownV2 路径——消息绝不会丢失。遇到端点缺失错误时还会在本会话内停用后续富消息尝试，避免每条消息都付出一次注定失败的往返。洪水控制（`retry_after`）会像旧版路径一样原地等待并重试；其他瞬时/网络错误**不会**被静默重发（不会产生重复的最终消息）。

**MarkdownV2 回退。** 当富消息路径被禁用或不可用时，Hermes 会将 markdown 转换为 MarkdownV2。由于 MarkdownV2 没有原生表格语法，管道表格会被规范化为**行组项目符号**——每行在列标题下变为可读的项目符号列表。回退无需配置，按消息自动应用。
