...
```bash
savarez gateway setup        # 交互式配置所有消息平台
```
...
savarez gateway setup        # 交互式配置消息平台
savarez gateway install      # 安装为用户服务（Linux）/ launchd 服务（macOS）
sudo savarez gateway install --system   # 仅 Linux：安装开机启动的系统服务
savarez gateway start        # 启动默认服务
savarez gateway stop         # 停止默认服务
savarez gateway status       # 检查默认服务状态
savarez gateway status --system         # 仅 Linux：显式检查系统服务
```
...
| `/update` | 将 Savarez AI Agent 更新至最新版本 |
| `/help` | 显示可用命令 |
...
在 `~/.savarez/gateway.json` 中配置各平台的覆盖设置：
...
在 `~/.savarez/config.yaml` 中控制显示多少工具活动信息：
...
当运行后台会话的 agent 使用 `terminal(background=true)` 启动长时间运行的进程（服务器、构建等）时，网关可以向你的聊天推送状态更新。通过 `~/.savarez/config.yaml` 中的 `display.background_process_notifications` 控制：
...
```bash
savarez gateway install               # 安装为 launchd agent
savarez gateway start                 # 启动服务
savarez gateway stop                  # 停止服务
savarez gateway status                # 检查状态
...
1. **网关日志**（`~/.savarez/logs/gateway.log` 或 systemd / launchd 单元日志）。搜索平台名称以及 `circuit breaker`、`paused` 或 `disabled`。触发事件包含失败次数和最后一个错误。
