...
sidebar_position: 8
title: "Mattermost"
description: "将 Savarez AI Agent 配置为 Mattermost 机器人"
---
...
1. 打开你希望添加机器人的频道。
2. 点击频道名称 → **添加成员**。
3. 搜索你的机器人用户名（例如 `savarez`）并添加。

对于私信，直接与机器人开启私信即可——它将立即能够响应。

## 第四步：查找你的 Mattermost 用户 ID

Savarez AI Agent 使用你的 Mattermost 用户 ID 来控制谁可以与机器人交互。查找方式：
...
:::tip
你可以在后台运行 `savarez gateway`，或将其配置为 systemd 服务以持续运行。详情参见部署文档。
:::
...
**解决方法**：将你的用户 ID 添加到 `~/.savarez/.env` 中的 `MATTERMOST_ALLOWED_USERS`，然后重启 gateway。注意：用户 ID 是 26 位字母数字字符串，不是你的 `@用户名`。
...
有关保护 Savarez AI Agent 部署的更多信息，请参阅[安全指南](../security.md)。

## 说明

- **自托管友好**：适用于任何自托管的 Mattermost 实例。无需 Mattermost Cloud 账户或订阅。
- **无额外依赖**：适配器使用 `aiohttp` 处理 HTTP 和 WebSocket，该库已包含在 Savarez AI Agent 中。
