...
| | |
|---|---|
| 来源 | 可选 — 通过 `savarez skills install official/research/qmd` 安装 |
...
qmd 提供 MCP 服务器，可通过原生 MCP 客户端直接向 Savarez AI Agent 提供搜索工具。这是推荐的集成方式 — 配置完成后，agent 无需每次加载此 skill 即可自动获得 qmd 工具。

### 方案 A：Stdio 模式（简单）

在 `~/.savarez/config.yaml` 中添加：
...
然后配置 Savarez AI Agent 通过 HTTP 连接：
