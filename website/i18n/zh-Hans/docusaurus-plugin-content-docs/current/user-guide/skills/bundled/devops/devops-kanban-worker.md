---
title: "Kanban Worker — Savarez Kanban worker 的陷阱、示例与边界情况"
sidebar_label: "Kanban Worker"
description: "Savarez Kanban worker 的陷阱、示例与边界情况"
---
...
Savarez Kanban worker 的陷阱、示例与边界情况。生命周期本身会自动注入到每个 worker 的系统 prompt（提示词）中，作为 `KANBAN_GUIDANCE`（来自 `agent/prompt_builder.py`）；当你需要深入了解特定场景时，加载此 skill 即可。
...
> 你看到此 skill，是因为 Savarez Kanban 调度器以 `--skills kanban-worker` 参数将你作为 worker 派生——它会为每个被派发的 worker 自动加载。**生命周期**（6 个步骤：orient → work → heartbeat → block/complete）也存在于自动注入到你系统 prompt 中的 `KANBAN_GUIDANCE` 块里。此 skill 是更深层的细节：良好的交接形式、重试诊断、边界情况。
...
对于大多数涉及代码变更的任务，在人工审查者过目之前，工作并未真正*完成*。应使用 block 而非 complete，并在 `reason` 前加 `review-required: ` 前缀，以便仪表板将该行标记为待审查。先将结构化元数据（变更文件、测试计数、diff/PR url）写入 comment，因为 `kanban_block` 只携带人类可读的原因——comment 是持久化注释的渠道。审查者可执行 `savarez kanban unblock <id>` 批准（这会携带 comment 线程重新派生你以处理后续事项），或通过另一条 comment 要求修改。
...
**当指导已可用时，不要依赖 CLI。** `kanban_*` 工具可在所有终端后端（Docker、Modal、SSH）上工作。从你的终端工具执行 `savarez kanban <verb>` 在容器化后端中会失败，因为 CLI 未安装在那里。如有疑问，使用工具。

## CLI 回退（用于脚本）

每个工具都有对应的 CLI 等价命令，供人工操作员和脚本使用：
- `kanban_show` ↔ `savarez kanban show <id> --json`
- `kanban_complete` ↔ `savarez kanban complete <id> --summary "..." --metadata '{...}'`
- `kanban_block` ↔ `savarez kanban block <id> "reason"`
- `kanban_create` ↔ `savarez kanban create "title" --assignee <profile> [--parent <id>]`
- 等等。
