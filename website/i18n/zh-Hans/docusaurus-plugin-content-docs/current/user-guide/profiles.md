...
在同一台机器上运行多个独立的 Savarez AI agent——每个 agent 拥有各自的配置、API 密钥、记忆、会话、技能和 gateway 状态。
...
```bash
savarez profile create coder       # 创建 profile + "coder" 命令别名
...
你也可以稍后通过 `savarez profile describe` 设置或自动生成描述——完整路由模型请参阅 [Kanban 指南](./features/kanban#auto-vs-manual-orchestration)。
...
将当前 profile 的 `config.yaml`、`.env` 和 `SOUL.md` 复制到新 profile。API 密钥和模型相同，但会话和记忆是全新的。编辑 `~/.savarez/profiles/work/.env` 可使用不同的 API 密钥，编辑 `~/.savarez/profiles/work/SOUL.md` 可设置不同的人格。
...
### 粘性默认值（`savarez profile use`）

```bash
savarez profile use coder
savarez chat                   # 现在指向 coder
savarez tools                  # 配置 coder 的工具
savarez profile use default    # 切换回默认
```

设置默认值后，普通 `savarez` 命令将指向该 profile。类似于 `kubectl config use-context`。
...
- **`savarez profile`**：显示当前 profile 名称、路径、模型、gateway 状态
...
`savarez update` 拉取一次代码（共享），并自动将新的内置技能同步到**所有** profile：
...
```bash
savarez profile list           # 显示所有 profile 及其状态
savarez profile show coder     # 显示某个 profile 的详细信息
savarez profile rename coder dev-bot   # 重命名（同步更新别名和服务）
savarez profile export coder   # 导出为 coder.tar.gz
savarez profile import coder.tar.gz   # 从归档文件导入
```
...
使用 `--yes` 跳过确认：`savarez profile delete coder --yes`

:::note
你无法删除默认 profile（`~/.savarez`）。如需删除所有内容，请使用 `savarez uninstall`。
:::
...
默认 profile 就是 `~/.savarez` 本身。无需迁移——现有安装的工作方式完全不变。
