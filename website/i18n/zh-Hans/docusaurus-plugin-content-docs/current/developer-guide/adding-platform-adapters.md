...
Plugin 元数据。`requires_env` 和 `optional_env` 块会自动填充 `savarez config` UI 条目（参见下方[在 savarez config 中暴露环境变量](#surfacing-env-vars-in-hermes-config)）。
...
## 在 `savarez config` 中暴露环境变量 {#surfacing-env-vars-in-hermes-config}
