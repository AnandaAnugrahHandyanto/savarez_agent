...
- 构建自定义/本地工具而不修改 Savarez 核心？从 [构建 Savarez 插件](../guides/build-a-savarez-plugin.md) 开始
- 为 Savarez 本身构建新的内置核心工具？从 [添加工具](./adding-tools.md) 开始
...
# 创建全局访问的符号链接
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/savarez" ~/.local/bin/savarez
...
- **Profile 安全路径**：不得硬编码 `~/.savarez` — 代码路径使用 `hermes_constants` 中的 `get_hermes_home()`，面向用户的消息使用 `display_hermes_home()`。完整规则参见 [AGENTS.md](https://github.com/AnandaAnugrahHandyanto/savarez_agent/blob/main/AGENTS.md#profiles-multi-instance-support)。

## 跨平台兼容性

Savarez 官方支持 **Linux、macOS、WSL2 以及原生 Windows（通过 PowerShell 安装）**。原生 Windows 使用 [Git for Windows](https://git-scm.com/download/win) 提供的 Git Bash 执行 shell 命令。部分功能依赖 POSIX 内核原语，已做条件限制：dashboard 内嵌的 PTY 终端面板（`/chat` 标签页）仅支持 WSL2。如果您主要在 Windows 上开发，推送前请运行 Windows 陷阱（footgun）lint（`scripts/check-windows-footguns.py`）。
...
- 使用 [GitHub Issues](https://github.com/AnandaAnugrahHandyanto/savarez_agent/issues)
- 请包含：操作系统、Python 版本、Savarez 版本（`savarez version`）、完整错误堆栈
- 包含复现步骤
- 创建前请检查是否已有重复 issue
- 安全漏洞请私下报告
...
- **Discord**：[discord.gg/AnandaAnugrahHandyanto](https://discord.gg/AnandaAnugrahHandyanto)
- **GitHub Discussions**：用于设计提案和架构讨论
- **Skills Hub**：上传专业 skill 并与社区共享

## 许可证

提交贡献即表示您同意您的贡献将以 [MIT 许可证](https://github.com/AnandaAnugrahHandyanto/savarez_agent/blob/main/LICENSE) 授权。