...
`a2a_adapter/executor.py` 实现 a2a-sdk 的 `AgentExecutor` 接口（`execute` / `cancel`）。
...
- 在工作线程中运行 `AIAgent.run_conversation`（`asyncio.to_thread`）
...
`a2a_adapter/sessions.py` 将 `contextId` 映射到一个 `HermesSession`（一个 `AIAgent`、其滚动历史和一个取消事件）。它是线程安全的，惰性创建 agent，并接受 `agent_factory` 以便测试注入伪实现。真实构建镜像 `acp_adapter.session._make_agent`（`platform="a2a"`、`enabled_toolsets=["hermes-a2a", ...]`）。

### 事件桥接

`a2a_adapter/events.py` 将 AIAgent 回调转换为 `TaskUpdater` 事件：
...
`a2a_adapter/card.py` 根据 Hermes 版本加上精选的技能列表（通用 agent、研究）动态构建 `AgentCard`。与 ACP 签入的 `acp_registry/agent.json` 不同，A2A 的 card 在服务器启动时构建（没有需要保持同步的静态 JSON 清单），并在每次 `/.well-known/agent-card.json` 请求时重新序列化。
...
- `tests/a2a/` —— A2A 测试套件
- `toolsets.py` —— `hermes-a2a` 工具集定义
- `hermes_cli/main.py` —— `hermes a2a` CLI 子命令
