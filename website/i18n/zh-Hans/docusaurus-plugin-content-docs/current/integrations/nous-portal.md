...
但**不建议在 Hermes Agent 内部使用它们**。Hermes 4 更适合对话和推理，不适合 Hermes 这种需要频繁调用工具的工作流。你可以把它们用于 [Nous Chat](https://chat.nousresearch.com)、研究工作流，或通过[订阅代理](/user-guide/features/subscription-proxy)从其他工具调用。要在 Hermes Agent 里使用，建议从目录中选择更适合智能体任务的模型：
