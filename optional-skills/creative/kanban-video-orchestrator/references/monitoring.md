...
savarez kanban watch --tenant <project-slug>

# Snapshot of the board
savarez kanban list --tenant <project-slug>
savarez kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
savarez kanban stats --tenant <project-slug>

# Visual dashboard (browser)
savarez dashboard

# Inspect a specific task (includes comments + events)
savarez kanban show <task-id>

# Follow a single task's event stream
savarez kanban tail <task-id>
```

Verify available subcommands with `savarez kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `savarez kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `savarez kanban block <id>` then `savarez kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `savarez kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `savarez kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `savarez profile list` (profile exists?), `savarez status` (gateway/dispatcher up?), and verify tenant. |
...
savarez kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
savarez kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(savarez kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
savarez kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
savarez kanban block <task-id>
savarez kanban archive <task-id>

# Diagnose what happened
savarez kanban show <task-id>      # task body, comments, recent events
savarez kanban tail <task-id>      # follow the live event stream
savarez kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `savarez kanban create` for the director
...
while true; do
    clear
    savarez kanban list --tenant <slug>
    echo "---"
    savarez kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `savarez kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `savarez profile list` and start
