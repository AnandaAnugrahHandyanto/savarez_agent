#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `savarez kanban list` and
...
def hermes_available() -> bool:
    return shutil.which("savarez") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed savarez CLI."""
    try:
        out = subprocess.run(
            ["savarez", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `savarez kanban list`
    out = subprocess.run(
        ["savarez", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["savarez", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not hermes_available():
        print("ERROR: 'savarez' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
