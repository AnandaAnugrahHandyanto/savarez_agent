...
version: 0.1.0
author: Tim Koepsel (xzessmedia), Savarez AI Agent
license: MIT
platforms: [linux, macos]
metadata:
  savarez:
    tags: [Coding-Agent, OpenHands, Model-Agnostic, LiteLLM]
    related_skills: [claude-code, codex, opencode, savarez-agent]
---
...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from Savarez.
...
For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For Savarez-native subagents, use `delegate_task`.
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `savarez-agent` (Savarez subagents via `delegate_task`).
