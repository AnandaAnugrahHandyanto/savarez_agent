...
version: 0.1.0
author: Hugo Sequier (Hugo-SEQUIER), Savarez AI Agent
license: MIT
...
The script reads `~/.savarez/.env` for two optional defaults:
...
Helper script: `~/.savarez/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
```bash
python3 ~/.savarez/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py <command> [args]
```
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `~/.savarez/.env`.
...
```bash
python3 ~/.savarez/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py dexs

python3 ~/.savarez/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py \
