...
# become ``~/.savarez`` — the real Hermes home — not the broken ``~/.Hermes``).
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Hermes`` but maps ``openclaw`` → ``savarez`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.savarez/config.yaml`` (the real Hermes home) instead of the broken
    ``~/.Hermes/config.yaml``.
    """
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--target", default=os.environ.get("SAVAREZ_HOME") or str(Path.home() / ".hermes"), help="Hermes home directory")
    parser.add_argument(
        "--workspace-target",
