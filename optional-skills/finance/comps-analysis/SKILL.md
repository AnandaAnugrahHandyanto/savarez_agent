...
version: 1.0.0
author: Anthropic (adapted by Ananda Anugrah Handyanto)
license: Apache-2.0
...
Many passages below say "use the S&P Kensho MCP / Daloopa MCP / FactSet MCP". Those are commercial financial-data MCPs from the original Cowork plugin context. In Savarez:

- **If you have any structured financial-data MCP configured** (Savarez supports MCP — see `native-mcp` skill), prefer it for point-in-time comps, precedent transactions, and filings.
