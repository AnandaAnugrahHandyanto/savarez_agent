...
Usage (via cron with --no-agent):
    ...
    savarez cron create my-feed \\
      --schedule "*/15 * * * *" --no-agent \\
      --script "$SAVAREZ_HOME/skills/devops/watchers/scripts/watch_rss.py" \\
...
def main() -> int:
    ...
    try:
        req = urllib.request.Request(args.url, headers={"User-Agent": "Savarez-Watcher/1.0"})
