...
| INT-005 | `src/ouroboros/bigbang/interview.py` prompt construction path | Question generation should use the upstream InterviewEngine contract: prompt-safe initial context, answer prefixes, brownfield intent/decision instruction, ambiguity snapshot, and perspective panel. | BO-062 now stores `upstream_question_contract` from the vendored prompt builder on start/answer turns; gateway does not call a provider in this no-restart slice. | adapter_equivalent | `test_bo062_records_vendored_upstream_question_prompt_contract`, `test_bo062_updates_upstream_question_contract_after_answer` |
